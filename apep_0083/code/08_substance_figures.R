...
fig18 <- ggplot(bac_data, aes(x = max_bac)) +
  ...
  labs(
    ...
    subtitle = "Drivers in fatal crashes with positive BAC test, Western States 2001-2019",
...
# =============================================================================
# Figure 20: Poly-Substance Involvement (2018-2019)
# =============================================================================
...
poly_data <- fars %>%
  filter(year >= 2018, !is.na(thc_positive) & !is.na(drunk_dr)) %>%
  mutate(alc_involved = drunk_dr > 0) %>%
  group_by(ever_rec_legal) %>%
  summarise(
    total = n(),
    thc_only = sum(thc_positive & !alc_involved),
    alc_only = sum(!thc_positive & alc_involved),
    both = sum(thc_positive & alc_involved),
    neither = sum(!thc_positive & !alc_involved),
    .groups = "drop"
  ) %>%
  mutate(
...
fig20 <- ggplot(poly_data, aes(x = state_type, y = pct, fill = category)) +
  geom_col(position = "stack", alpha = 0.8) +
  scale_fill_manual(values = c(
    "THC Only" = unname(apep_colors["thc_positive"]),
    "Alcohol Only" = unname(apep_colors["alcohol_positive"]),
    "Both THC + Alcohol" = unname(apep_colors["both"]),
    "Neither" = unname(apep_colors["neither"])
  )) +
  labs(
    title = "Substance Involvement Breakdown",
    subtitle = "Western States, 2018-2019",
    x = NULL,
    y = "Percent of Crashes with Drug Records",
    fill = "Substance\nCategory",
    caption = "Source: FARS, 2018-2019. THC identification via DRUGRESNAME text matching."
  ) +
  theme_apep()
