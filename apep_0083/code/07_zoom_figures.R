  ...
  if (color_by == "substance_cat") {
    ...
    p <- p +
      ...
      scale_color_manual(
        ...
        labels = c("TRUE" = "THC Finding Present", "FALSE" = "No THC Finding"),
        name = "THC\nFinding"
      )
  }
...
# Filter to 2018-2019 where THC identification is reliable
fars_co_wy <- fars_sf %>%
  filter(year >= 2018,
...
fig10 <- create_zoom_map(
  ...
  subtitle = "2018-2019 (THC text-match identification reliable)",
...
fars_i25 <- fars_sf %>%
  filter(year >= 2018, state_abbr %in% c("CO", "WY"))
...
fig11 <- create_zoom_map(
  ...
  subtitle = "Denver to Wyoming border, 2018-2019",
...
fars_denver <- fars_sf %>%
  filter(year >= 2018, state_abbr == "CO")
...
fars_wa_id <- fars_sf %>%
  filter(year >= 2018, state_abbr %in% c("WA", "ID"))
...
fig13 <- create_zoom_map(
  ...
  subtitle = "2018-2019 (I-90 corridor)",
  color_by = "thc_positive"
)
...
fars_or_id <- fars_sf %>%
  filter(year >= 2018, state_abbr %in% c("OR", "ID"))
...
fig14 <- create_zoom_map(
  ...
  subtitle = "2018-2019 (I-84 corridor, rural)",
  color_by = "rec_legal"
)
