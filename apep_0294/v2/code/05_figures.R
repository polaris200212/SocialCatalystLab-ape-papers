...
# Keep top 4 categories, merge the rest into "Other"
top4 <- c("CPT Professional Services", "HCBS/State (T-codes)",
          "Behavioral Health (H-codes)", "Temporary/State (S-codes)")
fig2_data[!hcpcs_category %in% top4, hcpcs_category := "Other"]
fig2_data <- fig2_data[, .(paid = sum(paid), paid_billions = sum(paid_billions)),
                       by = .(month_date, hcpcs_category)]
...
# Simplified palette for 5 categories
cat_colors_5 <- c(
  "HCBS/State (T-codes)" = "#1565C0",
  "Behavioral Health (H-codes)" = "#00695C",
  "Temporary/State (S-codes)" = "#6A1B9A",
...
p2 <- ggplot(fig2_data, aes(x = month_date, y = paid_billions, fill = hcpcs_category)) +
  ...
  scale_fill_manual(values = cat_colors_5, name = "Service Category") +
  scale_x_date(date_breaks = "1 year", date_labels = "%Y") +
...
# Exclude December 2024 (incomplete due to claims processing lag)
fig4_data <- copy(panel_national[!(year == 2024 & month_num == 12), .(month_date, n_providers)])
...
# Aggregate to quarterly for cleaner bars
fig4_quarterly <- fig4_data[month_date > as.Date("2018-06-01") & month_date < max_month]
fig4_quarterly[, quarter_date := as.Date(paste0(year(month_date), "-",
  sprintf("%02d", (quarter(month_date) - 1) * 3 + 2), "-01"))]
fig4_quarterly <- fig4_quarterly[, .(new_providers = sum(new_providers),
                                      exiting_providers = sum(exiting_providers)),
                                  by = quarter_date]

p4b <- ggplot(fig4_quarterly, aes(x = quarter_date)) +
  geom_col(aes(y = new_providers), fill = accent_teal, alpha = 0.7, width = 70) +
  geom_col(aes(y = -exiting_providers), fill = covid_red, alpha = 0.7, width = 70) +
  geom_hline(yintercept = 0, color = "grey40") +
  scale_x_date(date_breaks = "1 year", date_labels = "%Y") +
  scale_y_continuous(labels = comma) +
  labs(title = "Quarterly Provider Entry and Exit",
       subtitle = "Green = new entrants; Red = last observed billing quarter",
       x = NULL, y = "Providers")
