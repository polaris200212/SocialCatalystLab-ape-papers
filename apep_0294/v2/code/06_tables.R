...
for (i in seq_len(nrow(annual))) {
  r <- annual[i]
  growth_p <- ifelse(is.na(r$paid_growth), "N/A", sprintf("%.1f", r$paid_growth))
  growth_c <- ifelse(is.na(r$claims_growth), "N/A", sprintf("%.1f", r$claims_growth))
...
cat("\\begin{tablenotes}[flushleft]\n")
cat("\\small\n")
cat("\\item \\textit{Notes:} Panel covers January 2018 through December 2024 (84 months). Provider tenure counts include December 2024 billing activity; monthly spending totals exclude December 2024 due to claims processing lag. Tenure = months with at least one billing claim. Median total = median of cumulative payments across all months active.\n")
cat("\\end{tablenotes}\n")
cat("\\end{threeparttable}\n")
cat("\\end{table}\n")
sink()
