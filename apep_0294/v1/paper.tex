...
The provider panel is extraordinarily unbalanced. \Cref{fig:tenure} shows the distribution of provider tenure---the number of months in which a billing NPI submits at least one claim. Only 6\% of providers are active in all 84 months. Over 37\% appear for fewer than 12 months. The median provider is active for 22 months.
...
\label{fig:tenure}
\floatfoot{\textit{Notes:} Active months = number of months with at least one billing claim. Full panel = 84 months (Jan 2018--Dec 2024). The heavy left tail reflects high workforce turnover in home- and community-based services.}
\end{figure}
...
\label{fig:dynamics}
\floatfoot{\textit{Notes:} Top panel: unique billing NPIs with at least one claim per month. Bottom panel: green bars = providers billing for the first time; red bars = providers billing for the last time.}
\end{figure}
...
The National Plan and Provider Enumeration System (NPPES) transforms anonymous NPI numbers into identified, geolocated, specialty-classified providers. The bulk NPPES download contains 329 fields per provider, of which we extract the most analytically valuable:
...
The NPPES match rate on billing NPI is 99.5\%: we successfully match virtually all billing NPIs in T-MSIS to NPPES records, enabling state-level analysis, provider-type stratification, and all downstream geographic joins.
...
The linkage architecture described above is an ingredient list. The research value comes from specific panels constructed for specific questions. We describe four panels that we have built and made available for future research, each targeting a distinct class of research questions.
...
What we find inside is surprising. Over half of Medicaid provider spending flows through procedure codes---for personal care, behavioral health, habilitation---that have no Medicare equivalent whatsoever. The provider workforce is extraordinarily dynamic: 37\% of billing NPIs appear for fewer than 12 months, and only 6\% persist across the full 84-month panel. The organizational structure---revealed through billing--servicing NPI relationships and NPPES parent organization identifiers---shows that two-thirds of claim rows flow through organizational billing arrangements, suggesting markets that may be substantially more concentrated than they appear from provider counts alone.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig3_top_hcpcs.png}
\caption{Top 10 HCPCS Codes by Total Spending}
\label{fig:top_codes}
\floatfoot{\textit{Notes:} Cumulative Medicaid payments, 2018--2024. Personal care (T1019) dominates. All top 6 codes are Medicaid-specific (T/H/S prefix).}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig7_billing_structure.png}
\caption{Spending by Billing Structure}
\label{fig:billing_app}
\floatfoot{\textit{Notes:} Self-billing = billing NPI equals servicing NPI. Solo = no servicing NPI. Organization = billing NPI differs from servicing NPI.}
\end{figure}
