...
# ============================================================================
# 1b. 2008 BASELINE CONTROL (Key Revision Addition)
# ============================================================================

cat("\n1b. 2008 Baseline Control Analysis\n")
cat("------------------------------------\n\n")
cat("Controlling for 2008 GOP share (McCain vs Obama) shows whether technology\n")
cat("predicts CHANGES in GOP support since 2008, not just levels.\n\n")

# Filter to observations with 2008 baseline
df_2008 <- df %>% filter(!is.na(gop_share_2008))

# Model with 2008 baseline control
m_base1 <- feols(trump_share ~ modal_age_mean + gop_share_2008,
                 data = df_2008, cluster = ~cbsa)

m_base2 <- feols(trump_share ~ modal_age_mean + gop_share_2008 | year,
                 data = df_2008, cluster = ~cbsa)

m_base3 <- feols(trump_share ~ modal_age_mean + gop_share_2008 + log_total_votes + is_metro | year,
                 data = df_2008, cluster = ~cbsa)

# Also model the CHANGE from 2008 directly
m_change <- feols(gop_change_from_2008 ~ modal_age_mean + log_total_votes + is_metro | year,
                  data = df_2008, cluster = ~cbsa)

cat("Models with 2008 Baseline Control:\n")
etable(m_base1, m_base2, m_base3, m_change,
       headers = c("No FE", "Year FE", "Full Controls", "Change from 2008"),
       dict = c(modal_age_mean = "Modal Tech Age (years)",
                gop_share_2008 = "GOP Share 2008 (%)",
                log_total_votes = "Log Total Votes",
                is_metroTRUE = "Metropolitan"),
       se.below = TRUE,
       title = "Technology Age and GOP Support: 2008 Baseline Control")

cat("\nKey finding: Technology age predicts GOP gains from 2008 baseline.\n")
cat(sprintf("A 10-year older tech base → +%.2f pp GOP support change since 2008.\n\n",
            coef(m_change)["modal_age_mean"] * 10))
...
etable(m_2012, m_2016, m_2020, m_2024,
       headers = c("2012", "2016", "2020", "2024"),
       dict = c(modal_age_mean = "Modal Tech Age (years)",
                log_total_votes = "Log Total Votes",
                is_metroTRUE = "Metropolitan"),
       se.below = TRUE,
...
models <- list(
  ...
  by_year_2024 = m_2024,
  terciles = m_tercile,
  baseline_2008 = m_base3,
  change_from_2008 = m_change
)
