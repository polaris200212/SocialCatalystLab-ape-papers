...
\title{Legal Status vs.\ Physical Access: Testing the Cannabis-Alcohol Substitution Hypothesis at State Borders\footnote{This paper is a revision of APEP-0093. See \url{https://github.com/anthropics/auto-policy-evals/tree/main/papers/apep_0093} for the original.}}
...
\begin{tablenotes}[flushleft]
\small
\item Notes: Estimates and standard errors are reported as decimals; multiply by 100 to convert to percentage points (e.g., 0.092 = 9.2 pp). Robust bias-corrected standard errors in parentheses following \citet{calonico2014}. P-values and confidence intervals use the bias-corrected ``robust'' inference from \texttt{rdrobust}, which accounts for both estimation uncertainty and bias correction; these may differ slightly from naive z-statistics computed from the point estimate and SE. Column (1) uses MSE-optimal bandwidth with local linear regression and triangular kernel. Column (2) uses local quadratic polynomial. Columns (3)--(5) vary bandwidth relative to the MSE-optimal choice. Treatment is defined as crossing from prohibition to legal territory (negative running variable = legal). Positive estimates indicate higher alcohol involvement in legal states.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
A key identification concern is whether cannabis access actually changes discontinuously at the border. If prohibition-state residents can easily access cannabis through day trips to legal states, the ``treatment'' may be weak even when legal status changes. To assess treatment intensity, I estimate RDD specifications using measures of cannabis access as outcomes rather than alcohol involvement.

\textbf{Critical data limitation:} The dispensary data come from a 2020 OSM snapshot, while the crash data span 2016--2019. Dispensary networks expanded substantially during this period---particularly in California after January 2018 and Nevada after July 2017---so the 2020 snapshot does not accurately represent the dispensary landscape at the time of each crash. This timing mismatch means the following ``first-stage'' analysis is \textit{suggestive} rather than definitive: it characterizes 2020-era access patterns as a proxy for the study period, but cannot rule out that first-stage relationships differed in 2016--2019 when dispensary networks were sparser. Results should be interpreted with this caveat in mind.

Table~\ref{tab:first_stage} reports first-stage estimates for three measures of cannabis access.

\begin{table}[H]
\centering
\caption{First-Stage: Cannabis Access Discontinuity at Border (Suggestive, Using 2020 Dispensary Data)}
...
\begin{tablenotes}[flushleft]
\small
\item Notes: Each row reports RDD estimate at legal-prohibition borders using the indicated cannabis access measure as the outcome. Estimates use MSE-optimal bandwidth with local linear polynomial and triangular kernel. Effective N is total observations within bandwidth on both sides. None of the estimates are statistically significant at $p < 0.05$. \textbf{Caution:} Dispensary distances are computed using 2020 OSM data; actual distances during 2016--2019 may have differed due to dispensary network expansion.
\end{tablenotes}
\end{threeparttable}
\end{table}

Contrary to expectations, the first-stage estimates are small and statistically insignificant. While mean distances to the nearest dispensary differ dramatically between legal states (24 km) and prohibition states (110 km) \textit{in the 2020 data}, this gap largely reflects locations far from borders. At the exact border---within the RDD bandwidth---cannabis access (as measured by 2020 dispensary locations) does not change sharply. One interpretation, consistent with the cross-border shopping literature \citep{lovenheim2008, knight2013}, is that prohibition-state residents near the border can easily cross into legal territory to purchase cannabis, effectively collapsing the treatment discontinuity. However, an alternative interpretation is that measurement error from using 2020 dispensary locations to characterize 2016--2019 access contributes to the null first-stage: if dispensary networks were sparser in 2016--2019, the true first-stage discontinuity during the study period may have been larger than these estimates suggest. \textbf{Given this ambiguity, the ``weak first stage'' finding should be treated as suggestive rather than definitive.} The ``treatment'' at the border is therefore best understood as a change in \textit{legal status} (the de jure law); whether it also represents a change in \textit{physical access} (the de facto availability) cannot be determined with certainty from the available data.
...
The meta-analytic pooled estimate is similar to the main specification, confirming that the null result is not driven by heterogeneous effects across borders averaging to zero. Most individual borders show null effects, with one exception: the CO--WY border shows a significant positive estimate at baseline (0 km, no donut exclusion) in the border-specific analysis (Table~\ref{tab:border_donut}, column 1). However, this result does not persist when a 2 km donut is applied (column 2), suggesting it may reflect local sample composition near the exact cutoff rather than a robust treatment effect.
...
\begin{tablenotes}[flushleft]
\small
\item Notes: Estimates are reported as decimals; multiply by 100 to convert to percentage points (e.g., 0.237 = 23.7 pp). Each row excludes crashes within the specified distance of the border. Estimates use MSE-optimal bandwidth selection from the remaining sample. The 2km donut produces a larger estimate, but this is sensitive to sample composition. Larger donuts (5km, 10km) return to estimates near zero, consistent with the baseline.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\subsubsection{Border-Specific Donut Analysis}

To investigate which border segment(s) drive the anomalous 2km donut result, I decompose the donut RDD by border region. Table~\ref{tab:border_donut} presents donut estimates for each major legal-prohibition border pair.

\begin{table}[H]
\centering
\caption{Border-by-Border Donut RDD Results}
\label{tab:border_donut}
\begin{threeparttable}
\begin{tabular}{lcccccc}
\toprule
 & \multicolumn{2}{c}{0 km (baseline)} & \multicolumn{2}{c}{2 km donut} & \multicolumn{2}{c}{5 km donut} \\
\cmidrule(lr){2-3} \cmidrule(lr){4-5} \cmidrule(lr){6-7}
Border & Estimate (SE) & N & Estimate (SE) & N & Estimate (SE) & N \\
\midrule
CO-WY & 0.395* (0.151) & 157 & 0.297 (0.331) & 155 & $-$0.096 (0.337) & 157 \\
NV/CA-AZ & 0.015 (0.101) & 344 & 0.215 (0.162) & 249 & 0.004 (0.167) & 225 \\
OR/WA-ID & 0.124 (0.095) & 505 & 0.194 (0.114) & 436 & 0.146 (0.138) & 405 \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
\small
\item Notes: Each cell shows the RDD estimate (standard error) and effective sample size (N) for the indicated donut size and border region. * denotes significance at $p < 0.05$. CO-NE, CO-KS, CO-NM, and CO/NV-UT borders omitted due to insufficient within-bandwidth observations after applying donut exclusions (fewer than 50 crashes on one side of the cutoff). Larger donut sizes (10 km) not shown as sample attrition yields imprecise estimates. Effective N is total observations within MSE-optimal bandwidth on both sides of the cutoff.
\end{tablenotes}
\end{threeparttable}
\end{table}

The border-by-border decomposition reveals a key finding: \textbf{no individual border shows a significant positive 2km donut result}. The OR/WA-ID border comes closest ($0.194$, SE $= 0.114$, $p = 0.089$), but even this is not statistically significant at conventional levels. The pooled 2km significance appears to arise from aggregation across borders rather than a consistent pattern within any single border region.

At baseline (0 km donut), the CO--WY border shows a significant positive estimate (0.395, SE = 0.151), but this significance disappears once the 2 km donut is applied (0.297, SE = 0.331). This pattern suggests the CO--WY result is driven by crashes very close to the border (within 2 km) rather than a robust treatment effect operating throughout the bandwidth.

Figure~\ref{fig:border_donut_forest} presents the 2km donut estimates as a forest plot.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig09_border_donut_forest.pdf}
\caption{2km Donut RDD Estimates by Border Segment}
\label{fig:border_donut_forest}
\small
Notes: Forest plot of 2km donut RDD estimates for each border region. Point size proportional to effective sample size. Dashed line indicates null effect. No individual border shows a significant positive effect, suggesting the pooled 2km result reflects aggregation artifacts rather than a robust treatment effect at any specific border.
\end{figure}

This analysis reinforces the interpretation that the 2km donut significance is not robust: it does not replicate within any individual border segment and likely reflects sample composition idiosyncrasies rather than a true treatment effect.
...
A striking pattern emerges when examining cross-border drivers specifically. Among prohibition-state residents (drivers licensed in AZ, ID, KS, MT, NE, NM, UT, WY) in the within-150km sample, alcohol involvement differs substantially depending on where they crashed:\footnote{This subsample of N = 1,847 prohibition-state licensed drivers is drawn from the full within-150km analysis sample (N = 5,442), not the narrower RDD effective sample (N $\approx$ 1,446 within the MSE-optimal bandwidth). The larger window is used here because the cross-border comparison is descriptive (not an RDD estimate) and requires sufficient observations in each cross-border category.}
...
Table~\ref{tab:mde} formalizes the power analysis for the main specification. The minimum detectable effect (MDE) at 80\% power and two-sided $\alpha = 0.05$ is calculated as $\text{MDE} = 2.80 \times \text{SE}$.

\begin{table}[H]
\centering
\caption{Statistical Power Analysis: Minimum Detectable Effects}
\label{tab:mde}
\begin{threeparttable}
\begin{tabular}{lccc}
\toprule
Specification & Effective N & SE & MDE (pp) \\
\midrule
Main RDD (all crashes) & 1,446 & 0.059 & 16.6 \\
2km Donut RDD & 1,392 & 0.082 & 23.0 \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
\small
\item Notes: MDE calculated as $2.80 \times \text{SE}$ for 80\% power at two-sided $\alpha = 0.05$. Effective N matches Table~\ref{tab:donut} specifications. This MDE represents the effect size we could reliably \textit{detect} ex ante; it does not change the ex post inference from the realized confidence interval.
\end{tablenotes}
\end{threeparttable}
\end{table}

The power analysis reveals that the study has 80\% power to detect effects of approximately 16.6 percentage points or larger. This limitation applies to ex ante detection probability: before seeing the data, we would have needed a true effect of at least 16.6 pp to expect rejection of the null with 80\% probability.

However, the ex post confidence interval [$-$2.0 pp, +21.0 pp] provides different information. This interval \textit{excludes} large substitution effects (e.g., reductions of 5 pp or more in alcohol involvement), ruling them out at the 5\% level. What the study cannot distinguish is whether the true effect is a very small substitution effect (e.g., $-$1 pp), zero, or a small complementarity effect---all are consistent with the data.

The distinction matters for interpretation: we can conclude with confidence that cannabis access does not produce large reductions in alcohol-involved crashes at borders, but we lack precision to characterize very small effects.
