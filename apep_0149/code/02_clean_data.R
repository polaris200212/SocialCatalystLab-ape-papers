...
df <- df %>%
  ...
  mutate(
    ...
    # because we have no post-treatment observations for them
    ...
    # Fill in state abbreviation for never-treated
...
cat(sprintf("Postpartum sample: %d rows\n", nrow(df_postpartum)))
...
# Sample 4: Non-postpartum women (placebo population)
df_nonpostpartum <- df %>% filter(postpartum == 0) %>%
  filter(low_income == 1)  # Match on income
cat(sprintf("Non-postpartum low-income (placebo): %d rows\n", nrow(df_nonpostpartum)))
