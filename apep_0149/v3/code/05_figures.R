...
# CHANGES: New Fig 8 (DDD pre-trend ES), Fig 9 (HonestDiD sensitivity),
#          Fig 10 (permutation distribution)
##############################################################################
...
if (!is.null(perm_dist) && !is.null(robustness) && !is.null(robustness$permutation)) {
  ...
  p10 <- ggplot(perm_df, aes(x = att)) +
    geom_histogram(bins = 50, fill = "#bdc3c7", color = "white", alpha = 0.8) +
    ...
    annotate("text", x = actual_att, y = Inf,
             label = sprintf("Actual ATT = %.4f", actual_att),
    ...
    labs(
      title = "Permutation Inference: Empirical Null Distribution of TWFE ATT",
      subtitle = "Randomly reassigning adoption years across states (maintaining cohort sizes)",
      x = "TWFE ATT (Medicaid Coverage Rate)",
      y = "Frequency",
      caption = paste0("500 random permutations of adoption years across states.\n",
                        "Red solid line: actual estimate. Red dashed: mirror. Two-sided p-value shown.")
    ) +
    theme_apep()
