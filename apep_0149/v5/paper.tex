...
\title{Does the Safety Net Bite Back? \\ Medicaid Postpartum Coverage Extensions \\ Through the Public Health Emergency and Beyond\thanks{This is a revision of \href{https://github.com/SocialCatalystLab/ape-papers/tree/main/apep_0149}{APEP Working Paper 0149} (v4). Changes in v5: explicit 2020 exclusion in code (previously relied on API null response), exact adoption dates with source citations and July 1 half-year rule verification, PHE timing documentation, strengthened Monte Carlo calibration disclosure, and improved DDD pre-trend joint test variance estimation.}}
\author{APEP Autonomous Research\thanks{Autonomous Policy Evaluation Project. Correspondence: scl@econ.uzh.ch} \and @ai1scl \\ @olafdrw}
\date{\today}
...
Treatment assignment is based on state-level adoption dates compiled from CMS press releases, Kaiser Family Foundation tracking data, MACPAC reports, and state Medicaid agency announcements. Exact effective dates (month and day) for each state are documented in the replication code (\texttt{01\_fetch\_data.R}) with per-state source citations. With data through 2024, the analysis includes 47 treated jurisdictions (4 adopting in 2021, 25 in 2022, 13 in 2023, and 5 in 2024) and 4 control jurisdictions: Arkansas and Wisconsin (never adopted) plus Idaho and Iowa (adopting in 2025).
...
\textbf{Treatment timing and ACS alignment.} I code a state as ``treated'' in survey year $t$ if the extension's effective date falls on or before July 1 of year $t$ (i.e., the extension was in place for at least half the reference year); if the effective date falls after July 1, treatment is coded beginning in year $t+1$. The exact effective date for each state is recorded in the replication code with source citations, and the resulting \texttt{adopt\_year} already encodes this July 1 rule. For the CS-DiD estimator, the treatment cohort year $G_s$ is the first calendar year in which the extension is coded as active. For the 2024 adoption cohort (5 states), this means that respondents interviewed early in 2024 may be reporting on a period before the policy was active; the July 1 rule ensures that \emph{at least half} of the reference year falls under treatment, but partial-year treatment attenuates the $e=0$ estimate for this cohort. This partial-year contamination is explicitly quantified in the Monte Carlo attenuation analysis (Section~\ref{sec:attenuation}). \Cref{tab:timing_map} illustrates the mapping from adoption cohort to event time for the ACS survey years used in the analysis.
...
\begin{figure}[H]
    ...
    \floatfoot{\textit{Notes:} Numbers above points show new adopters in each year. Gray shading: PHE period. The 2025 adopters (ID, IA) are included in the sample as not-yet-treated controls through the 2024 data window.}
\end{figure}
...
\Cref{tab:ddd_pretrend} reports the individual pre-period coefficients, standard errors, and $p$-values from the DDD event study, along with a joint Wald $F$-test of the null hypothesis that all pre-treatment coefficients are simultaneously zero.\footnote{The joint test uses the full variance-covariance matrix from the CS-DiD influence functions when available. If the influence function matrix is not accessible from the estimator output, the test falls back to a diagonal approximation that ignores covariance across event-time estimates. The diagonal approximation may overstate the test statistic if pre-period estimates are positively correlated; however, with only 2--3 pre-period coefficients, the practical impact is limited. The variance method used is reported in the replication code output.} A high joint $p$-value supports the DDD identifying assumption; a low joint $p$-value would indicate differential pre-trends requiring further investigation.
...
The 2024 adopters (AK, NE, TX, UT, NV) provide a particularly clean test. These states implemented the extension after the PHE ended, so their treatment effect is entirely post-PHE. The late-adopter TWFE yields a positive but imprecise 2.5 pp (SE = 3.1 pp, 95\% CI: $[-3.6, 8.7]$ pp). The estimate is statistically uninformative given the wide confidence interval, which spans both meaningful positive and negative effects. That said, the positive point estimate---in contrast to the negative standard DiD results---is directionally consistent with a beneficial policy effect in the specification with the least PHE contamination. The imprecision reflects the small number of clusters (5 treated, 4 control states), and we caution against over-interpreting this estimate.
...
\textbf{DDD power.} The DDD specification, which relies on only 4 control states and the within-state postpartum versus non-postpartum comparison, has wider standard errors than the standard DiD. With a DDD SE of approximately 1.2 pp, the MDE at 80\% power is $2.8 \times 1.2 = 3.4$ pp. The expected true DDD effect---the postpartum-specific component after differencing out the common unwinding shock---is the product of the true per-exposed effect (5--15 pp) and the ITT scaling factor (0.5--0.7), yielding an expected range of 2.5--10.5 pp. At 5 pp the DDD achieves 99\% power, but at 2.5 pp it achieves only 55\% power. This power limitation should be weighed when interpreting the insignificant DDD estimate: the data cannot rule out a modest positive DDD effect of 2--3 pp.
...
This paper was autonomously generated using Claude Code as part of the Autonomous Policy Evaluation Project (APEP). This is a revision of APEP Working Paper 0149 (v4), addressing code review findings: explicit year-2020 exclusion, exact adoption dates with source citations and July 1 half-year rule verification, PHE timing documentation, strengthened Monte Carlo calibration disclosure, and improved DDD pre-trend joint test variance estimation.
