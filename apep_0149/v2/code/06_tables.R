...
# CHANGES: Panel C (DDD), Panel D (Post-PHE), updated Table 3 with HonestDiD/WCB,
#          updated Table 4 status, new Table 5 (post-PHE heterogeneity)
##############################################################################
...
tab1_tex <- paste0(tab1_tex,
  ...
  "Control states: Arkansas, Wisconsin (never adopted), Idaho, Iowa (adopt 2025).\n",
  "\\end{tablenotes}\n",
  "\\end{table}\n"
)
...
# =========================================================
# Table 2: Main Results (EXPANDED with DDD and Post-PHE panels)
# =========================================================

cat("  Table 2: Main results (expanded)\n")
...
tab2_tex <- paste0(tab2_tex,
  ...
  "Panel C reports DDD estimates (treated $\\times$ postpartum indicator) with state$\\times$postpartum and year$\\times$postpartum FE, using low-income women.\n",
...
# =========================================================
# Table 3: Robustness Checks (EXPANDED)
# =========================================================
...
# DDD
tab3_tex <- paste0(tab3_tex,
  sprintf("Triple-difference (DDD) & %.4f & %.4f & ",
...
# =========================================================
# Table 4: Treatment Dates (UPDATED status column)
# =========================================================

cat("  Table 4: Treatment dates (updated status)\n")
...
sorted_dates <- treatment_dates %>%
  ...
  mutate(
    # CHANGE: Updated status to reflect 2024 sample end
    in_sample = case_when(
...
sorted_dates <- bind_rows(sorted_dates, never_rows) %>%
  arrange(
    is.na(adopt_year),  # Non-adopters last
    adopt_year,
    state_abbr
  ) %>%
...
n_control <- sum(sorted_dates$in_sample == "Control")
...
tab4_tex <- paste0(tab4_tex,
  ...
  sprintf("Status: ``Treated'' = adopted by %d (N=%d); ``NYT'' = not-yet-treated in sample (N=%d); ``Control'' = never adopted (N=%d).\n",
          max_year, n_treated_total, n_nyt, n_control),
...
# =========================================================
# NEW Table 5: Post-PHE Heterogeneity
# =========================================================
