...
# CHANGES: Added low-income employer insurance CS-DiD, ATT(g,t) matrix export
##############################################################################
...
# CS-DiD: Employer insurance for low-income (reconciliation for Fatal Error 2)
cs_agg_employer_low <- NULL
tryCatch({
  cs_employer_low <- att_gt(
    yname = "employer_rate",
    tname = "year",
    idname = "state_id",
    gname = "first_treat",
  ...
  cs_agg_employer_low <- aggte(cs_employer_low, type = "simple")
  cat("\nOverall ATT (Employer Insurance, Low-Income):\n")
  summary(cs_agg_employer_low)
}, error = function(e) {
  cat("\nCS-DiD Employer Low-Income failed:", e$message, "\n")
})
...
# =========================================================
# Export ATT(g,t) matrix for reconciliation (Fatal Error 1)
# =========================================================

cat("\n--- ATT(g,t) Matrix Export ---\n")

if (!is.null(cs_medicaid)) {
  att_gt_df <- data.frame(
    group = cs_medicaid$group,
    t = cs_medicaid$t,
    att = cs_medicaid$att,
    se = cs_medicaid$se
  )
  cat(sprintf("ATT(g,t) matrix: %d cells\n", nrow(att_gt_df)))

  # Print the matrix for verification
  for (g in sort(unique(att_gt_df$group[att_gt_df$group > 0]))) {
    g_data <- att_gt_df[att_gt_df$group == g, ]
    cat(sprintf("  Cohort %d:", g))
    for (i in seq_len(nrow(g_data))) {
      cat(sprintf(" t=%d:%.3f", g_data$t[i], g_data$att[i]))
    }
    cat("\n")
  }

  # Verify: reconstruct simple ATT from post-treatment cells
  post_cells <- att_gt_df[att_gt_df$group > 0 & att_gt_df$t >= att_gt_df$group, ]
  if (nrow(post_cells) > 0) {
    simple_mean <- mean(post_cells$att)
    cat(sprintf("\n  Simple mean of post-treatment cells: %.4f\n", simple_mean))
    cat(sprintf("  aggte(type='simple') ATT: %.4f\n", cs_agg_medicaid$overall.att))
    cat("  Note: These may differ because aggte uses group-size weights\n")
  }
}
...
results <- list(
  ...
  cs_agg = list(
    ...
    employer_low = if (exists("cs_agg_employer_low")) cs_agg_employer_low else NULL
  ),
  cs_dyn = list(
...
if (!is.null(cs_agg_employer_low)) {
  cat(sprintf("CS-DiD ATT (Employer, Low-Inc PP):  %.4f (SE: %.4f)\n",
              cs_agg_employer_low$overall.att, cs_agg_employer_low$overall.se))
}
