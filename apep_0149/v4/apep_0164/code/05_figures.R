...
# CHANGES: DDD event study enhancement, non-postpartum event study figure, power curve
##############################################################################
...
if (!is.null(results$ddd$cs_dyn_medicaid)) {
  ddd_es_data <- data.frame(
    ...
    ddd_es_data <- ddd_es_data %>%
      mutate(period = ifelse(e < 0, "Pre-treatment", "Post-treatment"))
  ...
  p8 <- ggplot(ddd_es_data, aes(x = e, y = att)) +
    ...
    # Binned post-treatment average overlay
    post_avg <- ddd_es_data %>% filter(e >= 0) %>%
      summarise(mean_att = mean(att), mean_ci_low = mean(ci_low), mean_ci_high = mean(ci_high))

    if (nrow(ddd_es_data %>% filter(e >= 0)) > 0) {
      p8 <- p8 +
        geom_rect(aes(xmin = -0.3, xmax = max(e) + 0.3,
                      ymin = post_avg$mean_ci_low, ymax = post_avg$mean_ci_high),
                  alpha = 0.1, fill = "#e67e22", inherit.aes = FALSE) +
        geom_hline(yintercept = post_avg$mean_att, linetype = "dashed",
                   color = "#e67e22", linewidth = 0.7)
    }

    # Annotate F-test p-value
    if (!is.null(robustness) && !is.null(robustness$ddd_pretrend) &&
        !is.null(robustness$ddd_pretrend$f_pval)) {
      p8 <- p8 +
        annotate("text", x = min(ddd_es_data$e) + 0.5, y = max(ddd_es_data$ci_high) * 0.9,
                 label = sprintf("Joint F-test p = %.3f", robustness$ddd_pretrend$f_pval),
...
if (!is.null(perm_dist) && !is.null(robustness) && !is.null(robustness$permutation)) {
  ...
  p10 <- ggplot(perm_df, aes(x = att)) +
    ...
    labs(
      ...
      subtitle = sprintf("Randomly reassigning adoption years across states (%d permutations)", length(perm_dist)),
      x = "CS-DiD ATT (Medicaid Coverage Rate)",
      y = "Frequency",
      caption = paste0(sprintf("%d random permutations of adoption years across states.\n", length(perm_dist)),
...
# =========================================================
# NEW Figure 11: Non-Postpartum Event Study
# =========================================================

cat("  Figure 11: Non-postpartum event study\n")

if (!is.null(robustness) && !is.null(robustness$nonpp_event) &&
    !is.null(robustness$nonpp_event$cs_dyn)) {

  p11 <- plot_event_study(robustness$nonpp_event$cs_dyn,
                           "Non-Postpartum Low-Income Women: Medicaid Event Study",
                           "ATT (Medicaid Coverage Rate)",
                           "#95a5a6")
  p11 <- p11 + labs(
    subtitle = "Mechanism validation: non-postpartum women should show unwinding but no postpartum-specific effect",
    caption = paste0("CS-DiD on non-postpartum low-income women (comparison group for DDD).\n",
                     "Negative post-treatment effects confirm the unwinding mechanism.\n",
                     "This validates the DDD design's use of non-postpartum women as a control for secular shocks.")
  )

  ggsave(file.path(fig_dir, "fig11_nonpp_event_study.pdf"), p11,
         width = 8, height = 6, dpi = 300)
} else {
  cat("  Non-postpartum event study results not available, skipping Fig 11\n")
}

# =========================================================
# NEW Figure 12: Power Curve
# =========================================================

cat("  Figure 12: Power curve\n")

main_se <- results$cs_agg$medicaid$overall.se
# Use CS-DiD DDD SE (primary spec) for power curve, not TWFE DDD SE
ddd_se <- if (!is.null(results$ddd$cs_agg_medicaid)) {
  results$ddd$cs_agg_medicaid$overall.se
} else {
  se(results$ddd$twfe_medicaid)[1]
}

effect_sizes <- seq(0, 0.12, by = 0.005)
power_df <- data.frame(
  effect = rep(effect_sizes * 100, 2),
  power = c(
    sapply(effect_sizes, function(e) {
      ncp <- e / main_se
      1 - pnorm(1.96 - ncp) + pnorm(-1.96 - ncp)
    }),
    sapply(effect_sizes, function(e) {
      ncp <- e / ddd_se
      1 - pnorm(1.96 - ncp) + pnorm(-1.96 - ncp)
  ...
  specification = rep(c("Main CS-DiD", "DDD"), each = length(effect_sizes))
)

p12 <- ggplot(power_df, aes(x = effect, y = power * 100, color = specification,
                              linetype = specification)) +
  geom_line(linewidth = 1.1) +
  geom_hline(yintercept = 80, linetype = "dotted", color = "gray50") +
  annotate("text", x = 11, y = 82, label = "80% power", color = "gray50", size = 3) +
  scale_color_manual(values = c("Main CS-DiD" = "#3498db", "DDD" = "#8e44ad")) +
  scale_linetype_manual(values = c("Main CS-DiD" = "solid", "DDD" = "dashed")) +
  scale_y_continuous(limits = c(0, 100), breaks = seq(0, 100, 20)) +
  labs(
    title = "Statistical Power by True Effect Size",
    subtitle = "Two-sided test at 5% significance level",
    x = "True Effect Size (percentage points)",
    y = "Power (%)",
    color = NULL, linetype = NULL,
    caption = sprintf("Main CS-DiD SE = %.3f pp; DDD SE = %.3f pp.\nMDE at 80%% power: Main = %.1f pp, DDD = %.1f pp.",
                       main_se * 100, ddd_se * 100,
                       2.8 * main_se * 100, 2.8 * ddd_se * 100)
  ) +
  theme_apep()

ggsave(file.path(fig_dir, "fig12_power_curve.pdf"), p12,
       width = 8, height = 6, dpi = 300)
