...
# CHANGES: CS-DiD permutation (replacing TWFE), WCB for CS-DiD estimates,
#          DDD pre-trend formal tests, balance tests, Monte Carlo attenuation
#          with multiple scenarios, cohort-specific ATTs, DDD power analysis,
#          expanded HonestDiD
##############################################################################
...
cat("\n--- Monte Carlo Attenuation Calibration ---\n")
cat("Simulating 100k women with random birth months under multiple adoption date scenarios\n")
cat("NOTE: This is a calibration exercise (numerical integration), NOT analysis data.\n")
...
for (i in seq_len(nrow(adoption_scenarios))) {
  ...
  # Simulation: woman benefits if past 60-day cliff AND extension active
  ...
  sim_exposed <- (sim_birth_month_ago >= 3) &
    (sim_cliff_month >= am | sim_survey_month >= am)

  frac_sim <- mean(sim_exposed)
  ...
  attenuation_table <- rbind(attenuation_table, data.frame(
    scenario = sc,
    adopt_month = am,
    frac_exposed_sim = round(frac_sim, 3),
    frac_exposed_analytic = round(frac_analytic, 3),
    measured_as_fraction_true = round(frac_analytic, 3),
    stringsAsFactors = FALSE
  ))

  cat(sprintf("  %s (month %d): sim=%.1f%%, analytic=%.1f%%\n",
              sc, am, frac_sim * 100, frac_analytic * 100))
}
