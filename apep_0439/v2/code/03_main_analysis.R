...
# With municipality-level controls (log_eligible + avg_turnout)
m1_controlled <- feols(yes_share ~ is_french * is_catholic + log_eligible + avg_turnout | vote_date,
                       data = gender_panel, cluster = ~mun_id)
...
main_effects <- list(
  ...
  interaction_controlled = coef(m1_controlled)["is_frenchTRUE:is_catholicTRUE"]
)
cat("\nMain effects:\n")
...
cat("  Interaction (French x Catholic):", round(main_effects$interaction, 4), "\n")
cat("  Language within canton:", round(main_effects$language_within_canton, 4), "\n")
cat("  Interaction (controlled):", round(main_effects$interaction_controlled, 4), "\n")

# Compute and store 95% confidence intervals for all main coefficients
cat("\n=== 95% Confidence Intervals ===\n")
ci_list <- list(
  m1_lang = as.data.frame(confint(m1_lang)),
  m1_relig = as.data.frame(confint(m1_relig)),
  m1_both = as.data.frame(confint(m1_both)),
  m1_interaction = as.data.frame(confint(m1_interaction)),
  m1_canton = as.data.frame(confint(m1_canton)),
  m1_controlled = as.data.frame(confint(m1_controlled))
)

# Print key CIs
ci_lang <- ci_list$m1_lang["is_frenchTRUE", ]
cat("  Language gap CI: [", round(ci_lang[[1]], 4), ",", round(ci_lang[[2]], 4), "]\n")
ci_relig <- ci_list$m1_relig["is_catholicTRUE", ]
cat("  Religion gap CI: [", round(ci_relig[[1]], 4), ",", round(ci_relig[[2]], 4), "]\n")
ci_int <- ci_list$m1_interaction["is_frenchTRUE:is_catholicTRUE", ]
cat("  Interaction CI: [", round(ci_int[[1]], 4), ",", round(ci_int[[2]], 4), "]\n")
ci_int_ctrl <- ci_list$m1_controlled["is_frenchTRUE:is_catholicTRUE", ]
cat("  Interaction (controlled) CI: [", round(ci_int_ctrl[[1]], 4), ",", round(ci_int_ctrl[[2]], 4), "]\n")
