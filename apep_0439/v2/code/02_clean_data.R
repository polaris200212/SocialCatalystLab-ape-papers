...
# Known French-speaking municipalities in Bern (Bernese Jura / Jura bernois)
french_mun_patterns_be <- c(
  "Moutier", "Court", "Reconvilier", "Tavannes", "Tramelan",
  "Saint-Imier", "Corgémont", "Cortébert", "Cormoret", "Péry",
  "La Neuveville", "Nods", "Diesse", "Lamboing",
  "Biel/Bienne", "Bienne",
  "Sonceboz", "Orvin", "Romont", "Saicourt", "Saules",
  "Grandval", "Crémines", "Eschert", "Corcelles",
  "Malleray", "Bévilard", "Pontenet", "Champoz",
  "Sorvilier", "Loveresse", "Rebévelier",
  "Sonvilier", "Renan", "Villeret", "Corgemont",
  "Plagne", "Vauffelin", "Romont"
)

# Known German-speaking municipalities in Valais (Upper Valais / Oberwallis)
german_mun_patterns_vs <- c(
  "Brig", "Visp", "Naters", "Zermatt", "Saas-Fee", "Saas-Grund",
  "Leuk", "Raron", "Stalden", "Grächen", "St. Niklaus", "Täsch",
  "Mund", "Ausserberg", "Baltschieder", "Lalden", "Eggerberg",
  "Turtmann", "Gampel", "Steg", "Niedergesteln",
  "Bratsch", "Erschmatt", "Guttet", "Feschel", "Leukerbad",
  "Brig-Glis", "Ried-Brig", "Simplon", "Zwischbergen",
  "Bitsch", "Mörel", "Filet", "Ernen", "Fiesch", "Bellwald",
  "Binn", "Grengiols", "Blitzingen", "Niederwald", "Münster",
  "Reckingen", "Gluringen", "Geschinen", "Ulrichen", "Oberwald",
  "Obergoms", "Lax", "Fieschertal",
  "Saas-Almagell", "Saas-Balen", "Eisten",
  "Embd", "Törbel", "Zeneggen", "Bürchen",
  "Unterbäch", "Eischoll", "Rarogne",
  "Ferden", "Kippel", "Wiler", "Blatten",
  "Albinen", "Inden", "Varen", "Salgesch",
  "Agarn"
)
...
votes <- votes %>%
  mutate(
    ...
    # Refine for bilingual cantons using municipality name matching
    mun_language = case_when(
      ...
      # Bern: mostly German, but Bernese Jura municipalities are French
      canton_id == 2 & mun_name %in% french_mun_patterns_be ~ "French",
      canton_id == 2 & grepl("Moutier|Court|Reconvilier|Tavannes|Tramelan|Saint-Imier|Corgémont|Cortébert|Cormoret|Péry|La Neuveville|Nods|Diesse|Lamboing|Biel/Bienne|Bienne|Sonceboz|Orvin|Grandval|Crémines|Eschert|Malleray|Bévilard|Pontenet|Champoz|Sorvilier|Loveresse|Rebévelier|Sonvilier|Renan|Villeret|Plagne|Vauffelin", mun_name, ignore.case = TRUE) ~ "French",
      canton_id == 2 ~ "German",
      # Valais: Lower Valais is French (default), Upper Valais is German
      canton_id == 23 & mun_name %in% german_mun_patterns_vs ~ "German",
      canton_id == 23 & grepl("Brig|Visp|Naters|Zermatt|Saas|Leuk|Raron|Stalden|Grächen|St\\. Niklaus|Täsch|Mund|Ausserberg|Baltschieder|Lalden|Eggerberg|Turtmann|Gampel|Steg|Niedergesteln|Bratsch|Erschmatt|Guttet|Feschel|Leukerbad|Simplon|Zwischbergen|Bitsch|Mörel|Filet|Ernen|Fiesch|Bellwald|Binn|Grengiols|Blitzingen|Niederwald|Münster|Reckingen|Gluringen|Geschinen|Ulrichen|Oberwald|Obergoms|Lax|Fieschertal|Eisten|Embd|Törbel|Zeneggen|Bürchen|Unterbäch|Eischoll|Rarogne|Ferden|Kippel|Wiler|Blatten|Albinen|Inden|Varen|Salgesch|Agarn", mun_name, ignore.case = TRUE) ~ "German",
      canton_id == 23 ~ "French",
...
# Add municipality-level controls (proxy for size/urbanization and civic engagement)
gender_index <- gender_index %>%
  mutate(
    log_eligible = log(avg_eligible + 1)
  )
...
# ============================================================================
# 6B. MERGE MUNICIPALITY CONTROLS INTO PANEL
# ============================================================================
cat("\n=== Merging municipality controls ===\n")

# Compute municipality-level controls from the full data
mun_controls <- gender_panel %>%
  group_by(mun_id) %>%
  summarize(
    log_eligible = log(mean(eligible, na.rm = TRUE) + 1),
    avg_turnout = mean(turnout, na.rm = TRUE),
    .groups = "drop"
  )

gender_panel <- gender_panel %>%
  left_join(mun_controls, by = "mun_id")

cat("  Added log_eligible and avg_turnout controls to gender panel\n")
cat("  Mean log_eligible:", round(mean(gender_panel$log_eligible, na.rm = TRUE), 2), "\n")
cat("  Mean avg_turnout:", round(mean(gender_panel$avg_turnout, na.rm = TRUE), 2), "\n")

# Also merge into falsification panel
