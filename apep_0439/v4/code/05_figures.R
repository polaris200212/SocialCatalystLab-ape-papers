...
# ============================================================================
# FIGURE 6: Ridgeline plot — distributions by culture group (replaces forest plot)
# ============================================================================
cat("=== Figure 6: Ridgeline distribution plot ===\n")
...
ridgeline_data <- fig2_data %>%
  mutate(
    culture_label = factor(culture_label,
                           levels = c("German-Catholic", "German-Protestant",
                                      "French-Catholic", "French-Protestant"))
  )

fig6 <- ggplot(ridgeline_data, aes(x = gender_index, y = culture_label,
                                    fill = culture_label)) +
  ...
  scale_x_continuous(labels = percent_format()) +
  labs(
    title = "Distribution of Gender Progressivism by Culture Group",
    subtitle = "Each ridge shows the municipality-level distribution of average yes-share across 6 referenda",
    x = "Gender Progressivism Index",
    y = NULL,
    caption = paste0("Note: N = ", nrow(ridgeline_data), " municipalities. ",
                     "Substantial overlap between groups confirms within-group heterogeneity.")
  ) +
  theme(axis.text.y = element_text(size = 10))

ggsave(file.path(fig_dir, "fig6_ridgeline.pdf"), fig6,
       width = 8, height = 5)
cat("  Saved fig6_ridgeline.pdf\n")
...
# Use the MAIN falsification model (m6_falsi from 03_main_analysis.R)
# which uses the pre-selected falsification referenda panel, NOT the extended
# random sample from robustness
falsi_comparison <- bind_rows(
  ...
  tibble(
    ...
    french = coef(results$m6_falsi)["is_frenchTRUE"],
    french_se = se(results$m6_falsi)["is_frenchTRUE"],
    interaction = coef(results$m6_falsi)["is_frenchTRUE:is_catholicTRUE"],
    interaction_se = se(results$m6_falsi)["is_frenchTRUE:is_catholicTRUE"]
  )
)
