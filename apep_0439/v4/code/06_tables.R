...
tab1_data <- gender_index %>%
  ...
  summarize(
    ...
    `N Referenda` = sprintf("%.1f", mean(n_referenda)),
    .groups = "drop"
  )
...
tab1_tex <- tab1_data %>%
  kbl(format = "latex", booktabs = TRUE,
      ...
      col.names = c("Culture Group", "Municipalities", "Gender Index",
                    "SD", "Turnout (\\%)", "Eligible Voters", "Avg. Ref.")) %>%
...
# Compute dependent variable mean for each specification's sample
dep_mean_all <- round(mean(gender_panel$yes_share, na.rm = TRUE), 3)
dep_mean_ctrl <- round(mean(gender_panel$yes_share[!is.na(gender_panel$log_eligible) & !is.na(gender_panel$avg_turnout)], na.rm = TRUE), 3)
...
tab2_tex <- etable(
  ...
  fitstat = c("n", "r2", "ar2"),
  extralines = list(
    `Dep. var. mean` = c(dep_mean_all, dep_mean_all, dep_mean_all,
                         dep_mean_all, dep_mean_all, dep_mean_ctrl)
  ),
  notes = paste0(
...
# Convert all values to percentage points for consistency with other tables
obs_lang_pp <- robustness$permutation$obs_lang * 100
obs_int_pp <- robustness$permutation$obs_interaction * 100
perm_lang_pp <- robustness$permutation$perm_lang_coefs * 100
perm_int_pp <- robustness$permutation$perm_coefs * 100

# Canton-level permutation p-value (new in v4)
perm_p_canton_str <- if (!is.null(robustness$canton_permutation)) {
  round(robustness$canton_permutation$perm_p_canton, 3)
} else {
  "---"
}
...
tab5_tex <- paste0(
  ...
  " & & Municipality & Canton \\\\\n",
  "\\midrule\n",
  "Observed coefficient (pp) & ", sprintf("%.2f", obs_lang_pp),
  " & ", sprintf("%.2f", obs_int_pp),
  " & ", sprintf("%.2f", obs_int_pp), " \\\\\n",
  ...
  " & ", if (!is.null(robustness$canton_permutation)) length(robustness$canton_permutation$perm_canton_coefs) else "---", " \\\\\n",
  "Permutations exceeding observed & ", robustness$permutation$n_exceed_lang,
  " & ", robustness$permutation$n_exceed_interaction,
  " & ", if (!is.null(robustness$canton_permutation)) robustness$canton_permutation$n_exceed_canton else "---", " \\\\\n",
  "Permutation mean (pp) & ",
  sprintf("%.2f", mean(perm_lang_pp)),
  " & ", sprintf("%.2f", mean(perm_int_pp)),
  " & ", if (!is.null(robustness$canton_permutation)) sprintf("%.2f", mean(robustness$canton_permutation$perm_canton_coefs * 100)) else "---", " \\\\\n",
  "Permutation SD (pp) & ",
  sprintf("%.2f", sd(perm_lang_pp)),
  " & ", sprintf("%.2f", sd(perm_int_pp)),
  " & ", if (!is.null(robustness$canton_permutation)) sprintf("%.2f", sd(robustness$canton_permutation$perm_canton_coefs * 100)) else "---", " \\\\\n",
  "\\bottomrule\n",
  "\\end{tabular}\n",
  "\\begin{minipage}{0.9\\textwidth}\n",
  "\\footnotesize\n",
  "\\textit{Notes:} All values in percentage points. Permutation inference based on 500 random reassignments. ",
  "Municipality-level permutation shuffles language and religion labels across municipalities independently. ",
  "Canton-level permutation shuffles confessional status at the canton level, respecting the historical ",
  "assignment mechanism, while keeping municipality-level language labels fixed. ",
  "Two-sided $p$-values report the fraction of permuted coefficients with absolute value at least as large ",
  "as the observed coefficient. Reported as $< 1/N$ when no permutation exceeds the observed value.\n",
  "\\end{minipage}\n",
  "\\end{table}\n"
)
...
# ============================================================================
# TABLE 7: Covariate Balance Across Culture Groups
# ============================================================================
cat("=== Table 7: Covariate Balance ===\n")

# Show means of pre-determined covariates across the 4 culture groups
balance_data <- gender_index %>%
  filter(culture_group %in% c("French-Protestant", "French-Catholic",
                               "German-Protestant", "German-Catholic")) %>%
  group_by(culture_group) %>%
  summarize(
    N = n(),
    `Log Population` = round(mean(log_eligible, na.rm = TRUE), 2),
    `Avg. Turnout (\\%)` = round(mean(avg_turnout, na.rm = TRUE), 1),
    `Avg. Eligible` = round(mean(avg_eligible, na.rm = TRUE), 0),
    `N Referenda` = round(mean(n_referenda, na.rm = TRUE), 2),
    .groups = "drop"
  )

tab7_tex <- balance_data %>%
  kbl(format = "latex", booktabs = TRUE,
      caption = "Covariate Balance Across Culture Groups",
      label = "balance",
      col.names = c("Culture Group", "N", "Log Pop.",
                    "Turnout (\\%)", "Eligible Voters", "Avg. Ref."),
      escape = FALSE) %>%
  kable_styling(latex_options = "hold_position") %>%
  footnote(general = paste0(
    "Pre-determined municipality characteristics by culture group. ",
    "Log population is the log of average eligible voters across referenda. ",
    "Turnout is the average turnout rate across referenda. ",
    "Eligible voters is the mean number of eligible voters. ",
    "Avg. Ref. is the average number of referenda observed per municipality (max 6). ",
    "All variables are municipality-level averages, not referendum-level."
  ), threeparttable = TRUE)

writeLines(tab7_tex, file.path(tab_dir, "tab7_balance.tex"))
cat("  Saved tab7_balance.tex\n")

# ============================================================================
# TABLE 6 ENHANCEMENT: Add BH q-values column for interaction
# ============================================================================
cat("=== Enhancing Table 6 with BH q-values ===\n")

# If BH-adjusted results are available, add q-value column to Table 6
if (!is.null(robustness$ref_bh_adjusted)) {
  bh_data <- robustness$ref_bh_adjusted

  # Rebuild Table 6 with q-value column
  tab6_data_v2 <- time_gaps %>%
    filter(!is.na(lang_gap)) %>%
    left_join(
      bh_data %>% select(vote_date, bh_qval),
      by = "vote_date"
    ) %>%
    mutate(
      lang_p = 2 * pnorm(-abs(lang_gap / lang_se)),
      ...
      int_stars = ifelse(is.na(int_p), "",
                         ifelse(int_p < 0.01, "***",
                                ifelse(int_p < 0.05, "**",
                                       ifelse(int_p < 0.10, "*", "")))),
      Referendum = format(vote_date, "%b %d, %Y"),
      `Language Gap` = paste0(sprintf("%.1f", lang_gap * 100), lang_stars),
      `Lang SE` = paste0("(", sprintf("%.1f", lang_se * 100), ")"),
      `Religion Gap` = ifelse(!is.na(relig_gap),
                              paste0(sprintf("%.1f", relig_gap * 100), relig_stars), "---"),
      `Relig SE` = ifelse(!is.na(relig_se),
      ...
      `Int SE` = ifelse(!is.na(interaction_se),
                        paste0("(", sprintf("%.1f", interaction_se * 100), ")"), ""),
      `BH q` = ifelse(!is.na(bh_qval), sprintf("%.3f", bh_qval), "---"),
      N = format(n, big.mark = ",")
    )
  ...
  tab6_tex_lines_v2 <- c(
    ...
    " & \\multicolumn{2}{c}{Language Gap} & \\multicolumn{2}{c}{Religion Gap} & \\multicolumn{2}{c}{Interaction} & & \\\\",
    "\\cmidrule(lr){2-3} \\cmidrule(lr){4-5} \\cmidrule(lr){6-7}",
    "Referendum & Coef & (SE) & Coef & (SE) & Coef & (SE) & BH $q$ & N \\\\"
  )
  tab6_tex_lines_v2 <- c(tab6_tex_lines_v2, "\\midrule")

  for (i in seq_len(nrow(tab6_data_v2))) {
    row <- tab6_data_v2[i, ]
    line <- paste0(
      row$Referendum, " & ",
      row$`Language Gap`, " & ", row$`Lang SE`, " & ",
    ...
    tab6_tex_lines_v2 <- c(tab6_tex_lines_v2, line)
  }

  tab6_tex_lines_v2 <- c(tab6_tex_lines_v2,
    "\\bottomrule",
    "\\end{tabular}",
    "\\begin{minipage}{0.95\\textwidth}",
    "\\footnotesize",
    paste0("\\textit{Notes:} All values in percentage points. ",
           "Language gap: French minus German yes-share. ",
           "Religion gap: Catholic minus Protestant yes-share. ",
           "Interaction: French $\\times$ Catholic coefficient. ",
           "BH $q$: Benjamini-Hochberg adjusted $q$-values for the six interaction tests, ",
           "controlling the false discovery rate across referenda. ",
           "Standard errors in parentheses from OLS with municipality-level data. ",
           "* $p<0.10$, ** $p<0.05$, *** $p<0.01$."),
    "\\end{minipage}",
    "\\end{table}"
  ...
  writeLines(tab6_tex_lines_v2, file.path(tab_dir, "tab6_time_gaps.tex"))
  cat("  Updated tab6_time_gaps.tex with BH q-values\n")
}
