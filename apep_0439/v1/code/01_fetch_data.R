...
# Download Swiss municipality boundaries from BFS
# Using the ThemaKart API for official boundaries
...
canton_religion <- tibble(
  ...
  hist_religion = c(
    ...
    "Catholic",    # SO - Catholic (initially, complex history)
    "Protestant",  # BS - Reformed 1529
    "Protestant",  # BL - Reformed
    "Protestant",  # SH - Reformed 1529
    "Protestant",  # AR - Protestant
    "Catholic",    # AI - Catholic
    "Catholic",    # SG - Mixed, Catholic majority historically
    "Mixed",       # GR - Mixed (Three Leagues, complex)
    "Mixed",       # AG - Mixed (Baden Catholic, former Bern Protestant)
    "Protestant",  # TG - Protestant majority
...
# Try BFS package for municipal population data
tryCatch({
  catalog <- bfs_get_catalog_data(language = "en")

  # Find population by municipality dataset
  pop_datasets <- catalog[grepl("population.*commune|bevölkerung.*gemeinde|permanent.*resident",
                                 catalog$title, ignore.case = TRUE), ]
  cat("  Found", nrow(pop_datasets), "population datasets\n")

  if (nrow(pop_datasets) > 0) {
    cat("  Titles:\n")
    for (i in seq_len(min(5, nrow(pop_datasets)))) {
      cat("    ", pop_datasets$title[i], "\n")
    }

    # Try to download the first relevant dataset
    pop_data <- tryCatch({
      bfs_get_data(number_bfs = pop_datasets$number_bfs[1], language = "en")
    }, error = function(e) {
    ...
    if (!is.null(pop_data)) {
      cat("  Downloaded population data:", nrow(pop_data), "rows\n")
      saveRDS(pop_data, file.path(data_dir, "population_raw.rds"))
    }
  }
}, error = function(e) {
...
# ============================================================================
# 5. MARRIAGE / DIVORCE STATISTICS (BFS)
# ============================================================================
cat("\n=== Fetching vital statistics ===\n")

tryCatch({
  catalog <- bfs_get_catalog_data(language = "en")

  # Search for marriage/divorce data
  vital_ds <- catalog[grepl("marriage|divorce|heirat|scheidung",
                            catalog$title, ignore.case = TRUE), ]
  cat("  Found", nrow(vital_ds), "vital statistics datasets\n")

  if (nrow(vital_ds) > 0) {
    for (i in seq_len(min(3, nrow(vital_ds)))) {
      cat("    ", vital_ds$title[i], "\n")
    }
  }
}, error = function(e) {
  cat("  Vital stats error:", e$message, "\n")
})
