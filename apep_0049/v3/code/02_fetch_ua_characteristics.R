...
# Fetch 2020 population for urban areas
ua_pop_response <- GET(
  census_2020_url,
  query = list(
    get = "NAME,P1_001N",  # P1_001N is total population in 2020 PL94
    `for` = "urban area:*"
  )
)

if (status_code(ua_pop_response) == 200) {
  ...
  stop("Failed to fetch 2020 Census population data: ", status_code(ua_pop_response))
}
