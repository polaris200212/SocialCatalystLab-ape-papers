...
\title{Does Federal Transit Funding Improve Local Labor Markets? \\ Evidence from a Population Threshold\footnote{This paper is a revision of APEP-0129. See \url{https://github.com/SocialCatalystLab/auto-policy-evals/tree/main/papers/apep_0129} for the previous version.}}
...
This paper provides the first causal evidence on whether federal transit formula grants improve local outcomes. Exploiting a sharp statutory discontinuity at the 50,000 population threshold for FTA Section 5307 eligibility, I find precise null effects: crossing the threshold has no detectable impact on transit ridership, employment, vehicle ownership, or commute times. The confidence intervals are narrow enough to rule out effects larger than 1 percentage point. This finding is informative for policy: it suggests that population-based eligibility thresholds in federal grant programs may not effectively target resources where they can generate benefits.

The Federal Transit Administration distributes over \$5 billion annually through the Urbanized Area Formula Program (Section 5307). Eligibility depends on Census Bureau classification as an ``urbanized area,'' which requires population of at least 50,000. This creates a sharp discontinuity: an area with 49,999 residents receives zero Section 5307 funding, while an area with 50,001 becomes eligible for formula grants of roughly \$1.5--2.5 million annually. The threshold is statutory, determined by federal enumeration rather than local application, making it attractive for regression discontinuity analysis.
...
Second, the first stage from eligibility to funding is not directly observable with available data. While eligibility is perfectly sharp by statute, actual fund utilization depends on local capacity, matching funds, and project readiness. Data on obligations and expenditures at the urban area level would permit estimation of a fuzzy RD, but such data are not readily available.

Third, the ITT interpretation is consistent with the null finding. If some eligible areas successfully utilize funding while others do not, and if utilization produces effects, then the ITT would still be positive (just attenuated relative to TOT). The null ITT suggests either that effects are zero conditional on utilization, or that utilization rates near the threshold are low enough to make average effects undetectable.

Future work with administrative data on fund utilization could distinguish these explanations by estimating the first stage and, if take-up is substantial, a fuzzy RD that scales the ITT by the first stage.
...
The most likely explanation for the null is straightforward: funding at the margin is too small to matter. Section 5307 formula grants for a 50,000-person area amount to roughly \$1.5--2.5 million annually---enough to purchase 3--5 transit buses over several years, but not enough to transform service. A single bus costs \$300,000--500,000, and operating expenses consume resources quickly. Compare this to the interventions that have shown detectable effects: Bogot\'{a}'s TransMilenio involved \$240 million in initial capital investment \citep{tsivanidis2023evaluating}, and the Los Angeles Metro represents billions in cumulative spending \citep{severen2021commuting}. Annual formula funding for a small urbanized area is orders of magnitude smaller.
...
With robust standard errors of approximately 0.43 percentage points for transit share, the design can detect effects of about 0.85 pp with 80\% power at the 5\% level. This represents roughly 115\% of the sample mean---the design rules out doublings of ridership but could miss smaller effects.

For context, 0.85 percentage points represents about 115\% of the sample mean transit share. The design can rule out doublings of transit ridership but may miss smaller effects---say, 20--50\% increases---that would be policy-relevant. Whether such modest effects would justify the billions of dollars in federal transit funding is a separate policy question.

For employment, the minimum detectable effect is approximately 1.6 percentage points. Given mean employment rate of 94.1\%, this represents detection of changes in the unemployment rate of about 1.6 percentage points---a meaningful but not enormous labor market effect.
