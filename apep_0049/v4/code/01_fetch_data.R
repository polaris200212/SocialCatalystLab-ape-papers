...
# 01_fetch_data.R - Fetch Census and ACS data for RDD analysis
#
# This script fetches:
#   1. 2010 Decennial Census urbanized area populations (running variable)
#   2. 2016-2020 ACS 5-year outcomes at the urban area level
#
# Design: 2010 Census classification -> FY2012+ eligibility -> 2016-2020 ACS outcomes
# FTA apportionment data is fetched separately in 01b_fetch_fta_data.R
# ============================================================================
...
ua_response_2010 <- NULL
for (attempt in 1:5) {
  query_params <- list(get = "NAME,P001001", `for` = "urban area:*")
  if (nchar(census_key) > 0) query_params$key <- census_key
  ua_response_2010 <- tryCatch(
    GET(census_url_2010, query = query_params, timeout(60)),
    error = function(e) NULL
  )
  if (!is.null(ua_response_2010) && status_code(ua_response_2010) == 200) break
  wait_secs <- 30 * attempt
  cat(sprintf("  Attempt %d failed (status %s), retrying in %ds...\n",
              attempt, ifelse(is.null(ua_response_2010), "error",
                              status_code(ua_response_2010)), wait_secs))
  Sys.sleep(wait_secs)
}

if (!is.null(ua_response_2010) && status_code(ua_response_2010) == 200) {
  ...
  stop("Failed to fetch 2010 Census data after 5 attempts. Last status: ",
       ifelse(is.null(ua_response_2010), "connection error", status_code(ua_response_2010)))
}
...
# Commuting variables - with retry
acs_vars <- paste0(
  "NAME,",
...
acs_response <- NULL
for (attempt in 1:5) {
  acs_query <- list(get = acs_vars, `for` = "urban area:*")
  if (nchar(census_key) > 0) acs_query$key <- census_key
  acs_response <- tryCatch(
    GET(acs_url, query = acs_query, timeout(60)),
    error = function(e) NULL
  )
  if (!is.null(acs_response) && status_code(acs_response) == 200) break
  wait_secs <- 30 * attempt
  cat(sprintf("  ACS attempt %d failed (status %s), retrying in %ds...\n",
              attempt, ifelse(is.null(acs_response), "error",
                              status_code(acs_response)), wait_secs))
  Sys.sleep(wait_secs)
}

if (!is.null(acs_response) && status_code(acs_response) == 200) {
  ...
  stop("Failed to fetch ACS data after 5 attempts. Last status: ",
       ifelse(is.null(acs_response), "connection error", status_code(acs_response)))
}
...
# NOTE ON SAMPLE SELECTION: We drop areas with missing ACS outcomes.
# This is NOT outcome-based selection bias because:
# 1. Missing values arise when ACS cannot produce reliable estimates for very small areas
#    (populations below ACS geographic publication thresholds)
# 2. These areas are FAR from the 50k threshold - the smallest urban clusters
# 3. Missingness is determined by population size (running variable), not by outcome values
# 4. We do NOT drop based on outcome values, only on ACS publication availability
# The main analysis bandwidth (populations 25k-75k) has near-complete coverage.
ua_final <- ua_merged %>%
