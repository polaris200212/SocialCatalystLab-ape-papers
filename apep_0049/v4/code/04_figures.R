...
# v4 additions: Real FTA funding first-stage figure
# ============================================================================
...
# ============================================================================
# 8. FIGURE 7: FIRST STAGE - REAL FTA APPORTIONMENT DATA
# ============================================================================

cat("Creating Figure 7: First stage (real FTA funding data)...\n")
...
if (file.exists(fta_file)) {
  ...
  # Create binned scatter of per-capita funding by population
  fta_plot_data <- fta_data %>%
    filter(abs(running_var) <= 30000, !is.na(fta_per_capita)) %>%
    mutate(bin = cut(running_var, breaks = seq(-30000, 30000, by = 3000),
                     include.lowest = TRUE)) %>%
    group_by(bin) %>%
    summarise(
      mean_x = mean(running_var),
      mean_y = mean(fta_per_capita, na.rm = TRUE),
      n = n(),
      se = sd(fta_per_capita, na.rm = TRUE) / sqrt(n())
    ) %>%
    filter(!is.na(bin), !is.na(mean_y))

  fig7 <- ggplot() +
    geom_point(data = fta_plot_data, aes(x = mean_x / 1000, y = mean_y),
               color = "grey50", size = 3, alpha = 0.8) +
    geom_errorbar(data = fta_plot_data,
                  aes(x = mean_x / 1000,
                      ymin = mean_y - 1.96 * se,
                      ymax = mean_y + 1.96 * se),
                  color = "grey50", width = 0.5, alpha = 0.5) +
    geom_smooth(data = fta_data %>% filter(running_var < 0, abs(running_var) <= 30000),
                aes(x = running_var / 1000, y = fta_per_capita),
                method = "loess", formula = y ~ x, span = 0.8,
                color = apep_colors[2], fill = apep_colors[2],
                alpha = 0.2, linewidth = 1.2) +
    geom_smooth(data = fta_data %>% filter(running_var >= 0, abs(running_var) <= 30000,
                                           !is.na(fta_per_capita)),
                aes(x = running_var / 1000, y = fta_per_capita),
                method = "loess", formula = y ~ x, span = 0.8,
    ...
    geom_vline(xintercept = 0, linetype = "dashed", color = "grey30", linewidth = 0.8) +
    labs(
      title = "First Stage: Actual FTA Section 5307 Per-Capita Funding",
      subtitle = "Sharp discontinuity in federal transit funding at 50,000 population threshold",
      x = "Population Relative to 50,000 Threshold (thousands)",
      y = "FTA Section 5307 Funding Per Capita ($)",
      caption = "Source: FTA Table 3 apportionments, FY 2020. Urban clusters below threshold receive $0."
    ) +
    theme_apep() +
    annotate("text", x = -15, y = 10, label = "Urban Clusters\n$0 formula funding",
  ...
  cat("  WARNING: FTA data not found. Run 01b_fetch_fta_data.R first.\n")
  cat("  Creating statutory eligibility figure as fallback...\n")

  # Fallback: binary eligibility figure
...
cat("  - fig7_first_stage.png (real FTA apportionment data)\n")
