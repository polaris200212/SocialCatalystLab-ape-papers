...
## Changes from v1: 6 specialties (NPs folded in), joint F-test,
## mean dep var, MD-only appendix results
## ============================================================
...
# Restrict to counties with population data and US states
...
cat("  After restrictions (all clinicians):", nrow(panel), "\n")
cat("  After restrictions (MD/DO only):", nrow(panel_mdonly), "\n")
...
sumstats <- panel[, .(
  ...
  mean_per_10k_med = round(mean(providers_per_10k_medicaid, na.rm = TRUE), 3),
  pct_desert = round(100 * mean(is_desert, na.rm = TRUE), 1),
  mean_dep_var = round(mean(ln_providers), 3)
), by = specialty]

cat("Summary by specialty (all clinicians):\n")
...
m1 <- feols(ln_providers ~ treat_intensity |
            ...
            data = panel,
            cluster = ~state_fips)
cat("Model 1 (pooled, all clinicians):\n")
print(summary(m1))
...
pre_rows <- grepl("rel_quarter::-[2-8]", rownames(es_coefs))
if (sum(pre_rows) > 0) {
  pre_f <- tryCatch({
    wald(m_es, keep = rownames(es_coefs)[pre_rows])
  }, error = function(e) {
...
cat("\n=== MD/DO-Only Results (Appendix) ===\n")

m1_mdonly <- feols(ln_providers ~ treat_intensity |
                     cs_id + quarter,
                   data = panel_mdonly,
                   cluster = ~state_fips)
cat("MD/DO-only pooled:\n")
cat("  Coef:", round(coef(m1_mdonly)["treat_intensity"], 4),
    "  SE:", round(se(m1_mdonly)["treat_intensity"], 4), "\n")

m2_mdonly <- feols(ln_providers ~ treat_intensity |
                     cs_id + quarter,
                   data = panel_mdonly,
                   cluster = ~state_fips,
                   split = ~specialty)
m2_mdonly_list <- list()
for (i in seq_along(m2_mdonly)) {
  nm <- names(m2_mdonly)[i]
  m2_mdonly_list[[nm]] <- m2_mdonly[[i]]
  cat("  ", nm, ": Coef:", round(coef(m2_mdonly[[i]])["treat_intensity"], 4),
      "  SE:", round(se(m2_mdonly[[i]])["treat_intensity"], 4), "\n")
}

cat("\n=== Mean Dependent Variable by Specialty ===\n")
mean_depvar <- panel[, .(
  mean_ln_providers = round(mean(ln_providers), 3),
  mean_n_providers = round(mean(n_providers), 2),
  mean_desert = round(mean(is_desert, na.rm = TRUE), 3)
), by = specialty]
print(mean_depvar)

cat("\n=== Save Results ===\n")

# Extract SA ATT values before saving (avoid re-evaluation issues)
sa_agg_saved <- summary(m_sa, agg = "ATT")
sa_att_coef <- as.numeric(coef(sa_agg_saved))
sa_att_se <- as.numeric(se(sa_agg_saved))
sa_att_n <- nobs(m_sa)
cat("  SA ATT for robustness table:", sa_att_coef, "SE:", sa_att_se, "N:", sa_att_n, "\n")

results <- list(
  m1 = m1, m2 = m2_list, m3 = m3,
  m4_urban = m4_urban, m4_rural = m4_rural,
  m_sa = m_sa,
  sa_att_coef = sa_att_coef, sa_att_se = sa_att_se, sa_att_n = sa_att_n,
  m_es = m_es, es_results = es_results,
  sumstats = sumstats, trends = trends,
  m1_mdonly = m1_mdonly, m2_mdonly = m2_mdonly_list,
