...
# Prefer full-panel (incl. post-treatment 2020-2023) if available
...
if (!is.null(heart_es_file)) {
  ...
  cat("  Heart placebo source:", heart_es_file, "\n")
}
...
if (!is.null(cancer_es_file)) {
  ...
  cat("  Cancer placebo source:", cancer_es_file, "\n")
}
...
if (length(placebo_panels) > 0) {
  ...
  fig5 <- ggplot(placebo_combined, aes(x = event_time, y = att)) +
    ...
    labs(
      ...
      subtitle = paste0(
        "Callaway-Sant'Anna event studies on placebo outcomes",
        ifelse(file.exists("data/placebo_heart_event_study_full.rds"),
               " (full panel incl. 2020-2023)", " (1999-2017)")
      ),
      x = "Years Relative to Copay Cap Implementation",
      y = "ATT",
      caption = paste0(
        ...
        ifelse(file.exists("data/placebo_heart_event_study_full.rds"),
               "Placebo data covers full panel (1999-2023) including post-treatment periods.",
               "Placebo data covers 1999-2017 only."),
        " Null effects confirm identification."
      )
    ) +
    theme_apep()
