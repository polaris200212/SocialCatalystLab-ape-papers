\begin{table}[htbp]
\centering
\caption{Pre-Treatment Balance: Treated vs.\ Control States (1999--2017)}
\label{tab:balance}
\begin{tabular}{lcc}
\hline\hline
 & Ever-Treated & Never-Treated \\
\hline
Diabetes Mortality (All Ages) & 23.50 & 23.35 \\
 \quad (SD) & (5.37) & (4.71) \\
Heart Disease Mortality & 192.31 & 200.75 \\
Cancer Mortality (All Ages) & 176.31 & 179.77 \\
N States & 17 & 34 \\
N State-Years & 323 & 646 \\
\hline\hline
\end{tabular}
\begin{tablenotes}
\small
\item \textit{Notes:} Pre-treatment period: 1999--2017 (NCHS historical data, before any state adopted caps; 2018--2019 not available from either CDC source).
\item All rates per 100,000 population. Source: CDC WONDER.
\end{tablenotes}
\end{table}
