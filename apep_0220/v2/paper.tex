...
\title{Who Believes God Forgives? Divine Punishment Beliefs Across Cultures and Economies\thanks{This paper is a revised version of APEP Working Paper apep\_0220. The original paper is available at \url{https://github.com/SocialCatalystLab/ape-papers/tree/main/apep_0220}.}}
...
The stakes are economic, not just theological. Beliefs about whether God punishes transgressors or forgives sinners correlate with risk aversion \citep{hoffmann2013}, social trust \citep{shariff2011}, tax compliance \citep{torgler2006}, cooperative behavior in economic games \citep{norenzayan2013}, and support for redistribution \citep{scheve2006}. The ``Big Gods'' hypothesis in cultural evolution holds that belief in moralizing, punitive deities was a prerequisite for the emergence of large-scale cooperation and complex societies \citep{norenzayan2013, botero2014}. If true, understanding the distribution and correlates of these beliefs is foundational for political economy.
...
This paper bridges these literatures with three contributions. \textit{First}, we integrate every freely accessible dataset measuring divine punishment or forgiveness---five sources spanning individual surveys, ethnographic records, and historical polities---into a unified portrait. No prior study has combined these heterogeneous sources. \textit{Second}, we estimate correlates of divine belief at both individual and cross-cultural levels: demographics and religious characteristics in the GSS, and societal complexity in the Standard Cross-Cultural Sample. \textit{Third}, we catalog all known datasets on divine temperament beliefs, including six restricted-access sources, as a resource for future research.

The analysis reveals three patterns. In the United States, divine forgiveness is nearly universal among the religious, while divine punishment concentrates among the less educated and lower-income---inconsistent with a ``more religious = more punitive God'' model and suggestive of religious distress rather than devotion. Cross-culturally, moralizing high gods track political complexity and subsistence mode, consistent with the hypothesis that supernatural enforcement scales with societal scale. Finally, measurement profoundly shapes conclusions: the same society can appear ``punitive'' or ``forgiving'' depending on whether one codes high gods, afterlife beliefs, or experienced divine relationships.

\subsection{Related Literature}

This paper contributes to several literatures.

The \textit{economics of religion} has grown rapidly since the foundational work of \citet{iannaccone1992} on rational choice models of religious participation and \citet{barro2003} on religion and economic growth. Recent contributions examine how religious beliefs affect economic preferences \citep{benjamin2016}, labor supply \citep{gruber2004}, education \citep{botticini2012}, and political attitudes \citep{scheve2006}. Quasi-experimental evidence from \citet{clingingsmith2009} demonstrates that the Hajj pilgrimage increases religious tolerance; \citet{campante2015} exploit Ramadan timing variation to show that religious practice affects economic growth and subjective well-being; and \citet{bentzen2019} shows that natural disasters increase religiosity globally, suggesting that exogenous shocks to existential security shape belief formation. Within this literature, the specific content of belief---not just intensity of religiosity---has received increasing attention. \citet{shariff2011} show that belief in hell (but not heaven) predicts lower national crime rates, suggesting that the \textit{punitive} dimension of religion carries distinct behavioral implications.

The \textit{cultural evolution} literature on moralizing gods, spearheaded by \citet{norenzayan2013}, argues that belief in gods who monitor and punish moral transgressions facilitated the emergence of large-scale cooperation beyond kinship networks. Empirical tests using cross-cultural databases \citep{botero2014, peoples2016, whitehouse2019} find that moralizing gods are more common in larger, more complex societies---paralleling \citet{nunn2012}'s emphasis on geography's role in shaping institutional development---though the direction of causality remains debated \citep{beheim2021, slingerland2020}. A landmark experimental study by \citet{purzycki2016} demonstrates across eight diverse populations that belief in moralistic, knowledgeable gods predicts greater impartiality in economic games, providing micro-level behavioral evidence for the cultural evolution hypothesis. Our analysis contributes by examining these patterns across multiple databases simultaneously and by connecting the ethnographic coding to individual-level survey measurement.
...
\subsection{General Social Survey (GSS), 1972--2024 (Belief Modules: 1991--2018)}
...
Table \ref{tab:summary_stats} reports summary statistics for all variables. Panel A shows the GSS individual-level data: the cumulative file contains 75,699 respondents, but the divine belief modules (COPE4, FORGIVE3, God-image battery) were fielded only in selected years, yielding substantially smaller analytic samples of 1,400--4,800. The belief variables are binary or ordinal; real income exhibits substantial right skew (mean \$26,483, SD \$26,773). Panel B summarizes the four cross-cultural databases. The EA provides the largest ethnographic coverage (775 societies with non-missing high gods data), while Seshat offers the only temporal dimension (348 polity-periods across 33 Natural Geographic Areas).
...
The most direct measures of divine temperament are COPE4 (``I feel that God is punishing me for my sins,'' coded 1 = a great deal to 4 = not applicable) and FORGIVE3 (``I know that God forgives me,'' coded 1 = a great deal to 4 = not applicable), asked to approximately 1,400 respondents in the 1998 GSS. The gap is dramatic: 79\% report knowing God forgives them (responding ``a great deal'' or ``somewhat,'' i.e., FORGIVE3 $\leq$ 2), while only 17\% report feeling divinely punished (COPE4 $\leq$ 2). Table \ref{tab:summary_stats} reports the raw ordinal means (FORGIVE3 mean = 1.39, COPE4 mean = 3.69), which confirm the same pattern: forgiveness responses cluster near the floor (1 = strong agreement), while punishment responses cluster near the ceiling (4 = not applicable). This is not a ceiling effect of religious participation---even among weekly attenders, fewer than 25\% feel punished while over 90\% report forgiveness.
...
\input{tables/tab_gss_beliefs_religion.tex}
...
\input{tables/tab_ea_region.tex}
...
These are \textit{correlational} estimates, not causal effects \citep{angrist2009}. The goal is to identify systematic patterns that generate hypotheses for future work with credible identification strategies, ideally paired with sensitivity analysis for unobserved confounders \citep{oster2019}.

\input{tables/tab_gss_regressions.tex}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig_coef_plot.pdf}
\caption{Coefficient estimates from OLS regressions of divine belief measures on individual characteristics (Table \ref{tab:gss_regressions}). Points show coefficient estimates; horizontal bars show 95\% confidence intervals based on heteroskedasticity-robust standard errors. Dependent variables differ in scale: Heaven, Hell, and Afterlife are binary (0--1); Forgive and Cope are ordinal (1--4). The college degree coefficient is negative for afterlife beliefs but near zero for forgiveness, illustrating education's selective association with lower punitive beliefs.}
\label{fig:coef_plot}
\end{figure}

Table \ref{tab:gss_regressions} reports heteroskedasticity-robust standard errors with 95\% confidence intervals throughout. Figure \ref{fig:coef_plot} visualizes the key coefficients. The most striking result is the education-punishment gap: college graduates are significantly less likely to feel divinely punished, yet no less likely to believe in a forgiving God.

\textit{Education.} College education is negatively correlated with belief in heaven ($-$9 pp) and hell ($-$11 pp), and with less divine punishment experience (COPE4 coefficient $+0.10$, where higher values indicate less punishment). The effect on divine forgiveness (FORGIVE3) is small and insignificant. Education thus appears selectively associated with lower punitive beliefs while leaving benevolent ones intact.
...
\textit{Tax compliance.} \citet{torgler2006} finds that religiosity predicts tax morale, but our results suggest a more nuanced mechanism. The forgiveness-punishment gap implies that for most religious Americans, the operative channel is \textit{not} fear of divine retribution for tax evasion. Among the 79\% who experience God as primarily forgiving, the deterrence mechanism is weak: a forgiving God is unlikely to condemn you for underreporting deductions. But among the 17\% who feel divinely punished---disproportionately lower-income and less educated---the effective tax on noncompliance includes a spiritual penalty. This generates a regressive pattern: divine enforcement supplements state enforcement most effectively for those who are already economically vulnerable, precisely the group with the least to gain from evasion.
...
\input{tables/tab_datasets_overview.tex}
...
\input{tables/tab_restricted_datasets.tex}
...
\input{tables/tab_sccs_correlations.tex}
