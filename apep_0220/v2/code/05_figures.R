...
###############################################################################
# Figure 9: Coefficient Plot for GSS Regressions
###############################################################################
cat("\n=== Figure 9: Coefficient Plot ===\n")

reg_results_file <- file.path(data_dir, "regression_results.rds")
if (file.exists(reg_results_file)) {
  reg_results <- readRDS(reg_results_file)
  ...
  # Filter to key covariates (exclude intercept)
  plot_data <- reg_results %>%
    filter(term != "(Intercept)") %>%
    mutate(
      term_label = ifelse(term %in% names(label_map), label_map[term], term),
      term_label = factor(term_label,
                          levels = rev(c("Age", "Female", "College degree",
                                         "Log(income)", "Attendance",
                                         "Catholic", "Jewish",
                                         "Other religion", "No religion"))),
      model = factor(model, levels = c("Heaven", "Hell", "Afterlife",
                                        "Forgive", "Cope"))
    )

  p9 <- ggplot(plot_data, aes(x = estimate, y = term_label, color = model, shape = model)) +
    geom_vline(xintercept = 0, linetype = "dashed", color = "grey50", linewidth = 0.4) +
    geom_pointrange(aes(xmin = ci_low, xmax = ci_high),
                    position = position_dodge(width = 0.6),
                    size = 0.4, linewidth = 0.5) +
    scale_color_manual(
      values = c("Heaven" = "#2166AC", "Hell" = "#B2182B",
                 "Afterlife" = "#4393C3", "Forgive" = "#1B7837",
                 "Cope" = "#762A83"),
      name = "Dependent Variable"
    ) +
    scale_shape_manual(
      values = c("Heaven" = 16, "Hell" = 15, "Afterlife" = 17,
                 "Forgive" = 18, "Cope" = 4),
      name = "Dependent Variable"
    ) +
    labs(
      title    = "Coefficient Estimates: Determinants of Divine Belief",
      subtitle = "OLS with heteroskedasticity-robust 95% CIs (Table 4)",
  ...
  ggsave(file.path(fig_dir, "fig_coef_plot.pdf"), p9,
         width = 9, height = 7, device = cairo_pdf)
  cat("Saved fig_coef_plot.pdf\n")
} else {
  cat("WARNING: regression_results.rds not found. Run 06_tables.R first.\n")
}
