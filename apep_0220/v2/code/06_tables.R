...
if (length(models) > 0) {
  ...
  # Compute 95% CIs using robust SEs
  robust_ci_low <- lapply(seq_along(model_list), function(i) {
    coef(model_list[[i]]) - 1.96 * robust_ses[[i]]
  })
  robust_ci_high <- lapply(seq_along(model_list), function(i) {
    coef(model_list[[i]]) + 1.96 * robust_ses[[i]]
  })
  ...
  stargazer(
    ...
    out = reg_file,
    se = robust_ses,
    ci.custom = lapply(seq_along(model_list), function(i) {
      cbind(robust_ci_low[[i]], robust_ci_high[[i]])
    }),
    ...
    report = "vc*s",
    notes = c(
      "Heteroskedasticity-robust 95\\% confidence intervals in parentheses.",
      "Reference category: Protestant. * p<0.05, ** p<0.01, *** p<0.001.",
      "Heaven, Hell, Afterlife: binary (1 = yes definitely/probably); positive coefficient = stronger belief.",
      "Forgive (FORGIVE3): 1 = a great deal, 4 = not applicable; positive coefficient = less forgiveness.",
      "Cope (COPE4): 1 = a great deal, 4 = not applicable; positive coefficient = less divine punishment."
    ),
    notes.align = "l",
    notes.append = FALSE
  )
  ...
  # Save regression results for coefficient plot
  reg_results <- lapply(names(models), function(nm) {
    m <- models[[nm]]
    ...
    data.frame(
      ...
      estimate = coefs,
      se = robust_se,
      ci_low = coefs - 1.96 * robust_se,
      ci_high = coefs + 1.96 * robust_se,
      stringsAsFactors = FALSE,
      row.names = NULL
    )
  })
  reg_results_df <- do.call(rbind, reg_results)
  saveRDS(reg_results_df, file.path(data_dir, "regression_results.rds"))
  cat("Saved regression_results.rds for coefficient plot\n")
} else {
