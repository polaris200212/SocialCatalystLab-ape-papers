...
## ---- Figure 1: Treatment Variation Map ----
cat("Figure 1: Stringency variation across states...\n")

# Simple bar chart of peak stringency by state (sorted)
...
## ---- Figure 2: Raw Trends (HCBS vs BH by stringency group) ----
cat("Figure 2: Raw trends by service type and stringency...\n")
...
fig2a <- ggplot(trends, aes(x = month_date, y = mean_paid,
  ...
  annotate("text", x = as.Date("2020-04-01"), y = max(trends$mean_paid) * 0.95,
           label = "Lockdown onset", hjust = -0.05, size = 3, color = "grey40") +
  ...
  scale_linetype_manual(values = c("HCBS" = "solid", "BH" = "dashed"), name = "Service Type") +
  labs(
...
## ---- Figure 3: HCBS/BH Ratio Over Time by Stringency ----
...
## ---- Figure 4: Dynamic DDD Event Study ----
cat("Figure 4: Event study coefficients...\n")

m_dynamic <- results$m_dynamic
ct <- as.data.table(coeftable(m_dynamic), keep.rownames = TRUE)
setnames(ct, c("rn", "estimate", "se", "tval", "pval"))

# Parse quarter from coefficient names (format: "rel_quarter::N:str_x_hcbs")
ct[, quarter := as.numeric(gsub("rel_quarter::", "", gsub(":str_x_hcbs", "", rn)))]
ct <- ct[!is.na(quarter)]

# Add reference period (quarter = -1, coef = 0, se = 0)
...
fig4 <- ggplot(ct, aes(x = quarter, y = estimate)) +
  ...
  annotate("text", x = -0.5, y = max(ct$estimate + 1.96 * ct$se, na.rm = TRUE) * 0.9,
           label = "Lockdown →", hjust = -0.1, size = 3.5, color = "grey40") +
  scale_x_continuous(breaks = seq(-8, 18, 2)) +
  labs(
    title = "Dynamic Triple-Difference: Effect of Lockdown Stringency on HCBS vs BH",
    subtitle = "Coefficient on Stringency × HCBS × Quarter relative to Q-1 (Jan-Mar 2020)",
...
ri_data <- data.table(coef = robustness$ri_perm_coefs)

fig5 <- ggplot(ri_data, aes(x = coef)) +
  geom_histogram(bins = 50, fill = "grey70", color = "white") +
  geom_vline(xintercept = robustness$ri_actual_coef,
             color = "#D55E00", linewidth = 1.2) +
  annotate("text", x = robustness$ri_actual_coef,
           y = Inf, vjust = 2,
           label = sprintf("Actual = %.3f\nRI p = %.3f",
                          robustness$ri_actual_coef,
                          robustness$ri_pvalue),
           color = "#D55E00", size = 3.5, hjust = -0.1) +
  labs(
    title = "Randomization Inference: Permuted DDD Coefficients",
...
ggsave(file.path(FIGURES, "fig5_ri_distribution.pdf"), fig5,
       width = 8, height = 5)
...
fig6 <- ggplot(ct7, aes(x = period, y = estimate)) +
...
ggsave(file.path(FIGURES, "fig6_period_effects.pdf"), fig6,
       width = 8, height = 5)
