...
# Wild cluster bootstrap (try loading; graceful fallback)
wcb_available <- requireNamespace("fwildclusterboot", quietly = TRUE)
if (wcb_available) {
  library(fwildclusterboot)
  cat("fwildclusterboot loaded successfully.\n")
} else {
  cat("fwildclusterboot not available; will skip wild cluster bootstrap.\n")
}
