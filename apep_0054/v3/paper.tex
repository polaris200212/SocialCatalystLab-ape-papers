...
\title{Shining Light on Paychecks: \\ The Effects of Salary Transparency Laws on Wages and the Gender Pay Gap\footnote{This paper is a revision of APEP-0146. See \url{https://github.com/SocialCatalystLab/auto-policy-evals/tree/main/papers/apep_0146} for the original. Key revisions: (1) fixed code integrity issues (Table 1 state counts, border state definitions); (2) sharpened economic contribution framing around equity-efficiency trade-off; (3) tightened prose throughout.}}
...
Pay transparency creates a trade-off between equity and efficiency. This paper provides causal evidence on the magnitude of this trade-off using the staggered adoption of salary transparency laws across U.S. states between 2021 and 2024. I find that transparency laws reduce average wages by 1--2\%, consistent with \citeauthor{cullen2023pay}'s (\citeyear{cullen2023pay}) theoretical prediction that employer commitment to posted ranges weakens worker bargaining power. However, transparency also narrows the gender wage gap by approximately 1 percentage point, as women---who faced larger information deficits---benefit disproportionately from disclosure.

The policy context offers a clean natural experiment. Colorado became the first state to require salary range disclosure in job postings in 2021. By 2024, California, New York, Washington, and five other states had followed suit with similar requirements. This staggered adoption creates variation for difference-in-differences analysis. I implement heterogeneity-robust estimators \citep{callaway2021difference, sun2021estimating} that avoid the biases of standard two-way fixed effects with staggered treatment.
...
My findings support both predictions. First, transparency reduces wages by 1.2--1.8\% across specifications. Second, transparency narrows the gender gap: the triple-difference coefficient shows women's wages decline less than men's by approximately 1 percentage point. Third, effects concentrate in high-bargaining occupations (management, finance, technology) where individual negotiation matters, while standardized-wage occupations show muted effects. This heterogeneity pattern provides mechanism evidence for the bargaining-power channel.

\textbf{Contribution.} This paper makes three contributions. First, I provide the first causal estimates of \emph{job-posting} salary transparency---a stronger intervention than the ``right-to-ask'' laws studied by \citet{cullen2023pay} or internal disclosure policies studied by \citet{baker2023pay}. Job-posting requirements affect all applicants ex ante, not just workers who actively inquire. Second, I quantify the equity-efficiency trade-off: a 2\% wage reduction ``buys'' a 1 percentage point reduction in the gender gap. Policymakers motivated by equity should recognize this cost. Third, the occupational heterogeneity results provide direct evidence for the \citet{cullen2023pay} bargaining mechanism, extending their theoretical predictions to a new policy setting.
...
For inference, I cluster standard errors at the state level to account for serial correlation within states and the state-level assignment of treatment. With 50+ clusters (51 states including DC), cluster-robust standard errors are generally appropriate \citep{cameron2008bootstrap}. Eight states adopted transparency laws during the study period, though only six (CO, CT, NV, RI, CA, WA) have post-treatment data in the sample; New York and Hawaii (first affected income year 2024) fall outside the data window (through income year 2023). The moderate number of treated clusters with post-treatment exposure motivates multiple robustness checks and sensitivity analyses reported below.
...
\item Excluding border states: Dropping states adjacent to treated states to reduce spillover contamination yields -0.011
