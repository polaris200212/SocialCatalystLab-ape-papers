...
p_trends <- ggplot(trends, aes(x = income_year, y = mean_wage, color = group)) +
  ...
  labs(x = "Income Year", y = "Mean Hourly Wage ($)", color = NULL, fill = NULL) +
  theme_apep() +
...
p_gap <- ggplot(gap_trends, aes(x = income_year, y = wage_gap_pct, color = group)) +
  ...
  scale_color_manual(values = c("Treated States" = color_treated,
                                "Control States" = color_control)) +
  scale_x_continuous(breaks = seq(2014, 2023, 2)) +
  labs(x = "Income Year", y = "Gender Wage Gap (%)", color = NULL) +
  theme_apep() +
...
# ============================================================================
# Figure 2 Combined: CPS Trends (2-panel patchwork)
# Panel (a): Mean Hourly Wage  |  Panel (b): Gender Wage Gap
# ============================================================================

cat("\nCreating combined CPS trends figure (2-panel)...\n")

# Strip legends from individual panels — shared legend via patchwork
p_trends_panel <- p_trends +
  labs(tag = "(a)") +
...
p_cps_combined <- p_trends_panel + p_gap_panel +
  plot_layout(ncol = 2) +
  plot_annotation(theme = theme(plot.margin = margin(5, 5, 5, 5))) &
  theme(legend.position = "bottom")

# Collect legends
p_cps_combined <- p_cps_combined + plot_layout(guides = "collect") &
  theme(legend.position = "bottom")

ggsave("figures/fig2_cps_combined.pdf", p_cps_combined, width = 12, height = 5.5)
cat("Saved figures/fig2_cps_combined.pdf\n")
...
p_cohorts <- ggplot(cohort_trends, aes(x = income_year, y = mean_wage, color = cohort)) +
  ...
  labs(x = "Income Year", y = "Mean Hourly Wage ($)", color = "Cohort") +
  theme_apep() +
