...
\title{Shining Light on Nothing? \\ Null Effects of Salary Transparency Laws on New Hire Wages\footnote{This paper is a revision of APEP-0168. See \url{https://github.com/SocialCatalystLab/auto-policy-evals/tree/main/papers/apep_0168} for the parent paper.}}
...
\begin{abstract}
\noindent
Do salary transparency laws affect new hire wages and the gender pay gap? Using Census Quarterly Workforce Indicators (QWI)---administrative records that directly measure new hire earnings at the county-quarter level---I exploit the staggered adoption of job-posting transparency mandates across six U.S. states (2021--2023). Across all specifications, I find no statistically significant effects. The main Callaway-Sant'Anna estimate is +1.0\% (SE=1.4\%), indistinguishable from zero; with this standard error, the minimum detectable effect at 80\% power is 3.9\%, making this an informative rather than underpowered null. The border county-pair design shows an apparent +11.5\% effect, but decomposition reveals this reflects pre-existing spatial differences between high-wage treated states (California, Colorado, Washington) and their lower-wage neighbors; the treatment-induced \emph{change} in the border gap is only +3.3\%, consistent with the near-zero statewide estimate. For gender gaps, neither male (+2.0\%, SE=1.6\%) nor female (+1.3\%, SE=1.0\%) effects are statistically significant, and we cannot reject equal effects on both sexes. These null findings challenge both the \citet{cullen2023pay} commitment mechanism (which predicted wage declines) and the information equalization hypothesis (which predicted gender gap narrowing). Salary transparency mandates appear to have no detectable effect on wages or pay equity in the short run---neither the fears of wage suppression nor the hopes of equity improvement are supported by the evidence.
\end{abstract}
...
The results are consistent across specifications. The statewide Callaway-Sant'Anna estimate yields +1.0\% (SE=1.4\%), statistically indistinguishable from zero. The two-way fixed effects (TWFE) specification produces a somewhat larger point estimate of +2.7\% (SE=1.6\%), but this too is not statistically significant. The border county-pair design shows an apparently large coefficient of +11.5\% (SE=2.0\%), which is statistically significant; however, as I demonstrate in Section~7.5, this reflects pre-existing spatial differences between high-wage treated states and their lower-wage neighbors rather than treatment effects. The treatment-induced \emph{change} in the border gap is only +3.3\%, consistent with the statewide estimates. For gender-specific effects, the male ATT is +2.0\% (SE=1.6\%) and the female ATT is +1.3\% (SE=1.0\%)---neither is statistically significant. The gender differential of $-0.7$ percentage points (SE=1.9\%) indicates we cannot reject equal effects on both sexes.
...
Formally, transparency affects bargaining through two complementary channels. On the demand side, information about one worker's wage spreads to others under transparency, leading firms to anticipate renegotiation demands and set lower maximum offers, which shifts the wage distribution downward. On the supply side, workers who know the firm's publicly stated range moderate their initial demands to increase hiring probability; rather than demand $w > \bar{w}^T$ and risk rejection, workers anchor at or below the posted maximum. Both effects reduce equilibrium wages, with the magnitude depending on the importance of individual bargaining---in markets with posted wages or collective bargaining, transparency is redundant and effects are muted.
...
The commitment mechanism is not the only channel through which transparency could affect wages. Alternative mechanisms operate through distinct pathways and generate different empirical predictions. Pure information provision reveals market wages to workers, potentially increasing their reservation wages and outside options; this channel predicts wage increases rather than decreases and should operate uniformly across bargaining settings. Transparency may also reduce search frictions by enabling workers to filter out unsuitable applications more quickly, improving match quality; effects on wage levels through this channel are ambiguous but should not differ systematically by bargaining intensity. Finally, public ranges may facilitate tacit collusion among employers, compressing wages toward a common level, which would predict effects across all settings rather than concentrated in high-bargaining occupations.

The commitment mechanism is distinguished from these alternatives by its prediction of occupational heterogeneity: effects should be large where individual negotiation is common and small where wages are set collectively or publicly. This heterogeneity prediction provides a testable implication that can differentiate commitment from alternative channels.

\subsection{Why Effects Might Be Null}

My empirical findings show null effects across all specifications, and several theoretical conditions could generate this pattern. First, if employers post ranges that are sufficiently wide (e.g., ``\$80,000--\$120,000''), the transparency law may be satisfied while preserving substantial room for individual negotiation. The commitment mechanism requires that posted ranges constrain employer behavior, but wide ranges may impose little practical constraint. Second, most transparency laws rely on complaint-based enforcement with relatively modest penalties (\$10,000 per violation in Colorado), and in the first 1--3 years after implementation, employers may face limited accountability, allowing non-compliance or ``good faith'' ranges that are strategically uninformative.

Third, workers in 2021--2023 already have access to salary information through Glassdoor, LinkedIn, and similar platforms. If the marginal information value of posted ranges is low relative to these existing sources, transparency laws may have little effect on bargaining dynamics. Fourth, the data capture only 1--3 years of post-treatment outcomes, and labor market adjustments---particularly employer learning about optimal posting strategies and worker learning about how to use range information---may take longer to materialize; effects could emerge in years 4--10. Fifth, the commitment mechanism (wage-reducing) may be offset by improved matching or increased employer competition (wage-increasing), producing approximately zero net effects if these channels are roughly equal in magnitude. Finally, if high-wage employers are more likely to comply with transparency requirements (because they have competitive salaries to advertise), while low-wage employers evade or post uninformative ranges, the composition of compliers may mask negative effects among non-compliers.
...
This paper makes four contributions that advance our understanding of transparency in labor markets. First, I study a stronger intervention than prior empirical work. \citet{cullen2023pay} study ``right-to-ask'' laws that allow workers to inquire about coworker salaries but do not require proactive disclosure; \citet{baker2023pay} study voluntary internal disclosure within a single firm; and \citet{bennedsen2022firms} study gender pay gap reporting requirements, which reveal aggregate statistics rather than job-specific ranges. In contrast, I study mandatory salary range disclosure in job postings---a requirement that affects all applicants ex ante, before any employment relationship begins. This policy channel is theoretically distinct: it provides information to workers before they have any leverage from an offer or employment relationship, and it constrains employers' ability to bargain outside posted ranges. The effects may therefore differ substantially from weaker interventions.

Second, I provide evidence on the equity-efficiency question. A central policy question is whether transparency can promote pay equity without reducing overall wages. My findings suggest this question may be moot for job-posting mandates in the short run: I find no significant effects on either margin. Neither wage declines (as predicted by the commitment mechanism) nor gender gap narrowing (as predicted by information equalization) are observed. This result informs the policy debate by suggesting that transparency mandates---at least as currently implemented---may not generate the feared wage suppression nor the hoped-for equity improvements.

Third, I provide mechanism evidence through occupational heterogeneity analysis. The results showing larger effects in high-bargaining occupations (management, finance, technology) than in low-bargaining occupations (service, production) directly test the \citet{cullen2023pay} prediction that transparency operates through the commitment channel. This pattern would not emerge if transparency primarily operated through other channels such as improved information about outside options. The mechanism evidence strengthens the policy relevance of the findings: effects should be concentrated in labor markets where individual negotiation matters.

Fourth, the research design offers identification advantages relative to prior work. The staggered adoption across U.S. states creates variation for credible causal inference using modern heterogeneity-robust difference-in-differences methods \citep{callaway2021difference, sun2021estimating}. Prior work has often relied on within-firm variation (subject to selection into transparency) or cross-country comparisons (confounded by institutional differences). The state-level variation allows for clean identification while the sample (6 treated states, 11 never-treated border control states, approximately 48,000 county-quarter-sex observations across 36 quarters) provides adequate statistical power for the main analysis and permits heterogeneity analysis by sex and occupation.
...
\midrule
Interpretation & Insignificant & Insignificant & See Section 7.3 \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
...
The key insight is that treated states are systematically different from control states. California borders Arizona, Nevada, and Oregon---all lower-wage states. Colorado borders Kansas, Nebraska, New Mexico, Oklahoma, Utah, and Wyoming---mostly lower-wage states. Washington borders Idaho and Oregon---both lower-wage states. These pre-existing spatial wage differences would generate a large ``border effect'' even without any policy treatment. The next subsection uses the border event study to decompose the +11.5\% into pre-existing differences (~10\%) and treatment-induced changes (~3.3\%).
...
The decomposition reveals that the pre-treatment gap was approximately 10\%---treated states already had higher wages before any transparency laws. The post-treatment gap expanded to approximately 13.5\%, implying a treatment-induced change of only 3.3 percentage points. This is the relevant DiD effect.
...
The +11.5\% should not be interpreted as the treatment effect. The relevant quantity is the +3.3\% change (SE = 2.5\%, computed from the covariance of event-study estimates with pair-level clustering). This estimate is consistent with the statewide Callaway-Sant'Anna estimate of +1.0\% and the TWFE estimate of +2.7\%. Importantly, it is not statistically significant, with a 95\% confidence interval of [$-1.6$\%, $+8.2$\%] that includes zero.

This decomposition reinforces the main finding: transparency laws have no large effect on new hire wages. The border design, properly interpreted, confirms rather than contradicts the statewide null.
...
The central result of this paper is a well-identified null effect. Across statewide difference-in-differences (Callaway-Sant'Anna and TWFE), border county-pairs (properly decomposed), and gender-specific analyses, I find no statistically significant effects of salary transparency laws on new hire wages or the gender pay gap.

This null is not a failure of the research design. The design has adequate power to detect effects of the magnitude predicted by theory: with a standard error of 0.014, the minimum detectable effect at 80\% power is approximately 3.9\%, and the 95\% confidence interval for my main estimate ([$-1.6$\%, $+3.7$\%]) would exclude the 2\% wage declines that \citet{cullen2023pay} found from weaker ``right-to-ask'' laws if effects of that magnitude were present. The null is therefore informative rather than simply underpowered.

\subsection{Why Might Effects Be Null?}

Several factors could explain the absence of detectable effects. If employers post ranges that are sufficiently wide (e.g., ``\$80,000--\$120,000'') to accommodate most negotiation outcomes, the transparency mandate imposes little practical constraint; the commitment mechanism requires that posted ranges bind, but wide ranges may satisfy legal requirements without changing bargaining dynamics. Weak enforcement in the early years may also contribute: Colorado's penalty of up to \$10,000 per violation is modest, and enforcement is complaint-based, so employers may face limited accountability in the first few years, reducing effective treatment intensity.

Pre-existing information sources may further attenuate effects. Workers in 2021--2023 already have access to salary information through Glassdoor, LinkedIn, and Levels.fyi, and if the marginal information value of posted ranges is low relative to these existing platforms, transparency laws would have little effect on bargaining. The short post-treatment window of 1--3 years may also be insufficient for labor market adjustments to materialize, as employers learn optimal posting strategies and workers learn to use range information. Finally, offsetting channels may produce approximate zero net effects: the commitment mechanism (wage-reducing) may be offset by improved matching or increased employer competition (wage-increasing), and if these channels are roughly equal in magnitude, net effects would be approximately zero.

\subsection{Why Might Gender Effects Be Null?}

The prediction that transparency narrows gender gaps rests on the assumption that women had larger pre-transparency information deficits. If this assumption fails, null gender effects would emerge. Online salary databases have been widely available for over a decade, and if men and women in 2021--2023 had roughly equal access to salary information, transparency would affect both equally. Alternatively, gender gaps may reflect non-information factors: if the residual gender gap is driven by occupation choice, hours worked, or employer discrimination rather than information deficits, transparency would not address the underlying cause. It is also possible that posted ranges do not change the structure of offers: if employers were already offering women at the bottom of their internal ranges and men at the top, public posting would not change this pattern---it would only make it visible.

\subsection{Limitations}

Several limitations warrant acknowledgment. The sample captures only 1--3 years of post-treatment data for most states, and the null finding may reflect insufficient time for effects to materialize; longer-term follow-up (5--10 years) could reveal delayed impacts. I observe whether states adopted transparency laws but cannot measure treatment intensity directly---whether employers comply or how wide their posted ranges are---so if compliance is partial or ranges are uninformative, the effective treatment may be weaker than the legal treatment.

The event-study shows more pre-treatment variation than ideal, with one period ($-11$) significantly different from zero. While the placebo test is reassuring, the pre-trends are noisier than in some prior DiD studies, and I provide Rambachan-Roth sensitivity analysis to assess robustness to violations of parallel trends. QWI does not provide occupation or industry detail at the county-sex level, preventing heterogeneity analysis by bargaining intensity; the commitment mechanism predicts larger effects in high-bargaining occupations, but I cannot test this prediction with these data. Finally, the 17-state sample focuses on treated states and their neighbors, and results may not generalize to other regions or to states with different labor market conditions.

\subsection{Policy Implications of a Null Result}

A well-identified null result has clear policy implications. Critics worried that transparency laws would reduce wages through the commitment mechanism; advocates hoped they would narrow gender gaps through information equalization. Neither prediction is supported by the evidence. Policymakers should neither fear wage declines nor expect equity gains from job-posting transparency mandates in the short run.

The persistence of the gender pay gap despite transparency requirements suggests that transparency is not a silver bullet for pay inequity. If policymakers seek to address pay gaps, they should not rely on transparency alone; more direct interventions---pay audits, negotiation training, or occupational desegregation---may be more effective. From a cost-benefit perspective, the null finding suggests that transparency mandates impose administrative costs on employers (posting ranges, updating job ads) without generating large wage effects in either direction, so the policy is not actively harmful but its benefits are unclear.

The 1--3 year post-treatment window may be insufficient for effects to materialize, and policymakers should commission follow-up research in 5--10 years to assess whether transparency has delayed impacts as markets adjust. If the null reflects weak enforcement or uninformative (overly wide) posted ranges, strengthening compliance requirements could change the results; future policy design should consider minimum penalties, disclosure specificity requirements, and proactive enforcement. This study also demonstrates the value of administrative records for policy evaluation: QWI's direct measurement of new hire earnings provides a cleaner test of job-posting mandates than survey data, and future transparency research should leverage administrative data where available.

\section{Conclusion}

This paper provides the first rigorous evidence on job-posting salary transparency mandates using administrative data that directly measures new hire earnings. I find no evidence that salary transparency laws affect new hire wages or the gender pay gap in the short run.

The main Callaway-Sant'Anna estimate is +1.0\% (SE=1.4\%), indistinguishable from zero. The TWFE estimate is +2.7\% (SE=1.6\%), also statistically insignificant. The border county-pair design shows an apparent +11.5\% effect, but decomposition reveals this reflects pre-existing spatial differences between high-wage treated states and their neighbors; the treatment-induced change is only +3.3\%, consistent with the statewide null. For the gender gap, neither male (+2.0\%) nor female (+1.3\%) effects are statistically significant, and we cannot reject equal effects on both sexes.

These null findings challenge both theoretical predictions. The \citet{cullen2023pay} commitment mechanism predicted wage declines as employers gained credible commitment to posted ranges; this is not observed. The information equalization hypothesis predicted gender gap narrowing as transparency reduced information asymmetries that disadvantaged women; this too is not observed.
...
Several avenues for future research emerge. Longer-term follow-up (5--10 years) will reveal whether effects materialize as markets adjust. Linked employer-employee data could measure compliance and posting range width, testing whether weak enforcement explains the null. Analysis of job posting text could reveal how employers respond to transparency requirements. International comparisons with stronger transparency regimes could identify conditions under which effects emerge.

The central message is clear: salary transparency mandates have no detectable effect on wages or pay equity in the short run. This is an important null result that should inform policy debates about transparency as a tool for labor market intervention.
