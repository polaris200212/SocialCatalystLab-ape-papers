...
\title{Shining Light on Paychecks: \\ The Effects of Salary Transparency Laws on Wages and the Gender Pay Gap\footnote{This paper is a revision of APEP-0148. See \url{https://github.com/SocialCatalystLab/auto-policy-evals/tree/main/papers/apep_0148} for the original. Key revisions: (1) fixed code integrity issues with systematic border state identification; (2) added conceptual framework formalizing the Cullen-Pakzad-Hurson mechanism; (3) strengthened introduction narrative around the equity-efficiency puzzle; (4) added wild cluster bootstrap inference for small-cluster robustness.}}
...
\textbf{Contribution to the literature.} This paper advances our understanding of how information shapes labor market outcomes in three ways.

First, I study a \emph{stronger intervention} than prior work. Existing empirical evidence on pay transparency comes from ``right-to-ask'' laws \citep{cullen2023pay}, voluntary internal disclosure \citep{baker2023pay}, or mandated gender gap reporting \citep{bennedsen2022firms, blundell2022wage}. Job-posting requirements are qualitatively different: they affect external labor markets, reach all applicants before hiring, and create commitment through public observability. The wage effects I estimate (1.5--2\%) are comparable to \citeauthor{cullen2023pay}'s findings despite the different policy channel, suggesting the commitment mechanism operates robustly across transparency regimes.

Second, I \emph{quantify the equity-efficiency trade-off}. A central question for policymakers is whether transparency can promote pay equity without depressing wages. My estimates provide a concrete answer: approximately 2\% wage decline ``buys'' 1 percentage point reduction in the gender gap. Whether this trade-off is worthwhile depends on normative judgments, but the magnitudes are now known. For a firm with a 20\% gender gap, achieving parity through transparency would require a 40\% wage decline---clearly prohibitive. Transparency is a complement to, not a substitute for, other equity interventions.

Third, the \emph{occupational heterogeneity} results provide direct mechanism evidence. The \citeauthor{cullen2023pay} model makes a sharp prediction: effects should be large where individual bargaining matters and small where it does not. The concentration of effects in high-bargaining occupations---with precisely zero effect in unionized sectors---is exactly what the theory predicts and would not emerge under alternative mechanisms (e.g., pure information provision). This confirms that transparency operates primarily through the commitment channel.
...
For inference, I cluster standard errors at the state level to account for serial correlation within states and the state-level assignment of treatment. With 50+ clusters (51 states including DC), cluster-robust standard errors are generally appropriate \citep{cameron2008bootstrap}. However, with only 8 treated states (and 6 with post-treatment data in the sample), inference may be unreliable under standard asymptotics \citep{conley2011inference}. I therefore supplement the main results with wild cluster bootstrap inference \citep{mackinnon2017wild} and randomization inference that permutes treatment assignment across states. These procedures are robust to having few treated clusters and provide a more conservative assessment of statistical significance.
...
\begin{tablenotes}
\small
\item \textit{Notes:} Standard errors clustered at state level (51 clusters: 6 treated with post-treatment data, 45 control/not-yet-treated) in parentheses. Column (1) uses state-year aggregates (51 states $\times$ 10 years = 510 obs). Columns (2)-(4) use individual-level CPS ASEC data with survey weights (ASECWT); observation counts are survey-weighted effective sample sizes (unweighted N $\approx$ 650,000 person-years). Demographics include age, education, race, and marital status. * p$<$0.10, ** p$<$0.05, *** p$<$0.01.
\end{tablenotes}
\end{threeparttable}
\end{table}
