...
# ============================================================================
#
# Drop each of the 8 treated states with post-treatment data one at a time.
...
# States with post-treatment data (first_treat <= 2024, since data covers through 2024)
treated_with_post <- transparency_laws %>%
  filter(first_treat > 0, first_treat <= 2024)
...
# ============================================================================
# 25. Lee (2009) Bounds for Sample Selection (WS3)
# ============================================================================
#
# Addresses the significant Pct High-Bargaining composition shift (p=0.012).
# Lee bounds trim the treated group's wage distribution to create worst-case
# bounds on the treatment effect under the assumption that treatment affects
# sample selection monotonically.
# Reference: Lee, D.S. (2009). "Training, Wages, and Sample Selection."
# Review of Economic Studies, 76(3), 1071-1102.

cat("\n==== 25. Lee (2009) Bounds for Sample Selection ====\n")

# Calculate excess mass: proportion of extra observations in treated post-period
# due to composition shift
comp_high_bargain <- composition_results[["pct_high_bargain"]]
excess_pct <- comp_high_bargain$estimate  # ~0.0197

cat("Excess high-bargaining share in treated states:", round(excess_pct, 4), "\n")
cat("This represents the proportion to trim for Lee bounds.\n")

# Lee bounds: trim top/bottom of treated post-treatment wage distribution
# by the excess proportion
treated_post_wages <- df %>%
  filter(treat_post == 1) %>%
  pull(log_hourly_wage)

n_treated_post <- length(treated_post_wages)
n_trim <- ceiling(abs(excess_pct) * n_treated_post)

cat("Treated post-treatment observations:", n_treated_post, "\n")
cat("Observations to trim:", n_trim, "\n")

# Upper bound: trim lowest wages (selection brought in low-wage workers)
sorted_wages <- sort(treated_post_wages)
upper_bound_wages <- sorted_wages[(n_trim + 1):length(sorted_wages)]

# Lower bound: trim highest wages (selection brought in high-wage workers)
lower_bound_wages <- sorted_wages[1:(length(sorted_wages) - n_trim)]

# Re-estimate ATT with trimmed samples
# Create trimmed datasets
trim_upper_threshold <- sorted_wages[n_trim]
trim_lower_threshold <- sorted_wages[length(sorted_wages) - n_trim + 1]

# Upper bound dataset: exclude lowest-wage treated-post observations
df_lee_upper <- df %>%
  filter(!(treat_post == 1 & log_hourly_wage <= trim_upper_threshold))

# Lower bound dataset: exclude highest-wage treated-post observations
df_lee_lower <- df %>%
  filter(!(treat_post == 1 & log_hourly_wage >= trim_lower_threshold))

# Re-estimate DDD on trimmed samples
lee_upper_ddd <- tryCatch({
  feols(log_hourly_wage ~ treat_post * female |
          statefip + income_year + occ_major + educ_cat + age_group,
        data = df_lee_upper, weights = ~ASECWT, cluster = ~statefip)
}, error = function(e) { cat("Lee upper DDD failed:", conditionMessage(e), "\n"); NULL })

lee_lower_ddd <- tryCatch({
  feols(log_hourly_wage ~ treat_post * female |
          statefip + income_year + occ_major + educ_cat + age_group,
        data = df_lee_lower, weights = ~ASECWT, cluster = ~statefip)
}, error = function(e) { cat("Lee lower DDD failed:", conditionMessage(e), "\n"); NULL })

if (!is.null(lee_upper_ddd) && !is.null(lee_lower_ddd)) {
  lee_upper_coef <- coef(lee_upper_ddd)["treat_post:female"]
  lee_lower_coef <- coef(lee_lower_ddd)["treat_post:female"]
  cat("\nLee Bounds for Gender DDD (treat_post:female):\n")
  cat("  Lower bound:", round(lee_lower_coef, 4), "\n")
  cat("  Upper bound:", round(lee_upper_coef, 4), "\n")
  cat("  Main estimate:", round(coef(results$ddd_result)["treat_post:female"], 4), "\n")

  if (lee_lower_coef > 0) {
    cat("  Gender DDD is positive under BOTH Lee bounds -- robust to selection.\n")
  } else {
    cat("  WARNING: Lower Lee bound crosses zero.\n")
  }
}
...
lee_bounds <- list(
  ...
  n_trim = n_trim,
  lee_upper_ddd = lee_upper_ddd,
  lee_lower_ddd = lee_lower_ddd,
  upper_coef = if (!is.null(lee_upper_ddd)) coef(lee_upper_ddd)["treat_post:female"] else NA,
  lower_coef = if (!is.null(lee_lower_ddd)) coef(lee_lower_ddd)["treat_post:female"] else NA
)
saveRDS(lee_bounds, "data/lee_bounds.rds")
cat("Saved Lee bounds to data/lee_bounds.rds\n")

# ============================================================================
# 26. Synthetic DiD (Arkhangelsky et al. 2021) (WS4)
# ============================================================================
#
# Complement C-S with SDID as an alternative estimator that constructs
# synthetic control weights. Apply to the Colorado cohort (longest
# post-treatment window) for a clean single-treatment design.

cat("\n==== 26. Synthetic DiD (Arkhangelsky et al. 2021) ====\n")

if (requireNamespace("synthdid", quietly = TRUE)) {
  library(synthdid)

  # Focus on Colorado (earliest adopter, 2021) for clean SDID
  # Create balanced panel: all states, income years 2014-2024
  # Treated = Colorado (statefip 8), Control = never-treated states
  sdid_panel <- state_year %>%
    filter(g == 0 | statefip == 8) %>%  # Never-treated + Colorado
    select(statefip, income_year, y) %>%
    filter(!is.na(y), !is.infinite(y))

  # Create treatment matrix
  sdid_panel <- sdid_panel %>%
    mutate(
      W = as.integer(statefip == 8 & income_year >= 2021)
    )

  # Convert to matrix format required by synthdid
  # Rows = units, columns = time periods
  sdid_wide_y <- sdid_panel %>%
    select(statefip, income_year, y) %>%
    pivot_wider(names_from = income_year, values_from = y)

  sdid_wide_w <- sdid_panel %>%
    select(statefip, income_year, W) %>%
    pivot_wider(names_from = income_year, values_from = W)

  # Remove unit column and convert to matrices
  Y_mat <- as.matrix(sdid_wide_y[, -1])
  W_mat <- as.matrix(sdid_wide_w[, -1])
  rownames(Y_mat) <- sdid_wide_y$statefip
  rownames(W_mat) <- sdid_wide_w$statefip

  # Remove rows/cols with NA
  complete_rows <- complete.cases(Y_mat)
  Y_mat <- Y_mat[complete_rows, ]
  W_mat <- W_mat[complete_rows, ]

  # Run SDID
  sdid_est <- tryCatch({
    sdid_setup <- panel.matrices(Y_mat, W_mat)
    tau_sdid <- synthdid_estimate(sdid_setup$Y, sdid_setup$N0, sdid_setup$T0)
    tau_sdid
  }, error = function(e) {
    cat("SDID estimation failed:", conditionMessage(e), "\n")
    # Try alternative approach with manual setup
    tryCatch({
      N0 <- sum(rownames(Y_mat) != "8")
      T0 <- sum(as.numeric(colnames(Y_mat)) < 2021)
      tau <- synthdid_estimate(Y_mat, N0, T0)
      tau
    }, error = function(e2) {
      cat("SDID alternative also failed:", conditionMessage(e2), "\n")
      NULL
    })
  })

  if (!is.null(sdid_est)) {
    cat("SDID estimate (Colorado vs. synthetic control):", round(as.numeric(sdid_est), 4), "\n")
    cat("Main C-S ATT:", round(results$att_simple$overall.att, 4), "\n")

    # Also run standard SC and DiD for comparison
    sc_est <- tryCatch({
      N0 <- sum(rownames(Y_mat) != "8")
      T0 <- sum(as.numeric(colnames(Y_mat)) < 2021)
      sc_estimate(Y_mat, N0, T0)
    }, error = function(e) NULL)

    did_est <- tryCatch({
      N0 <- sum(rownames(Y_mat) != "8")
      T0 <- sum(as.numeric(colnames(Y_mat)) < 2021)
      did_estimate(Y_mat, N0, T0)
    }, error = function(e) NULL)

    if (!is.null(sc_est)) cat("SC estimate:", round(as.numeric(sc_est), 4), "\n")
    if (!is.null(did_est)) cat("DiD estimate:", round(as.numeric(did_est), 4), "\n")
  }

  sdid_results <- list(
    sdid_est = sdid_est,
    sc_est = if (exists("sc_est")) sc_est else NULL,
    did_est = if (exists("did_est")) did_est else NULL,
    Y_mat = Y_mat,
    W_mat = W_mat
  )
  saveRDS(sdid_results, "data/sdid_results.rds")
  cat("Saved SDID results to data/sdid_results.rds\n")

} else {
  cat("synthdid package not available. Skipping SDID estimation.\n")
  sdid_results <- NULL
}

# ============================================================================
# 27. Excluding NY and HI Robustness Check
# ============================================================================
#
# Gemini reviewer requested: verify that adding NY and HI (new in this
# revision) does not drive results. Re-estimate excluding these two states.

cat("\n==== 27. Excluding NY and HI Robustness Check ====\n")

state_year_no_nyhi <- state_year %>%
  filter(!statefip %in% c(36, 15))  # Exclude NY (36) and HI (15)

cs_no_nyhi <- tryCatch({
  att_gt(
    yname = "y", tname = "income_year", idname = "statefip", gname = "g",
    data = as.data.frame(state_year_no_nyhi),
    control_group = "nevertreated", anticipation = 0, est_method = "dr",
    print_details = FALSE
  )
}, error = function(e) { cat("C-S without NY/HI failed:", conditionMessage(e), "\n"); NULL })

if (!is.null(cs_no_nyhi)) {
  att_no_nyhi <- aggte(cs_no_nyhi, type = "simple")
  cat("ATT excluding NY and HI:", round(att_no_nyhi$overall.att, 4),
      "(SE:", round(att_no_nyhi$overall.se, 4), ")\n")
  cat("Main ATT (all states):", round(results$att_simple$overall.att, 4), "\n")
}

# DDD excluding NY/HI
df_no_nyhi <- df %>% filter(!statefip %in% c(36, 15))
ddd_no_nyhi <- tryCatch({
  feols(log_hourly_wage ~ treat_post * female |
          statefip + income_year + occ_major + educ_cat + age_group,
        data = df_no_nyhi, weights = ~ASECWT, cluster = ~statefip)
}, error = function(e) NULL)

if (!is.null(ddd_no_nyhi)) {
  cat("DDD (treat_post:female) excl. NY/HI:",
      round(coef(ddd_no_nyhi)["treat_post:female"], 4),
      "(SE:", round(se(ddd_no_nyhi)["treat_post:female"], 4), ")\n")
}

nyhi_exclusion <- list(
  att_no_nyhi = if (!is.null(cs_no_nyhi)) att_no_nyhi else NULL,
  ddd_no_nyhi = ddd_no_nyhi
)
saveRDS(nyhi_exclusion, "data/nyhi_exclusion.rds")
cat("Saved NY/HI exclusion results to data/nyhi_exclusion.rds\n")

# ============================================================================
# Update final robustness results
# ============================================================================
robustness_results$lee_bounds <- lee_bounds
robustness_results$sdid_results <- sdid_results
robustness_results$nyhi_exclusion <- nyhi_exclusion
saveRDS(robustness_results, "data/robustness_results.rds")

cat("\n==== All Robustness Checks Complete (v7) ====\n")
