...
# Load standard errors from actual estimation results
...
# Extract standard errors from model objects
se_main <- att_overall$overall.se
se_twfe <- fixest::se(twfe_result)["postTRUE"]
se_male <- results_by_sex$Male$overall.se
se_female <- results_by_sex$Female$overall.se
# Gender differential SE: sqrt(var(male) + var(female)) assuming independence
se_gender_diff <- sqrt(se_male^2 + se_female^2)
# Border change SE from border analysis (if available, otherwise use estimate)
