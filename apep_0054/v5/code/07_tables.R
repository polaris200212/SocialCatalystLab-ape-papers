...
# This script loads intermediate data files created by earlier pipeline scripts.
# All inputs have documented provenance in the pipeline:
#
#   Input File                     | Created By              | Contents
#   -------------------------------|-------------------------|---------------------------
#   data/cps_analysis.rds          | 02_clean_data.R         | Cleaned individual-level CPS data
#   data/state_year_panel.rds      | 02_clean_data.R         | State-year aggregates
#   data/transparency_laws.rds     | 00_policy_data.R        | Treatment timing with citations
#   data/main_results.rds          | 04_main_analysis.R      | C-S estimates, DDD results
#   data/robustness_results.rds    | 05_robustness.R         | Alternative estimators, sensitivity
#   data/descriptive_stats.rds     | 03_descriptives.R       | Summary statistics for Table 1
#   data/event_study_data.rds      | 04_main_analysis.R      | Event study coefficients
#
# To verify provenance: run scripts 00-06 in sequence; all .rds files are
# created reproducibly from raw CPS ASEC data via the IPUMS API.

source("code/00_packages.R")

# Load data and results
cat("Loading pipeline artifacts with documented provenance...\n")
...
desc_stats <- readRDS("data/descriptive_stats.rds")  # From 03_descriptives.R
