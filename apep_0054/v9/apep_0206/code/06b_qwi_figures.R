...
if (!is.null(es_earns) && nrow(es_earns) > 0) {
  ...
  p1 <- ggplot(es_earns_plot, aes(x = event_time, y = att)) +
    ...
    labs(
      title = "Effect of Transparency Laws on Average Earnings (QWI)",
      subtitle = "Callaway-Sant'Anna quarterly event study",
      x = "Quarters relative to treatment",
      y = "ATT (log average monthly earnings)",
      caption = "Source: Census QWI, state-quarter panel. 95% CI shown."
    ) +
    theme_apep()
...
if (!is.null(es_gap) && nrow(es_gap) > 0) {
  ...
  p2 <- ggplot(es_gap_plot, aes(x = event_time, y = att)) +
    ...
    labs(
      title = "Effect on Gender Earnings Gap (QWI)",
      subtitle = "Callaway-Sant'Anna quarterly event study; negative = gap narrowing",
      x = "Quarters relative to treatment",
      y = "ATT (male-female log earnings gap)",
      caption = "Source: Census QWI, sex-disaggregated state-quarter panel."
    ) +
    theme_apep()
...
p3 <- ggplot(gap_trends, aes(x = date, y = mean_gap, color = group)) +
  ...
  labs(
    title = "Gender Earnings Gap: Treated vs. Control States",
    subtitle = "Male-female log monthly earnings gap (QWI administrative data)",
    x = "", y = "Log earnings gap (male - female)",
    color = "",
    caption = "Source: Census QWI, aggregate industry, private sector."
  ) +
  theme_apep()
...
if (length(dyn) > 0) {
  ...
  p4 <- ggplot(dyn_df %>% filter(estimator == "TWFE"),
    ...
    labs(
      title = "Labor Market Dynamism Under Transparency (QWI)",
      subtitle = "DiD estimates: effect of transparency laws on labor market flows",
      x = "Treatment effect (TWFE with state and quarter FE)",
      y = "",
      shape = "Significance",
      caption = "Source: Census QWI, state-quarter panel, private sector."
    ) +
    theme_apep() +
...
if (length(ind_res) > 0) {
  ...
  if (nrow(ind_earns) > 0) {
    p5 <- ggplot(ind_earns, aes(x = coef, y = reorder(industry, coef),
      ...
      labs(
        title = "Industry Heterogeneity in Earnings Effects (QWI)",
        subtitle = "DiD estimates by NAICS sector",
        x = "Treatment effect on log earnings",
        y = "",
        color = "",
        caption = "Source: Census QWI, state-quarter-industry panel."
      ) +
      theme_apep() +
      theme(legend.position = "right")
...
p6 <- ggplot(earns_trends, aes(x = date, y = mean_earns, color = group)) +
  ...
  scale_color_manual(values = c("Treated States" = color_treated,
                                 "Control States" = color_control)) +
  labs(
    title = "Average Monthly Earnings: Treated vs. Control States",
    subtitle = "QWI administrative data, all industries, private sector",
    x = "", y = "Log average monthly earnings",
    color = "",
    caption = "Source: Census QWI."
  ) +
  theme_apep()
