...
cat("Trimming extreme wages...\n")

# Winsorize at 1st and 99th percentiles
wage_bounds <- quantile(df$hourly_wage, c(0.01, 0.99), na.rm = TRUE)
cat("Wage bounds: $", round(wage_bounds[1], 2), " - $", round(wage_bounds[2], 2), "\n")
...
state_year <- df %>%
  ...
  summarize(
    ...
    # Log wage gap
    log_wage_gap = weighted.mean(log_hourly_wage[female == 0],
                                 ASECWT[female == 0], na.rm = TRUE) -
                   weighted.mean(log_hourly_wage[female == 1],
                                 ASECWT[female == 1], na.rm = TRUE),

    # Wage dispersion
    wage_sd = sqrt(wtd.var(hourly_wage, ASECWT)),
    wage_p90p10 = wtd.quantile(hourly_wage, ASECWT, probs = 0.9) /
                  wtd.quantile(hourly_wage, ASECWT, probs = 0.1),

    # Demographics
    pct_female = weighted.mean(female, ASECWT, na.rm = TRUE) * 100,
    pct_college = weighted.mean(educ_cat == "BA or higher" |
                                educ_cat == "Graduate degree",
...
# Recalculate with proper functions
state_year <- df %>%
  group_by(statefip, income_year, first_treat, ever_treated, post, treat_post) %>%
  summarize(
    n = n(),
