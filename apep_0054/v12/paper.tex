...
\title{Shining Light on Nothing? \\ Null Effects of Salary Transparency Laws on New Hire Wages\footnote{This paper is a revision of APEP-0172. See \url{https://github.com/SocialCatalystLab/auto-policy-evals/tree/main/papers/apep_0172} for the parent paper.}}
...
Eight U.S. states mandated salary transparency between 2021 and 2024. Advocates promised pay equity; critics warned of wage suppression. Both were wrong. Using 48,000 county-quarter observations from administrative records, I find nothing: no wage effects, no gender gap changes, nothing at all. This paper asks why.

The question of whether mandated information disclosure changes market outcomes is fundamental to economics. \citet{stigler1961economics} showed that search costs generate price dispersion; transparency policies aim to reduce these frictions. \citet{akerlof1970market} demonstrated how information asymmetries cause market failures; disclosure mandates seek to restore efficiency. In labor markets specifically, \citet{mortensen1986job} established that search frictions and incomplete information generate substantial wage variation even among identical workers. If these theories are correct, forcing employers to reveal wages should matter.

Yet theory makes conflicting predictions about the direction. The commitment mechanism of \citet{cullen2023pay} predicts wage \emph{declines}: posted ranges become credible ceilings that shift bargaining power to firms. The information equalization hypothesis predicts the opposite for women---transparency should close gender gaps by revealing market rates to workers who lacked salary information through informal networks. Under monopsony \citep{manning2003monopsony, azar2022labor}, transparency could raise wages by increasing the elasticity of labor supply. These predictions cannot all be true.

I test them using administrative data on new hires---the population directly affected by job-posting mandates---and modern difference-in-differences methods. The Census Quarterly Workforce Indicators (QWI) provide average monthly earnings of new hires at the county-quarter-sex level, derived from unemployment insurance records covering 95\% of private employment. Unlike survey data, QWI captures exactly who the policy targets.

The staggered adoption of transparency laws across six states enables causal identification using the \citet{callaway2021difference} estimator, which avoids the biases of two-way fixed effects under treatment effect heterogeneity. County-level granularity also permits a border discontinuity design following \citet{dube2010minimum}, comparing adjacent counties across state lines.

The results are unambiguous. The Callaway-Sant'Anna estimate yields +1.0\% (SE=1.4\%), statistically indistinguishable from zero. The TWFE estimate is +2.7\% (SE=1.6\%), also null. The border design shows an apparently large +11.5\% (SE=2.0\%), but decomposition reveals this reflects pre-existing spatial differences---California and Washington simply pay more than Arizona and Idaho---not treatment effects. The treatment-induced change is only +3.3\%, consistent with the statewide null. For the gender gap, male (+2.0\%) and female (+1.3\%) effects are both insignificant; the differential of $-0.7$ percentage points (SE=1.9\%) provides no evidence of gap narrowing.

\textbf{Contribution.} This paper makes three contributions. First, I document a \emph{well-identified null} that challenges multiple theoretical predictions: neither wage declines (commitment), wage increases (monopsony), nor gender gap narrowing (information equalization) materialize. With a minimum detectable effect of 3.9\%, the null is informative---I can rule out effects of the magnitude that theory predicts. Second, I demonstrate how to interpret border designs when treated and control states have pre-existing level differences, showing that decomposition into levels and changes is essential. Third, I use administrative data targeting the exact population affected by the policy, providing a cleaner test than prior survey-based work.

Why might transparency mandates produce nothing? Several explanations are consistent with the null. Employers may post ranges so wide (``\$80,000--\$120,000'') that the commitment constraint never binds. Workers in 2021--2023 already access salary data through Glassdoor and LinkedIn, so the marginal information value of posted ranges may be small. Weak enforcement and short post-treatment windows (1--3 years) may be insufficient for effects to emerge. Or countervailing mechanisms---commitment lowering wages while improved matching raises them---may offset almost exactly. The null is a genuine puzzle that invites further investigation.

The paper proceeds as follows. Section~2 provides institutional background. Section~3 develops a conceptual framework deriving testable predictions. Section~4 reviews related literature on information in labor markets, pay transparency, and gender gaps. Section~5 describes the data. Section~6 presents the empirical strategy. Section~7 reports results. Section~8 discusses mechanisms and policy implications. Section~9 concludes.
