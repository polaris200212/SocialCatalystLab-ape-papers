...
# ============================================================================
# E. Industry Adoption — Comprehensive Analysis
# ============================================================================

cat("\n=== E. Industry Adoption (Expanded) ===\n")

# Define all industry outcomes to test
industry_outcomes <- c(
  "has_industry_cite", "n_industry_cites", "ln_industry_cites",
  "industry_cite_share", "has_tier1_cite", "has_tier2_cite",
  "n_industry_cites_1y", "n_industry_cites_3y", "n_industry_cites_5y",
  "ln_industry_cites_3y", "ln_industry_cites_5y",
  "industry_speed", "n_distinct_companies"
)

# Filter to outcomes that exist
industry_outcomes <- industry_outcomes[industry_outcomes %in% names(df)]

industry_results <- list()

for (outcome in industry_outcomes) {
  y <- df[[outcome]]
  ...
  # Use full sample (2012-2020; all citation windows complete through 2025)
  ...
  if (!is.null(rdd_out)) {
    industry_results[[outcome]] <- data.table(
      outcome = outcome,
      coef_conv = rdd_out$coef["Conventional", ],
      se_conv = rdd_out$se["Conventional", ],
      p_conv = rdd_out$pv["Conventional", ],
    ...
    cat(sprintf("  %s: coef = %.4f (se = %.4f), p = %.4f, N = %d, mean = %.3f\n",
                outcome,
                rdd_out$coef["Robust", ], rdd_out$se["Robust", ],
                rdd_out$pv["Robust", ], rdd_out$N_h[1] + rdd_out$N_h[2],
                mean(y, na.rm = TRUE)))
  }
}
...
if (length(industry_results) > 0) {
  ...
  fwrite(industry_df, file.path(TAB_DIR, "industry_adoption_full.csv"))
  cat(sprintf("\nSaved %d industry outcome estimates\n", nrow(industry_df)))
}
