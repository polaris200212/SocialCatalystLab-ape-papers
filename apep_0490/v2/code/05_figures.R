...
p1 <- ggplot(fig1_data, aes(x = run_var)) +
  ...
  labs(
    ...
    subtitle = "AI/ML papers (cs.AI, cs.CL, cs.LG, stat.ML, cs.CV, cs.IR), 2012-2020"
  ) +
...
if (file.exists(year_file) && file.info(year_file)$size > 10) {
  ...
  if (nrow(year_df) == 0) {
    cat("Skipping fig8 — no year-by-year data\n")
  } else {
...
# ============================================================================
# Figure 10: RDD Plot — Industry Citations
# ============================================================================

cat("=== Figure 10: Industry RDD Plot ===\n")

IND_OUTCOME <- "ln_industry_cites_3y"
if (!(IND_OUTCOME %in% names(df))) IND_OUTCOME <- "ln_industry_cites"

if (IND_OUTCOME %in% names(df)) {
  ind_df <- df

  bin_width <- 5
  fig10_data <- ind_df[abs(run_var) <= 90 & !is.na(get(IND_OUTCOME))]
  fig10_data[, bin := floor(run_var / bin_width) * bin_width + bin_width / 2]

  fig10_means <- fig10_data[, .(
    mean_y = mean(get(IND_OUTCOME), na.rm = TRUE),
    se_y = sd(get(IND_OUTCOME), na.rm = TRUE) / sqrt(.N),
    n = .N
  ), by = bin]

  p10 <- ggplot(fig10_means, aes(x = bin, y = mean_y)) +
    geom_point(aes(size = n), color = "darkorange", alpha = 0.8) +
    geom_errorbar(aes(ymin = mean_y - 1.96 * se_y,
                      ymax = mean_y + 1.96 * se_y),
                  width = 1, color = "darkorange", alpha = 0.5) +
    geom_vline(xintercept = 0, linetype = "dashed", color = "red", linewidth = 0.8) +
    geom_smooth(data = fig10_data[run_var < 0], aes(x = run_var, y = get(IND_OUTCOME)),
                method = "loess", color = "darkorange4", fill = "bisque", alpha = 0.3) +
    geom_smooth(data = fig10_data[run_var > 0], aes(x = run_var, y = get(IND_OUTCOME)),
                method = "loess", color = "darkorange4", fill = "bisque", alpha = 0.3) +
    labs(
      x = "Minutes from 14:00 ET cutoff",
      y = "Log(3-year industry citations + 1)",
      title = "Industry Adoption at the Cutoff",
      subtitle = "Citations from major AI/tech industry labs"
    ) +
    scale_size_continuous(guide = "none") +
    theme_apep

  ggsave(file.path(FIG_DIR, "fig10_industry_rdd.pdf"), p10, width = 8, height = 5)
  cat("Saved fig10_industry_rdd.pdf\n")
}

# ============================================================================
# Figure 11: Industry Citation Dynamics — Coefficient Plot by Horizon
# ============================================================================

cat("=== Figure 11: Industry Dynamics ===\n")

ind_full_file <- file.path(TAB_DIR, "industry_adoption_full.csv")
if (file.exists(ind_full_file)) {
  ind_full <- fread(ind_full_file)

  # Extract horizon-based outcomes
  horizon_outcomes <- c("n_industry_cites_1y", "n_industry_cites_3y", "n_industry_cites_5y")
  horizon_data <- ind_full[outcome %in% horizon_outcomes]

  if (nrow(horizon_data) > 0) {
    horizon_data[, horizon := as.integer(gsub(".*_(\\d+)y$", "\\1", outcome))]
    horizon_data[, label := paste0(horizon, "-year")]

    p11 <- ggplot(horizon_data, aes(x = horizon, y = coef_robust)) +
      geom_point(size = 3, color = "darkorange") +
      geom_errorbar(aes(ymin = ci_robust_lower, ymax = ci_robust_upper),
                    width = 0.3, color = "darkorange") +
      geom_hline(yintercept = 0, linetype = "dashed", color = "grey50") +
      labs(
        x = "Citation horizon (years after publication)",
        y = "RDD estimate (robust)",
        title = "Industry Citation Dynamics",
        subtitle = "Does the visibility premium grow or fade for industry adoption?"
      ) +
      scale_x_continuous(breaks = c(1, 3, 5), labels = c("1 year", "3 years", "5 years")) +
      theme_apep

    ggsave(file.path(FIG_DIR, "fig11_industry_dynamics.pdf"), p11, width = 7, height = 5)
    cat("Saved fig11_industry_dynamics.pdf\n")
  }
}

# ============================================================================
# Figure 12: Multi-Outcome Coefficient Plot
# ============================================================================

cat("=== Figure 12: Multi-Outcome Coefficient Plot ===\n")

# Combine main results and industry results
main_file <- file.path(TAB_DIR, "main_results.csv")
if (file.exists(main_file) && file.exists(ind_full_file)) {
  main_res <- fread(main_file)
  ind_res <- fread(ind_full_file)

  # Select key outcomes
  selected <- rbind(
    main_res[outcome == "ln_cite_3y", .(outcome, coef = coef_robust, se = se_robust,
                                          ci_lo = ci_robust_lower, ci_hi = ci_robust_upper)],
    ind_res[outcome == "ln_industry_cites_3y", .(outcome, coef = coef_robust, se = se_robust,
                                                   ci_lo = ci_robust_lower, ci_hi = ci_robust_upper)],
    ind_res[outcome == "has_tier1_cite", .(outcome, coef = coef_robust, se = se_robust,
                                            ci_lo = ci_robust_lower, ci_hi = ci_robust_upper)],
    ind_res[outcome == "has_tier2_cite", .(outcome, coef = coef_robust, se = se_robust,
                                            ci_lo = ci_robust_lower, ci_hi = ci_robust_upper)],
    ind_res[outcome == "industry_cite_share", .(outcome, coef = coef_robust, se = se_robust,
                                                  ci_lo = ci_robust_lower, ci_hi = ci_robust_upper)],
    fill = TRUE
  )
  ...
  if (nrow(selected) > 0) {
    outcome_labels <- c(
      ...
      "ln_industry_cites_3y" = "Log(3y industry cites)",
      "has_tier1_cite" = "Frontier lab citation",
      "has_tier2_cite" = "Big Tech citation",
      "industry_cite_share" = "Industry cite share"
    )
    selected[, label := outcome_labels[outcome]]
    selected[, label := factor(label, levels = rev(outcome_labels[outcome_labels %in% label]))]

    # Color by type
    selected[, type := fifelse(outcome == "ln_cite_3y", "General", "Industry")]

    p12 <- ggplot(selected, aes(x = coef, y = label, color = type)) +
      geom_point(size = 3) +
      geom_errorbarh(aes(xmin = ci_lo, xmax = ci_hi), height = 0.2) +
      geom_vline(xintercept = 0, linetype = "dashed", color = "grey50") +
      scale_color_manual(values = c("General" = "steelblue", "Industry" = "darkorange")) +
      labs(
        x = "RDD estimate (robust, with 95% CI)",
        y = "",
        title = "Multi-Outcome RDD Estimates at the arXiv Cutoff",
        subtitle = "General citations vs. industry-specific adoption measures",
        color = ""
      ) +
      theme_apep +
      theme(legend.position = "bottom")

    ggsave(file.path(FIG_DIR, "fig12_multi_outcome.pdf"), p12, width = 8, height = 5)
    cat("Saved fig12_multi_outcome.pdf\n")
  }
}
