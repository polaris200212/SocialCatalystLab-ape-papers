...
# ============================================================================
# J. Industry-Specific Robustness
# ============================================================================

cat("\n=== J. Industry Robustness ===\n")

INDUSTRY_PRIMARY <- "ln_industry_cites_3y"
if (!(INDUSTRY_PRIMARY %in% names(df))) INDUSTRY_PRIMARY <- "ln_industry_cites"
if (!(INDUSTRY_PRIMARY %in% names(df))) {
  cat("WARNING: No industry outcome found. Skipping industry robustness.\n")
} else {

  ind_df <- df

  # J.1 Industry bandwidth sensitivity
  cat("  J.1 Industry Bandwidth Sensitivity\n")
  rdd_ind_base <- tryCatch(
    rdrobust(y = ind_df[[INDUSTRY_PRIMARY]], x = ind_df$run_var, c = 0),
    error = function(e) NULL
  )

  if (!is.null(rdd_ind_base)) {
    h_ind_opt <- rdd_ind_base$bws["h", "left"]
    ind_bw_results <- list()

    for (mult in c(0.5, 0.75, 1.0, 1.25, 1.5, 2.0)) {
      h <- h_ind_opt * mult
      rdd_bw <- tryCatch(
        rdrobust(y = ind_df[[INDUSTRY_PRIMARY]], x = ind_df$run_var, c = 0, h = h),
        error = function(e) NULL
      )
      if (!is.null(rdd_bw)) {
        ind_bw_results[[as.character(mult)]] <- data.table(
    ...
    if (length(ind_bw_results) > 0) {
      fwrite(rbindlist(ind_bw_results), file.path(TAB_DIR, "industry_bandwidth_sensitivity.csv"))
    }
  }

  # J.2 Industry donut RDD
  cat("  J.2 Industry Donut RDD\n")
  ind_donut_results <- list()
  for (donut in c(2, 5)) {
    df_donut <- ind_df[abs(run_var) > donut]
    rdd_donut <- tryCatch(
      rdrobust(y = df_donut[[INDUSTRY_PRIMARY]], x = df_donut$run_var, c = 0),
      error = function(e) NULL
    )
    if (!is.null(rdd_donut)) {
      ind_donut_results[[as.character(donut)]] <- data.table(
        donut_minutes = donut,
  ...
  if (length(ind_donut_results) > 0) {
    fwrite(rbindlist(ind_donut_results), file.path(TAB_DIR, "industry_donut_rdd.csv"))
  }

  # J.3 Industry placebo cutoffs
  cat("  J.3 Industry Placebo Cutoffs\n")
  ind_placebo_results <- list()
  for (pc in c(10*60, 12*60, 13*60, 15*60, 16*60, 18*60)) {
    full[, placebo_run_ind := minutes_since_midnight - pc]
    df_placebo <- full[abs(placebo_run_ind) <= 120]
    if (nrow(df_placebo) < 100 || !(INDUSTRY_PRIMARY %in% names(df_placebo))) next
    rdd_plac <- tryCatch(
      rdrobust(y = df_placebo[[INDUSTRY_PRIMARY]], x = df_placebo$placebo_run_ind, c = 0),
      error = function(e) NULL
    )
    if (!is.null(rdd_plac)) {
      ind_placebo_results[[as.character(pc)]] <- data.table(
  ...
  # Add the real cutoff
  if (!is.null(rdd_ind_base)) {
    ind_placebo_results[["real"]] <- data.table(
      cutoff_hour = 14, is_real = TRUE,
      coef = rdd_ind_base$coef["Robust", ], se = rdd_ind_base$se["Robust", ],
      p_value = rdd_ind_base$pv["Robust", ],
      n_eff = rdd_ind_base$N_h[1] + rdd_ind_base$N_h[2]
    )
  }
  if (length(ind_placebo_results) > 0) {
    fwrite(rbindlist(ind_placebo_results, fill = TRUE),
           file.path(TAB_DIR, "industry_placebo_cutoffs.csv"))
  }

  cat("  Industry robustness complete\n")
}
