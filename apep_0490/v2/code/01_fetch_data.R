...
if (!file.exists(arxiv_file) || file.info(arxiv_file)$size < 100) {
  ...
  for (cat in c("cs.AI", "cs.CL", "cs.LG", "stat.ML", "cs.CV", "cs.IR")) {
    cat(sprintf("\nFetching %s papers...\n", cat))
    start <- 0
    ...
    for (batch_num in 1:500) {  # Up to 100K papers per category
      ...
      # Target: ~2000 papers per category
      if (cat_count >= 2000) {
...
# ============================================================================
# Step 3: Query citing works for industry affiliation data
# ============================================================================

cat("\n=== Step 3: Query citing works for industry citations ===\n")

industry_file <- file.path(DATA_DIR, "industry_citations.csv")

if (!file.exists(industry_file) || file.info(industry_file)$size < 100) {

  # For each paper with an OpenAlex ID, fetch citing works and check affiliations
  query_citing_works <- function(openalex_id, pub_year_oa) {
    ...
    first_industry_year <- NA_integer_
    total_citing <- length(body$results)

    for (citing in body$results) {
      cite_year <- as.integer(substr(citing$publication_date %||% "9999", 1, 4))
      years_since <- cite_year - pub_year_oa

      # Check affiliations of citing paper's authors
      is_industry <- FALSE
      cite_tier <- NA_character_
      cite_company <- NA_character_
            ...
            for (inst in auth$institutions) {
              ...
              for (lab in INDUSTRY_LABS) {
                if (grepl(lab, inst_name, ignore.case = TRUE)) {
                  ...
                  cite_company <- lab
                  # Determine tier
                  if (lab %in% INDUSTRY_LABS_TIER1) cite_tier <- "tier1"
                  else if (lab %in% INDUSTRY_LABS_TIER2) cite_tier <- "tier2"
                  else cite_tier <- "tier3"
                  break
                }
              }
              if (is_industry) break
            }
          }
          if (is_industry) break
        }
      ...
      if (is_industry) {
        n_industry <- n_industry + 1L
        if (!is.na(cite_company)) companies <- c(companies, cite_company)
        if (!is.na(cite_tier) && cite_tier == "tier1") n_tier1 <- n_tier1 + 1L
        if (!is.na(cite_tier) && cite_tier == "tier2") n_tier2 <- n_tier2 + 1L
        if (!is.na(cite_tier) && cite_tier == "tier3") n_tier3 <- n_tier3 + 1L

        if (!is.na(years_since)) {
          if (years_since >= 0 && years_since <= 1) n_industry_1y <- n_industry_1y + 1L
          if (years_since >= 0 && years_since <= 3) n_industry_3y <- n_industry_3y + 1L
          if (years_since >= 0 && years_since <= 5) n_industry_5y <- n_industry_5y + 1L
          if (is.na(first_industry_year) || cite_year < first_industry_year) {
            first_industry_year <- cite_year
          }
        }
      }
    }

    # Handle pagination: if total_count > 200, we need more pages
    total_count <- body$meta$count %||% total_citing
    pages_fetched <- 1L

    if (total_count > 200) {
      cursor <- body$meta$next_cursor %||% NULL
      while (!is.null(cursor) && pages_fetched < 10) {
        next_url <- sprintf(
    ...
    industry_speed <- if (!is.na(first_industry_year)) first_industry_year - pub_year_oa else NA_real_

    data.table(
      openalex_id = openalex_id,
      n_industry_cites = n_industry,
      n_tier1_cites = n_tier1,
      n_tier2_cites = n_tier2,
      n_tier3_cites = n_tier3,
      n_industry_cites_1y = n_industry_1y,
      n_industry_cites_3y = n_industry_3y,
      n_industry_cites_5y = n_industry_5y,
      n_distinct_companies = length(unique(companies)),
      companies_list = paste(unique(companies), collapse = "; "),
      industry_speed = industry_speed,
      total_citing_checked = total_count
    )
  }

  # Only query papers that have OpenAlex IDs
  ...
  for (i in seq_len(n_to_query)) {
    result <- query_citing_works(
      oa_with_id$openalex_id[i],
      oa_with_id$pub_year_oa[i]
    )

    if (!is.null(result)) {
      # Add arxiv_id for merging
    ...
    # Checkpoint every 200 papers
    if (i %% 200 == 0 && length(industry_results) > 0) {
      checkpoint <- rbindlist(industry_results, fill = TRUE)
      fwrite(checkpoint, file.path(DATA_DIR, "industry_checkpoint.csv"))
...
if (exists("industry_df")) cat(sprintf("Industry citation records: %d\n", nrow(industry_df)))
