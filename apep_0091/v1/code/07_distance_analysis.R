...
# Print key results
# =============================================================================
# PART 11: Wild Cluster Bootstrap (Small-Cluster Correction)
# =============================================================================

cat("\n")
cat(paste0(rep("=", 60), collapse = ""), "\n")
cat("PART 11: WILD CLUSTER BOOTSTRAP\n")
cat(paste0(rep("=", 60), collapse = ""), "\n\n")

# With only 7-8 state clusters, standard cluster-robust SEs may be unreliable
...
if (requireNamespace("fwildclusterboot", quietly = TRUE)) {
  ...
  # Run WCB on main specification
  cat("Running wild cluster bootstrap (1000 iterations)...\n")
  ...
  wcb_post <- tryCatch({
    boottest(
      ...
      clustid = ~ NAME,
      B = 999,
      type = "webb"  # Webb weights for small clusters
    )
  }, error = function(e) {
  ...
  if (!is.null(wcb_post)) {
    ...
    cat(sprintf("  Coefficient: %.4f\n", wcb_post$point_estimate))
    cat(sprintf("  WCB p-value: %.4f\n", wcb_post$p_val))
    cat(sprintf("  WCB 95%% CI: [%.4f, %.4f]\n", wcb_post$conf_int[1], wcb_post$conf_int[2]))
    cat(sprintf("  Standard clustered p-value: %.4f\n",
                2 * pt(-abs(coef(model_post)["log_dist"] / se(model_post)["log_dist"]),
                       df = length(unique(crashes_prohib_df$NAME)) - 1)))

    # Save WCB results
    wcb_results <- list(
      estimate = wcb_post$point_estimate,
      wcb_pvalue = wcb_post$p_val,
      wcb_ci = wcb_post$conf_int,
      clustered_se = se(model_post)["log_dist"],
      n_clusters = length(unique(crashes_prohib_df$NAME))
    )
    saveRDS(wcb_results, "../data/wcb_results.rds")
  }
} else {
  cat("Note: fwildclusterboot package not installed.\n")
  cat("Install with: install.packages('fwildclusterboot')\n")
  cat("Small cluster correction not performed.\n")

  # Save placeholder
  wcb_results <- list(
    note = "fwildclusterboot not available",
    n_clusters = length(unique(crashes_prohib_df$NAME))
  )
  saveRDS(wcb_results, "../data/wcb_results.rds")
}
...
cat(sprintf("4. Number of clusters: %d (wild cluster bootstrap recommended for <20 clusters)\n",
            length(unique(crashes_prohib_df$NAME))))
