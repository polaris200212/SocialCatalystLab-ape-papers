...
\title{Testing the Substitution Hypothesis: Cannabis Dispensary Access and Alcohol Involvement in Fatal Crashes\footnote{This paper is a revision of APEP-0110. See \url{https://github.com/anthropics/auto-policy-evals/tree/main/papers/apep_0110} for the original.}}
...
The donut RDD results show some sensitivity to specification. The 2km donut yields a larger positive estimate that is statistically significant, but this is driven by changes in sample composition near the cutoff. Larger donuts (5km, 10km) produce estimates closer to zero, consistent with the baseline. Overall, the donut analysis does not overturn the main conclusion: there is no robust evidence of substitution effects.
...
The main finding is a null result: I detect no discontinuity in alcohol involvement among fatal crashes at legal-prohibition borders. The main RDD specification yields a point estimate of 9.2 percentage points (SE = 5.9 pp), which is not statistically significant (p = 0.127). The 95\% confidence interval spans -2.0 to +21.0 percentage points, encompassing zero but ruling out large substitution effects (reductions greater than 2.0 pp).

The null result is generally robust across specification checks, though with some sensitivity. Importantly, first-stage analysis reveals that physical dispensary access does \textit{not} change sharply at borders within the RDD bandwidth---the RDD estimate for distance to nearest dispensary is only 3.2 km (SE = 10.3 km, p = 0.757), far smaller than the unconditional difference in means (110 km vs. 24 km). This weak first stage complicates interpretation: the null may reflect either genuine absence of substitution effects or insufficient treatment intensity at the exact border, as prohibition-state residents near the border can easily cross to purchase cannabis. Border-by-border analysis shows no systematic heterogeneity, with individually null effects at each border segment. Donut RDD specifications show some sensitivity: excluding crashes within 2 km produces a larger positive estimate (0.237, SE = 0.082) that is statistically significant, but larger exclusion radii (5 km, 10 km) return to null estimates. Complementary analysis using distance to the nearest dispensary within prohibition states also produces null results (coefficient: -0.006, SE = 0.014), and this conclusion is robust to wild cluster bootstrap inference correcting for small cluster counts.
