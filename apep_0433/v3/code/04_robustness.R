## ============================================================
## 04_robustness.R — RDD validity and robustness checks (v3)
...
## v3: expanded MHT hierarchy, facility MDE, equivalence
## ============================================================
...
## ----------------------------------------------------------
## 2. Covariate Balance at Threshold (v3: add BPE 2018)
## ----------------------------------------------------------
...
# v3: add BPE 2018 as balance covariate if available
if ("bpe_total_2018" %in% names(df)) {
  balance_vars[["Total facilities (2018)"]] <- "bpe_total_2018"
}
...
## ----------------------------------------------------------
## 8. Multiple Hypothesis Correction (v3: pre-specified hierarchy)
## ----------------------------------------------------------

cat("\n--- 8. Multiple Hypothesis Correction (v3 hierarchy) ---\n")

results_v2 <- readRDS("../data/rd_results_v2.rds")

# PRIMARY: Holm within labor family
primary_pvals <- sapply(results_v2$primary, function(r) r$pv_robust)
primary_names <- sapply(results_v2$primary, function(r) r$name)

holm_primary <- p.adjust(primary_pvals, method = "holm")

cat("  PRIMARY (Holm-adjusted):\n")
for (i in seq_along(primary_names)) {
  cat(sprintf("    %s: raw=%.4f, Holm=%.4f\n",
      primary_names[i], primary_pvals[i], holm_primary[i]))
}

# SECONDARY: raw p-values, clearly labeled
cat("\n  SECONDARY (raw p-values, no adjustment):\n")
secondary_families <- list(
  pipeline = results_v2$pipeline,
  spending = results_v2$spending,
  facilities = results_v2$facilities
)
for (fam_name in names(secondary_families)) {
  fam <- secondary_families[[fam_name]]
  for (v in names(fam)) {
    r <- fam[[v]]
    cat(sprintf("    [%s] %s: p=%.4f\n", fam_name, r$name, r$pv_robust))
  }
}

# EXPLORATORY: raw p-values
cat("\n  EXPLORATORY (raw p-values):\n")
for (v in names(results_v2$exploratory)) {
  r <- results_v2$exploratory[[v]]
  cat(sprintf("    %s: p=%.4f\n", r$name, r$pv_robust))
}

# Also compute all-outcome Holm for supplementary
all_pvals <- c(
  primary_pvals,
  sapply(results_v2$pipeline, function(r) r$pv_robust),
  sapply(results_v2$spending, function(r) r$pv_robust),
  sapply(results_v2$facilities, function(r) r$pv_robust)
)
all_names <- c(
  primary_names,
  sapply(results_v2$pipeline, function(r) r$name),
  sapply(results_v2$spending, function(r) r$name),
  sapply(results_v2$facilities, function(r) r$name)
)
...
saveRDS(list(
  primary_holm = data.frame(name = primary_names, p_raw = primary_pvals,
                            p_holm = holm_primary),
  all_holm = if (length(all_pvals) > 0)
    data.frame(name = all_names, p_raw = all_pvals, p_holm = holm_all) else NULL,
  # v2 compat
  labor_holm = data.frame(name = primary_names, p_raw = primary_pvals,
                          p_holm = holm_primary)
), "../data/mht_correction.rds")

## ----------------------------------------------------------
## 9. Minimum Detectable Effect (MDE) Analysis (v3: expanded)
## ----------------------------------------------------------

cat("\n--- 9. Minimum Detectable Effect (v3 expanded) ---\n")
...
# MDE for all outcome families
all_families <- c(results_v2$primary, results_v2$pipeline,
                  results_v2$spending, results_v2$facilities)

for (r in all_families) {
  if (is.null(r)) next
  mde <- (z_alpha + z_beta) * r$se_robust
  cat(sprintf("  %s: MDE = %.4f\n", r$name, mde))
  ...
  mde_results <- rbind(mde_results, data.frame(
    ...
    mde_pct = NA_real_,
    family = case_when(
      r$name %in% primary_names ~ "Primary",
      r$name %in% sapply(results_v2$pipeline, function(x) x$name) ~ "Pipeline",
      r$name %in% sapply(results_v2$spending, function(x) x$name) ~ "Spending",
      r$name %in% sapply(results_v2$facilities, function(x) x$name) ~ "Facilities",
      TRUE ~ "Other"
    )
  ))
}
