...
acs_clean <- acs_raw %>%
  mutate(
    ...
    # Self-employment decomposition (critical for Levine & Rubinstein 2017 distinction)
    self_employed = as.integer(COW %in% c(6, 7)),
    incorporated = as.integer(COW == 7),      # Incorporated self-employed (professional practices, consultancies)
    unincorporated = as.integer(COW == 6),    # Unincorporated self-employed (sole proprietors, gig workers)
    wage_worker = as.integer(COW %in% c(1, 2, 3, 4, 5)),

    # Self-employment type indicator (3 categories: 0=wage, 1=unincorporated, 2=incorporated)
    selfemp_type = case_when(
      COW %in% c(1, 2, 3, 4, 5) ~ 0L,  # Wage worker
      COW == 6 ~ 1L,                   # Unincorporated
      COW == 7 ~ 2L,                   # Incorporated
      TRUE ~ NA_integer_
    ),

    # Keep gig_proxy for backward compatibility
    ...
    # -------------------------------------------------------------------------
    # Use PINCP (total personal income) to capture both wage and self-employment income
    # WAGP only captures wage/salary income, missing self-employment income
    wage_income = WAGP,
    total_income = PINCP,

    # For self-employed workers, use total personal income as earnings measure
    # For wage workers, wage income equals total personal income from work
    # This ensures we compare like-with-like: total work income for both groups
...
# Detailed comparison by self-employment type (incorporated vs unincorporated)
cat("\n=== Comparison by Self-Employment Type ===\n")

comparison_type <- acs_analysis %>%
  filter(!is.na(selfemp_type)) %>%
  group_by(selfemp_type) %>%
  summarise(
    n = n(),
    mean_age = weighted.mean(age, weight, na.rm = TRUE),
    pct_female = weighted.mean(female, weight, na.rm = TRUE) * 100,
    ...
    mean_hours = weighted.mean(hours_usual, weight, na.rm = TRUE),
    pct_homeowner = weighted.mean(homeowner, weight, na.rm = TRUE) * 100,
    pct_fulltime = weighted.mean(full_time, weight, na.rm = TRUE) * 100,
    .groups = "drop"
  ) %>%
  mutate(
    group = case_when(
      selfemp_type == 0 ~ "Wage Worker",
      selfemp_type == 1 ~ "Unincorporated Self-Emp",
      selfemp_type == 2 ~ "Incorporated Self-Emp"
    )
  )

print(comparison_type %>% select(group, everything(), -selfemp_type))

# Save type comparison
saveRDS(comparison_type, file.path(data_dir, "comparison_by_type.rds"))
...
analysis_vars <- c(
  ...
  # Treatment: Self-employment (with incorporated/unincorporated distinction)
  "self_employed", "incorporated", "unincorporated", "selfemp_type", "gig_proxy",
