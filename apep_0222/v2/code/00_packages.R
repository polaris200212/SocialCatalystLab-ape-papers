##############################################################################
# 00_packages.R — Load required libraries and set global options
# APEP-0221: Educational Content Restriction Laws and Teacher Labor Markets
##############################################################################

# Core tidyverse
library(tidyverse)
library(lubridate)  # Date manipulation (also loaded via tidyverse)

# Econometrics
library(fixest)     # TWFE, Sun-Abraham, cluster-robust SEs
library(did)        # Callaway-Sant'Anna estimator
library(HonestDiD)  # Rambachan-Roth sensitivity analysis
library(bacondecomp) # Goodman-Bacon decomposition

# Data handling
library(httr)       # API calls
library(jsonlite)   # JSON parsing

# Tables and figures
library(ggplot2)
library(patchwork)  # Multi-panel figures
library(latex2exp)  # LaTeX labels
library(knitr)
library(kableExtra)
library(modelsummary) # Regression tables

# Set global options
options(
  scipen = 999,
  digits = 4,
  dplyr.summarise.inform = FALSE
)

# Set seed for reproducibility
set.seed(20260211)

# APEP theme for publication-quality figures
theme_apep <- function() {
  theme_minimal(base_size = 12) +
    theme(
      panel.grid.minor = element_blank(),
      panel.grid.major = element_line(color = "grey90", linewidth = 0.3),
      axis.line = element_line(color = "grey30", linewidth = 0.4),
      axis.ticks = element_line(color = "grey30", linewidth = 0.3),
      axis.title = element_text(size = 11, face = "bold"),
      axis.text = element_text(size = 10, color = "grey30"),
      legend.position = "bottom",
      legend.title = element_text(size = 10, face = "bold"),
      legend.text = element_text(size = 9),
      plot.title = element_text(size = 13, face = "bold", hjust = 0),
      plot.subtitle = element_text(size = 10, color = "grey40", hjust = 0),
      plot.caption = element_text(size = 8, color = "grey50", hjust = 1),
      plot.margin = margin(10, 15, 10, 10)
    )
}

apep_colors <- c(
  "#0072B2",  # Blue (treated/education)
  "#D55E00",  # Orange (control/healthcare)
  "#009E73",  # Green (third group)
  "#CC79A7",  # Pink (fourth)
  "#F0E442",  # Yellow (fifth)
  "#56B4E9"   # Light blue (sixth)
)

cat("Packages loaded successfully.\n")
