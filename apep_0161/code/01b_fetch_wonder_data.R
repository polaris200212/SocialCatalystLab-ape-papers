...
# ============================================================
# PART 4: Fallback if WONDER API fails
# ============================================================
# If CDC WONDER API is inaccessible (data use agreement, rate limits),
# construct working-age estimates from the all-ages data with a note.
...
if (is.null(d76_processed) && is.null(d176_processed)) {
  ...
  # Load the all-ages data from 01_fetch_data.R
  stopifnot(file.exists("data/mortality_data.rds"))
  allages <- readRDS("data/mortality_data.rds")
  ...
  # but represent a population where commercially insured share is ~60% vs ~15% overall
  # Source: CDC WONDER multiple cause of death, national totals
  ...
  working_age_fallback <- allages %>%
    mutate(
      # Scale deaths by working-age share
      wa_deaths = round(mortality_deaths * WORKING_AGE_DEATH_SHARE),
      # Crude rate adjustment: deaths/pop_share = approximate working-age rate
    ...
    mutate(population = NA_integer_) %>%
    select(state_fips, state_abbr, state_name, year, deaths, population,
           crude_rate, is_suppressed, data_source)

  cat("  Fallback working-age proxy:", nrow(working_age_fallback), "obs\n")
  cat("  NOTE: This is a scaled approximation. Results should be interpreted\n")
  cat("  with caution. Age-restricted CDC WONDER data would provide true rates.\n")

  mortality_working_age <- working_age_fallback
} else {
