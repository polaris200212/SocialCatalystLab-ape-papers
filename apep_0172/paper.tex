...
\title{Shining Light on Nothing? \\ Null Effects of Salary Transparency Laws on New Hire Wages\footnote{This paper is a revision of APEP-0171. See \url{https://github.com/SocialCatalystLab/auto-policy-evals/tree/main/papers/apep_0171} for the parent paper.}}
...
\begin{abstract}
\noindent
When employers must post salary ranges, two opposing forces collide. The \emph{commitment mechanism} \citep{cullen2023pay} predicts wage declines: posted ranges become credible ceilings that shift bargaining power to firms. The \emph{information equalization} hypothesis predicts the opposite for women---transparency closes gender gaps by revealing market rates to workers who lacked access to salary information through informal networks. I test both predictions using administrative data on new hires---the population directly affected by job-posting mandates---exploiting the staggered adoption of transparency laws across six U.S. states (2021--2023). Neither prediction holds. The commitment mechanism implies wage declines; I find near-zero effects (+1.0\%, SE=1.4\%, MDE=3.9\%). Information equalization implies gender gap narrowing; I find no differential by sex. This informative null survives border-county comparisons, placebo tests, and heterogeneity-robust estimation. The result suggests a puzzle: either the mechanisms that theory identifies are weaker than expected, or countervailing forces---improved matching, strategic non-compliance, pre-existing information from Glassdoor and LinkedIn---offset them almost exactly. Policymakers should temper expectations: salary posting mandates appear inert in the short run, neither suppressing wages nor advancing pay equity.
\end{abstract}
...
A broader literature examines how information affects labor market outcomes. \citet{autor2001rise} document the dramatic increase in information availability through online job postings. \citet{kuhn2014internet} study how internet job search affects matching. \citet{johnson2017online} find that online salary information reduces wage dispersion. The foundational treatment of wage dispersion among similar workers is \citet{mortensen2003wage}, which shows how search frictions and incomplete information can generate substantial wage variation even among observationally identical workers.
...
Table~\ref{tab:border_decomp} presents the numeric decomposition. The pre-treatment gap was approximately 10\%---treated states already had higher wages before any transparency laws. The post-treatment gap expanded to approximately 13.5\%, implying a treatment-induced change of only 3.3 percentage points. This is the relevant DiD effect.

\begin{table}[H]
\centering
\caption{Border Design Decomposition: Level vs. Change}
...
Significant at 5\%? & \multicolumn{2}{c}{No} \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item \textit{Notes:} Pre-treatment gap is the average border coefficient for event times $e < 0$. Post-treatment gap is the average for $e \geq 0$. Treatment-induced change is the difference (post minus pre), which equals the true DiD effect. Standard errors computed from the covariance of event-study estimates with pair-level clustering (129 pairs). The na\"ive border coefficient of +11.5\% (Table~\ref{tab:main}) conflates the pre-existing level difference with treatment effects.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
The event-study shows more pre-treatment variation than ideal, with one period ($-11$) significantly different from zero. While the placebo test is reassuring, the pre-trends are noisier than in some prior DiD studies, and I provide Rambachan-Roth sensitivity analysis to assess robustness to violations of parallel trends. QWI does not provide \emph{occupation} detail, preventing heterogeneity analysis by occupation-level bargaining intensity as in \citet{cullen2023pay}. However, QWI does provide industry (NAICS) breakdowns at the county-quarter level (without sex disaggregation), which I use in the Appendix to proxy for bargaining intensity using high-negotiation industries (Professional Services, Finance) versus low-negotiation industries (Retail, Food Service). This industry-level test is less precise than occupation-level analysis but provides suggestive evidence. Finally, the 17-state sample focuses on treated states and their neighbors, and results may not generalize to other regions or to states with different labor market conditions.
