...
# ============================================================================
#
# PREREQUISITE: Download the modal_age.csv file from Tarek Hassan's Dropbox.
#
# DATA PROVENANCE:
#
# 1. Technology vintage data (modal_age.csv):
#    - Source: Prof. Tarek Hassan (Boston University)
#    - Dropbox link: https://www.dropbox.com/scl/fi/5onmury5pqun42g2l5yrr/modal_age.csv
#    - Coverage: 917 CBSAs, 2010-2023
#    - The data measures the modal age of capital equipment and production
#      technologies across U.S. establishments.
#    - NOTE: Earlier versions of this paper (APEP-0141, APEP-0143) incorrectly
#      used simulated data. This version uses the actual data from Prof. Hassan.
#
...
# ============================================================================
# 1. Load and clean technology data (modal_age.csv from Tarek Hassan)
# ============================================================================

cat("Loading technology data from Prof. Tarek Hassan's CSV...\n")
tech_raw <- read_csv("../data/modal_age.csv", show_col_types = FALSE)

# Filter to valid CBSAs (cbsa != -9999) and valid years (not NA)
# The raw data has multiple rows per CBSA-year (industry level), collapse to mean
tech_cbsa <- tech_raw %>%
  filter(cbsa != -9999) %>%    # Remove non-CBSA rows
  filter(!is.na(year)) %>%     # Remove aggregated rows with NA year
  group_by(cbsa, year) %>%
  summarize(
    ...
    density = first(density),       # Population density from Hassan data
    cbsa_title = first(CBSATitle),  # CBSA name from Hassan data
    n_sectors = n(),
    .groups = "drop"
  )
