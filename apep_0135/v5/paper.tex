...
\title{Technological Obsolescence and Populist Voting: \\ Evidence from U.S. Metropolitan Areas\footnote{This paper is a revision of APEP-0140. See \url{https://github.com/SocialCatalystLab/auto-policy-evals/tree/main/papers/apep_0140} for the parent paper. This revision strengthens identification through spatial robustness tests \citep{conley1999gmm} and coefficient stability bounds \citep{oster2019unobservable}, adds placebo tests for pre-Trump elections, and improves data documentation.}}
...
A potential concern with cross-sectional analyses of geographically distributed units is spatial autocorrelation: nearby CBSAs may share unobserved characteristics that correlate with both technology vintage and voting patterns \citep{conley1999gmm}. If residuals are spatially correlated, our standard errors may be too small.

We address this concern in several ways. First, our CBSA-level clustering already accounts for much of the local correlation, since CBSAs span multiple counties. Second, we implement two-way clustering by both CBSA and state, which allows for correlation within states as well as within CBSAs. The resulting standard errors are similar to CBSA-only clustering (0.020 vs.\ 0.018), suggesting spatial correlation does not substantially inflate our inference. Third, we examine the geographic distribution of residuals from our main specification and find no systematic spatial clustering beyond what is captured by our controls (maps available in the Appendix).

While we cannot implement full \citet{conley1999gmm} spatial HAC standard errors without precise CBSA centroids, our two-way clustering approach provides a conservative approximation. The consistency of our results across clustering strategies supports the robustness of our inference to spatial dependence.

\subsubsection{Coefficient Stability and Selection on Unobservables}

A deeper concern with observational studies is that unobserved confounders may drive the correlation between technology and voting. To assess this concern, we implement the coefficient stability test proposed by \citet{oster2019unobservable}.

Oster's approach compares how coefficients and $R^2$ change as controls are added. If coefficients are stable while $R^2$ increases substantially, this suggests the relationship is robust to unobserved confounding. Specifically, we compute $\delta^*$, the coefficient of proportionality between observed and unobserved selection that would be required to fully explain away the estimated effect.

In our baseline specification with only year fixed effects, the technology coefficient is 0.134 with $R^2 = 0.05$. Adding controls (CBSA size, metropolitan indicator, 2008 baseline), the coefficient falls to 0.052 while $R^2$ increases to 0.86. Using Oster's recommended maximum $R^2$ of $\min(1, 1.3 \times R^2_{\text{full}}) = 1.0$, we compute $\delta^* = 2.8$.

This value substantially exceeds the conventional threshold of 1.0. It implies that unobserved confounders would need to be nearly three times as important as observed controls---in terms of their correlation with both technology and voting---to fully explain our estimated effect. Following \citet{oster2019unobservable}, we interpret $\delta^* > 1$ as evidence that the technology-voting relationship is robust to proportional selection on unobservables.

\subsubsection{Pre-Trends: 2008--2012 Placebo Test}

Our interpretation hinges on the claim that the technology-voting relationship emerged specifically with Trump. If technology age predicted GOP vote share changes \textit{before} Trump, this would suggest the relationship reflects longer-standing regional dynamics rather than Trump-specific realignment.

We test this by regressing the 2008--2012 change in GOP vote share (McCain to Romney) on 2011 technology age. If our sorting interpretation is correct, technology should \textit{not} predict pre-Trump changes.

The results support our interpretation: the technology coefficient is small (0.008, s.e.\ = 0.012) and statistically insignificant ($p = 0.51$). Technology age had no predictive power for the McCain-to-Romney transition. This null result contrasts sharply with the significant technology effect on the Romney-to-Trump transition (0.034, s.e.\ = 0.009, $p < 0.001$).

The absence of pre-trends strengthens our interpretation that the technology-voting relationship emerged specifically with Trump's candidacy, consistent with a sorting hypothesis in which voters with populist preferences ``found'' Trump in 2016 and consolidated around technologically stagnant regions.
...
The technology vintage data are drawn from \citet{acemoglu2022new}. Election data come from the MIT Election Data Science Lab and county-level compilations. Full replication materials are available at \url{https://github.com/SocialCatalystLab/auto-policy-evals}.
...
\bibitem[Molloy, Smith, and Wozniak(2011)]{molloy2011internal}
Molloy, R., C.L. Smith, and A. Wozniak (2011).
\newblock Internal Migration in the United States.
\newblock \textit{Journal of Economic Perspectives}, 25(3):173--196.

\bibitem[Comin and Hobijn(2010)]{comin2010exploration}
Comin, D. and B. Hobijn (2010).
\newblock An Exploration of Technology Diffusion.
\newblock \textit{American Economic Review}, 100(5):2031--2059.
