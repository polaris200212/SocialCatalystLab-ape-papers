...
# Scatter: 2016 tech age vs 2016-2020 gains
fig6a <- df_gains %>%
  ...
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
  labs(
    title = "A. 2016 Technology Age vs. Trump Gains (2016-2020)",
    x = "Modal Technology Age in 2015 (years)",
    y = "Change in Trump Vote Share (pp)"
  )

# Scatter: 2016 tech age vs 2016 level (for comparison)
fig6b <- df %>%
  filter(year == 2016) %>%
  ggplot(aes(x = modal_age_mean, y = trump_share)) +
  geom_point(alpha = 0.3, size = 1.5) +
  geom_smooth(method = "lm", se = TRUE, color = "#E41A1C") +
  labs(
    title = "B. 2016 Technology Age vs. Trump Vote Share Level",
    x = "Modal Technology Age in 2015 (years)",
...
fig6 <- fig6a + fig6b +
  plot_annotation(
    title = "Testing Causal vs. Sorting: Technology Age and Voting",
    subtitle = "Technology age predicts vote share levels (B) but not changes (A)",
    caption = "Note: Lines show OLS fit with 95% CI. The null effect in Panel A suggests sorting, not causation."
  )
