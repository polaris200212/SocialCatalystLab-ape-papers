...
## Format numeric columns — v6: use 4 decimal places for small-magnitude variables
## (network fuel exposure has SD ~ 0.02, needs precision beyond 3dp)
sumstats_display <- sumstats_combined %>%
  mutate(
    ## Detect rows where values are very small (< 1) to use higher precision
    needs_precision = (!is.na(SD) & abs(SD) < 0.1) | (!is.na(Mean) & abs(Mean) < 0.1),
    Mean = ifelse(is.na(Mean), "", ifelse(needs_precision, fmt(Mean, 4), fmt(Mean))),
    SD   = ifelse(is.na(SD),   "", ifelse(needs_precision, fmt(SD, 4), fmt(SD))),
    Min  = ifelse(is.na(Min),  "", ifelse(needs_precision, fmt(Min, 4), fmt(Min))),
    Max  = ifelse(is.na(Max),  "", ifelse(needs_precision, fmt(Max, 4), fmt(Max))),
    N    = ifelse(is.na(N),    "", fmt0(N))
  ) %>%
  select(-needs_precision)
...
## ============================================================================
## v5 NEW: TABLE 14 — TIMING DECOMPOSITION (WS2)
## ============================================================================

cat("\n=== v5 Table 14: Timing Decomposition ===\n")

if (!is.null(robustness_results$timing_decomposition)) {
  td_model <- robustness_results$timing_decomposition$model

  tab14_tex <- etable(td_model,
                      tex = TRUE,
                      headers = "Timing Decomposition",
                      title = "Timing Decomposition: Carbon Tax Era vs.\\ Post-\\textit{Gilets Jaunes}",
                      label = "tab:timing_decomp",
                      style.tex = style.tex("aer"),
                      se.below = TRUE,
                      fitstat = ~ n + r2 + wr2,
                      notes = paste0(
                        "\\\\\\\\textit{Notes:} ",
                        "D\\\\'{e}partement-level regressions weighted by registered voters. ",
                        "Post Carbon Only = 1 for 2014-2017 elections (carbon tax at \\\\euro7--30.5/tCO$_2$). ",
                        "Post GJ Only = 1 for 2019+ elections (post-\\\\textit{Gilets Jaunes}, \\\\euro44.6/tCO$_2$). ",
                        "SEs clustered by d\\\\'{e}partement.",
                        " $^{***}p<0.01$, $^{**}p<0.05$, $^{*}p<0.10$."))

  writeLines(tab14_tex, file.path(TABLES_DIR, "tab_timing_decomposition.tex"))
  cat("  Saved tab_timing_decomposition.tex\n")
} else {
  cat("  WARNING: Timing decomposition results not available.\n")
}


## ============================================================================
## v5 NEW: TABLE 15 — ADDITIONAL ROBUSTNESS (DONUT + TRIPLE-DIFF + PRE-TREND)
## ============================================================================

cat("\n=== v5 Table 15: Additional Robustness ===\n")

add_rob_models <- list()
add_rob_headers <- character()

if (!is.null(robustness_results$donut_design)) {
  add_rob_models$donut <- robustness_results$donut_design$model
  add_rob_headers <- c(add_rob_headers, "Donut")
}

if (!is.null(robustness_results$pre_trend_adjusted)) {
  add_rob_models$detrend <- robustness_results$pre_trend_adjusted$model
  add_rob_headers <- c(add_rob_headers, "Pre-Trend Adj.")
}

if (length(add_rob_models) >= 1) {
  tab15_tex <- do.call(etable,
    c(add_rob_models, list(
      tex = TRUE,
      headers = add_rob_headers,
      title = "Additional Robustness: Donut Design and Pre-Trend Adjustment",
      label = "tab:additional_robustness",
      style.tex = style.tex("aer"),
      se.below = TRUE,
      fitstat = ~ n + r2 + wr2,
      notes = paste0(
        "\\\\\\\\textit{Notes:} ",
        "D\\\\'{e}partement-level, weighted by registered voters. ",
        "Donut: drops 2012 and 2014 elections. ",
        "Pre-Trend Adjusted: adds Network Fuel $\\\\times$ (year $-$ 2012) linear trend. ",
        "SEs clustered by d\\\\'{e}partement.",
        " $^{***}p<0.01$, $^{**}p<0.05$, $^{*}p<0.10$."))))

  writeLines(tab15_tex, file.path(TABLES_DIR, "tab_additional_robustness.tex"))
  cat("  Saved tab_additional_robustness.tex\n")
}


## ============================================================================
## v5 NEW: TABLE 16 — RI COMPARISON TABLE
## ============================================================================

cat("\n=== v5 Table 16: RI Comparison ===\n")

ri_comparison <- tibble(
  Method = c("Standard RI (full permutation)",
             "Region-Block RI (NUTS-2)",
             "Urbanization-Block RI",
             "Shift-Level RI (density terciles)",
             "Wild Cluster Bootstrap"),
  `p-value` = c(
    robustness_results$randomization_inference$ri_p_net,
    robustness_results$block_ri$block_ri_p_net,
    if (!is.null(robustness_results$urban_block_ri)) robustness_results$urban_block_ri$p_network else NA,
    if (!is.null(robustness_results$shift_level_ri)) robustness_results$shift_level_ri$p_network else NA,
    robustness_results$wild_bootstrap$p_network
  ),
  Permutations = c(
    robustness_results$randomization_inference$n_permutations,
    robustness_results$block_ri$n_permutations,
    if (!is.null(robustness_results$urban_block_ri)) robustness_results$urban_block_ri$n_permutations else NA,
    if (!is.null(robustness_results$shift_level_ri)) robustness_results$shift_level_ri$n_permutations else NA,
    robustness_results$wild_bootstrap$n_boot
  )
) %>%
  filter(!is.na(`p-value`))

ri_tab_tex <- kbl(ri_comparison,
                   format = "latex",
                   booktabs = TRUE,
                   caption = "Randomization Inference: Network Fuel Exposure Coefficient",
                   label = "tab:ri_comparison",
                   digits = 4,
                   col.names = c("Method", "$p$-value", "Permutations"),
                   escape = FALSE) %>%
  kable_styling(latex_options = c("hold_position")) %>%
  footnote(general = paste0(
    "All tests are for the network fuel exposure coefficient from the ",
    "département-level, population-weighted specification (D2). ",
    "Standard RI permutes fuel vulnerability across all départements. ",
    "Region-Block RI permutes within 13 NUTS-2 regions. ",
    "Urbanization-Block RI permutes within fuel-vulnerability quartiles. ",
    "Shift-Level RI permutes within population density terciles."),
    threeparttable = TRUE)

writeLines(ri_tab_tex, file.path(TABLES_DIR, "tab_ri_comparison.tex"))
cat("  Saved tab_ri_comparison.tex\n")


## ============================================================================
## v6 NEW: TABLE 17 — BARTIK RESIDUALIZATION + ORTHOGONALIZED CHANNELS
## ============================================================================

cat("\n=== v6 Table 17: Bartik Residualization & Orthogonalized Channels ===\n")

decomp_models <- list()
decomp_headers <- character()

if (!is.null(robustness_results$bartik_residualization)) {
  decomp_models$bartik <- robustness_results$bartik_residualization$model
  decomp_headers <- c(decomp_headers, "Bartik Residualized")
}

if (!is.null(robustness_results$orthogonalized_channels)) {
  decomp_models$orth <- robustness_results$orthogonalized_channels$model
  decomp_headers <- c(decomp_headers, "Orthogonalized")
}

if (length(decomp_models) >= 1) {
  tab17_tex <- do.call(etable,
    c(decomp_models, list(
      tex = TRUE,
      headers = decomp_headers,
      title = "Channel Decomposition: Bartik Residualization and Orthogonalized Fuel Exposure",
      ...
      notes = paste0(
        "\\\\\\\\textit{Notes:} ",
        "D\\\\'{e}partement-level, weighted by registered voters. ",
        "Column 1 residualizes fuel vulnerability on immigration, education, and urbanization ",
        "before constructing the shift-share (Borusyak et al., 2022). ",
        "Column 2 orthogonalizes network fuel exposure on network immigration exposure; ",
        "the `pure fuel' residual captures variation unrelated to immigration proximity. ",
        "SEs clustered by d\\\\'{e}partement.",
        " $^{***}p<0.01$, $^{**}p<0.05$, $^{*}p<0.10$."))))

  writeLines(tab17_tex, file.path(TABLES_DIR, "tab_channel_decomposition.tex"))
  cat("  Saved tab_channel_decomposition.tex\n")
}
...
cat("\n", strrep("=", 60), "\n")
cat("TABLE GENERATION COMPLETE (v6)\n")
cat(strrep("=", 60), "\n")
