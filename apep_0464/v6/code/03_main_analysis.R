...
## ============================================================================
## v6: PRE-TREND EQUIVALENCE TEST
## The p=0.03 joint F-test rejects exact zeros, but pre-treatment coefficients
## are NEGATIVE (opposite sign to post-treatment). A one-sided test against
## positive pre-trends (i.e., testing whether pre-trends go in the SAME
## direction as the effect) is the relevant threat to identification.
## ============================================================================

cat("\n=== v6: Pre-Trend Equivalence Test ===\n")

## Extract pre-treatment network coefficients from the event study
es_both_coefs <- coeftable(es_both)
pre_idx <- grepl("network_fuel_std", rownames(es_both_coefs)) &
  grepl("::([1-4]):", rownames(es_both_coefs))
pre_estimates <- es_both_coefs[pre_idx, "Estimate"]
pre_ses <- es_both_coefs[pre_idx, "Std. Error"]

cat("  Pre-treatment network coefficients:\n")
for (i in seq_along(pre_estimates)) {
  cat("    Period", i, ":", round(pre_estimates[i], 4),
      " (SE:", round(pre_ses[i], 4), ")\n")
}

## Standard joint F-test (two-sided: H0: all pre-treatment = 0)
pre_f_stat <- sum((pre_estimates / pre_ses)^2) / length(pre_estimates)
pre_f_pval <- pf(pre_f_stat, length(pre_estimates),
                  nobs(es_both) - length(coef(es_both)),
                  lower.tail = FALSE)
cat("\n  Joint F-test (two-sided, H0: pre = 0):\n")
cat("    F =", round(pre_f_stat, 3), ", p =", round(pre_f_pval, 4), "\n")

## One-sided test: H0: pre-treatment coefficients >= 0 (the threatening direction)
## vs H1: pre-treatment coefficients < 0 (which they are -- opposite to effect)
## Use multivariate one-sided test: all pre-treatment coefficients are non-positive
## Simple approach: test each individually, then Bonferroni-combine
one_sided_pvals <- pnorm(pre_estimates / pre_ses)  # P(Z < z) — tests for negative
cat("\n  One-sided tests (H0: pre >= 0 vs H1: pre < 0):\n")
for (i in seq_along(one_sided_pvals)) {
  cat("    Period", i, ": p =", round(one_sided_pvals[i], 4),
      if (pre_estimates[i] < 0) "(negative, supporting ID)" else "(positive, threatening ID)",
      "\n")
}

## Fraction of pre-treatment coefficients that are negative (same sign as threat = positive)
n_negative <- sum(pre_estimates < 0)
cat("\n  Pre-treatment coefficients negative (opposite to post-treatment):",
    n_negative, "of", length(pre_estimates), "\n")

## Equivalence test: are pre-treatment effects within a bound of zero?
## Use TOST (two one-sided tests) with bound = magnitude of smallest post-treatment effect
post_idx <- grepl("network_fuel_std", rownames(es_both_coefs)) &
  grepl("::([6-9]|10):", rownames(es_both_coefs))
post_estimates <- es_both_coefs[post_idx, "Estimate"]
equiv_bound <- min(abs(post_estimates)) / 2  # Half the smallest post-treatment effect
cat("  Equivalence bound (half smallest post-treatment):", round(equiv_bound, 4), "\n")

## TOST: reject if |pre| < bound
tost_p <- sapply(seq_along(pre_estimates), function(i) {
  t_lower <- (pre_estimates[i] - (-equiv_bound)) / pre_ses[i]
  t_upper <- (pre_estimates[i] - equiv_bound) / pre_ses[i]
  max(pnorm(-t_lower), pnorm(t_upper))  # Conservative
})
tost_reject <- all(tost_p < 0.05)
cat("  TOST equivalence test (bound =", round(equiv_bound, 4), "):",
    if (tost_reject) "REJECT H0 of non-equivalence (pre-trends equivalent to zero)"
    else "Cannot reject non-equivalence",
    "\n")
...
pretrend_tests <- list(
  ...
  pre_ses = pre_ses,
  joint_f_stat = pre_f_stat,
  joint_f_pval = pre_f_pval,
  one_sided_pvals = one_sided_pvals,
  n_negative = n_negative,
  n_total_pre = length(pre_estimates),
  equiv_bound = equiv_bound,
  tost_pvals = tost_p,
  tost_reject = tost_reject
)
saveRDS(pretrend_tests, file.path(DATA_DIR, "pretrend_tests.rds"))
