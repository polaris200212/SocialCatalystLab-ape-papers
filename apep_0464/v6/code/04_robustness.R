## ============================================================================
## 04_robustness.R — Connected Backlash (apep_0464 v6)
...
if (file.exists(geo_file)) {
  ...
  ## v6 fix: add population weights (consistent with main spec D2)
...
## ============================================================================
## v6 NEW: 17b. BARTIK RESIDUALIZATION (per Borusyak et al.)
## Residualize fuel vulnerability shifts on observables (immigration, education,
## urbanization) before constructing the shift-share instrument.
## If the coefficient survives, the fuel channel is not mechanically driven by
## correlated observables.
## ============================================================================

cat("\n=== 17b. Bartik Residualization (v6 WS3b) ===\n")

## Residualize own fuel vulnerability on correlated observables
## (immigration share, education, urbanization)
dept_cross <- dept_panel_hr %>%
  filter(election_num == 5) %>%  # pre-treatment cross-section (2012)
  select(dept_code, co2_commute, network_fuel_norm,
         share_immigrant, any_of(c("pct_bac_plus", "pct_urban",
                                    "pop_density", "median_income")))

## Identify available controls
avail_controls <- intersect(
  c("share_immigrant", "pct_bac_plus", "pct_urban", "pop_density", "median_income"),
  names(dept_cross)
)
cat("  Available controls for residualization:", paste(avail_controls, collapse = ", "), "\n")

if (length(avail_controls) >= 1) {
  ## Residualize own fuel on observables
  resid_formula_own <- as.formula(paste("co2_commute ~", paste(avail_controls, collapse = " + ")))
  resid_own <- lm(resid_formula_own, data = dept_cross, na.action = na.exclude)
  dept_cross$co2_commute_resid <- residuals(resid_own)

  cat("  Own fuel R² from observables:", round(summary(resid_own)$r.squared, 3), "\n")
  cat("  (Higher R² = more variation explained by confounders)\n")

  ## Residualize network fuel on observables
  resid_formula_net <- as.formula(paste("network_fuel_norm ~", paste(avail_controls, collapse = " + ")))
  resid_net <- lm(resid_formula_net, data = dept_cross, na.action = na.exclude)
  dept_cross$network_fuel_resid <- residuals(resid_net)

  cat("  Network fuel R² from observables:", round(summary(resid_net)$r.squared, 3), "\n")

  ## Merge residualized exposures back to panel
  resid_map <- dept_cross %>%
    select(dept_code, co2_commute_resid, network_fuel_resid) %>%
    filter(!is.na(co2_commute_resid) & !is.na(network_fuel_resid))

  dept_panel_resid <- dept_panel_hr %>%
    left_join(resid_map, by = "dept_code") %>%
    filter(!is.na(co2_commute_resid) & !is.na(network_fuel_resid)) %>%
    mutate(
      own_fuel_resid_std = (co2_commute_resid - mean(co2_commute_resid, na.rm = TRUE)) /
        sd(co2_commute_resid, na.rm = TRUE),
      network_fuel_resid_std = (network_fuel_resid - mean(network_fuel_resid, na.rm = TRUE)) /
        sd(network_fuel_resid, na.rm = TRUE)
    )

  ## Re-estimate with residualized exposure
  bartik_resid <- feols(rn_share ~ own_fuel_resid_std:post_carbon +
                           network_fuel_resid_std:post_carbon |
                           dept_code + id_election,
                         data = dept_panel_resid,
                         cluster = ~dept_code,
                         weights = ~total_registered)

  cat("\n  Bartik residualized results:\n")
  cat("    Own fuel (resid) x Post:     ",
      round(get_coef(bartik_resid, "own_fuel_resid"), 4),
      " (", round(get_se(bartik_resid, "own_fuel_resid"), 4), ")\n")
  cat("    Network fuel (resid) x Post: ",
      round(get_coef(bartik_resid, "network_fuel_resid"), 4),
      " (", round(get_se(bartik_resid, "network_fuel_resid"), 4), ")\n")

  bartik_survived <- abs(get_coef(bartik_resid, "network_fuel_resid") /
                           get_se(bartik_resid, "network_fuel_resid")) > 1.645
  cat("  VERDICT: Residualized network fuel",
      ifelse(bartik_survived, "SURVIVES", "does NOT survive"), "\n")

  robustness_results$bartik_residualization <- list(
    model = bartik_resid,
    own_r2 = summary(resid_own)$r.squared,
    network_r2 = summary(resid_net)$r.squared,
    controls_used = avail_controls,
    coef_network = get_coef(bartik_resid, "network_fuel_resid"),
    se_network = get_se(bartik_resid, "network_fuel_resid"),
    survived = bartik_survived,
    description = "Bartik residualization: fuel shifts purged of immigration/education/urbanization"
  )
} else {
  cat("  WARNING: No controls available for residualization. Skipping.\n")
  robustness_results$bartik_residualization <- NULL
}


## ============================================================================
## v6 NEW: 17c. ORTHOGONALIZED CHANNELS
## Regress network fuel on network immigration, use residual as "pure fuel"
## exposure. This addresses the VIF=3.9 collinearity concern directly.
## ============================================================================

cat("\n=== 17c. Orthogonalized Channels (v6 WS3c) ===\n")

## Orthogonalize: residualize network fuel on network immigration
## cross-section level (one obs per dept)
dept_cross_orth <- dept_panel_hr %>%
  filter(election_num == 5) %>%
  select(dept_code, network_fuel_std, network_immig_std) %>%
  filter(!is.na(network_fuel_std) & !is.na(network_immig_std))

orth_reg <- lm(network_fuel_std ~ network_immig_std, data = dept_cross_orth)
dept_cross_orth$network_fuel_orth <- residuals(orth_reg)

cat("  R² of network fuel on network immigration:",
    round(summary(orth_reg)$r.squared, 3), "\n")
cat("  (This is the fraction of fuel variation explained by immigration proximity)\n")

## Merge orthogonalized fuel back to panel
orth_map <- dept_cross_orth %>% select(dept_code, network_fuel_orth)

dept_panel_orth <- dept_panel_hr %>%
  left_join(orth_map, by = "dept_code") %>%
  filter(!is.na(network_fuel_orth)) %>%
  mutate(
    network_fuel_orth_std = (network_fuel_orth - mean(network_fuel_orth, na.rm = TRUE)) /
      sd(network_fuel_orth, na.rm = TRUE)
  )

## Estimate with orthogonalized fuel + original immigration
orth_model <- feols(rn_share ~ own_fuel_std:post_carbon +
                      network_fuel_orth_std:post_carbon +
                      network_immig_std:post_carbon |
                      dept_code + id_election,
                    data = dept_panel_orth,
                    cluster = ~dept_code,
                    weights = ~total_registered)

cat("\n  Orthogonalized results:\n")
cat("    Own fuel x Post:              ",
    round(get_coef(orth_model, "own_fuel_std"), 4),
    " (", round(get_se(orth_model, "own_fuel_std"), 4), ")\n")
cat("    Pure fuel (orth) x Post:      ",
    round(get_coef(orth_model, "network_fuel_orth"), 4),
    " (", round(get_se(orth_model, "network_fuel_orth"), 4), ")\n")
cat("    Network immigration x Post:   ",
    round(get_coef(orth_model, "network_immig_std"), 4),
    " (", round(get_se(orth_model, "network_immig_std"), 4), ")\n")

orth_fuel_survived <- abs(get_coef(orth_model, "network_fuel_orth") /
                            get_se(orth_model, "network_fuel_orth")) > 1.645
cat("  VERDICT: Orthogonalized 'pure fuel' channel",
    ifelse(orth_fuel_survived, "SURVIVES", "does NOT survive"), "\n")

robustness_results$orthogonalized_channels <- list(
  model = orth_model,
  fuel_immig_r2 = summary(orth_reg)$r.squared,
  coef_pure_fuel = get_coef(orth_model, "network_fuel_orth"),
  se_pure_fuel = get_se(orth_model, "network_fuel_orth"),
  coef_immig = get_coef(orth_model, "network_immig_std"),
  se_immig = get_se(orth_model, "network_immig_std"),
  fuel_survived = orth_fuel_survived,
  description = "Orthogonalized channels: fuel residualized on immigration to isolate pure fuel effect"
)
