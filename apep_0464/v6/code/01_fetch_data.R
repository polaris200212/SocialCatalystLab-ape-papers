...
if (!file.exists(migration_file)) {
  ...
  ## Method 1: INSEE RP2013 "Couples, familles, ménages" (contains mobility table)
  ## Source: INSEE Recensement de la population 2013, table T_COUPL_FAM_MEN
  ## URL: https://www.insee.fr/fr/statistiques/2862068
  ...
  tryCatch({
    ...
    cat("  Downloaded RP2013 couples/familles/ménages.\n")
    mig_success <- TRUE
  }, error = function(e) {
    cat("  WARNING: Method 1 (RP2013 couples) failed:", e$message, "\n")
  })

  ## Method 2: INSEE RP2013 "Tableaux détaillés — Mobilité résidentielle" by département
  ## Source: INSEE Recensement de la population 2013, table TD_MOB_DEP
  ## URL: https://www.insee.fr/fr/statistiques/2866283
  if (!mig_success) {
    ...
    tryCatch({
      ...
      cat("  WARNING: Method 2 (RP2013 mobility) failed:", e$message, "\n")
    })
  }

  ## v6: NO silent fallback to proxy construction
  ## Both methods must be tried; if both fail, stop with clear error
  if (!mig_success) {
    cat("  ERROR: Both INSEE migration download methods failed.\n")
    cat("  Migration data is required for the migration-proxy robustness check.\n")
    cat("  Please manually download from: https://www.insee.fr/fr/statistiques/2866283\n")
    cat("  Place the extracted CSV at:", migration_file, "\n")
    cat("  Continuing without migration data (migration robustness will be skipped).\n")
    ## Write a flag file so downstream scripts know migration failed
    writeLines("DOWNLOAD_FAILED", file.path(migration_dir, "MIGRATION_FAILED"))
  } else {
    ## SHA256 verification of downloaded files
    downloaded_files <- list.files(migration_dir, pattern = "\\.csv$", full.names = TRUE)
    if (length(downloaded_files) > 0) {
      for (f in downloaded_files) {
        hash <- digest::digest(file = f, algo = "sha256")
        cat("  SHA256(", basename(f), "):", hash, "\n")
      }
    }
  }
} else {
