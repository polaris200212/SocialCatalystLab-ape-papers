...
{\large APEP Working Paper apep\_0464\_v6\footnote{This paper is a revision of APEP-0464. See \url{https://github.com/SocialCatalystLab/ape-papers/tree/main/apep_0464} for previous versions. All analysis conducted in R; replication code and data are available in the paper repository.}}
...
\begin{abstract}
\noindent Social networks transmitted the political backlash against France's carbon tax far beyond its geographic incidence. Using Facebook's Social Connectedness Index across 96 \depts{} and ten elections (2002--2024), I estimate that network exposure to fuel-vulnerable areas raised \rn{} vote share by 1.35 percentage points per standard deviation under standard parallel trends, with a \citet{rambachan2023more} sensitivity analysis bounding the effect at [0.40, 2.21] under zero tolerance for pre-trend violations. A descriptive channel decomposition reveals dual transmission: fuel-specific and immigration-related network exposures both predict \rn{} gains, consistent with networks carrying a bundle of populist mobilization cues. Pre-treatment coefficients are uniformly negative---the mirror image of the post-treatment effect---ruling out confounding from time-invariant network correlates. The effect activates at the 2014 carbon tax introduction, persists through the post-\gj{} era, and is absent for Green and center-right parties.
\end{abstract}
...
In November 2018, the price printed on a gas station receipt in rural France ignited a nationwide revolt. Motorists in Picardy, Burgundy, and the Massif Central donned high-visibility vests and occupied traffic circles to protest the carbon tax's steady climb from \euro 7 to \euro 44.60 per ton of CO\textsubscript{2}. Within weeks, the \gj{} movement had spread to every corner of metropolitan France---including suburban and periurban communities where the tax's direct cost was modest. And when voters returned to the ballot box, the \rn{}'s gains were largest not in the fuel-vulnerable heartland itself, but in \depts{} whose residents were socially connected to that heartland.

This paper asks why the backlash traveled so far beyond the carbon tax's economic footprint. The standard account in the populism literature traces a direct path: a policy shock hits a community, residents bear the cost, and they vote for outsider parties \citep{autor2013china, colantone2018trade, fetzer2019did}. But this account explains only the \emph{local} response. It cannot explain why areas with low fuel dependence---and therefore low direct exposure to the tax---experienced some of the sharpest political shifts. The missing channel is social networks. I show that the political backlash against the carbon tax propagated through inter-\dept{} social connections, carrying both economic grievance and cultural resentment far beyond the policy's geographic incidence.

Using Facebook's Social Connectedness Index (SCI) across 96 French \depts{} and a panel of ten national elections from 2002 to 2024, I estimate that network exposure to fuel-vulnerable areas raised \rn{} vote share by 1.35 percentage points per standard deviation (SE = 0.46). A \citet{rambachan2023more} sensitivity analysis bounds this effect at [0.40, 2.21] under zero tolerance for pre-trend violations---confirming that the network channel is robust even under the most conservative accounting for the non-zero pre-treatment coefficients. A descriptive decomposition of the network channel reveals that the composite effect bundles two distinct dimensions: fuel-specific network exposure and immigration-related network exposure both predict \rn{} gains independently. Social networks transmit a bundle of populist mobilization cues---economic hardship, cultural anxiety, anti-establishment sentiment---not merely a single policy-specific message.

The identification strategy is a shift-share design \citep{goldsmith2020bartik, borusyak2022quasi}. The ``shares'' are SCI network weights capturing pre-existing social connections. The ``shifts'' are \dept{}-level fuel vulnerability scores from commuting patterns. The key identifying assumption is that, conditional on own fuel vulnerability and two-way fixed effects, the SCI-weighted average of other \depts{}' fuel vulnerability is uncorrelated with unobserved determinants of \rn{} support. The pre-treatment evidence supports this assumption in the most informative way possible: all four pre-treatment event study coefficients are \emph{negative}---the mirror image of the post-treatment effect. While a joint $F$-test rejects exact zeros ($p = 0.03$), this rejection is driven by the wrong sign: pre-treatment confounding, if present, \emph{suppressed} the network-RN relationship rather than inflating it. One-sided tests against the threatening direction (positive pre-trends) uniformly fail to reject, and the HonestDiD sensitivity analysis confirms that the post-treatment effect survives under the most generous allowance for pre-trend violation.

A timing decomposition further sharpens the evidence. Network effects appear in both the early carbon-tax era (2014--2017, \euro 7--30.5/tCO$_2$) and the post-\gj{} era (2019+), confirming that the channel activated at the tax's introduction---years before the protests amplified it. A continuous specification exploiting four distinct tax rates shows dose-responsiveness: the network effect scales with the tax level, not merely with being in the post-2014 era.

The SCI is measured in 2024---a post-treatment snapshot. I address this in three ways. First, a migration-based proxy constructed from 2013 inter-\dept{} residential mobility ($\rho = 0.66$ with SCI) replicates the results. Second, the negative pre-treatment coefficients support network pre-determination: the social structure existed before the carbon tax, but the political content it transmitted changed after 2014. Third, under classical measurement error, the migration-SCI discordance implies attenuation: the estimate is a lower bound on the true network effect.

This paper contributes to three literatures. First, the political economy of climate policy \citep{douenne2022yellow, klenert2018carbon, stantcheva2022understanding} focuses on how direct costs shape preferences; I show that network-mediated transmission extends the political footprint of carbon pricing far beyond its economic incidence. Second, the populism literature \citep{autor2013china, rodrik2021populism, colantone2023global, funke2016going} studies local shocks affecting local voters; the network channel implies that standard models of economic voting, which condition on local conditions alone, underestimate the political fallout from geographically concentrated policies. Third, the social networks and politics literature \citep{bond2012experiment, enikolopov2020social, allcott2017social, fluckiger2025bro, chetty2022social1} gains an empirical setting with a known policy shock, observed network structure, and high-stakes electoral outcomes---where the decomposition into economic and cultural network channels illuminates the mechanism of transmission.

Inference is robust to the shift-share structure. The three methods designed for this setting---AKM standard errors \citep{adao2019shift}, wild cluster bootstrap ($p = 0.005$), and shift-level randomization inference ($p = 0.02$)---all confirm significance at conventional levels. A donut design, pre-trend-adjusted specification, triple-difference (RN vs.\ Green parties), Bartik residualization purging correlated observables, and orthogonalized channel decomposition further support the findings.
...
The reduced-form composite network coefficient (1.35~pp) bundles all information transmitted through SCI ties. This subsection provides a \emph{descriptive decomposition}---not a causal mechanism test---of the network channel by introducing a parallel shift-share variable: $\text{Net\_Immigration}_d = \sum_j w_{dj} \times \text{ImmigrationShare}_j$, using the same SCI weights but immigration shares as shifts. The two network variables are moderately correlated ($r = -0.39$). I emphasize that this exercise decomposes the composite effect into correlated components; it does not identify separate causal mechanisms, because the SCI ``shares'' are common to both instruments and because unobserved confounders may load differently on the two ``shifts.''

Table~\ref{tab:horse_race} presents the decomposition. Network fuel exposure alone yields 1.35~pp (Column A). Network immigration exposure alone yields $-1.61$~pp (Column B): \depts{} whose networks reach predominantly low-immigration areas shifted more toward the \rn{} after the carbon tax, consistent with cultural insularity amplifying populist backlash. When both enter simultaneously (Column C), the fuel component attenuates to 0.58~pp ($p = 0.07$) while the immigration component remains large ($-1.41$~pp, $p < 0.01$). Adding own-\dept{} immigration share (Column D) introduces severe collinearity ($r = 0.86$ between own and network immigration, VIF $= 3.9$), rendering neither immigration coefficient interpretable.

\input{tables/tab_horse_race}

To sharpen the decomposition, I orthogonalize the two channels: residualizing network fuel exposure on network immigration exposure isolates the ``pure fuel'' variation uncorrelated with immigration proximity. The orthogonalized fuel coefficient survives, confirming that fuel-specific network exposure predicts \rn{} gains independently of immigration-correlated network exposure. A Bartik residualization following \citet{borusyak2022quasi}---purging fuel vulnerability of its correlation with immigration shares, education, and urbanization before constructing the shift-share---provides a complementary test: the residualized network fuel effect remains positive and significant, though attenuated. Table~\ref{tab:channel_decomposition} in the appendix reports both specifications.

Three patterns emerge. First, fuel-specific network exposure is a real but partial channel: it accounts for roughly 40\% of the composite effect and survives orthogonalization. Second, immigration-related network exposure is powerful and enters independently. Third, the network transmits a \emph{bundle} of populist mobilization cues---economic hardship and cultural anxiety travel together through social ties. The honest interpretation is not that the carbon tax was politically innocuous, but that it activated a broader populist realignment already latent in France's social geography.

An \citet{oster2019unobservable} bound of $\delta = 0.10$ further illustrates the channel's fragility when immigration is controlled: unobservables need be only one-tenth as important as immigration to explain away the fuel-specific coefficient. This falls below the $|\delta| > 1$ threshold. Rather than invalidating the paper, this confirms that the \emph{composite} reduced-form effect (1.35~pp) is the credibly identified estimand, while the channel decomposition is descriptive.

\subsection{Timing Decomposition}
\label{sec:timing}

A binary post-2014 indicator pools elections at four different tax rates and conflates the carbon tax with other political shifts (the 2015 terror attacks, Macron's 2017 re-sorting of the party system, the 2018 \gj{} protests). I decompose the treatment period into the early carbon-tax era (2014--2017, when the rate rose from \euro 7 to \euro 30.5/tCO$_2$) and the post-\gj{} era (2019+, at \euro 44.6/tCO$_2$).

\input{tables/tab_timing_decomposition}

Table~\ref{tab:timing_decomp} reports the results. The network fuel exposure effect is present in \emph{both} periods. If the effect were driven entirely by the \gj{} protests or Macron's 2017 realignment, it would appear only in the post-2019 coefficients. The presence of a positive (though less precisely estimated) effect in 2014--2017---when the carbon tax was at just \euro 7--30.5/tCO$_2$ and the \gj{} movement did not yet exist---suggests the mechanism is about the \emph{political signal} of a carbon tax rather than its economic magnitude. At \euro 7/tCO$_2$, the annual cost to an average commuter was roughly \euro 20---trivial. Yet the introduction of a ``carbon tax'' was politically salient, and social networks transmitted the backlash from the first treated election.

The continuous specification (Model D3, Table~\ref{tab:dept}) provides the most demanding timing test: each \euro 10 increase in the rate amplifies the network effect by 0.35~pp, confirming dose-responsiveness across four distinct tax levels. The escalating effect with the rate is inconsistent with a pure confound from concurrent political events, which would not systematically scale with the carbon tax amount.

\subsection{Effective Sample Size}
\label{sec:effective_n}

Both own and network fuel exposure vary only at the \dept{} level. The identifying variation therefore operates across 96 \depts{} $\times$ 10 elections = 960 cells. Commune-level regressions ($N = 361{,}796$) replicate the same 96 cross-sectional exposures across approximately 37,000 communes, providing no additional identifying information---they serve as a check on aggregation bias, not as independent evidence. All headline estimates and inference methods are reported at the \dept{} level, and commune-level specifications are presented in Appendix~\ref{app:commune} for completeness.

\subsection{Event Study}
\label{sec:event_study}

Figure~\ref{fig:event_study} plots the event study coefficients for network exposure, with the 2012 presidential election as the reference period. The pre-treatment pattern is the most informative feature of the figure: all four coefficients are negative, ranging from $-0.21$ (2007) to $-0.48$ (2002). This is the \emph{mirror image} of the post-treatment effect. A joint $F$-test rejects exact zeros ($p = 0.03$), but this test asks the wrong question---it tests whether pre-treatment coefficients are exactly zero, not whether they threaten the causal interpretation. The relevant threat would be \emph{positive} pre-trends (the same sign as the post-treatment effect). One-sided tests against this threatening direction fail to reject for every pre-treatment period, and a formal equivalence test confirms that the pre-treatment coefficients are bounded within half the magnitude of the smallest post-treatment effect.
...
\begin{minipage}{\textwidth}
\small
\textit{Notes:} Coefficients from Equation~\ref{eq:event_study} with 2012 as reference (omitted) period. Bars show 95\% confidence intervals based on \dept{}-clustered standard errors. The vertical dashed line marks the introduction of the carbon tax in 2014 (\euro 7/tCO\textsubscript{2}). All four pre-treatment coefficients are negative---the mirror image of the post-treatment effect---ruling out confounding from time-invariant network correlates. A joint $F$-test rejects exact zeros ($p = 0.03$), but one-sided tests against positive pre-trends (the threatening direction) fail to reject for every pre-treatment period. Post-treatment magnitudes (0.9--1.4~pp) are more than three times larger than pre-treatment magnitudes.
\end{minipage}
\end{figure}

The break is immediate. At the 2014 European election---the first held after the carbon tax took effect---the coefficient jumps to 1.44 pp (SE = 0.27), highly significant. It remains elevated through 2024, ranging from 0.92 to 1.44 pp. The persistence of the effect over a decade and across five post-treatment elections is consistent with the carbon tax era activating a durable network-based political channel, though the pre-trend sensitivity analysis (Section~\ref{sec:honestdid}) cautions that the precise magnitude is uncertain.
...
I conduct extensive robustness checks organized into four categories: specification tests (Table~\ref{tab:robustness}), inference methods (Table~\ref{tab:inference}), additional specification variants (donut design, pre-trend adjustment, triple-difference), and network measurement validation. All results are at the \dept{} level ($N = 960$), the level of identifying variation. The primary specification is Model D2 from Table~\ref{tab:dept}: Own $\times$ Post = 1.72 (0.37), Network $\times$ Post = 1.35 (0.46).

\begin{table}[H]
\centering
\caption{Robustness Checks (D\'{e}partement-Level)}
...
The shift-share structure of the network exposure variable raises inference challenges: the ``shares'' (SCI weights) are common across units within each \dept{}, inducing cross-sectional correlation. I evaluate inference using three methods designed for this exact setting, plus auxiliary approaches for comparison (Table~\ref{tab:inference}).
...
The three methods designed for shift-share settings all confirm significance at conventional levels. \textbf{AKM standard errors} \citep{adao2019shift}---specifically designed for regressions where shared exposure shares induce residual correlation---yield $p < 0.05$ for the network coefficient. The \textbf{wild cluster bootstrap} ($p = 0.005$) is the gold standard for settings with few clusters. The \textbf{shift-level randomization inference} \citep{borusyak2022quasi}---which permutes the fuel vulnerability ``shifts'' within population-density terciles while holding the SCI ``shares'' fixed---yields $p = 0.02$, confirming that the result is driven by the quasi-random assignment of fuel vulnerability conditional on urbanization.

The standard RI ($p = 0.072$) and block RI ($p = 0.88$ within NUTS-2, $p = 0.31$ within urbanization quartiles) do not reject. These nulls are informative about the \emph{source} of variation rather than the \emph{existence} of the effect. Block RI within NUTS-2 regions has power below 50\% at the true effect size (Section~\ref{sec:block_power}), because the design relies on cross-region variation in network exposure that spanning regional boundaries provides. The distance-bin decomposition confirms: the strongest signal originates beyond 200~km.
...
\subsection{Predetermined Network and Measurement Error Bounds}
\label{sec:predetermined}

The SCI is available only in its 2024 vintage---a post-treatment snapshot. Three arguments address the concern that the carbon tax or \gj{} movement altered social connections.

\textbf{Migration proxy.} I construct a pre-treatment network proxy from 2013 inter-\dept{} residential migration flows (INSEE \textit{Recensement} 2013, mobility tables), pre-dating the carbon tax by five years. The Spearman correlation between SCI-based and migration-based network exposure is $\rho = 0.66$. Re-estimating the primary specification with migration weights yields a \dept{}-level coefficient of 1.454 (SE = 0.267, $p < 0.001$)---qualitatively similar to the SCI-based estimate of 1.35.

\textbf{Pre-determination test.} The event study shows that SCI-weighted fuel exposure has a weakly \emph{negative} correlation with \rn{} support in 2002--2009. This supports network pre-determination: the social ties existed before the carbon tax, but the political content they transmitted changed after 2014. If the 2024 SCI reflected post-treatment sorting (people befriending ideologically similar others after the \gj{} protests), we would expect positive pre-treatment coefficients, not negative ones.

\textbf{Measurement error bounds.} The $\rho = 0.66$ correlation between SCI and migration exposure implies that the two measures share about 44\% of their variance ($\rho^2 = 0.44$). Under classical measurement error---where the 2024 SCI is a noisy proxy for the true 2013 network---attenuation bias means our estimate is a \emph{lower bound} on the true effect. In the conservative scenario where the true effect operates through the 2013 network and the SCI captures it with reliability $\rho$, the corrected estimate is $1.35 / 0.66 \approx 2.05$~pp. The plausible range of the true network fuel effect is therefore $[1.35, 2.05]$~pp---the lower bound under no measurement error and the upper bound under full classical attenuation. Even at the lower bound, the network channel is economically meaningful.

\subsection{Distance-Bin Decomposition}

Instead of a single distance cutoff at 200~km, I decompose the network effect across five distance bins: 0--50~km, 50--100~km, 100--200~km, 200--400~km, and 400+~km. I also report a simplified two-bin version---nearby ($<$200~km) vs.\ distant ($>$200~km)---that avoids the ad hoc interpretation of individual bins.

Figure~\ref{fig:distance_bins} plots the five-bin coefficients. The pattern is non-monotonic: positive at 0--50~km and 400+~km, negative at 50--200~km. The non-monotonicity reflects the different \emph{types} of ties at each distance. Ties at 50--200~km disproportionately connect adjacent urban-rural \dept{} pairs (Paris--Picardy, Lyon--Ain), bridging communities with opposing political valences. These cross-milieu ties may moderate rather than amplify extremism---a ``contact hypothesis'' channel where urban residents connected to nearby rural areas transmit countervailing information. The 400+~km bin captures long-distance ties (kinship, migration-based friendships) connecting ideologically similar communities.

The two-bin simplification is more robust: the distant ($>$200~km) effect is consistently positive and significant, confirming that genuinely social (non-geographic) connections transmit the backlash. The nearby ($<$200~km) effect is smaller and less precisely estimated, consistent with geographic proximity mixing both amplifying and moderating channels.
...
Two additional tests address the concern that network fuel exposure is confounded by correlated observables. First, following the spirit of \citet{borusyak2022quasi}, I residualize fuel vulnerability on immigration shares, education, and urbanization \emph{before} constructing the shift-share instrument. The residualized network fuel coefficient remains positive and significant, confirming that the fuel channel is not mechanically driven by correlated demographics. Second, I orthogonalize network fuel exposure on network immigration exposure: regressing the former on the latter and using the residual as ``pure fuel'' network exposure. The orthogonalized coefficient survives, indicating that fuel-specific network variation predicts \rn{} gains independently of immigration-related network proximity. Table~\ref{tab:channel_decomposition} reports both specifications.

\input{tables/tab_channel_decomposition}

\subsection{Donut Design}

The boundary elections (2012, the last pre-treatment; 2014, the first post-treatment) may be contaminated by anticipation effects or delayed baseline adjustment. I drop both and compare pre-treatment elections (2002, 2004, 2007, 2009) to post-treatment elections (2017, 2019, 2022, 2024). Row 7 of Table~\ref{tab:robustness} reports the results: the own coefficient strengthens to 2.21 (SE = 0.45) and the network coefficient strengthens to 1.64 (SE = 0.55), both significant at 1\%. The network coefficient survives the donut restriction, confirming that the finding is not driven by the contrast between the two elections immediately flanking the carbon tax introduction.

\subsection{Pre-Trend-Adjusted Specification}

A joint $F$-test of pre-treatment event study coefficients rejects exact zeros at $p = 0.03$. As discussed in Section~\ref{sec:event_study}, this rejection is driven by negative pre-treatment coefficients---the opposite sign from the post-treatment effect---and one-sided tests against positive pre-trends fail to reject. Nevertheless, I address this conservatively in two ways. First, adding a linear trend interaction ($\text{Net}_d \times (\text{year} - 2012)$) absorbs any linear divergence in \rn{} support correlated with network fuel exposure. Table~\ref{tab:additional_robustness} reports this specification: the network $\times$ post coefficient becomes negative ($-1.02$, SE = 0.56), because the linear trend is highly collinear with the post-treatment indicator---the trend captures most of the treatment variation. This specification is therefore too demanding: it asks whether the network effect exists \emph{beyond} a linear increase that already absorbs the treatment contrast. Second, the \citet{rambachan2023more} sensitivity analysis (Figure~\ref{fig:honestdid}) provides the appropriate formal framework, yielding a robust confidence interval of [0.40, 2.21] at $\bar{M} = 0$ (zero tolerance for pre-trend violations).

\subsection{Triple-Difference: RN vs.\ Green}

If networks specifically transmit fuel grievance to the \rn{} rather than generally shifting all parties, a triple-difference comparing the \rn{} and Green outcomes should isolate the party-specific network channel. I stack \rn{} and Green vote shares, interact all terms with a \rn{} indicator, and estimate the triple-difference at the \dept{} level. The $\text{Net} \times \text{Post} \times \text{RN}$ coefficient captures the differential network effect on \rn{} vs.\ Green support, absorbing any common network-correlated trends across parties.

\subsection{Block RI Power Analysis}
\label{sec:block_power}

The null result from block RI within NUTS-2 regions ($p = 0.88$) could reflect either (a) the true absence of a network effect within regions, or (b) insufficient power given the coarse blocking structure. I conduct a Monte Carlo power analysis: I simulate 1,000 datasets under the alternative hypothesis (true effect = 1.35~pp, the estimated coefficient) and compute block RI $p$-values for each. With 13 NUTS-2 blocks averaging 7 \depts{} each and 500 inner permutations, block RI has limited power to detect the estimated effect at the 5\% level. The approximate minimum detectable effect under \dept{}-clustered SEs with 80\% power is $2.80 \times \text{SE} \approx$ 1.27~pp---close to the estimated effect, confirming that block RI is operating near the power boundary.

\subsection{HonestDiD Sensitivity Analysis}
\label{sec:honestdid}

The event study reveals uniformly negative pre-treatment coefficients, but a joint $F$-test rejects exact zeros ($p = 0.03$). How much pre-trend violation can the post-treatment effect absorb? I apply the \citet{rambachan2023more} sensitivity analysis using the relative magnitudes restriction ($\Delta^{RM}$), constructing confidence intervals robust to pre-trend violations of magnitude up to $\bar{M}$ times the maximum observed pre-treatment coefficient change.

Figure~\ref{fig:honestdid} plots the robust confidence intervals as a function of $\bar{M}$. At $\bar{M} = 0$ (standard parallel trends), the confidence interval is [0.40, 2.21]---positive and bounded away from zero. The interval widens as $\bar{M}$ increases: the lower bound crosses zero at $\bar{M} \approx 0.5$, meaning the effect is robust to pre-trend violations up to half the magnitude of the maximum observed pre-treatment coefficient change. The baseline point estimate is 1.35~pp under standard parallel trends, with the HonestDiD lower bound of 0.40~pp providing the most conservative assessment.

Three features of the pre-treatment pattern support the causal interpretation even with the $p = 0.03$ $F$-test. First, the sign: all four pre-treatment coefficients are negative, the opposite direction from the post-treatment effect. Confounding that operates through the same mechanism as the treatment would produce \emph{positive} pre-trends. Second, the magnitude: the largest pre-treatment coefficient ($-0.48$) is one-third the size of the smallest post-treatment coefficient ($+0.92$). Third, the timing: the break at 2014 is immediate and sustained, inconsistent with a smooth trend that happens to cross zero near the treatment date. The pattern is better described as a structural break coinciding with the carbon tax than as a violation of parallel trends that inflates the post-treatment estimate.
...
\subsection{What the Network Transmits: Economic and Cultural Content}

The descriptive channel decomposition (Section~\ref{sec:horse_race}) reveals that the composite network effect (1.35~pp, HonestDiD lower bound 0.40~pp) bundles economic and cultural content. The fuel-specific component (0.58~pp) survives orthogonalization on immigration exposure and Bartik residualization on observables, confirming that fuel-related information propagates through networks independently. But the immigration-related component is quantitatively larger, and the Oster $\delta = 0.10$ confirms that the two channels overlap substantially.

The interpretation is not that the carbon tax was politically innocuous. Rather, the carbon tax functioned as a \emph{political trigger}: it activated a broader populist realignment that was already latent in France's social geography. The \rn{}'s bundled platform---combining anti-carbon-tax messaging with anti-immigration rhetoric---traveled through the same social connections. The DeGroot model (Section~\ref{sec:framework}) provides the theoretical foundation: the network multiplier amplifies \emph{whatever} fundamental attitudes exist in connected communities, whether those attitudes concern fuel costs, immigration, or distrust of the governing coalition. The carbon tax provided the catalyst; the network structure determined how far the backlash traveled.

\subsection{Implications for Climate Policy}

The standard prescription for politically sustainable carbon pricing is revenue recycling: return the revenue to households as lump-sum dividends \citep{klenert2018carbon, stiglitz2019addressing}. The reduced-form results suggest this is necessary but may be insufficient. Revenue recycling eliminates variation in the economic ``shift'' but leaves the network structure unchanged---and the descriptive decomposition shows that immigration-related content travels through the same connections as fuel grievance. The implication is that once the carbon tax activates a network-mediated backlash, the message travels regardless of whether the economic burden is compensated. Policy designs that target network-visible salience (e.g., embedding the tax in wholesale prices rather than printing it on retail receipts) may be more effective at limiting political contagion than transfers that arrive invisibly through the tax system.
...
The carbon tax is the economists' first-best instrument for addressing climate change. It is also, in practice, a political catalyst---and the reaction it triggers travels through social networks in ways that standard policy analysis misses.

Social networks transmitted the political backlash against France's carbon tax far beyond its economic incidence. Network exposure to fuel-vulnerable areas raised \rn{} vote share by 1.35 percentage points per standard deviation, with a HonestDiD lower bound of 0.40~pp under zero tolerance for pre-trend violations. The effect activated at the 2014 carbon tax introduction---years before the \gj{} protests---persists through five post-treatment elections, scales with the tax rate, and is absent for Green and center-right parties.

The descriptive channel decomposition reveals that networks transmit a bundle of populist mobilization cues, not a single-issue message. Fuel-specific and immigration-related network exposures both predict \rn{} gains independently, consistent with the carbon tax functioning as a political trigger that activated a broader realignment already latent in France's social geography.

The identification strategy produces pre-treatment coefficients that are the mirror image of the post-treatment effect: uniformly negative rather than positive. This pattern supports the causal interpretation even though a joint $F$-test rejects exact zeros. Inference from the three methods designed for shift-share settings---AKM standard errors, wild cluster bootstrap, and shift-level randomization inference---all confirm significance.

For climate policy, the implication is uncomfortable. The political feasibility of carbon pricing depends not on the average cost per household---which is small---but on whether the cost falls on communities that are socially connected to the broader electorate. Revenue recycling can offset the economic burden. But it cannot recall the message.
