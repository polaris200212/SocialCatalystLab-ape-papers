## ============================================================================
## 05_figures.R -- Connected Backlash (apep_0464)
...
## Election years (vertical reference lines)
election_years <- c(2012, 2014, 2017, 2019, 2022, 2024)

p1 <- ggplot(carbon_tax, aes(x = year, y = rate_eur_tco2)) +
  ...
  annotate("text", x = election_years, y = -3,
           ...
           angle = 0) +
  scale_x_continuous(breaks = seq(2013, 2024, 1), minor_breaks = NULL,
  ...
  labs(
    ...
    subtitle = "Vertical dashed lines mark national election years",
    x = "Year",
...
save_fig(p1, "fig_carbon_tax", width = 8, height = 5)
...
## ============================================================================
## FIGURE 6: Event Study Plot (Two Panels)
## ============================================================================
...
p6a <- ggplot(es_own, aes(x = year, y = estimate)) +
  ...
  ## Annotation
  annotate("text", x = 2014, y = max(es_own$ci_hi, na.rm = TRUE) * 1.1,
           label = "Ref.", size = 2.8, color = "grey50",
           fontface = "italic") +
  scale_x_continuous(breaks = c(2012, 2014, 2017, 2019, 2022, 2024)) +
  labs(
...
p6 <- p6a + p6b +
  plot_annotation(
    ...
    subtitle = paste0("Interaction of standardized exposure with election ",
                      "indicators (ref. = 2014); 95% CIs clustered by d\u00e9partement"),
    theme = theme_apep() +
      theme(
...
save_fig(p6, "fig_event_study", width = 12, height = 5.5)
...
p7 <- ggplot(rn_by_q, aes(x = year, y = mean_rn,
  ...
  labs(
    ...
    subtitle = "D\u00e9partement-level means weighted by total votes; 95% CIs",
    x = "Election Year",
...
save_fig(p7, "fig_rn_trajectory", width = 9, height = 5.5)
...
## Compute RN change (2024 - 2012) at the département level
rn_change <- dept_panel %>%
  ...
  pivot_wider(names_from = year, values_from = rn_share,
              names_prefix = "rn_") %>%
  mutate(delta_rn = rn_2024 - rn_2012) %>%
...
p8 <- ggplot(bin_means, aes(x = x_mean, y = y_mean)) +
  ...
  labs(
    ...
    y = expression(bold(Delta * " RN vote share 2024 \u2013 2012 (pp, residualized)"))
  ) +
  theme_apep()
