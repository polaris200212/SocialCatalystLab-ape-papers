...
{\large APEP Working Paper apep\_0464\_v5\footnote{This paper is a revision of APEP-0464. See \url{https://github.com/SocialCatalystLab/ape-papers/tree/main/apep_0464} for previous versions. All analysis conducted in R; replication code and data are available in the paper repository.}}
...
\begin{abstract}
\noindent Social networks transmitted the political backlash against France's carbon tax through economic and cultural channels. Using Facebook's Social Connectedness Index across 96 \depts{} and ten elections (2002--2024), I estimate a shift-share network fuel exposure effect of 1.35~pp on \rn{} vote share (0.87--1.35 under pre-trend adjustment). A horse-race reveals dual channels: fuel-specific network exposure contributes 0.58~pp ($p = 0.07$) while network exposure to low-immigration areas independently predicts RN support ($-1.41$~pp, $p < 0.01$), though the channels become collinear with local controls. An event study shows activation at the 2014 carbon tax introduction---before the 2018 \gj{} protests---and a timing decomposition confirms effects in both the early tax period and post-protest era. The effect is absent for Green and center-right parties, robust to a pre-treatment migration proxy, and survives a donut design.
\end{abstract}
...
How do localized economic shocks produce nationwide political realignment? The standard account in the populism literature traces a direct path: a trade shock or austerity measure hits a community, residents feel the cost, and they vote for outsider parties \citep{autor2013china, colantone2018trade, fetzer2019did}. This account explains why affected communities shift politically, but not why the shift radiates far beyond the geographic footprint of the shock itself. France's carbon tax provides a striking illustration: the \gj{} movement erupted in November 2018 across communities where the tax's direct cost was modest, and the \rn{}'s vote share surged most in \depts{} whose residents were socially connected to fuel-vulnerable areas, not only in the fuel-vulnerable areas themselves.

This paper shows that social networks amplify and transmit the populist realignment triggered by the carbon tax era, through multiple channels. I measure network structure using Facebook's Social Connectedness Index (SCI) across 96 French \depts{} and construct shift-share exposure measures that combine network weights with \dept{}-level fuel vulnerability and immigration shares. In a panel of ten national elections from 2002 to 2024 ($N = 960$ at the \dept{} level), I estimate a reduced-form composite network fuel exposure effect of 1.35 percentage points per standard deviation on \rn{} vote share. But this composite bundles multiple channels. A horse-race decomposition---the paper's central result---reveals that \emph{both} economic vulnerability and cultural attitudes propagate through networks: the fuel-specific component is 0.58~pp ($p = 0.07$) while SCI-weighted immigration network exposure enters with $-1.41$~pp ($p < 0.01$)---\depts{} whose networks reach low-immigration areas shift more toward the \rn{}. Social networks do not merely transmit fuel grievance; they transmit the full package of populist mobilization that the carbon tax era activated.

The identification strategy is a shift-share design \citep{goldsmith2020bartik, borusyak2022quasi}. The ``shares'' are SCI network weights capturing pre-existing social connections. The ``shifts'' are \dept{}-level fuel vulnerability scores from commuting patterns. The key assumption is that, conditional on own fuel vulnerability and two-way fixed effects, the SCI-weighted average of other \depts{}' fuel vulnerability is uncorrelated with unobserved determinants of \rn{} support. An event study spanning five pre-treatment and five post-treatment elections shows pre-treatment coefficients that are uniformly negative (opposite sign from the post-treatment effect), though a joint $F$-test rejects exact parallel trends at $p = 0.03$. A \citet{rambachan2023more} sensitivity analysis confirms that the post-treatment effect survives under plausible violations of the parallel trends assumption.

A timing decomposition addresses the concern that the binary post-2014 indicator conflates the carbon tax with other political shifts. I decompose the treatment period into the early carbon-tax era (2014--2017, \euro 7--30.5/tCO$_2$) and the post-\gj{} era (2019+, \euro 44.6/tCO$_2$). Network effects are present in \emph{both} periods, confirming that the channel activated at the carbon tax's introduction---years before the protests amplified it. The continuous specification exploiting four distinct tax rates (\euro 0, 7, 30.5, 44.6) shows dose-responsiveness: the network effect scales with the tax level, not merely with being in the post-2014 era.

The SCI is measured in 2024---a post-treatment snapshot. I address this in three ways. First, a migration-based proxy constructed from 2013 inter-\dept{} residential mobility ($\rho = 0.66$ with SCI) replicates the results. Second, the fact that SCI ``predicts'' \rn{} support with the \emph{opposite sign} in the pre-treatment period supports network pre-determination: the social structure existed before the carbon tax, but the political content it transmitted changed after 2014. Third, under classical measurement error, the migration-SCI discordance implies attenuation: our estimate is a lower bound on the true network effect if the 2024 SCI is a noisy proxy for the 2013 network.

I contribute to three literatures. The political economy of climate policy literature \citep{douenne2022yellow, klenert2018carbon, stantcheva2022understanding} focuses on how direct costs shape preferences. I show that network-mediated transmission extends the political footprint of carbon pricing far beyond its economic incidence, and that the network channel carries both economic (fuel) and cultural (immigration) content. The populism literature \citep{autor2013china, rodrik2021populism, funke2016going, guiso2019global} typically studies local shocks affecting local voters. The network channel implies that standard models of economic voting, which condition on local conditions alone, underestimate the political fallout from geographically concentrated shocks. The social networks literature \citep{fluckiger2025bro, enikolopov2020social, bailey2018social, chetty2022social1} gains an empirical setting with a known policy shock, observed network structure, and high-stakes electoral outcomes where the horse-race between economic and cultural network channels decomposes the mechanism.

The \dept{}-level specification ($N = 960$) operates at the level of the identifying variation, with inference robust to the shift-share structure \citep{adao2019shift} and corroborated by seven methods including wild cluster bootstrap ($p = 0.005$). A donut design dropping boundary elections, a pre-trend-adjusted specification, triple-differencing (RN vs.\ Green parties), and \citet{rambachan2023more} sensitivity analysis further support the findings. Block randomization inference within NUTS-2 regions produces a null ($p = 0.88$), consistent with the design relying on cross-region variation in network exposure---a formal power analysis confirms this test is severely underpowered for the effect size and blocking structure.

Section~\ref{sec:background} describes the institutional background. Sections~\ref{sec:framework}--\ref{sec:strategy} lay out the framework, data, and empirical strategy. Section~\ref{sec:results} presents results including the horse-race decomposition and timing analysis. Section~\ref{sec:robustness} conducts extensive robustness checks. Section~\ref{sec:spatial} provides a spatial dependence analysis. Section~\ref{sec:discussion} interprets the findings.
...
\subsection{Horse-Race: Fuel vs.\ Immigration Network Channels}
\label{sec:horse_race}

The reduced-form composite network coefficient (1.35~pp) bundles all information transmitted through SCI ties. This subsection decomposes the network channel by running a horse-race between SCI-weighted fuel vulnerability and SCI-weighted immigration exposure. I construct a parallel network variable: $\text{Net\_Immigration}_d = \sum_j w_{dj} \times \text{ImmigrationShare}_j$, using the same SCI weights but immigration shares as shifts. The two network variables are moderately correlated ($r = -0.39$).

Table~\ref{tab:horse_race} presents the horse-race. Column (A) shows the baseline: network fuel exposure alone yields 1.35~pp (SE = 0.46). Column (B) replaces fuel with immigration: network immigration exposure alone has a coefficient of $-1.61$ (SE = 0.29), indicating that \depts{} whose social networks reach predominantly low-immigration areas shift more strongly toward the \rn{} after the carbon tax---consistent with cultural insularity amplifying populist backlash. Column (C) is the key result: when both network variables enter simultaneously, the fuel coefficient attenuates to 0.58 (SE = 0.32, $p = 0.07$)---a 57\% reduction---while network immigration remains large and significant ($-1.41$, SE = 0.26). Column (D) adds own-\dept{} immigration share $\times$ Post; the fuel coefficient falls further to 0.47 (SE = 0.28, $p < 0.10$). Notably, the network immigration coefficient flips sign and becomes insignificant in Column (D) (0.37, SE = 0.72). This reflects severe collinearity between network and own immigration exposure ($r = 0.86$, VIF $= 3.9$): the two variables compete for the same cross-sectional variation, making it impossible to separately identify network vs.\ local immigration channels. I interpret Column (C)---which separates fuel from immigration network channels at moderate collinearity ($r = -0.39$, VIF $= 1.2$)---as the most informative specification, and Column (D) as documenting the collinearity limit rather than providing a credible decomposition.

\input{tables/tab_horse_race}

The horse-race yields three findings. First, fuel network exposure partially survives: the attenuated coefficient remains positive and marginally significant, consistent with fuel-specific information propagating through social ties. Second, immigration network exposure is an independent and powerful predictor---\depts{} whose networks reach culturally homogeneous (low-immigration) areas shifted more toward the \rn{} after 2014, consistent with insularity amplifying populist backlash. Third, both channels operate simultaneously through the same network structure. The original reduced-form specification attributed some immigration-driven network effects to fuel vulnerability because the two shift-share variables share common SCI ``shares.'' The honest interpretation is that social networks transmitted the full populist realignment---economic grievance \emph{and} cultural backlash---not a single-issue carbon tax message.

An \citet{oster2019unobservable} bound of $\delta = 0.10$ confirms this decomposition: unobservables would need to be only one-tenth as important as immigration controls to fully explain away the fuel-specific network coefficient. This falls below the $|\delta| > 1$ threshold, reinforcing that the fuel channel is real but modest once immigration is separated out.
...
The shift-share structure of the network exposure variable raises inference challenges: the ``shares'' (SCI weights) are common across units within each \dept{}, inducing correlation that standard cluster-robust SEs may not fully capture. I address this with nine inference methods, summarized in Table~\ref{tab:inference}.
...
Block RI (NUTS-2 regions) & $< 0.01$ & 0.883 & 10,000 & Within 13 regions \\
Block RI (urbanization quartiles) & 0.025 & 0.308 & 2,000 & Within fuel-vuln quartiles \\
...
\begin{tablenotes}
\small
\item \textit{Notes:} All methods test $H_0: \beta = 0$ in the \dept{}-level specification (Model D2, population-weighted). AKM SEs account for cross-sectional correlation induced by shared shift-share ``shares'' \citep{adao2019shift}. Two-way clustering allows for both \dept{} and election-level dependence. Conley SEs use a 300~km bandwidth based on \dept{} centroids \citep{conley1999gmm}. Standard RI permutes fuel vulnerability across all \depts{}. Block RI (NUTS-2) permutes within 13 administrative regions. Block RI (urbanization) permutes within quartiles of fuel vulnerability. Shift-level RI follows \citet{borusyak2022quasi}: permutes shifts within population-density terciles holding SCI shares fixed. WCB follows \citet{cameron2008bootstrap}.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
Five of the analytical methods yield $p$-values below 0.05 for the network coefficient: clustered SEs, AKM, two-way clustering, Conley HAC, and the wild cluster bootstrap ($p = 0.005$). The standard randomization inference test produces a marginal $p = 0.072$---suggestive but not conventionally significant---reflecting the conservative nature of permutation-based tests with 96 clusters.

The block RI within NUTS-2 regions ($p = 0.883$) is far from significant. This result requires transparent interpretation. Block RI within NUTS-2 regions tests whether the network effect survives when only \emph{within-region} variation in fuel vulnerability is permuted---but with only 5--10 \depts{} per region, the permutation space is small. A Monte Carlo power analysis (Section~\ref{sec:block_power}) confirms this: simulating data under the alternative hypothesis with the true effect size, block RI within 13 NUTS-2 regions has power well below 50\% to reject at the 5\% level. The block RI null is consistent with the design relying on \emph{cross-region} variation in network exposure---the distance-bin decomposition confirms the strongest signal originates beyond 200~km, spanning regional boundaries.

To assess whether the null is specific to the NUTS-2 blocking structure, I also run block RI within urbanization quartiles and shift-level RI within population-density terciles \citep{borusyak2022quasi}. Block RI within urbanization quartiles yields $p = 0.31$ for the network coefficient---also null, confirming that within-group permutation lacks power in small samples. However, the shift-level RI \citep{borusyak2022quasi}---which permutes the \emph{shifts} (fuel vulnerability) across \depts{} within density terciles while holding the SCI ``shares'' fixed---yields $p = 0.02$ for the network coefficient. This is the inference method specifically designed for shift-share designs and confirms that the result is driven by the quasi-random assignment of fuel vulnerability conditional on urbanization, not by the network structure itself. \input{tables/tab_ri_comparison}

Table~\ref{tab:ri_comparison} reports all RI variants. The pattern across blocking structures reveals which dimensions of variation drive the result---cross-region ties between differently urbanized areas, consistent with the paper's core mechanism.
...
The joint $F$-test of pre-treatment event study coefficients rejects at $p = 0.03$, indicating a mild pre-existing correlation between network fuel exposure and \rn{} trends. I address this by adding linear trend interactions ($\text{Own}_d \times (\text{year} - 2012)$ and $\text{Net}_d \times (\text{year} - 2012)$) to the main specification, which absorb any linear divergence in \rn{} support correlated with fuel exposure. Table~\ref{tab:additional_robustness} reports the pre-trend-adjusted result: the network coefficient falls from 1.35 to 0.87 (SE = 0.34, $p < 0.05$), providing a lower bound on the effect. The \citet{rambachan2023more} sensitivity analysis provides additional quantification: the effect is robust to pre-trend violations up to $\bar{M}$ times the maximum observed pre-treatment coefficient change (Figure~\ref{fig:honestdid}).
...
Figure~\ref{fig:honestdid} plots the robust confidence intervals as a function of $\bar{M}$. At $\bar{M} = 0$ (the standard parallel trends assumption), the confidence interval is positive and bounded away from zero. However, the interval widens as $\bar{M}$ increases: the lower bound crosses zero at moderate levels of $\bar{M}$, reflecting the non-trivial pre-treatment coefficients. I report results for three estimands: the last post-period (2024), the first post-period (2014), and the average across all five post-periods. The first post-period is the most informative for the timing question: if the effect at 2014 survives pre-trend adjustment, it confirms that the network channel activated at the carbon tax's introduction rather than emerging gradually.

The sensitivity analysis yields a candid assessment. The overall pattern---negative pre-treatment coefficients followed by a positive break at 2014---is more consistent with a treatment effect than a smooth trend. However, a joint $F$-test rejects exact parallel trends ($p = 0.03$), and the formal bounds confirm that the result is sensitive to moderate pre-trend violations. I therefore present the pre-trend-adjusted specification (which adds a linear trend interaction and yields a network coefficient of 0.87, SE = 0.34) as a lower bound on the effect, and the baseline estimate of 1.35~pp as an upper bound. The true network effect plausibly lies between these estimates.
...
\subsection{Dual Channels: Economic Vulnerability and Cultural Backlash}

The horse-race decomposition is the paper's central finding. The reduced-form composite network effect (1.35~pp) bundles two distinct channels: fuel-specific network exposure (0.58~pp, marginally significant) and immigration-connected network exposure ($-1.41$~pp, highly significant). This decomposition has three implications.

First, the fuel-specific channel is \emph{real but modest}. The carbon tax did generate network-mediated political backlash through fuel vulnerability, but the pure economic channel explains less than half the composite effect. This is consistent with the carbon tax functioning as a \emph{political trigger}---it activated broader populist grievances that were already latent in the network structure.

Second, the immigration channel is quantitatively larger and operates through the same SCI network structure. D\'{e}partements connected to high-immigration areas shifted toward the \rn{} after 2014, through social ties. This is consistent with the \rn{}'s bundled platform: the party combined anti-carbon-tax messaging with anti-immigration rhetoric, and both messages traveled through the same social networks.

Third, the Oster $\delta = 0.10$ confirms that the fuel-specific coefficient is sensitive to omitted variables at the scale of immigration controls. This is not a weakness---it is a precise quantification of how much the two channels overlap. The honest conclusion is that social networks transmitted the full populist realignment, not a single-issue carbon tax message. The DeGroot model (Section~\ref{sec:framework}) provides the theoretical foundation: the network multiplier amplifies \emph{whatever} fundamental attitudes exist in connected communities, whether economic or cultural.
...
This paper makes three contributions. First, I show that social networks transmitted the populist backlash against France's carbon tax through multiple channels. A horse-race decomposition reveals that both fuel vulnerability (0.58~pp, $p = 0.07$) and immigration attitudes ($-1.41$~pp, $p < 0.01$) propagate through SCI-weighted social ties, with the composite reduced-form effect at 1.35~pp. The carbon tax did not merely generate economic grievance---it activated a network-mediated populist realignment that bundled economic and cultural channels.

Second, the network channel activated at the carbon tax's 2014 introduction, years before the \gj{} protests. A timing decomposition confirms effects in both the early carbon-tax era (2014--2017) and the post-protest period (2019+), and the continuous specification shows dose-responsiveness across four tax levels. The effect is specific to the \rn{}---absent for Green and center-right parties---and robust to a pre-treatment migration proxy, a donut design, and distance restrictions isolating social from geographic proximity.

Third, I provide a spatial dependence analysis bounding the total network effect: the reduced-form estimate is a lower bound, while the SAR counterfactual provides an illustrative upper bound of approximately 2.4$\times$ amplification, contingent on the SAR interpretation. The gap reflects the fundamental difficulty of distinguishing network contagion from spatially correlated shocks.

For policymakers, the implication is uncomfortable. The political feasibility of carbon pricing depends not on the average cost per household---which is small---but on whether the cost falls on communities that are well-connected to the rest of the electorate. Revenue recycling can offset the economic burden. But it cannot recall the message that the carbon tax sent through social networks---a message that bundled fuel costs with broader populist mobilization and reached communities far beyond the policy's direct economic footprint.
