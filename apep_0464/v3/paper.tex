...
{\large APEP Working Paper apep\_0464\_v3\footnote{This paper is a revision of APEP-0464. See \url{https://github.com/SocialCatalystLab/ape-papers/tree/main/apep_0464} for previous versions. All analysis conducted in R; replication code and data are available in the paper repository.}}
...
\begin{abstract}
\noindent On November 17, 2018, 300,000 French citizens donned yellow vests to protest a carbon tax---but the protests spread far beyond where the tax hit hardest. This paper shows that social networks transmitted the backlash. Using Facebook's Social Connectedness Index to measure ties between French \depts{}, I construct a shift-share measure of network exposure to fuel-vulnerable communities. At the \dept{}-level ($N = 960$), a one-standard-deviation increase in network exposure raises \rn{} vote share by 1.35 percentage points after the carbon tax, significant under shift-share-robust inference \citep{adao2019shift}. An event study shows a sharp structural break at 2014 with no positive pre-trend. The effect is absent for Green and center-right parties, robust to a pre-treatment migration-based network proxy, individual time-varying controls, and distance restrictions isolating social from geographic proximity. A spatial dependence analysis bounds the total network effect between 1.35 pp (reduced-form) and approximately 11 pp (structural upper bound).
\end{abstract}
...
The answer is yes, and the network channel is large. In a panel of 96 metropolitan \depts{} observed across ten national elections from 2002 to 2024 ($N = 960$), I estimate that a one-standard-deviation increase in network fuel exposure raises the \rn{} first-round vote share by 1.35 percentage points in the post-carbon-tax period, with population-weighted \dept{}-level regressions as the primary specification. The effect of a \dept{}'s own fuel vulnerability is comparable: 1.72 percentage points. Both effects are robust across a battery of spatial and shift-share inference methods, including AKM standard errors \citep{adao2019shift}, two-way clustering, Conley spatial HAC, and the wild cluster bootstrap ($p = 0.005$).
...
Six features of the research design support this assumption. First, an event study spanning ten elections---five before and five after the carbon tax introduction in 2014---shows that pre-treatment network exposure coefficients are uniformly small and negative (opposite sign from the post-treatment effect), with a sharp structural break at 2014. Second, I exploit the continuous variation in the carbon tax rate itself, which rose from zero to \euro 44.60, providing four distinct dose levels. Third, restricting the SCI matrix to \dept{} pairs separated by more than 200 kilometers preserves the network effect, and a distance-bin decomposition shows that the strongest identifying signal originates from ties beyond 400~km---supporting social over geographic proximity. Fourth, a pre-treatment migration-based network proxy (constructed from 2013 inter-\dept{} residential mobility flows, which pre-dates the carbon tax by five years) produces qualitatively similar results, addressing the concern that the 2024-vintage SCI may reflect post-treatment network changes. Fifth, a battery of time-varying controls---unemployment, education, immigration, and industry shares, each interacted with Post---leaves the network coefficient significant when added individually, though the most demanding kitchen-sink specification with \dept{}-specific linear trends absorbs it (an expected cost of combining time-invariant shift-share ``shares'' with unit-specific trends). Sixth, placebo outcomes confirm specificity: network fuel exposure has no effect on Green or center-right parties, and a placebo timing test assigning a fake treatment date of 2009 produces an insignificant network coefficient, while the 2007 placebo yields a smaller but marginally significant effect (consistent with the limited pre-treatment sample).
...
Several methodological features distinguish this paper. The \dept{}-level specification ($N = 960$) operates at the level of the identifying variation, with inference robust to the shift-share structure \citep{adao2019shift} and corroborated by seven distinct methods including AKM standard errors, two-way clustering, Conley spatial HAC, randomization inference, and wild cluster bootstrap. The expanded panel with five pre-treatment elections provides a stronger test of parallel trends than typical in the political economy literature. The continuous treatment specification exploits the sixfold variation in the carbon rate. The migration-proxy validation addresses a fundamental concern about post-treatment network measurement that all SCI-based research faces. And the spatial dependence analysis (SAR vs.\ SEM vs.\ SDM) follows best practices in spatial econometrics \citep{lesage2009introduction, anselin1988spatial}, with results framed as bounds rather than point estimates.

Section~\ref{sec:background} describes the institutional background. Sections~\ref{sec:framework}--\ref{sec:strategy} lay out the framework, data, and empirical strategy, including a formal statement of the estimand under interference. Sections~\ref{sec:results}--\ref{sec:robustness} present results. Section~\ref{sec:spatial} provides a spatial dependence analysis. Section~\ref{sec:discussion} interprets the findings.
...
\subsection{D\'{e}partement-Level Results (Primary)}

Table~\ref{tab:dept} presents the primary \dept{}-level results. The population-weighted specification (Model D2) yields a network coefficient of 1.35~pp (SE = 0.46, $p < 0.01$) and an own-exposure coefficient of 1.72~pp (SE = 0.37). The unweighted specification (Model D1) shows a larger own effect (1.80~pp) but an attenuated, insignificant network coefficient (0.41, SE = 0.46), reflecting greater noise in small-population \depts{}. The continuous specification (Model D3) confirms dose-responsiveness. Model D4 applies two-way clustering to the unweighted specification; the own coefficient remains significant ($p < 0.05$).
...
\begin{tablenotes}
\small
\item \textit{Notes:} Dependent variable is aggregate \rn{}/\fn{} vote share at the \dept{} level. $N = 960$ reflects 96 metropolitan \depts{} $\times$ 10 elections. Pop-weighted uses registered voters. Model D4 clusters standard errors by both \dept{} and election. $^{***}p<0.01$, $^{**}p<0.05$, $^{*}p<0.10$.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
If the network effect reflects a genuine response to the carbon tax rather than a pre-existing differential trend, assigning fake treatment dates before 2014 should produce null effects. I estimate the primary specification with $\text{Post}_t = \mathbf{1}[t \geq k]$ for $k \in \{2007, 2009\}$, restricting the sample to pre-2014 elections.

The 2009 placebo yields a network coefficient of 0.30 (SE = 0.17, $t = 1.75$), statistically indistinguishable from zero. The 2007 placebo produces a coefficient of 0.51 (SE = 0.21, $t = 2.50$), which is marginally significant---though notably less than half the magnitude of the actual treatment effect (1.35~pp). This partial failure has a mechanical explanation: with the sample restricted to pre-2014 elections, the 2007 cutoff leaves only two pre-treatment elections (2002, 2004) and three ``post'' elections (2007, 2009, 2012), yielding a severely unbalanced panel where any idiosyncratic election-to-election variation can register as a ``treatment effect.'' The 2009 placebo, which provides a more balanced split (three pre, two post), produces a null result. The actual treatment date (2014) produces the significant coefficient of 1.35~pp at the \dept{} level (Model D2), more than double the 2007 placebo effect and estimated on the full panel with five pre- and five post-treatment elections, consistent with a genuine structural break at the carbon tax introduction rather than a continuation of pre-existing trends.
...
A spatial dependence analysis bounds the total network effect: the reduced-form estimate of 1.35~pp per standard deviation (population-weighted \dept{}-level specification) is a lower bound, while the SAR counterfactual suggests an upper bound of approximately 11~pp through network amplification. The gap between these bounds reflects the fundamental difficulty of distinguishing network contagion from spatially correlated shocks with observational data. The reduced-form evidence for network transmission is strong; the structural interpretation requires the additional assumption that the SAR correctly models the data-generating process.
