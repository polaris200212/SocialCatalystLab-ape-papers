...
packages <- c(
  ...
  # Data access
  "httr", "jsonlite", "fredr"  # FRED API for economic controls
)
...
# Optional: fwildclusterboot for wild cluster bootstrap inference
# May not be available for all R versions
HAS_FWILDCLUSTERBOOT <- FALSE
if (!require("fwildclusterboot", quietly = TRUE)) {
  tryCatch({
    install.packages("fwildclusterboot")
    library(fwildclusterboot)
    HAS_FWILDCLUSTERBOOT <- TRUE
    cat("fwildclusterboot installed successfully.\n")
  }, error = function(e) {
    cat("Note: fwildclusterboot not available. Wild bootstrap inference will be simulated via standard methods.\n")
  })
} else {
  HAS_FWILDCLUSTERBOOT <- TRUE
}

# Optional: DIDmultiplegt for de Chaisemartin-D'Haultfoeuille estimator
# Has complex dependencies (GLPK, lme4) that may not be available
HAS_DIDMULTIPLEGT <- FALSE
if (!require("DIDmultiplegt", quietly = TRUE)) {
  tryCatch({
    install.packages("DIDmultiplegt")
    library(DIDmultiplegt)
    HAS_DIDMULTIPLEGT <- TRUE
    cat("DIDmultiplegt installed successfully.\n")
  }, error = function(e) {
    cat("Note: DIDmultiplegt not available (complex dependencies). Using did and fixest for heterogeneity-robust estimation.\n")
  })
} else {
  HAS_DIDMULTIPLEGT <- TRUE
}
...
if (!require("HonestDiD", quietly = TRUE)) {
  tryCatch({
    if (require("Rglpk", quietly = TRUE)) {
      ...
      library(HonestDiD)
      cat("HonestDiD installed successfully.\n")
    } else {
      cat("Note: HonestDiD not available (requires GLPK). Sensitivity analysis will use alternative methods.\n")
    }
  }, error = function(e) {
    cat("Note: HonestDiD not available. Sensitivity analysis will use alternative methods.\n")
  })
}
...
cat("\n============================================\n")
cat("Paper 166: Packages and environment loaded\n")
cat("(Revision of apep_0076 - EITC and Crime)\n")
