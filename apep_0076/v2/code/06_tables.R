...
# Get 2019 generosity from the historical rates
eitc_rates_2019 <- analysis_data %>%
  filter(year == 2019, has_eitc == 1) %>%
  select(state_abbr, eitc_generosity) %>%
  distinct()

eitc_details <- eitc_policy %>%
  left_join(eitc_rates_2019, by = "state_abbr") %>%
  arrange(eitc_adopted) %>%
  select(
    State = state_name,
    `Year Adopted` = eitc_adopted,
    `% of Federal (2019)` = eitc_generosity,
    Refundable = refundable
  ) %>%
  mutate(
    Refundable = if_else(Refundable, "Yes", "No"),
    `% of Federal (2019)` = round(`% of Federal (2019)`, 0)
  )
...
# Extract key results - use correct component names from how we saved
cs_summary <- data.frame(
  Estimator = c("Overall ATT (Property Crime)", "Overall ATT (Violent Crime)"),
  Estimate = c(
    cs_results$property_agg$overall.att,
    cs_results$violent_agg$overall.att
  ),
  SE = c(
    cs_results$property_agg$overall.se,
    cs_results$violent_agg$overall.se
  )
)
