...
## v2: Refine detection to use T1019-specific rates where available,
## plus add median payment per claim as robustness metric
...
detect_rate_change <- function(dt, threshold = 0.15, sustain_months = 3,
  ...
  for (s in states) {
    ...
    ## v2: Use T1019-specific rate if available and requested
    if (use_t1019 && !is.null(sd$t1019_rate) && sum(!is.na(sd$t1019_rate)) > 12) {
      sd[, rate_var := t1019_rate]
      ## Fill gaps with aggregate
      sd[is.na(rate_var), rate_var := avg_paid_per_claim]
    } else {
      sd[, rate_var := avg_paid_per_claim]
    }

    ## Compute rolling 3-month median to smooth noise
    sd[, rate_smooth := frollmean(rate_var, n = 3, align = "right", fill = NA)]
    ...
    pre_rate <- sd[month_date < jump_date, mean(rate_var, na.rm = TRUE)]

    if (nrow(post) >= sustain_months) {
      post_rate <- mean(post$rate_var, na.rm = TRUE)
...
## ---- 2b. v2: Median-based rate detection (robustness) ----
cat("\n--- Median-based rate detection (robustness) ---\n")

detect_rate_change_median <- function(dt, threshold = 0.15, sustain_months = 3) {
  states <- unique(dt$state)
  results <- list()
  ...
  for (s in states) {
    sd <- dt[state == s][order(month_date)]
    if (nrow(sd) < 12) next
    sd[, rate_smooth := frollmean(median_paid_per_claim, n = 3,
                                  align = "right", fill = NA)]
    sd[, rate_pct_change := (rate_smooth - shift(rate_smooth, 1)) /
                             shift(rate_smooth, 1)]
    jumps <- sd[!is.na(rate_pct_change) & rate_pct_change >= threshold]
    if (nrow(jumps) == 0) {
      results[[s]] <- data.table(state = s, treat_date = as.Date(NA), treated = FALSE)
      next
    }
    first_jump <- jumps[1]
    jump_date <- first_jump$month_date
    post <- sd[month_date >= jump_date][1:min(sustain_months + 1, .N)]
    pre_rate <- sd[month_date < jump_date, mean(median_paid_per_claim, na.rm = TRUE)]
    if (nrow(post) >= sustain_months) {
      post_rate <- mean(post$median_paid_per_claim, na.rm = TRUE)
      sustained <- (post_rate / pre_rate - 1) >= threshold * 0.5
    } else {
      sustained <- FALSE
    }
    results[[s]] <- data.table(state = s,
...
rc_median <- detect_rate_change_median(panel_pc, threshold = 0.15)
cat(sprintf("Median-based detection: %d treated, %d never-treated\n",
            sum(rc_median$treated), sum(!rc_median$treated)))
...
## v2: Log of intensive margin
panel_pc[, log_claims_per_provider := log(claims_per_provider + 1)]
...
saveRDS(rc_median,     file.path(DATA, "rate_changes_median.rds"))
