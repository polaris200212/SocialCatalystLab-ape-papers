...
## Pick 4 illustrative states (large rate changes + 2 never-treated)
...
## Add vertical line data for treatment dates
...
p2 <- ggplot(treat_timing, aes(x = treat_date)) +
  ...
  labs(x = "Date of Rate Increase", y = "Number of States",
       ...
       subtitle = "Based on detected sustained jumps in personal care payment rates",
...
p5 <- ggplot(trends, aes(x = month_date, y = mean_providers, color = group)) +
...
## Compute post-treatment average outcomes by state
...
p8 <- ggplot(post_means, aes(x = pct_change * 100, y = mean_log_prov)) +
