...
## ----------------------------------------------------------
## 6. Construct Election-Term Dataset
##    KEY REVISION: use election-year-specific population as
##    running variable (not averaged across all years)
##    Addresses reviewer concern about sharp assignment
## ----------------------------------------------------------

cat("\n=== Constructing election-term dataset ===\n")

# Define election terms and the budget year to use for population
# Budget data starts 2010, so 2007 term uses 2010 population
election_terms <- data.table(
  election_year = c(2007L, 2011L, 2015L, 2019L, 2023L),
  pop_year      = c(2010L, 2011L, 2015L, 2019L, 2023L),
  term_start    = c(2010L, 2011L, 2015L, 2019L, 2023L),
  term_end      = c(2010L, 2014L, 2018L, 2022L, 2023L)
)

# Extract election-year population for each municipality-term
pop_election <- pop_lookup[, .(ine_code, year, pop_elec = poblacion)]

share_cols_all <- grep("^share_", names(panel), value = TRUE)
outcome_cols <- c(share_cols_all, "edu_hhi", "edu_pc", "spending_pc",
                  "social_pc", "security_pc", "edu_share_total",
                  "total_edu", "total_spending")

# Per-capita education subcategory spending (levels, not shares)
edu_level_cols <- grep("^share_", names(panel), value = TRUE)
edu_3digit_codes <- gsub("share_", "", edu_level_cols)

# Build election-term level dataset
term_list <- list()

for (i in seq_len(nrow(election_terms))) {
  ey    <- election_terms$election_year[i]
  py    <- election_terms$pop_year[i]
  t_s   <- election_terms$term_start[i]
  t_e   <- election_terms$term_end[i]

  # Average outcomes within this term
  term_data <- panel[election_year == ey & year >= t_s & year <= t_e,
    lapply(.SD, function(x) mean(x, na.rm = TRUE)),
    by = .(ine_code),
    .SDcols = intersect(outcome_cols, names(panel))
  ]

  # Merge election-year specific population
  term_pop <- pop_election[year == py, .(ine_code, pop_elec)]
  term_data <- merge(term_data, term_pop, by = "ine_code", all.x = TRUE)

  # Merge council gender composition for this election
  term_gender <- council_gender[election_year == ey,
    .(ine_code, female_share, n_female, n_total)]
  term_data <- merge(term_data, term_gender, by = "ine_code", all.x = TRUE)

  term_data[, election_year := ey]

  # Treatment indicators based on election-year population
  term_data[, treated_5000 := (pop_elec >= 5000)]
  term_data[, treated_3000 := (pop_elec >= 3000)]
  term_data[, pop_centered_5000 := pop_elec - 5000]
  term_data[, pop_centered_3000 := pop_elec - 3000]

  # Per-capita education subcategory spending (EUR)
  for (code in edu_3digit_codes) {
    share_var <- paste0("share_", code)
    level_var <- paste0("edu_pc_", code)
    if (share_var %in% names(term_data) && "edu_pc" %in% names(term_data)) {
      # level = share * total_edu / pop (approximate from share * edu_pc)
      term_data[, (level_var) := get(share_var) * edu_pc]
    }
  }

  # Extensive margin: indicator of positive spending per subcategory
  for (code in edu_3digit_codes) {
    share_var <- paste0("share_", code)
    ext_var <- paste0("has_", code)
    if (share_var %in% names(term_data)) {
      term_data[, (ext_var) := fifelse(!is.na(get(share_var)) & get(share_var) > 0,
                                        1L, 0L)]
    }
  }

  term_list[[as.character(ey)]] <- term_data
  cat("  Election", ey, ": N =", nrow(term_data),
      "municipalities, pop_year =", py, "\n")
}

election_term_panel <- rbindlist(term_list, fill = TRUE)

cat("\nElection-term panel:", nrow(election_term_panel), "municipality-terms\n")
cat("Unique municipalities:", uniqueN(election_term_panel$ine_code), "\n")
cat("Elections:", paste(sort(unique(election_term_panel$election_year)),
    collapse = ", "), "\n")
cat("With election-year pop:", sum(!is.na(election_term_panel$pop_elec)), "\n")
cat("With council gender:", sum(!is.na(election_term_panel$female_share)), "\n")

fwrite(election_term_panel, file.path(data_dir, "election_term_panel.csv"))
cat("Saved: election_term_panel.csv\n")
