...
Two additional analyses sharpen the interpretation but also introduce an important caveat. Commercial electricity consumption---also targeted by many EERS programs---falls by 6.5 percent ($p < 0.01$), consistent with commercial-sector program components contributing to overall savings. But total electricity tells a more troubling story: the event study traces a near-linear decline across the entire event window, with no visible break at adoption. This pre-trend failure is driven by industrial consumption, which declines by 19 percent---a figure too large to be the work of energy audits and likely reflecting deindustrialization in the Northeastern and Midwestern states that adopted EERS. The residential event study, critically, does not share this linear pre-trend: pre-treatment coefficients are flat, and the decline begins at adoption. But the total electricity pattern is a warning that EERS and non-EERS states differ along dimensions beyond energy policy, and the residential estimate should be interpreted with this context in mind.
...
Column 4 reports the effect on total per-capita electricity, and here the picture becomes considerably more complicated. The point estimate is large ($-0.090$, $p < 0.01$), but the event study for total electricity (Figure~\ref{fig:alt_outcomes}, ``Total Electricity'' panel) reveals a pattern that should give any reader pause: coefficients trace an almost perfectly linear decline from approximately $+0.10$ at event time $-10$ to $-0.20$ at event time $+15$, with no visible break at the moment of EERS adoption. This is what a pre-existing differential trend looks like, and it fails the parallel trends assumption that underpins the entire identification strategy. The total electricity result should not be interpreted causally, and I report it only for transparency.

This raises an obvious and important question: if total electricity shows such clear pre-trend violations, why should we trust the residential result? The answer hinges on the decomposition. Total electricity equals the sum of residential, commercial, and industrial consumption. As documented in Section~\ref{sec:robustness_placebo}, industrial electricity in EERS states declines by 19.3 percent---a figure far too large to reflect energy audits and likely driven by the broader deindustrialization of the Northeastern and Midwestern states that disproportionately adopted EERS. The industrial component, which constitutes roughly 25--30 percent of total electricity in these states, is large enough to mechanically generate the linear pre-trend observed in total electricity even if residential pre-trends are genuinely flat. The residential event study (Figure~\ref{fig:event_study})---the identification-relevant test---shows pre-treatment coefficients centered on zero with no systematic drift, a pattern qualitatively different from the total electricity trajectory. The causal claim rests on the residential-specific event study, not on the total electricity result.

That said, the total electricity pre-trend is a warning. It demonstrates that EERS states and non-EERS states differ in ways that extend well beyond energy efficiency programs. If some portion of the industrial decline reflects forces that also affect residential consumption---say, population loss in manufacturing-heavy EERS states reducing household electricity demand through vacancy and lower housing density---then the residential event study may pass pre-trend tests while still capturing confounded variation. The robustness to census division-by-year fixed effects (which absorbs region-specific economic shocks) and the COVID exclusion test provide some reassurance, but they cannot fully resolve this concern. I return to this limitation in the Discussion.

The effect on residential prices (Column 5) is $+0.035$ (SE $= 0.021$), positive but statistically insignificant, providing only weak evidence of price pass-through.
...
EERS & $-$0.0415*** & $-$0.0260 & $-$0.0238* & $-$0.0904*** & 0.0345 \\
 ...
 & [$-$0.060, $-$0.023] & [$-$0.061, 0.009] & [$-$0.051, 0.003] & [$-$0.110, $-$0.071] & [$-$0.006, 0.075] \\
\addlinespace
\midrule
...
\begin{tablenotes}
\small
\item \textit{Notes:} $^{*}p<0.10$, $^{**}p<0.05$, $^{***}p<0.01$. Standard errors clustered at the state level in parentheses; 95\% confidence intervals in brackets. CS-DiD refers to the Callaway and Sant'Anna (2021) doubly-robust estimator. Column (1) is the preferred specification using never-treated states as the comparison group. $^\dagger$TWFE uses all 51 states (28 treated, 23 never-treated), with identification from variation in treatment timing across cohorts. Column (4) fails the pre-trends test (see text) and should not be interpreted causally.
\end{tablenotes}
\end{table}
...
\begin{table}[H]
\centering
\caption{Honest Confidence Intervals at Selected Event Times (Residential Electricity)}
...
\begin{tablenotes}[flushleft]
\footnotesize
\item \textit{Notes:} Outcome is log per-capita residential electricity consumption (same as Table~3, Column 1). $M$ bounds the maximum change in slope of differential trends between consecutive periods. Under $M = 0$, all estimates are significant at the 5\% level; under $M = 0.02$, none are significant.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
Industrial electricity consumption, which EERS programs do not typically target directly, shows a large and statistically significant decline of $-19.3\%$ (SE $= 0.038$), visible as the outlier in Figure~\ref{fig:forest}. This result is difficult to reconcile with a pure EERS mechanism and likely reflects broader economic composition shifts or pre-existing differential trends in industrial activity between EERS and non-EERS states. The industrial result prevents a clean falsification test of the form ``EERS affects residential but not industrial consumption,'' and warrants caution about interpreting the residential effect as entirely program-driven. However, the specificity of the residential event study---flat pre-trends, gradual post-treatment growth matching institutional program maturation---provides identification-based evidence that the residential effect reflects genuine program impacts rather than compositional confounders.
...
The total electricity and industrial results merit particular attention because they bear directly on the credibility of the residential estimate. The total electricity event study traces an almost linear decline across the full event window, with no visible break at adoption---a textbook parallel trends failure. Mechanically, this pre-trend is generated by the industrial component: EERS states experienced a 19.3 percent decline in industrial electricity that almost certainly reflects deindustrialization rather than energy audits. The critical question is whether the forces driving industrial decline also contaminate the residential estimate.

Two pieces of evidence suggest they do not, though neither is decisive. First, the residential event study shows flat pre-trends and a clear break at adoption, qualitatively unlike the total electricity trajectory. If the same confounders were driving both, we would expect residential pre-trends to drift as well. Second, the result is robust to census division-by-year fixed effects, which absorb region-specific economic shocks including manufacturing decline. But ``flat pre-trends'' is a necessary condition, not sufficient proof, and region-year fixed effects cannot capture within-region variation in deindustrialization. A skeptic could reasonably argue that the residential effect partially reflects economic restructuring that manifests with different timing across sectors---flat in the pre-period for residential by coincidence, not by identification. I cannot rule this out with the data at hand.

Future work could address this concern by controlling directly for state-level manufacturing employment, industrial output, or plant closures---variables that would absorb deindustrialization-driven changes in residential electricity demand (through vacancy rates, population composition, and local income effects). Until such controls are included, the residential estimate should be interpreted with the caveat that the industrial and total electricity results complicate the clean causal narrative, even if they do not directly invalidate the residential identification.
...
Two cautions temper these conclusions. The estimates are fragile to violations of the parallel trends assumption, as the Honest DiD analysis demonstrates. Under exact parallel trends, the effects are statistically significant at all horizons examined; under modest violations ($M = 0.02$), none survive. This sensitivity is inherent in state-level panels with slow-moving confounders and should inform how policymakers weight the evidence. More fundamentally, the total electricity event study reveals a near-linear pre-trend that fails parallel trends entirely. While this failure is mechanically driven by the industrial component (a 19.3 percent decline likely reflecting deindustrialization rather than energy policy), it demonstrates that EERS and non-EERS states differ along dimensions that extend well beyond efficiency programs. The residential event study passes pre-trend tests convincingly, but the total electricity result is a reminder that ``flat pre-trends'' for one outcome does not guarantee unconfoundedness across all margins.
...
The primary dependent variable is the natural logarithm of per-capita residential electricity consumption, constructed as SEDS series ESRCB (Billion Btu) divided by state population. Residential electricity price is the average retail price in cents per kilowatt-hour from EIA retail sales data. The EERS indicator equals one in all years at or after the state's EERS adoption year and zero otherwise; it is zero for all years in never-treated states. For the Callaway--Sant'Anna estimator, the first treatment year ($G_s$) is set to the adoption year for treated states (e.g., $G_s = 2008$ for Massachusetts) and $G_s = 0$ for never-treated states, following the encoding convention of the \texttt{did} R package (v2.1), where $G_s = 0$ signals never-treated status. This is functionally equivalent to the $G_s = \infty$ notation used in the theoretical literature.
