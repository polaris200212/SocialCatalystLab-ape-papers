...
panel <- panel %>%
  mutate(
    ...
    # Log transformations
    log_res_elec_pc = log(res_elec_pc),
    log_total_elec_pc = log(total_elec_pc),
    log_res_elec = log(res_elec_btu),
    log_total_elec = log(total_elec_btu),
    log_res_price = log(res_price),

    # Per-capita sales (MWh / population) — alternative measure
    res_sales_pc = res_sales_mwh / population,
    log_res_sales_pc = log(res_sales_pc),
