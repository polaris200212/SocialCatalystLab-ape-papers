...
# =============================================================================
# 1. Download FARS Data (2001-2023)
# =============================================================================
...
fetch_fars_year <- function(year) {
  ...
  # Construct URL - format changed over time
  if (year >= 2016) {
  ...
  tryCatch({
    ...
    # Read DRUGS file (detailed drug test results, available from some years)
...
# Download years 2001-2023
message("Starting FARS data download (2001-2023)...")
message("This will take several minutes...")

fars_years <- 2001:2023
fars_list <- list()
...
standardize_drugs <- function(df, year) {
  ...
  df <- df %>%
    mutate(
      ...
      veh_no = as.numeric(veh_no),
      per_no = as.numeric(per_no),
      drugspec = as.numeric(ifelse("drugspec" %in% names(df), drugspec, NA)),
      drugres = as.numeric(ifelse("drugres" %in% names(df), drugres, NA)),
      year = as.numeric(year)
    )
...
message("\nGeocoding rate by year:")
print(geocode_by_year %>% filter(year %in% c(2001, 2005, 2010, 2015, 2020, 2023)), n = 10)
...
# THC drug codes in FARS (codes 600-695 are cannabinoids)
THC_CODES <- 600:695
