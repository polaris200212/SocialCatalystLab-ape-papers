...
fig24 <- ggplot(border_counts, aes(x = dist_num, y = crashes, fill = side)) +
  ...
  labs(
    ...
    subtitle = "Western marijuana borders, 2016-2019",
...
for (i in 1:nrow(border_pairs)) {
  ...
  legal_data <- fars %>%
    filter(state_abbr == bp$legal, year >= 2016, !is.na(thc_positive)) %>%
    summarise(
      state = bp$legal,
      side = "Legal",
      tested = n(),
  ...
  illegal_data <- fars %>%
    filter(state_abbr == bp$illegal, year >= 2016, !is.na(thc_positive)) %>%
    summarise(
      state = bp$illegal,
      side = "Illegal",
      tested = n(),
...
fig28 <- ggplot(pair_df, aes(x = pair, y = pct_thc, fill = side)) +
  ...
  labs(
    ...
    subtitle = "2016-2019, comparing adjacent legal vs illegal states",
    x = "Border Pair",
    y = "Percent THC-Positive",
    fill = "State Type",
...
fig29 <- ggplot(co_wy_binned, aes(x = dist_bin, y = pct_thc, color = state_abbr)) +
  ...
  labs(
    ...
    subtitle = "RDD-style plot, 2016-2019 (10km bins)",
