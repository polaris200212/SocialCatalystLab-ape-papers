...
fig1 <- ggplot() +
  ...
  labs(
    title = "Fatal Traffic Crashes in Western United States, 2001-2023",
    subtitle = paste0("N = ", format(nrow(fars %>% filter(state_fips != "02")), big.mark = ","),
                      " geocoded crashes (Alaska excluded for visualization)"),
    caption = "Source: FARS (NHTSA), 2001-2023. Each point is one fatal crash."
  ) +
  theme_apep() +
  theme(
...
# =============================================================================
# Figure 3: THC-Positive Rate Over Time
# =============================================================================

message("Creating Figure 3: THC-positive rate over time...")

# Calculate THC-positive rate by year and policy status
thc_by_year <- fars %>%
...
fig3 <- ggplot(thc_by_year %>% filter(tested >= 50),
               aes(x = year, y = pct_thc, color = state_type)) +
  geom_line(linewidth = 1) +
  geom_point(size = 2) +
  geom_vline(xintercept = 2012, linetype = "dashed", color = "grey50") +
  annotate("text", x = 2012.2, y = max(thc_by_year$pct_thc, na.rm = TRUE) * 0.95,
           label = "CO/WA Legalize", hjust = 0, size = 3, color = "grey40") +
  ...
  labs(
    ...
    color = "State Type",
    caption = "Source: FARS drug test data. Limited to crashes with drug test results available."
  ) +
  theme_apep() +
...
# =============================================================================
# Figure 4: Alcohol-Positive Rate Over Time
# =============================================================================

message("Creating Figure 4: Alcohol-positive rate over time...")

alc_by_year <- fars %>%
  filter(!is.na(any_alc_positive)) %>%
  group_by(year, ever_rec_legal) %>%
  summarise(
    tested = n(),
    alc_pos = sum(any_alc_positive),
    pct_alc = 100 * alc_pos / tested,
    .groups = "drop"
  ) %>%
  mutate(
    state_type = ifelse(ever_rec_legal, "States that Legalized", "Never-Legal States")
...
fig4 <- ggplot(alc_by_year %>% filter(tested >= 50),
  ...
  scale_x_continuous(breaks = seq(2001, 2023, 2)) +
  labs(
    title = "Alcohol-Positive Rate Among Tested Fatal Crash Drivers",
    subtitle = "Western States, 2001-2023",
    x = "Year",
...
# =============================================================================
# Figure 6: Crashes by Day of Week
# =============================================================================
...
fig6 <- ggplot(fars %>% filter(!is.na(day_week) & day_week >= 1 & day_week <= 7),
  ...
  labs(
    ...
    y = "Number of Crashes",
    fill = "Substance\nInvolvement",
    caption = "Source: FARS (NHTSA). Sunday = 1, Saturday = 7."
  ) +
  theme_apep()
...
fig7 <- ggplot(geocode_quality, aes(x = year, y = pct_geocoded)) +
  ...
  labs(
    ...
    x = "Year",
    y = "Percent Geocoded",
    caption = "Source: FARS (NHTSA). Valid coordinates = within continental US bounds."
  ) +
  theme_apep()
...
# =============================================================================
# Figure 8: Drug Testing Rate by State
# =============================================================================
...
testing_by_state <- fars %>%
  ...
  summarise(
    total_crashes = n(),
    tested = sum(!is.na(thc_positive)),
    pct_tested = 100 * tested / total_crashes,
    .groups = "drop"
  ) %>%
...
fig8 <- ggplot(testing_by_state, aes(x = reorder(state_abbr, pct_tested), y = pct_tested)) +
  ...
  annotate("text", x = 1, y = mean(testing_by_state$pct_tested) + 2,
           label = paste0("Mean: ", round(mean(testing_by_state$pct_tested), 1), "%"),
           hjust = 0, size = 3, color = "red") +
  coord_flip() +
  labs(
    title = "Drug Testing Rate by State",
    subtitle = "Percent of fatal crashes with THC test results available",
    x = NULL,
    y = "Percent with Drug Test Results",
    caption = "Source: FARS drug test data, 2001-2023. Excludes crashes with missing test data."
  ) +
  theme_apep()
...
fig9 <- ggplot(crashes_by_state, aes(x = reorder(state_abbr, crashes), y = crashes, fill = state_type)) +
  ...
  coord_flip() +
  labs(
    title = "Total Fatal Crashes by State, 2001-2023",
