...
fig2 <- ggplot(df_trends, aes(x = year, y = pension_rate, color = treated_group)) +
  ...
  labs(
    ...
    caption = "Note: Shaded areas show 95% confidence intervals. Dashed line indicates first program launch (Oregon, July 2017; first CPS with exposure: March 2018)."
  ) +
  theme_apep() +
...
cat("\nCreating Figure 6: Leave-one-out analysis...\n")

if (file.exists(file.path(data_dir, "leave_one_out_full.csv"))) {

  loo_df <- read_csv(file.path(data_dir, "leave_one_out_full.csv"),
                     show_col_types = FALSE) %>%
    mutate(
      excluded_state = factor(excluded_state,
                               levels = c(excluded_state[excluded_state == "None (Full)"],
                                          excluded_state[excluded_state != "None (Full)"][order(att[excluded_state != "None (Full)"])])),
      is_full = excluded_state == "None (Full)"
    )

  fig6 <- ggplot(loo_df, aes(x = att, y = excluded_state)) +
    geom_vline(xintercept = 0, linetype = "dashed", color = "grey40") +
    geom_errorbarh(aes(xmin = att - 1.96 * se, xmax = att + 1.96 * se,
                       color = is_full),
                   height = 0.3, linewidth = 0.7) +
    geom_point(aes(color = is_full, size = is_full)) +
    scale_color_manual(values = c("TRUE" = apep_colors[2], "FALSE" = apep_colors[1]),
                       guide = "none") +
    scale_size_manual(values = c("TRUE" = 4, "FALSE" = 2.5), guide = "none") +
    scale_x_continuous(labels = scales::percent_format(accuracy = 0.1)) +
    labs(
      title = "Systematic Leave-One-Out Analysis",
      subtitle = "Sensitivity of main result to exclusion of each treated state",
      x = "ATT on Retirement Plan Coverage",
      y = "Excluded State",
      caption = "Note: Orange point shows full sample estimate. Blue points show estimates excluding each state.\nOregon's exclusion has the largest effect on the estimated ATT."
    ) +
    theme_apep() +
    theme(
      panel.grid.major.y = element_blank()
  ...
  ggsave(file.path(fig_dir, "fig6_leave_one_out.pdf"), fig6, width = 9, height = 7)
  ggsave(file.path(fig_dir, "fig6_leave_one_out.png"), fig6, width = 9, height = 7, dpi = 300)
  cat("Saved: fig6_leave_one_out.pdf\n")
}

# -----------------------------------------------------------------------------
# FIGURE 7: DDD Event Study
# -----------------------------------------------------------------------------

cat("\nCreating Figure 7: DDD event study...\n")

if (file.exists(file.path(data_dir, "ddd_event_study_coefs.csv"))) {

  ddd_es <- read_csv(file.path(data_dir, "ddd_event_study_coefs.csv"),
                     show_col_types = FALSE)

  fig7 <- ggplot(ddd_es, aes(x = event_time, y = att)) +
    geom_ribbon(aes(ymin = ci_lower, ymax = ci_upper),
    ...
    geom_hline(yintercept = 0, linetype = "dashed", color = "grey40") +
    geom_vline(xintercept = -0.5, linetype = "dotted", color = "grey60") +
    scale_x_continuous(breaks = seq(min(ddd_es$event_time), max(ddd_es$event_time), 1)) +
    scale_y_continuous(labels = scales::percent_format(accuracy = 0.1)) +
    labs(
      title = "Triple-Difference Event Study",
      subtitle = "Differential effect on small firm workers (targeted by mandates)",
      x = "Years Relative to Mandate Implementation",
      y = "DDD Coefficient: Effect on Small Firms vs Large Firms",
      caption = "Note: Estimates show differential effect of mandates on small firm workers relative to large firm workers.\nSmall firms (<100 employees) are the primary target of auto-IRA mandates."
    ) +
    theme_apep()

  ggsave(file.path(fig_dir, "fig7_ddd_event_study.pdf"), fig7, width = 9, height = 6)
  ggsave(file.path(fig_dir, "fig7_ddd_event_study.png"), fig7, width = 9, height = 6, dpi = 300)
  cat("Saved: fig7_ddd_event_study.pdf\n")
}
...
cat("- fig6_leave_one_out.pdf: Systematic LOO analysis\n")
cat("- fig7_ddd_event_study.pdf: Triple-difference event study\n")
