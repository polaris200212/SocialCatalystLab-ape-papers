...
# Extract event study coefficients using fixest's native extraction
# fixest i() terms are named like "rel_year_binned::-6:high_flood"
es_tidy <- broom::tidy(es_model, conf.int = TRUE)
es_coefs <- es_tidy %>%
  filter(str_detect(term, "rel_year_binned|high_flood")) %>%
  mutate(
    rel_year = as.integer(str_extract(term, "-?\\d+"))
  ) %>%
  filter(!is.na(rel_year))

# If broom extraction failed, use fixest's coeftable directly
if (nrow(es_coefs) <= 1) {
  ct <- as.data.frame(coeftable(es_model))
  ct$term <- rownames(ct)
  es_coefs <- ct %>%
    filter(str_detect(term, "rel_year_binned|high_flood")) %>%
    mutate(
...
if (file.exists(paste0(data_dir, "cs_dynamic.rds"))) {
  ...
  cat("  CS-DiD results not available. Skipping Figure 4.\n")
}
