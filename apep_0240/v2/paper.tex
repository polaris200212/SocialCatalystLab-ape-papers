...
\title{Licensing to Disclose: Do State Flood Risk Disclosure Laws Capitalize into Housing Values?\thanks{This paper is a revision of \href{https://github.com/SocialCatalystLab/ape-papers/tree/main/apep_0240}{APEP Working Paper 0240 v1}. Changes in this version: fixed compilation errors, tightened introduction, corrected table formatting.}}
...
In many states, a home seller must disclose a cracked foundation, a faulty furnace, or a leaky roof---but can remain silent about a history of catastrophic flooding. Thirty U.S.\ states have now moved to close this gap through mandatory seller disclosure laws, requiring sellers to reveal whether a property sits in a FEMA-designated flood zone, whether it has experienced prior flood damage, and whether flood insurance is required. Adoption occurred in three distinct waves---a first wave in the 1990s, a second in the early 2000s, and a third beginning in 2019 \citep{NRDC2024}. The logic is straightforward: if buyers lack information about flood risk, mandating disclosure should cause flood-prone property values to decline as the market incorporates previously hidden hazard information. Yet whether these laws actually change how markets price flood risk remains an open question.
...
Second, I contribute to the literature on flood risk capitalization. \citet{BernsteinEtAl2019} document a 7\% discount for sea-level-rise-exposed properties, concentrated in non-believer areas, suggesting that climate beliefs mediate price effects. \citet{OrtegaTaspinar2018} estimate that Hurricane Sandy reduced prices in flood zones by 8--18\%. \citet{GallagherSad2020} show that flood insurance take-up spikes after nearby disasters but decays rapidly. \citet{KouskyShoemaker2019} document how NFIP premium subsidies distort risk pricing, creating a wedge between actuarial risk and the price signal buyers observe. \citet{GiglioEtAl2021} find that climate risk is only weakly reflected in long-run discount rates for real estate, suggesting limited market attention to slow-moving hazards. My results complement this work by showing that legislative disclosure mandates---as opposed to direct experience with floods---have limited marginal effects on prices.
...
\textbf{First Wave (1992--1999):} Sixteen states adopted flood disclosure requirements during this period, typically as part of broader property condition disclosure reforms (fifteen appear in the analysis panel; Alaska is excluded due to limited ZHVI coverage). Kentucky was the first in 1992, followed by Indiana, Michigan, Ohio, Oregon, and South Dakota in 1993. This wave was driven by consumer protection concerns and the professionalization of real estate transactions.
...
Merging these three sources yields a county-year panel of 54,503 observations covering 3,072 counties across 49 states (excluding Alaska), spanning 2000--2024. I restrict the panel to begin in 2000 to ensure adequate ZHVI coverage, as county-level coverage increases substantially after 1999. Of the 3,072 counties, 1,578 (51\%) are classified as high-flood, 1,907 (62\%) are in treated states, and the panel is approximately balanced with an average of 22 county-years per county.

An important implication of the 2000 panel start is that the 15 first-wave states in the analysis (adopted 1992--1999, excluding Alaska) enter the panel already treated: all their observations are post-treatment. These states contribute to the static DDD estimate---which compares the within-state flood-exposure gap in treated versus never-treated states---but not to the event study pre-trends. Pre-trend identification comes from the 5 second-wave states (adopted 2000--2006, with up to 6 pre-treatment years) and 10 third-wave states (adopted 2019--2024, with up to 19 pre-treatment years). This is a standard feature of staggered DiD with long treatment histories and does not compromise the main DDD design, which pools all cohorts.

A design consideration is that four states adopted disclosure requirements in 2024 (Florida, New Hampshire, North Carolina, Vermont). Because the treatment indicator is coded as $\text{Post}_{st} = \ind[\text{year} \geq \text{year\_adopted}]$, these states contribute one post-treatment year (2024 itself) to the analysis. In the annual panel, the 2024 observation aggregates all twelve months, so the post-treatment indicator captures the full calendar year regardless of the law's effective date within that year; this conservative coding treats partial-year exposure as full treatment, which if anything attenuates the estimated effect. While this provides limited statistical power for these cohorts individually, excluding them would reduce the treated sample. Importantly, the state-by-year fixed effects fully absorb any 2024-specific state-level shocks, so the DDD coefficient for these states is identified only from the within-state high- versus low-flood differential in 2024 relative to that differential in pre-adoption years. In robustness analysis (\Cref{tab:robustness}, Column 4), I restrict to third-wave adopters only and find a consistent null (0.005, SE = 0.006), confirming that the result is not driven by the 2024 cohort.
...
\label{eq:eventstudy}
\end{equation}
where $t^*_s$ is the adoption year for state $s$, and the sum runs over event-time indicators $k$ from $-6$ to $+8$, with $k = -1$ as the omitted reference period. Estimates are restricted to treated states (year adopted $> 0$). Because the panel begins in 2000, first-wave states (adopted 1992--1999) enter the panel already treated and contribute only to post-treatment bins ($k \geq 1$); the pre-treatment coefficients are identified exclusively from second-wave (2000--2006) and third-wave (2019--2024) states, which have ample pre-adoption observations. This is standard in staggered DiD designs where early cohorts lack pre-period data in the sample window. The pre-treatment coefficients ($\beta_k$ for $k < -1$) test for differential pre-trends in the flood-exposure gap across states that adopted at different times. If the parallel trends assumption holds, these coefficients should be statistically indistinguishable from zero.

\subsection{Callaway-Sant'Anna Estimator}

Staggered adoption with heterogeneous treatment effects can bias standard TWFE estimators \citep{GoodmanBacon2021, deChaisemartinHaultfoeuille2020}. While the DDD structure with state-by-year fixed effects already absorbs much of the variation that generates TWFE bias---since the problematic ``forbidden comparisons'' arise from using already-treated units as controls, and state-by-year effects partial this out---I directly address remaining concerns using the \citet{CallawayAndSantanna2021} estimator. This computes group-time average treatment effects for each cohort (defined by adoption year) and aggregates them into an overall ATT and dynamic treatment effect estimates. I use never-treated states as the comparison group and the outcome-regression estimand, with bootstrapped standard errors clustered at the county level. The CS-DiD results serve as a robustness check on the TWFE specification; concordance between the two approaches would confirm that heterogeneous treatment timing does not materially affect inference.
...
Disclosure does not move the needle on flood-zone housing prices. \Cref{tab:main} presents three variants of the DDD model, and none produces a statistically significant negative effect. The core specification (Column 1) yields a coefficient of 0.0072 log points (SE = 0.0091, $p = 0.43$)---less than 1\%, or roughly \$1,300 for the median home. The 95\% confidence interval of $[-0.011, 0.025]$ rules out effects larger than 2.5\% in either direction, allowing us to reject the large negative effects (4--8\%) found in earthquake disclosure studies \citep{PopeHuang2020}.

\input{tables/tab2_main_results}

Column (2) uses a broader flood exposure definition---any county with at least one pre-1992 flood declaration. This specification yields a coefficient of 0.030 log points (SE = 0.015, $p = 0.055$), marginally significant at the 10\% level. The larger magnitude and near-significance with the broader measure suggests that the disclosure effect, if any, may operate across the full spectrum of flood-exposed counties rather than being concentrated in the most flood-prone areas.

Column (3) uses the continuous count of pre-1992 flood declarations interacted with post-adoption. The coefficient of 0.0043 (SE = 0.0027, $p = 0.12$) is consistent with the binary specifications: positive but imprecisely estimated.

Across all three specifications, the $R^2$ (within) is 0.979, reflecting the strong predictive power of county and state-by-year fixed effects for housing values. The sample includes 54,479 county-year observations (from 54,503 raw county-years, with 24 singletons removed by \texttt{fixest}).
...
\Cref{fig:adoption} displays the adoption timeline, showing the three waves of state adoption. The staggered pattern---with clusters of adoption in 1993 (five states), 1994 (three states), 2019 (three states), and 2024 (four states)---provides identifying variation across cohorts. The full list of treated and control states with adoption dates, waves, and NRDC grades is provided in \Cref{tab:treatment} in the Appendix.
...
Several limitations warrant acknowledgment. The county-level ZHVI aggregates across properties with heterogeneous flood exposure, potentially attenuating effects that operate at the property level. The analysis captures law adoption but not enforcement intensity or compliance rates, which vary across states. And the pre-existing information environment---including FEMA maps, NFIP requirements, and real estate agent knowledge---may have already transmitted much of the information that disclosure laws codify, making the null a consequence of measurement timing rather than policy irrelevance.

For policymakers concerned about climate adaptation and housing investment in flood-prone areas, the results suggest that information provision alone is insufficient to redirect housing markets away from risky locations. Achieving that goal likely requires more direct interventions: actuarially fair flood insurance pricing \citep{KouskyShoemaker2019}, land-use regulations, managed retreat programs, or infrastructure investments that reduce flood risk directly. The persistent willingness of Americans to buy in flood zones---despite decades of increasingly available flood risk information---suggests that the problem is not lack of information, but a market that has already priced the risk.
...
\item \textbf{States:} 49 states (Alaska excluded due to limited county-level ZHVI data); 30 treated + 19 never-treated
...
\input{tables/tab4_treatment_states}
