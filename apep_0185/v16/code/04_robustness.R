...
# ============================================================================
# 15. Sun & Abraham Interaction-Weighted Estimator
# ============================================================================

cat("\n15. Sun & Abraham interaction-weighted estimator...\n")

# Define treatment cohort: year of largest MW exposure change for each county
sa_results <- NULL
...
tryCatch({
  panel_sa <- panel %>%
    ...
    arrange(yearq) %>%
    mutate(
      mw_change = network_mw_pop_out_state - lag(network_mw_pop_out_state, 4)
    ) %>%
    ungroup()

  # Identify cohort year (year of largest exposure change)
  county_cohorts <- panel_sa %>%
    filter(!is.na(mw_change)) %>%
    group_by(county_fips) %>%
    slice_max(abs(mw_change), n = 1, with_ties = FALSE) %>%
    select(county_fips, cohort_year = year) %>%
    ungroup()

  panel_sa <- panel_sa %>%
    left_join(county_cohorts, by = "county_fips") %>%
    filter(!is.na(cohort_year))

  cat("  Cohort distribution:\n")
  print(table(county_cohorts$cohort_year))

  # Sun & Abraham using fixest::sunab
  sa_est <- feols(
    log_emp ~ sunab(cohort_year, year) | county_fips + state_fips^yearq,
    data = panel_sa,
    cluster = ~state_fips
  )

  cat("  Sun & Abraham aggregated ATT:\n")
  sa_agg <- summary(sa_est, agg = "ATT")
  print(sa_agg)

  sa_results <- list(
    model = sa_est,
    cohort_distribution = table(county_cohorts$cohort_year)
  )

  cat("  Sun & Abraham estimation successful\n")
}, error = function(e) {
  cat("  WARNING: Sun & Abraham failed:", e$message, "\n")
  cat("  Continuing without SA results\n")
})
...
# ============================================================================
# 17. County-Specific Linear Trends
# ============================================================================

cat("\n17. County-specific linear trends robustness...\n")

county_trend_results <- NULL

tryCatch({
  # Create numeric time variable for trends
  panel <- panel %>%
    mutate(yearq_num = as.numeric(factor(yearq)))

  # OLS with county-specific linear trends
  ols_county_trends <- feols(
    log_emp ~ network_mw_pop | county_fips[yearq_num] + state_fips^yearq,
    data = panel,
    cluster = ~state_fips
  )

  cat("  OLS with county-specific linear trends:\n")
  cat("    Coefficient:", round(coef(ols_county_trends)[1], 4),
      "(SE:", round(se(ols_county_trends)[1], 4),
      ", p =", round(fixest::pvalue(ols_county_trends)[1], 4), ")\n")

  # Compare with baseline OLS
  ols_baseline <- feols(
    log_emp ~ network_mw_pop | county_fips + state_fips^yearq,
    data = panel,
    cluster = ~state_fips
  ...
  cat("    Baseline OLS (no trends):", round(coef(ols_baseline)[1], 4),
      "(SE:", round(se(ols_baseline)[1], 4), ")\n")

  attenuation <- 1 - abs(coef(ols_county_trends)[1] / coef(ols_baseline)[1])
  cat("    Attenuation from trends:", round(attenuation * 100, 1), "%\n")

  # 2SLS with county trends
  iv_county_trends <- tryCatch({
    feols(
      log_emp ~ 1 | county_fips[yearq_num] + state_fips^yearq |
        network_mw_pop ~ network_mw_pop_out_state,
      data = panel,
      cluster = ~state_fips
    )
  }, error = function(e) {
    cat("    WARNING: 2SLS with county trends failed:", e$message, "\n")
    NULL
  })

  if (!is.null(iv_county_trends)) {
    cat("    2SLS with county trends:", round(coef(iv_county_trends)[1], 4),
        "(SE:", round(se(iv_county_trends)[1], 4),
        ", p =", round(fixest::pvalue(iv_county_trends)[1], 4), ")\n")
    cat("    Baseline 2SLS:", round(coef(main_results$iv_2sls_pop)[1], 4), "\n")
  }

  county_trend_results <- list(
    ols_trends = ols_county_trends,
    ols_baseline = ols_baseline,
    iv_trends = iv_county_trends,
    attenuation = attenuation
  )

  cat("  County-trend robustness complete\n")
}, error = function(e) {
  cat("  WARNING: County trends failed:", e$message, "\n")
})
...
robustness <- list(
  ...
  # NEW: Enhanced Rambachan-Roth
  rr_enhanced = rr_enhanced,

  # NEW: County-specific linear trends
  county_trend_results = county_trend_results
)
