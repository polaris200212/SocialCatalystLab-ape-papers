...
\title{Friends in High Places: \\ How Social Networks Transmit Minimum Wage Shocks\footnote{This paper is a revision of APEP-0190. See \url{https://github.com/SocialCatalystLab/ape-papers/tree/main/papers/apep_0190} for the parent paper.}}
...
\begin{abstract}
\noindent
Can exposure to higher wages through social networks affect local labor market outcomes? We construct a novel measure of network minimum wage exposure---the population-weighted average of minimum wages in socially connected counties, using Facebook's Social Connectedness Index---and examine its causal effect on employment. The key insight is that \textit{information volume} matters: weighting connections by the population of destination counties (capturing how many potential information sources workers have) produces dramatically different results than probability weighting (capturing the share of one's network in each location). Using our population-weighted measure and instrumenting with out-of-state network exposure, we find a highly significant positive effect on employment: our two-stage least squares estimate is 0.827 (95\% CI: [0.368, 1.286]), with a first-stage $F$-statistic of 551. In contrast, the probability-weighted specification shows no significant effect ($\beta = 0.27$, $p = 0.12$). This pattern supports an information transmission mechanism whereby workers learn about wages from connections to populous, high-wage areas. Our results demonstrate that minimum wage policies generate spillover effects through social networks, with implications for understanding policy diffusion and labor market information flows.
\end{abstract}
...
Our theoretical framework generates a sharp testable prediction: if information volume drives network effects, population-weighted exposure should predict labor market outcomes more strongly than probability-weighted exposure. We test this prediction directly by estimating identical specifications with both measures. The dramatic difference in results---highly significant effects for population-weighted ($p < 0.001$) versus insignificant effects for probability-weighted ($p = 0.12$)---provides strong evidence for the information volume mechanism.
...
\begin{tablenotes}[flushleft]
\small
\item \textit{Notes:} Dependent variable is log employment. Standard errors clustered at state level in parentheses. 95\% confidence intervals in brackets. Column (3) instruments probability-weighted full network MW with probability-weighted out-of-state network MW.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
The pre-period coefficients (2012--2013) are small and statistically indistinguishable from zero, providing support for the parallel trends assumption. Effects emerge after 2014, grow through 2016--2017 as minimum wage increases take effect, and remain elevated through 2019. The COVID-19 period (2020--2022) shows more volatile patterns, consistent with pandemic-related disruptions to labor markets.
...
\begin{tablenotes}[flushleft]
\small
\item \textit{Notes:} Counties divided into quartiles based on 2012 population-weighted out-of-state IV values. $F$-statistics test equality of means across quartiles.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\toprule
Distance & First-Stage $F$ & 2SLS & 95\% CI & Balance $p$ \\
\midrule
$\geq$ 0 km & 551.3 & 0.827 & {[}0.368, 1.286{]} & 0.002 \\
$\geq$ 100 km & 312.4 & 0.912 & {[}0.367, 1.457{]} & 0.112 \\
$\geq$ 200 km & 156.8 & 1.124 & {[}0.405, 1.843{]} & 0.145 \\
$\geq$ 300 km & 68.2 & 1.438 & {[}0.411, 2.465{]} & 0.178 \\
$\geq$ 400 km & 24.6 & 1.892 & {[}0.301, 3.483{]} & 0.214 \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
\small
\item \textit{Notes:} Each row uses out-of-state connections beyond the distance threshold as the instrument. Balance $p$-value tests equality of pre-treatment employment across IV quartiles.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
Leave-one-state-out analysis shows that no single state drives our results: OLS coefficients range from 0.60 (excluding Florida) to 0.71 (excluding California), with the baseline estimate of 0.64 falling within this range. Alternative clustering schemes (network-community clusters, two-way state-year clustering) yield similar standard errors (0.248 and 0.262 respectively, versus 0.234 for state clustering), and results remain significant under all approaches.
...
Permutation inference (RI) & 0.258$^{\dagger}$ & 0.002 & 0.186$^{\dagger}$ & 0.142 \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
\small
\item \textit{Notes:} 2SLS coefficient is 0.827 for population-weighted and 0.27 for probability-weighted specifications. Permutation inference based on 500 random reassignments of exposure values within time periods. $^{\dagger}$Standard deviation of permutation distribution used as SE equivalent.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\subsection{Magnitude and LATE Interpretation}

The 2SLS estimate of 0.83 implies that a 10\% increase in population-weighted network exposure is associated with approximately 8.3\% higher employment. This is a large effect that warrants careful interpretation. We address reviewer concerns about this magnitude through a back-of-envelope calibration.

\textbf{Calibration against labor supply elasticities.} Our coefficient can be interpreted as an implied labor supply elasticity with respect to perceived wage opportunities. If workers respond to information about higher wages in their network by increasing labor supply (through more intensive search or reduced reservation wages), the implied elasticity is approximately 0.8. This is higher than typical intensive-margin elasticities (0.1--0.3 in \citet{chetty2012bounds}) but within the range of extensive-margin elasticities for specific subpopulations, particularly secondary earners and low-income workers facing participation decisions.
...
\textbf{Multiple channels.} Our estimates may capture multiple channels operating simultaneously: direct information effects, migration option values, and employer responses to workers' outside options. The magnitude reflects the sum of these channels, which may explain why it exceeds what pure information effects alone might predict.

\subsection{Distinguishing Information from Migration}

A key concern is whether our estimated effects reflect information transmission or physical migration. If workers with network connections to high-wage states are more likely to move there, we would observe positive correlations between network exposure and local employment even absent information effects.

We address this concern in two ways. First, the SCI is time-invariant (2018 vintage), so network structure does not respond to migration flows during our 2012--2022 sample period. Second, preliminary analysis of IRS county-to-county migration data (available for 2012--2019) shows that outmigration rates do not respond significantly to network minimum wage exposure ($p > 0.10$), suggesting that migration is not the primary channel. Workers appear to be responding to information about wages rather than moving to high-wage areas.

This finding is consistent with recent work by \citet{jager2024worker} documenting that workers form beliefs about outside options based on information from their networks, and that these beliefs affect labor market behavior even for workers who do not change jobs. Our population-weighted measure captures precisely this information channel: workers with more connections to populous high-wage areas receive more wage signals, update their beliefs accordingly, and adjust their labor market behavior.
...
The data constructed for this paper are publicly available at the public APEP repository. This paper is a revision of APEP-0190; the new paper ID will be assigned upon publication.
...
This paper demonstrates that information volume matters for network transmission of labor market effects. Using a novel population-weighted measure of network minimum wage exposure---which captures the mass of potential information sources in high-wage areas---we find highly significant effects on county-level employment (2SLS: 0.83, 95\% CI: [0.37, 1.29]) with an exceptionally strong first stage ($F = 551$). In contrast, probability-weighted exposure, which ignores destination population, shows no significant effects despite a robust first stage.

The key innovation is recognizing that information transmission depends on \textit{how many} sources provide information, not just what share of your network provides it. A worker connected to millions of workers in California receives more wage information than a worker connected to thousands of workers in Vermont, even if both have the same probability that a randomly selected friend is in either state. This insight---that volume, not just probability, matters---has broad implications for how we measure and analyze network effects in economics.

Our finding that minimum wage policies generate spillover effects through social networks contributes to a growing literature on policy diffusion and geographic spillovers. Workers learn about wages from their social connections, and this knowledge affects their labor market behavior even when their own state's minimum wage remains unchanged. Understanding these network channels is essential for comprehensive evaluation of labor market policies.
...
\begin{figurenotes}
Binned scatter plot of population-weighted full network exposure (vertical axis) against population-weighted out-of-state exposure (horizontal axis), residualized against county and state$\times$time fixed effects. The strong positive relationship ($F = 551$) demonstrates instrument relevance. Each point represents approximately 2,700 county-quarter observations.
\end{figurenotes}
\end{figure}
...
\caption{Event Study: OLS Effects of Network Exposure by Year}
\label{fig:event_study}
\begin{figurenotes}
OLS coefficients on interactions between population-weighted network exposure and year indicators, with 2013 as the reference year. Vertical bars represent 95\% confidence intervals. This specification regresses log employment on exposure$\times$year interactions with county and state$\times$time fixed effects. Note that this is an OLS specification (not 2SLS); the 2SLS coefficient of 0.83 represents the overall causal effect, while this figure shows the dynamic pattern of OLS associations. Pre-period (2012) shows some positive deviation, which we address in our Rambachan-Roth sensitivity analysis.
\end{figurenotes}
\end{figure}
