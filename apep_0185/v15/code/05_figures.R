...
fig4 <- ggplot(binscatter_data, aes(x = mean_iv, y = mean_full)) +
  ...
  annotate("label", x = min(binscatter_data$mean_iv) + 0.02,
           y = max(binscatter_data$mean_full) - 0.005,
...
# Employment event study panel
es_coefs$outcome <- "Employment"

fig5_emp <- ggplot(es_coefs, aes(x = year, y = coef)) +
  ...
  labs(
    title = "Panel A: Employment",
    x = "Year",
    y = "Coefficient on Network Exposure"
  ) +
...
es_earn_coefs <- NULL
if ("log_earn" %in% names(panel)) {
  tryCatch({
    es_earn_model <- feols(log_earn ~ i(year, pop_mw_full, ref = 2013) |
                            county_fips + state_fips^year^quarter,
                          data = panel, cluster = ~state_fips)

    es_earn_raw <- as.data.frame(coeftable(es_earn_model))
    es_earn_raw$term <- rownames(es_earn_raw)
    es_earn_coefs <- es_earn_raw %>%
      filter(grepl("year::", term)) %>%
      mutate(
        year = as.numeric(gsub("year::(\\d+):pop_mw_full", "\\1", term)),
        coef = Estimate,
    ...
    cat("  Earnings event study failed:", e$message, "\n")
  })
}

if (!is.null(es_earn_coefs)) {
  fig5_earn <- ggplot(es_earn_coefs, aes(x = year, y = coef)) +
    geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
    geom_vline(xintercept = 2013.5, linetype = "dotted", color = "gray50", size = 0.8) +
  ...
  fig5 <- fig5_emp / fig5_earn +
    plot_annotation(
      title = "Event Study: Effect of Network Exposure on Employment and Earnings",
...
ggsave("../figures/fig5_event_study.pdf", fig5, width = 9, height = if (!is.null(es_earn_coefs)) 10 else 6)
...
# Merge and compute ANNUAL averages by quartile (smooths seasonal noise)
balance_ts <- panel %>%
  left_join(baseline_iv %>% select(county_fips, iv_quartile), by = "county_fips") %>%
  filter(!is.na(iv_quartile)) %>%
  group_by(year, iv_quartile) %>%
  summarise(
...
fig6 <- ggplot(balance_ts, aes(x = year, y = mean_emp, color = iv_quartile)) +
  ...
  labs(
    ...
    subtitle = "Annual averages. Roughly parallel trends before 2014, divergence after major MW increases.",
    x = "Year",
...
# ============================================================================
# Figure 9: DUAL EVENT STUDY (CENTERPIECE)
# ============================================================================

cat("\nFigure 9: Dual event study (structural vs reduced-form)...\n")

main_res <- tryCatch(readRDS("../data/main_results.rds"), error = function(e) NULL)

if (!is.null(main_res) && !is.null(main_res$structural_es) && !is.null(main_res$rf_es_baseline)) {

  # Extract event study coefficients from quarterly yearq_int model
  # yearq_int = (year - 2012) * 4 + quarter, so yearq_int=1 is 2012Q1
  extract_es_coefs <- function(model, ref_yearq_int = 5) {
    ct <- as.data.frame(coeftable(model))
    ct$term <- rownames(ct)
    ct <- ct %>%
      filter(grepl("yearq_int::", term)) %>%
      mutate(
        yqi = as.numeric(gsub("yearq_int::([-0-9]+):.*", "\\1", term)),
        year_frac = 2012 + (yqi - 1) / 4,  # convert back to fractional year
        coef = Estimate,
        se = `Std. Error`,
        ci_lower = coef - 1.96 * se,
        ci_upper = coef + 1.96 * se
      ) %>%
      select(yqi, year_frac, coef, se, ci_lower, ci_upper)

    # Add reference period (2013Q1 = yearq_int 5)
    ref_year_frac <- 2012 + (ref_yearq_int - 1) / 4
    ct <- bind_rows(
      ct,
      tibble(yqi = ref_yearq_int, year_frac = ref_year_frac,
             coef = 0, se = 0, ci_lower = 0, ci_upper = 0)
    ) %>%
      arrange(yqi)
    ct
  }

  struct_coefs <- extract_es_coefs(main_res$structural_es)
  struct_coefs$type <- "Panel A: Structural (Endogenous Variable)"

  rf_coefs <- extract_es_coefs(main_res$rf_es_baseline)
  rf_coefs$type <- "Panel B: Reduced-Form (Instrument)"

  dual_es_data <- bind_rows(struct_coefs, rf_coefs)

  # Pre-trend p-values for annotation
  struct_p <- round(main_res$structural_es_pretrend_p, 3)
  rf_p <- round(main_res$rf_es_pretrend_p, 3)

  fig9 <- ggplot(dual_es_data, aes(x = year_frac, y = coef)) +
    facet_wrap(~type, ncol = 1, scales = "free_y") +
    geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
    geom_vline(xintercept = 2013.0, linetype = "dotted", color = "gray50", size = 0.8) +
    geom_ribbon(aes(ymin = ci_lower, ymax = ci_upper), alpha = 0.2, fill = "#4575b4") +
    geom_line(color = "#4575b4", size = 1) +
    geom_point(color = "#4575b4", size = 2) +
    scale_x_continuous(breaks = 2012:2022) +
    labs(
      title = "Structural vs. Reduced-Form Event Studies",
      subtitle = paste0("Structural pre-trend F-test p = ", struct_p,
                        ". Reduced-form pre-trend F-test p = ", rf_p, "."),
      x = "Year",
      y = "Coefficient"
    ) +
    theme_minimal(base_size = 11) +
  ...
  ggsave("../figures/fig9_dual_event_study.pdf", fig9, width = 9, height = 10)
  cat("  Saved fig9_dual_event_study.pdf\n")
} else {
  cat("  Skipping Figure 9: main_results.rds not found or missing event study models\n")
}


# ============================================================================
# Figure 10: DISTANCE-CREDIBILITY TRADEOFF
# ============================================================================

cat("\nFigure 10: Distance-credibility tradeoff...\n")

dist_cred <- tryCatch(readRDS("../data/distance_credibility.rds"), error = function(e) NULL)

if (!is.null(dist_cred) && length(dist_cred) > 0) {
  dc_df <- do.call(rbind, lapply(names(dist_cred), function(d) {
    r <- dist_cred[[d]]
    data.frame(
      distance = r$threshold,
      first_stage_f = r$first_stage_f,
      rf_pretrend_p = r$rf_pretrend_p,
      balance_p = r$balance_p,
      tsls_coef = ifelse(is.null(r$tsls_coef), NA, r$tsls_coef),
      stringsAsFactors = FALSE
    )
  }))

  # Scale factor for dual axis
  f_max <- max(dc_df$first_stage_f, na.rm = TRUE)
  scale_factor <- f_max / 1.0  # p-values go from 0 to 1

  fig10 <- ggplot(dc_df, aes(x = distance)) +
    # First-stage F (left axis)
    geom_line(aes(y = first_stage_f, color = "First-Stage F"), size = 1.2) +
    geom_point(aes(y = first_stage_f, color = "First-Stage F"), size = 3) +
    # RF pre-trend p (scaled to right axis)
    geom_line(aes(y = rf_pretrend_p * scale_factor, color = "RF Pre-Trend p"), size = 1.2) +
    geom_point(aes(y = rf_pretrend_p * scale_factor, color = "RF Pre-Trend p"), size = 3) +
    # Balance p (scaled to right axis)
    geom_line(aes(y = balance_p * scale_factor, color = "Balance p"), size = 1.2, linetype = "dashed") +
    geom_point(aes(y = balance_p * scale_factor, color = "Balance p"), size = 3, shape = 17) +
    # Reference lines
    geom_hline(yintercept = 10, linetype = "dotted", color = "gray40", size = 0.5) +
    geom_hline(yintercept = 0.05 * scale_factor, linetype = "dotted", color = "#d73027", size = 0.5) +
    annotate("text", x = max(dc_df$distance) - 20, y = 15,
             label = "F = 10 (weak IV)", size = 3, color = "gray40", hjust = 1) +
    annotate("text", x = max(dc_df$distance) - 20, y = 0.05 * scale_factor + 10,
             label = "p = 0.05", size = 3, color = "#d73027", hjust = 1) +
    scale_y_continuous(
      name = "First-Stage F-statistic",
      sec.axis = sec_axis(~./scale_factor, name = "p-value")
    ) +
    scale_color_manual(
      values = c("First-Stage F" = "#4575b4", "RF Pre-Trend p" = "#d73027", "Balance p" = "#fc8d59"),
      name = ""
    ) +
    scale_x_continuous(breaks = dc_df$distance) +
    labs(
      title = "Distance-Credibility Tradeoff",
      subtitle = "As distance increases, instrument strength declines but exogeneity improves",
  ...
  ggsave("../figures/fig10_distance_credibility.pdf", fig10, width = 9, height = 7)
  cat("  Saved fig10_distance_credibility.pdf\n")
} else {
  cat("  Skipping Figure 10: distance_credibility.rds not found\n")
}
