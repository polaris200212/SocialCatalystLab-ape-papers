...
cat("\n5B. Horse Race: Network (Pop) vs Geographic Exposure\n")
...
# ============================================================================
# PART 5C: EARNINGS AS CO-PRIMARY OUTCOME
# ============================================================================

cat("\n╔══════════════════════════════════════════════════════════════════╗\n")
cat("║  PART 5C: EARNINGS (CO-PRIMARY OUTCOME)                          ║\n")
cat("╚══════════════════════════════════════════════════════════════════╝\n\n")

ols_earn_pop <- NULL
ols_earn_pop_simple <- NULL
iv_earn_pop <- NULL
balance_p_earn <- NA

if ("log_earn" %in% names(panel_reg)) {
  # OLS: county FE only
  ols_earn_pop_simple <- feols(
    log_earn ~ network_mw_pop | county_fips + yearq,
    data = panel_reg,
    cluster = ~state_fips
  )
  ...
  cat("  OLS (county FE):", round(coef(ols_earn_pop_simple)[1], 4),
      "(SE:", round(se(ols_earn_pop_simple)[1], 4), ")\n")
  cat("  OLS (+ state×time FE):", round(coef(ols_earn_pop)[1], 4),
      "(SE:", round(se(ols_earn_pop)[1], 4), ")\n")

  # Balance test for earnings
  panel_2012_earn <- panel_reg %>%
    filter(year == 2012) %>%
    group_by(county_fips) %>%
    summarise(
      log_earn_2012 = mean(log_earn, na.rm = TRUE),
  ...
  balance_earn <- lm(log_earn_2012 ~ factor(iv_quartile), data = panel_2012_earn)
  f_earn <- summary(balance_earn)$fstatistic
  balance_p_earn <- if (!is.null(f_earn)) pf(f_earn[1], f_earn[2], f_earn[3], lower.tail = FALSE) else NA
  cat("  Balance p-value (log earn 2012):", round(balance_p_earn, 4),
      if(balance_p_earn > 0.05) "(PASS)" else "(CONCERN)", "\n")
  ...
  rf_earn <- feols(
    log_earn ~ network_mw_pop_out_state | county_fips + state_fips^yearq,
    data = panel_reg,
    cluster = ~state_fips
  )
  cat("  Reduced form (earnings):", round(coef(rf_earn)[1], 4),
      "(SE:", round(se(rf_earn)[1], 4), ")\n")
}

# ============================================================================
# PART 5D: USD-DENOMINATED SPECIFICATIONS
# ============================================================================

cat("\n╔══════════════════════════════════════════════════════════════════╗\n")
cat("║  PART 5D: USD-DENOMINATED SPECIFICATIONS                         ║\n")
cat("╚══════════════════════════════════════════════════════════════════╝\n\n")
...
if ("network_mw_pop_dollar" %in% names(panel_reg) &&
    "network_mw_pop_out_state_dollar" %in% names(panel_reg)) {

  # First stage (USD)
  fs_usd <- feols(
    network_mw_pop_dollar ~ network_mw_pop_out_state_dollar | county_fips + state_fips^yearq,
    data = panel_reg,
    cluster = ~state_fips
  )
  fs_f_usd <- (coef(fs_usd)[1] / se(fs_usd)[1])^2
  cat("  USD First Stage F:", round(fs_f_usd, 1), "\n")

  # Employment (USD)
  iv_usd_emp <- feols(
    log_emp ~ 1 | county_fips + state_fips^yearq |
      network_mw_pop_dollar ~ network_mw_pop_out_state_dollar,
    data = panel_reg,
    cluster = ~state_fips
  )

  cat("  USD 2SLS (employment): β =", round(coef(iv_usd_emp)[1], 4),
      "(SE:", round(se(iv_usd_emp)[1], 4), ", p =",
      round(fixest::pvalue(iv_usd_emp)[1], 4), ")\n")
  cat("  Interpretation: A $1 increase in network avg MW →",
      round(coef(iv_usd_emp)[1] * 100, 2), "% change in employment\n")

  # Earnings (USD)
  if ("log_earn" %in% names(panel_reg)) {
    iv_usd_earn <- feols(
      log_earn ~ 1 | county_fips + state_fips^yearq |
        network_mw_pop_dollar ~ network_mw_pop_out_state_dollar,
      data = panel_reg,
      cluster = ~state_fips
    )

    cat("  USD 2SLS (earnings): β =", round(coef(iv_usd_earn)[1], 4),
        "(SE:", round(se(iv_usd_earn)[1], 4), ", p =",
        round(fixest::pvalue(iv_usd_earn)[1], 4), ")\n")
    cat("  Interpretation: A $1 increase in network avg MW →",
        round(coef(iv_usd_earn)[1] * 100, 2), "% change in earnings\n")
  }

  # Magnitude context
  usd_sd <- sd(panel_reg$network_mw_pop_dollar, na.rm = TRUE)
  cat("\n  === MAGNITUDE CONTEXT ===\n")
  cat("  1-SD change in USD exposure: $", round(usd_sd, 2), "\n")
  cat("  Predicted employment change for 1-SD:", round(coef(iv_usd_emp)[1] * usd_sd * 100, 2), "%\n")
  if (!is.null(iv_usd_earn)) {
    cat("  Predicted earnings change for 1-SD:", round(coef(iv_usd_earn)[1] * usd_sd * 100, 2), "%\n")
  }
  cat("  Comparison: Cengiz et al. (2019) find direct MW employment elasticity ≈ -0.04 to 0\n")
  cat("  Our NETWORK effect is distinct: information transmission, not direct regulation\n")
} else {
  cat("  USD exposure variables not found in panel. Skipping USD specifications.\n")
}
...
# ============================================================================
# PART 11: REDUCED-FORM EVENT STUDIES (CENTERPIECE OF REVISION)
# ============================================================================

cat("\n╔══════════════════════════════════════════════════════════════════╗\n")
cat("║  PART 11: REDUCED-FORM EVENT STUDIES                            ║\n")
cat("╚══════════════════════════════════════════════════════════════════╝\n\n")

cat("KEY INSIGHT: The structural event study (OLS on endogenous network MW)\n")
cat("rejects parallel trends because it includes endogenous same-state variation.\n")
cat("The reduced-form event study (regressing outcome on the INSTRUMENT)\n")
cat("isolates the identifying variation and shows NO pre-trend problem.\n\n")

# 11A. Structural (endogenous) event study: OLS on full network MW × yearq
# Use yearq interactions for quarterly pre-trend test (matching the original F(4,50)=3.90)
cat("11A. Structural event study (endogenous variable × yearq)...\n")

# Create a yearq integer for i() syntax; reference = 2013Q1
panel_reg <- panel_reg %>%
  mutate(yearq_int = (year - 2012) * 4 + quarter,
         yearq_ref = ifelse(year == 2013 & quarter == 1, TRUE, FALSE))

# Reference period: 2013Q1 (yearq_int = 5)
structural_es <- tryCatch({
  feols(
    log_emp ~ i(yearq_int, network_mw_pop, ref = 5) | county_fips + state_fips^yearq,
    data = panel_reg,
    cluster = ~state_fips
  )
}, error = function(e) {
  cat("  WARNING: Structural event study failed:", e$message, "\n")
  NULL
})

structural_es_pretrend_p <- NA
structural_es_pretrend_F <- NA
if (!is.null(structural_es)) {
  # Extract pre-period coefficients (yearq_int 1-4 = 2012Q1-Q4)
  es_coefs <- coef(structural_es)
  es_vcov <- vcov(structural_es)
  # Pre-period: yearq_int 1,2,3,4 (= 2012 Q1-Q4, ref is yearq_int=5 = 2013Q1)
  pre_idx <- grep("yearq_int::[1-4]:", names(es_coefs))

  if (length(pre_idx) >= 1) {
    pre_coefs <- es_coefs[pre_idx]
    pre_vcov <- es_vcov[pre_idx, pre_idx, drop = FALSE]
    n_pre <- length(pre_coefs)

    if (n_pre == 1) {
      F_struct <- as.numeric((pre_coefs / sqrt(pre_vcov[1,1]))^2)
      structural_es_pretrend_p <- 2 * pt(abs(pre_coefs / sqrt(pre_vcov[1,1])), df = 50, lower.tail = FALSE)
    } else {
      F_struct <- as.numeric(t(pre_coefs) %*% solve(pre_vcov) %*% pre_coefs / n_pre)
      structural_es_pretrend_p <- pf(F_struct, n_pre, 50, lower.tail = FALSE)
    }
    structural_es_pretrend_F <- F_struct
    cat("  Pre-trend test (", n_pre, " pre-period coefs): F =", round(F_struct, 3),
        ", p =", round(structural_es_pretrend_p, 4), "\n")
    cat("  Pre-period coefs:", paste(round(pre_coefs, 4), collapse = ", "), "\n")
  }
}

# 11B. Reduced-form event study: outcome regressed on INSTRUMENT × yearq
# Use quarterly interactions to match the structural ES for apples-to-apples comparison
cat("\n11B. Reduced-form event study (instrument × yearq)...\n")

rf_es_baseline <- tryCatch({
  feols(
    log_emp ~ i(yearq_int, network_mw_pop_out_state, ref = 5) | county_fips + state_fips^yearq,
    data = panel_reg,
    cluster = ~state_fips
  )
}, error = function(e) {
  cat("  WARNING: RF event study failed:", e$message, "\n")
  NULL
})

rf_es_pretrend_p <- NA
rf_es_pretrend_F <- NA
if (!is.null(rf_es_baseline)) {
  es_coefs_rf <- coef(rf_es_baseline)
  es_vcov_rf <- vcov(rf_es_baseline)
  # Pre-period: yearq_int 1,2,3,4 (= 2012 Q1-Q4)
  pre_idx_rf <- grep("yearq_int::[1-4]:", names(es_coefs_rf))

  if (length(pre_idx_rf) >= 1) {
    pre_coefs_rf <- es_coefs_rf[pre_idx_rf]
    pre_vcov_rf <- es_vcov_rf[pre_idx_rf, pre_idx_rf, drop = FALSE]
    n_pre_rf <- length(pre_coefs_rf)

    if (n_pre_rf == 1) {
      F_rf <- as.numeric((pre_coefs_rf / sqrt(pre_vcov_rf[1,1]))^2)
      rf_es_pretrend_p <- 2 * pt(abs(pre_coefs_rf / sqrt(pre_vcov_rf[1,1])), df = 50, lower.tail = FALSE)
    } else {
      F_rf <- as.numeric(t(pre_coefs_rf) %*% solve(pre_vcov_rf) %*% pre_coefs_rf / n_pre_rf)
      rf_es_pretrend_p <- pf(F_rf, n_pre_rf, 50, lower.tail = FALSE)
    }
    rf_es_pretrend_F <- F_rf
    cat("  Pre-trend test (", n_pre_rf, " pre-period coefs): F =", round(F_rf, 3),
        ", p =", round(rf_es_pretrend_p, 4), "\n")
    cat("  Pre-period coefs:", paste(round(pre_coefs_rf, 4), collapse = ", "), "\n")
  }
}

cat("\n  *** COMPARISON ***\n")
cat("  Structural (endogenous) pre-trend p:", round(structural_es_pretrend_p, 4), "\n")
cat("  Reduced-form (instrument) pre-trend p:", round(rf_es_pretrend_p, 4), "\n")
if (!is.na(rf_es_pretrend_p) && rf_es_pretrend_p > 0.05) {
  cat("  --> CLEAN: Reduced-form event study shows NO pre-trend problem!\n")
  cat("  The pre-trend rejection in the structural ES is driven by the\n")
  cat("  endogenous same-state component, NOT by the identifying variation.\n")
}

# 11C. Reduced-form event studies at each distance cutoff
cat("\n11C. Reduced-form event studies by distance cutoff...\n")

rf_es_by_distance <- list()

for (d in distance_thresholds) {
  iv_col <- paste0("iv_pop_dist_", d)
  if (!iv_col %in% names(panel_reg)) next

  panel_d <- panel_reg %>% filter(!is.na(.data[[iv_col]]))
  if (nrow(panel_d) < 10000) next

  # Ensure yearq_int exists in distance subset
  if (!"yearq_int" %in% names(panel_d)) {
    panel_d <- panel_d %>% mutate(yearq_int = (year - 2012) * 4 + quarter)
  }

  rf_es_d <- tryCatch({
    feols(
      as.formula(paste0("log_emp ~ i(yearq_int, ", iv_col, ", ref = 5) | county_fips + state_fips^yearq")),
      data = panel_d,
      cluster = ~state_fips
    )
  }, error = function(e) NULL)
  ...
  rf_pretrend_p_d <- NA
  if (!is.null(rf_es_d)) {
    es_coefs_d <- coef(rf_es_d)
    es_vcov_d <- vcov(rf_es_d)
    pre_idx_d <- grep("yearq_int::[1-4]:", names(es_coefs_d))

    if (length(pre_idx_d) >= 1) {
      pre_coefs_d <- es_coefs_d[pre_idx_d]
      pre_vcov_d <- es_vcov_d[pre_idx_d, pre_idx_d, drop = FALSE]
      n_pre_d <- length(pre_coefs_d)
      if (n_pre_d == 1) {
        F_d <- as.numeric((pre_coefs_d / sqrt(pre_vcov_d[1,1]))^2)
        rf_pretrend_p_d <- 2 * pt(abs(pre_coefs_d / sqrt(pre_vcov_d[1,1])), df = 50, lower.tail = FALSE)
      } else {
        F_d <- as.numeric(t(pre_coefs_d) %*% solve(pre_vcov_d) %*% pre_coefs_d / n_pre_d)
        rf_pretrend_p_d <- pf(F_d, n_pre_d, 50, lower.tail = FALSE)
      }
    }
  }

  rf_es_by_distance[[as.character(d)]] <- list(
    threshold = d,
    model = rf_es_d,
    pretrend_p = rf_pretrend_p_d
  )

  cat(sprintf("  %4d km: RF pre-trend p = %.4f %s\n",
              d, rf_pretrend_p_d,
              ifelse(!is.na(rf_pretrend_p_d) && rf_pretrend_p_d > 0.05, "(CLEAN)", "(CONCERN)")))
}

# ============================================================================
# PART 12: ENHANCED DISTANCE-CREDIBILITY TABLE
# ============================================================================

cat("\n╔══════════════════════════════════════════════════════════════════╗\n")
cat("║  PART 12: DISTANCE-CREDIBILITY TABLE                            ║\n")
cat("╚══════════════════════════════════════════════════════════════════╝\n\n")

# Update distance_results with RF pre-trend p-values and AR CIs
distance_credibility <- list()
...
for (d in distance_thresholds) {
  ...
  # First stage
  fs_formula <- as.formula(paste("network_mw_pop ~", iv_col, "| county_fips + state_fips^yearq"))
  fs_d <- tryCatch(feols(fs_formula, data = panel_d, cluster = ~state_fips), error = function(e) NULL)
  if (is.null(fs_d)) next
  fs_f_d <- (coef(fs_d)[1] / se(fs_d)[1])^2

  # 2SLS
  tsls_d <- NULL
  if (fs_f_d > 5) {
    tsls_formula <- as.formula(paste("log_emp ~ 1 | county_fips + state_fips^yearq | network_mw_pop ~", iv_col))
    tsls_d <- tryCatch(feols(tsls_formula, data = panel_d, cluster = ~state_fips), error = function(e) NULL)
  }
  ...
  panel_2012_d <- panel_d %>%
    filter(year == 2012) %>%
    group_by(county_fips) %>%
    summarise(log_emp_2012 = mean(log_emp, na.rm = TRUE), iv_2012 = mean(.data[[iv_col]], na.rm = TRUE), .groups = "drop") %>%
    mutate(iv_q = ntile(iv_2012, 4))
  bal_d <- lm(log_emp_2012 ~ factor(iv_q), data = panel_2012_d)
  f_bal_d <- summary(bal_d)$fstatistic
  p_bal_d <- if (!is.null(f_bal_d)) pf(f_bal_d[1], f_bal_d[2], f_bal_d[3], lower.tail = FALSE) else NA

  # RF pre-trend p from part 11
  rf_pretrend_p_d <- NA
  if (as.character(d) %in% names(rf_es_by_distance)) {
    rf_pretrend_p_d <- rf_es_by_distance[[as.character(d)]]$pretrend_p
  }

  # Anderson-Rubin CI for this distance
  ar_ci_lo_d <- NA
  ar_ci_hi_d <- NA
  if (!is.null(tsls_d) && fs_f_d > 5) {
    beta_d <- coef(tsls_d)[1]
    se_d <- se(tsls_d)[1]
    beta_grid_d <- seq(beta_d - 4 * se_d, beta_d + 4 * se_d, length.out = 200)
    ar_pvals_d <- numeric(length(beta_grid_d))
    for (j in seq_along(beta_grid_d)) {
      panel_d$y_tilde_d <- panel_d$log_emp - beta_grid_d[j] * panel_d$network_mw_pop
      ar_d <- tryCatch(
        feols(as.formula(paste0("y_tilde_d ~ ", iv_col, " | county_fips + state_fips^yearq")),
              data = panel_d, cluster = ~state_fips),
        error = function(e) NULL)
      ar_pvals_d[j] <- if (!is.null(ar_d)) fixest::pvalue(ar_d)[1] else NA
    }
    panel_d$y_tilde_d <- NULL
    ar_in_d <- beta_grid_d[ar_pvals_d > 0.05]
    if (length(ar_in_d) > 0) {
      ar_ci_lo_d <- min(ar_in_d)
      ar_ci_hi_d <- max(ar_in_d)
    }
  }

  # Median connections at this distance
  n_conn_med <- median(panel_d[[iv_col]], na.rm = TRUE)

  distance_credibility[[as.character(d)]] <- list(
    threshold = d,
    n_obs = nrow(panel_d),
    first_stage_f = fs_f_d,
    balance_p = p_bal_d,
    rf_pretrend_p = rf_pretrend_p_d,
    tsls_coef = if (!is.null(tsls_d)) coef(tsls_d)[1] else NA,
    tsls_se = if (!is.null(tsls_d)) se(tsls_d)[1] else NA,
    tsls_p = if (!is.null(tsls_d)) fixest::pvalue(tsls_d)[1] else NA,
    ar_ci_lower = ar_ci_lo_d,
    ar_ci_upper = ar_ci_hi_d,
    median_iv = n_conn_med
  )
}

# Print enhanced distance-credibility table
cat(sprintf("%6s %8s %8s %10s %10s %8s %8s %12s\n",
            "Dist", "FS F", "Bal p", "RF Pre p", "2SLS", "SE", "p-val", "AR CI"))
cat(paste(rep("─", 85), collapse = ""), "\n")

for (d_name in names(distance_credibility)) {
  r <- distance_credibility[[d_name]]
  ar_str <- if (!is.na(r$ar_ci_lower)) sprintf("[%.2f,%.2f]", r$ar_ci_lower, r$ar_ci_upper) else "---"
  cat(sprintf("%4d km %8.1f %8.3f %10.3f %10.4f %8.4f %8.4f %12s\n",
              r$threshold, r$first_stage_f, r$balance_p,
              ifelse(is.na(r$rf_pretrend_p), NA, r$rf_pretrend_p),
              ifelse(is.na(r$tsls_coef), NA, r$tsls_coef),
              ifelse(is.na(r$tsls_se), NA, r$tsls_se),
              ifelse(is.na(r$tsls_p), NA, r$tsls_p),
              ar_str))
...
# Save distance credibility data for tables
saveRDS(distance_credibility, "../data/distance_credibility.rds")
cat("\nSaved distance_credibility.rds\n")

# ============================================================================
# PART 13: LATE / COMPLIER CHARACTERIZATION
# ============================================================================

cat("\n╔══════════════════════════════════════════════════════════════════╗\n")
cat("║  PART 13: LATE / COMPLIER CHARACTERIZATION                      ║\n")
cat("╚══════════════════════════════════════════════════════════════════╝\n\n")

cat("Characterizing IV compliers: counties whose full-network MW responds\n")
cat("most strongly to out-of-state variation (high IV sensitivity).\n\n")

# Compute IV sensitivity = ratio of out-of-state to full network exposure
panel_complier <- panel_reg %>%
  filter(year == 2013) %>%
  group_by(county_fips) %>%
  summarise(
    iv_sensitivity = mean(network_mw_pop_out_state / network_mw_pop, na.rm = TRUE),
    mean_emp = mean(exp(log_emp), na.rm = TRUE),
    mean_earn = if ("log_earn" %in% names(panel_reg)) mean(exp(log_earn), na.rm = TRUE) else NA,
    mean_log_emp = mean(log_emp, na.rm = TRUE),
    state_fips = first(state_fips),
    .groups = "drop"
  ) %>%
  mutate(
    iv_quartile = ntile(iv_sensitivity, 4),
    complier_label = case_when(
      iv_quartile == 4 ~ "Q4 (High Compliers)",
      iv_quartile == 3 ~ "Q3",
      iv_quartile == 2 ~ "Q2",
      iv_quartile == 1 ~ "Q1 (Low Compliers)"
    )
  )

# Check if we have population data
has_pop <- "population" %in% names(panel_reg)

complier_summary <- panel_complier %>%
  group_by(iv_quartile, complier_label) %>%
  summarise(
    n_counties = n(),
    mean_sensitivity = mean(iv_sensitivity, na.rm = TRUE),
    mean_employment = mean(mean_emp, na.rm = TRUE),
    mean_earnings = if (any(!is.na(mean_earn))) mean(mean_earn, na.rm = TRUE) else NA,
    mean_log_emp = mean(mean_log_emp, na.rm = TRUE),
    .groups = "drop"
  )

cat("Complier characterization by IV sensitivity quartile:\n\n")
cat(sprintf("  %-22s %6s %12s %12s %12s\n",
            "Quartile", "N", "Sensitivity", "Avg Emp", "Avg Log Emp"))
cat(paste(rep("-", 68), collapse = ""), "\n")
for (i in 1:nrow(complier_summary)) {
  r <- complier_summary[i, ]
  cat(sprintf("  %-22s %6d %12.4f %12.0f %12.3f\n",
              r$complier_label, r$n_counties, r$mean_sensitivity,
              r$mean_employment, r$mean_log_emp))
}

# Save complier characterization
saveRDS(list(
  complier_summary = complier_summary,
  complier_detail = panel_complier
), "../data/complier_characterization.rds")
cat("\nSaved complier_characterization.rds\n")
...
results <- list(
  ...
  # Earnings (co-primary)
  ols_earn_pop_simple = if (exists("ols_earn_pop_simple")) ols_earn_pop_simple else NULL,
  ols_earn_pop = if (exists("ols_earn_pop")) ols_earn_pop else NULL,
  iv_earn_pop = if (exists("iv_earn_pop")) iv_earn_pop else NULL,
  balance_p_earn = if (exists("balance_p_earn")) balance_p_earn else NA,

  # USD-denominated specifications
  fs_usd = if (exists("fs_usd")) fs_usd else NULL,
  iv_usd_emp = if (exists("iv_usd_emp")) iv_usd_emp else NULL,
  iv_usd_earn = if (exists("iv_usd_earn")) iv_usd_earn else NULL,
  ...
  # NEW: Reduced-form event studies
  structural_es = structural_es,
  structural_es_pretrend_p = structural_es_pretrend_p,
  rf_es_baseline = rf_es_baseline,
  rf_es_pretrend_p = rf_es_pretrend_p,
  rf_es_by_distance = rf_es_by_distance,

  # NEW: Enhanced distance-credibility
  distance_credibility = distance_credibility,

  # NEW: Complier characterization
  complier_summary = complier_summary
)
