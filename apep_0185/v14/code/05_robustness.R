...
sa_table <- map_dfr(names(sa_results), function(outcome) {
  ...
  if (!is.null(agg)) {
    tibble(
      ...
      ATT = coef(agg),
      SE = se(agg),
      CI_lower = coef(agg) - 1.96 * se(agg),
      CI_upper = coef(agg) + 1.96 * se(agg),
      p_value = pvalue(agg),
      N = m$nobs,
      N_clusters = m$summary_flags$n_clusters
    )
  } else {
...
dose_table <- map_dfr(names(dose_results), function(outcome) {
  ...
  tibble(
    ...
    p_value = pvalue(m)["broadband_rate"],
    N = m$nobs,
    N_clusters = n_clusters,
...
attrition_table <- tibble(
  ...
  p_value = c(
    ...
    if (!is.null(attrition_test3)) pvalue(attrition_test3)["treat_post"] else NA
  ),
  N = c(
    nrow(attrition_data),
    nrow(attrition_data),
