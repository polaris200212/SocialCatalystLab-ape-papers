...
for (outcome in main_outcomes) {
  ...
  cs_objects[[outcome]] <- tryCatch({
    ...
    # Fallback without covariates
    cat("  Retrying without covariates...\n")
    tryCatch({
      att_gt(
        yname = outcome,
        tname = "year",
...
for (fnd in foundations) {
  ...
  foundation_cs[[fnd]] <- tryCatch({
    ...
    cat(sprintf("FAILED: %s. Retrying without covariates... ", e$message))
    tryCatch({
      att_gt(
        yname = fnd,
        tname = "year",
      ...
      cat(sprintf("ALSO FAILED: %s", e2$message))
      NULL
    })
  })
...
twfe_comparison <- map_dfr(names(twfe_results_list), function(outcome) {
  ...
  tibble(
    outcome = outcome,
    twfe_coef = coef(m)["treat_post"],
    twfe_se = se(m)["treat_post"],
    twfe_pval = pvalue(m)["treat_post"],
    twfe_r2 = r2(m, type = "within"),
    twfe_n = m$nobs,
    twfe_n_clusters = m$nobs_origin,
    cs_att = cs_att_val,
    cs_se = cs_se_val,
