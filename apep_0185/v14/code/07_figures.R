...
if (file.exists("data/event_study_data.csv")) {
  ...
  cat(sprintf("  Loaded event_study_data: %d rows\n", nrow(es_data)))
} else {
...
tryCatch({
  ...
  state_means <- analysis %>%
    mutate(state_fips = substr(st_fips, 1, 2)) %>%
    group_by(state_fips) %>%
    summarise(
    ...
    # Overlay place-level points if shapefile available
    {
      if (!is.null(places_sf)) {
        place_means <- analysis %>%
          group_by(st_fips) %>%
          summarise(mean_univ = mean(universalism_index, na.rm = TRUE), .groups = "drop")

        places_plot <- places_sf %>%
          mutate(st_fips = paste0(STATEFP, PLACEFP)) %>%
          inner_join(place_means, by = "st_fips") %>%
          filter(!STATEFP %in% c("02", "15", "60", "66", "69", "72", "78"))
...
tryCatch({
  ...
  if (!"place_id" %in% names(analysis)) {
    analysis <- analysis %>% mutate(place_id = as.numeric(factor(st_fips)))
  }
...
tryCatch({
  ...
  # If it's a HonestDiD sensitivity results object (data frame)
  if (is.data.frame(hd_obj)) {
    hd_df <- hd_obj
    # Standardize column names
    if ("Mbar" %in% names(hd_df)) hd_df <- hd_df %>% rename(M = Mbar)
    if ("lb" %in% names(hd_df) && "ub" %in% names(hd_df)) {
      # Good to go
    } else if ("CI_lower" %in% names(hd_df)) {
      hd_df <- hd_df %>% rename(lb = CI_lower, ub = CI_upper)
    }
  } else if (is.list(hd_obj) && "SensitivityResults" %in% names(hd_obj)) {
...
tryCatch({
  ...
  if (!cs_loaded) stop("cs_results.RData not loaded. Need foundation_results object.")
  if (!exists("foundation_results")) stop("foundation_results object not found.")
  ...
  for (fnd in names(foundation_names)) {
    ...
    if (!is.null(foundation_results[[fnd]]$es)) {
      es_obj <- foundation_results[[fnd]]$es
      es_df <- data.frame(
        time = es_obj$egt,
        att  = es_obj$att.egt,
        se   = es_obj$se.egt
...
tryCatch({
  ...
  if (length(alt_panels) == 0) {
    ...
    for (thresh_pct in alt_thresholds) {
      ...
      df_alt <- did_data %>%
        left_join(treat_timing_alt, by = "st_fips") %>%
        mutate(gname_alt = ifelse(!is.na(treat_year_alt), treat_year_alt, 0))
  ...
  if (!is.null(het_rural_es)) {
    ...
    if ("population" %in% names(analysis)) {
      ...
      for (grp in c("Smaller Places", "Larger Places")) {
        df_sub <- analysis %>%
          filter(rural_group == grp) %>%
          mutate(gname = ifelse(treated, treat_year, 0))
