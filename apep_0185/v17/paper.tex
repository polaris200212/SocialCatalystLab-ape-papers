...
\title{Friends in High Places: \\ Minimum Wage Shocks and Social Network Propagation}
...
\begin{abstract}
\noindent
When California raises its minimum wage to \$15, does the shock reach workers in Texas---not through policy imitation, but through social connections? We construct a population-weighted measure of network minimum wage exposure using Facebook's Social Connectedness Index, capturing the \textit{breadth} of each county's connections to high-wage labor markets. Using out-of-state network ties as instruments ($F > 500$), we find that a \$1 increase in the network average minimum wage raises local earnings by 3.4\% and employment by 9\%. Effects strengthen monotonically with instrument distance, placebo shocks produce nulls, and Anderson-Rubin sets exclude zero. A built-in specification test reveals that probability-weighted exposure---ignoring destination population---shows no significant employment effects, confirming that the \textit{scale} of connections drives the response. Job flows reveal heightened churn; migration is negligible. Workers learn about distant wages through their networks and adjust local labor market behavior accordingly.
\end{abstract}
...
Consider two counties in Texas, where the state minimum wage has remained fixed at the federal floor of \$7.25 per hour since 2009. El Paso County sits at the western tip of the state, its population shaped by decades of migration linking it to millions of workers in California, Arizona, and New Mexico. Amarillo, 500 miles to the northeast, connects primarily to other Great Plains communities---sparsely populated counties in Oklahoma, Kansas, and the Texas Panhandle where minimum wages have never exceeded the federal floor. Legally, the two counties are identical; socially, they are worlds apart. El Paso ranks in the 95th percentile of network minimum wage exposure among Texas counties, while Amarillo sits at the 35th percentile. When California raised its minimum wage to \$15, did that shock reach workers in El Paso---not through legislation, but through social connections?

This paper asks how minimum wage shocks propagate through the economy via social networks. Between 2012 and 2022, eleven U.S.\ states raised their minimum wages above \$12 per hour while twenty states remained at the \$7.25 federal floor. We show that these shocks did not stay within the states that enacted them. Workers in low-wage states learned about distant wage increases through friends, family, and former classmates, and this information reshaped their local labor market behavior---their reservation wages, search intensity, and bargaining power.

We measure each county's exposure to distant minimum wage increases using Facebook's Social Connectedness Index (SCI) weighted by destination population. The key innovation is \textit{population weighting}: a connection to Los Angeles, with its 10 million residents, contributes roughly 1,000 times more to a county's network exposure than an equally strong connection to a rural California county with 9,000 residents. This captures the \textit{breadth} of wage signals a county receives, not just the share of its network in high-wage areas. The distinction proves consequential: population-weighted and probability-weighted exposure produce qualitatively different results.

Our identification strategy exploits out-of-state network connections as an instrument for full network exposure. State-by-time fixed effects absorb each county's own-state minimum wage and all state-level confounders. Identification comes from within-state variation in cross-state social ties: El Paso has dense connections to California; Amarillo's ties run to the Great Plains. The first stage is strong ($F > 500$).

A \$1 increase in the network average minimum wage raises county-level earnings by 3.4\% and employment by 9\% (2SLS; \Cref{tab:usd}). These are market-level equilibrium multipliers concentrated among high-connectivity counties, comparable in magnitude to the local multipliers of 1.5--3.0 documented by \citet{moretti2011local} and \citet{klinemoretti2014}. Probability-weighted exposure---ignoring the scale of connections---shows attenuated and insignificant employment effects despite a robust first stage ($F = 290$). This divergence confirms that the breadth of connections to high-wage areas, not merely the network share directed there, drives the response.

The identification withstands extensive scrutiny. Distance-restricted instruments show effects \textit{strengthening} as connections are limited to origins beyond 200, 300, or 500 kilometers, while pre-treatment balance improves---a pattern consistent with reduced attenuation bias and inconsistent with local confounders. Placebo instruments using GDP or employment shocks through identical network weights produce null effects. Anderson-Rubin confidence sets exclude zero at every distance threshold. Event studies show null pre-treatment coefficients and effects emerging after 2014, precisely when major state minimum wage increases were announced.

The mechanism is information, not migration. Job flow data reveal that network exposure increases both hiring and separations while net job creation remains zero---heightened churn consistent with more active search and more frequent job-to-job transitions. IRS migration data show negligible responses: controlling for migration attenuates the employment coefficient by less than 5\%. Effects concentrate in high-bite industries (retail, food services) where minimum wage information is directly relevant, with null effects in low-bite sectors. A descriptive policy diffusion exercise shows that network exposure to high minimum wages correlates with future own-state wage floor increases, consistent with---though not establishing---a political economy channel through which wage information shapes policy adoption.

This paper contributes to three literatures. First, we demonstrate that minimum wage shocks propagate through social networks over distances far exceeding the geographic spillovers studied by \citet{dube2014designing}, establishing social connections as a channel for cross-state policy transmission. Second, we show that the \textit{scale} of network connections---not just their existence---matters for economic outcomes, with implications for the growing body of research using SCI-based exposure measures \citep{bailey2018social, bailey2018house, chetty2022social}. Third, we contribute to the literature on how networks transmit information rather than people \citep{jager2024worker, kramarz2023, faberman2022}, showing that workers update their labor market behavior based on wage signals from distant connections even absent physical migration.
...
We compile state minimum wage histories from 2010 through 2022 using data from the U.S. Department of Labor, National Conference of State Legislatures, and the Vaghul-Zipperer minimum wage database. State minimum wages ranged from \$7.25 (the federal floor, maintained by 20 states) to over \$15 per hour (California, large employers, 2022). We use the effective statewide minimum wage for each state-quarter, which reflects the statutory floor applicable to the majority of covered workers. Twenty states maintained the federal minimum throughout our sample period, while California increased from \$8.00 to \$15.00 (large employers; \$14.00 for small employers by 2022), New York from \$7.25 to \$14.20 (statewide floor; \$15 in New York City), and Washington from \$9.04 to \$14.49. These large, staggered increases provide the variation that drives our shift-share instrument.
...
\begin{figurenotes}
Binned scatter plot of population-weighted full network exposure (vertical axis) against population-weighted out-of-state exposure (horizontal axis), after residualizing on county and year fixed effects. The slope and F-statistic displayed in the figure (0.836, $F = 551$) reflect this parsimonious specification with county and year fixed effects only; these differ from Table~\ref{tab:main} Column~2 ($\hat{\pi} = 0.579$, $F = 536$), which uses the full specification with state $\times$ year fixed effects and controls that absorb additional within-state variation. Each point represents approximately 2,714 county-quarter observations.
\end{figurenotes}
\end{figure}
...
The joint F-test of pre-period event-study coefficients yields $p = 0.007$. This reflects baseline \textit{level} differences in employment across exposure quartiles (\Cref{tab:balance}), not differential \textit{trends}: the pre-period coefficients are individually small and insignificant, but their joint test is powered by the non-monotonic level variation documented in the balance table ($p = 0.004$). County fixed effects absorb all time-invariant level differences, so identification relies on within-county variation over time. The reduced-form pre-trend F-test produces a similar pattern.

Counties with different network exposure have different employment levels (absorbed by fixed effects), but their employment \textit{trends} are parallel before the minimum wage shocks. Three additional diagnostics reinforce this interpretation: (i) distance-restricted instruments show monotonically improving balance with strengthening coefficients; (ii) controlling for baseline employment $\times$ linear time trend leaves the main coefficient stable; and (iii) formal sensitivity analysis following \citet{rambachanroth2023credible} shows post-treatment effects substantially exceed pre-period deviations.
...
Anderson-Rubin (weak-IV robust)\textsuperscript{a} & --- & $<$0.001 & --- & --- \\
Permutation inference (RI, $n=2{,}000$) & --- & $<$0.001 & --- & $<$0.001 \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
\footnotesize
\item \textit{Notes:} Employment 2SLS coefficient is 0.826 (population-weighted) and 0.323 (probability-weighted).
\textsuperscript{a}Anderson-Rubin confidence set for the population-weighted specification: $[0.51, 1.13]$,
following \citet{andrews2019weak}. Permutation inference based on 2,000 random reassignments of
exposure values within time periods. RI $p$-values test whether the observed coefficient is extreme
relative to the permuted null; they do not account for within-cluster correlation. The
probability-weighted RI $p < 0.001$ is consistent with cluster-robust $p = 0.070$: the coefficient
is large relative to the permuted distribution but imprecisely estimated once within-state
correlation is accounted for. This divergence reinforces the conclusion that the
population-weighted measure produces robust results across all inference procedures.
Network clustering uses SCI-based county groupings.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\paragraph{Reconciling Positive Employment with Zero Net Job Creation.} Rising employment (\Cref{sec:results}) alongside zero net job creation in job flows reflects two factors.

First, the samples differ: QWI job flow variables are missing for approximately 25\% of county-quarters due to confidentiality suppression (compared to 1\% for employment counts), and suppressed counties are disproportionately small and rural.

Second, employment measures the \textit{stock} of jobs at a point in time, while job flows capture gross \textit{flows} within each quarter. Higher hires \textit{and} higher separations can coexist with rising employment if the hiring rate increase slightly exceeds the separation rate increase. Our estimates are consistent: the 2SLS hire rate coefficient (0.058) modestly exceeds the separation rate coefficient (0.044), and a small but persistent excess accumulates over multiple quarters.
...
The small and statistically insignificant migration responses, combined with the strong earnings and employment effects, suggest that the dominant channel operates through information updating, revised reservation wages, and more active job search---not physical relocation. This interpretation is consistent with \citet{jager2024worker}, who document that workers form beliefs about outside options based on network information, and that these beliefs affect labor market behavior even for non-movers.

\subsection{Policy Diffusion}

If network connections transmit wage information that shifts workers' expectations and bargaining behavior, a natural downstream consequence is political pressure for higher local minimum wages. Workers who learn that friends in California earn \$15 per hour may demand similar policies from their own state legislatures, reducing the political cost of minimum wage increases. This is a social network channel for policy diffusion, distinct from the geographic and political similarity channels documented by \citet{shipan2008mechanisms}.

We examine this hypothesis descriptively using a state-year panel (51 states, 2013--2022). For each state-year, we construct the state-average population-weighted network minimum wage exposure by collapsing our county-level measure. The outcome is whether state $s$ raised its minimum wage in year $t+1$ (linear probability model) or the magnitude of the change ($\Delta\log(\text{MW})_{s,t+1}$). We control for lagged own minimum wage, state fixed effects, and year fixed effects. States already at the \$15 ceiling are excluded. We emphasize that this exercise is descriptive: political ideology, unionization, and other state-level factors correlated with both network exposure and MW adoption are not fully controlled. The estimates capture a conditional correlation, not a causal effect.

\input{tables/tab_policy_diffusion}

\Cref{tab:diffusion} reports the results. States whose workers have stronger network connections to high-minimum-wage areas are more likely to raise their own minimum wages in the following year. The linear probability model coefficient is positive, indicating that a one-unit increase in network exposure raises the probability of a future MW increase. The continuous specification confirms that the magnitude of wage floor changes also responds to network exposure.

This descriptive finding is consistent with---but does not establish---a political economy channel through which minimum wage shocks propagate. The correlation could reflect reverse causation (states that are already planning MW increases happen to have high-exposure workers), omitted political variables, or other confounds. We include it as suggestive evidence that network-transmitted wage information may shape not only individual labor market behavior but also the political environment in which wage policy is made.
...
Why does a raise in California create jobs in Texas? The evidence points to information, not relocation. Several lines of evidence identify the channel.
...
\paragraph{Assessing the 9\% Employment Magnitude.} The 9\% employment effect of a \$1 increase in the network average minimum wage is large. Four considerations contextualize this magnitude.

First, our 2SLS estimates capture a local average treatment effect (LATE) among compliers---counties whose full network exposure responds most strongly to out-of-state variation. These counties have extensive cross-state ties to populous metropolitan areas. The 9\% effect applies to this selected subset, not the average county.

Second, a \$1 network average minimum wage change is not a \$1 increase in the county's \textit{own} minimum wage. Network exposure reflects the population-weighted average across thousands of connected counties; a \$1 shift represents a substantial recomposition of the county's entire network wage environment. The 9\% is not comparable to conventional own-minimum-wage employment elasticities.

Third, spatial multipliers of similar magnitude appear in related contexts: \citet{klinemoretti2014} find local employment multipliers of 2--3, and \citet{moretti2011local} estimates multipliers of 1.5--2.5. Our market-level equilibrium multiplier incorporates analogous general equilibrium channels.

Fourth, the standard deviation of network average minimum wage is approximately \$0.96, so the economically relevant variation produces roughly 8.6\% employment changes---within the range of spatial multiplier estimates in the literature. Nonetheless, we caution that the magnitudes could partly reflect violations of the exclusion restriction (e.g., correlated origin-state policy bundles) or artifacts of the LATE weighting. We view the qualitative finding---that network-weighted MW exposure affects local labor markets through information rather than migration---as more robustly established than the precise point estimates.
...
Minimum wage policies generate spillover effects through social networks that extend far beyond state borders. When California raises its minimum wage, information about higher wages diffuses through social connections to workers in Texas, Mississippi, and other low-minimum-wage states, reshaping their labor market expectations and behavior. Traditional cost-benefit analyses focus on direct effects within the implementing jurisdiction; indirect effects through social networks are quantitatively important and should be considered in comprehensive policy evaluation.
...
Minimum wage shocks do not stay within the states that enact them. When California raises its wage floor, information about higher wages propagates through social connections to workers in Texas, Mississippi, and the Great Plains. These workers revise their expectations, search more intensively, bargain more aggressively, and cycle through jobs at higher rates. A \$1 increase in the network average minimum wage raises local earnings by 3.4\% and employment by 9\%, with a strong first stage ($F > 500$) and effects that strengthen as instruments are restricted to increasingly distant connections.

The population-versus-probability divergence reveals which dimension of social connections matters. Probability-weighted exposure---capturing only the \textit{share} of connections in high-wage areas---shows attenuated and insignificant employment effects despite a robust first stage ($F = 290$). The breadth of connections, not the network share, drives the response. This finding has implications for the growing body of research using SCI-based exposure measures across housing, trade, and public health.

The mechanism is information, not migration. Job flows reveal heightened churn---more hiring and more separations---without net expansion. Migration responses are negligible, accounting for less than 5\% of the employment effect. Effects concentrate in high-bite industries where minimum wage information directly shapes workers' decisions. Network exposure correlates with future own-state minimum wage increases, consistent with information feeding back into the political economy of wage policy.
...
Andrews, I., Stock, J. H., \& Sun, L. (2019).
...
\bibitem[de Chaisemartin and D'Haultf{\oe}uille(2024)]{dechaisemartin2024}
de Chaisemartin, C., \& D'Haultf{\oe}uille, X. (2024).
Difference-in-differences estimators of intertemporal treatment effects.
\textit{Review of Economics and Statistics}, 106(6), 1723--1736.
...
\textbf{Comparative statics.} The model yields four testable predictions.
First, $\frac{\partial \log(E_c)}{\partial \text{PopMW}_{ct}} > 0$ and
$\frac{\partial \log(W_c)}{\partial \text{PopMW}_{ct}} > 0$: higher population-weighted
exposure increases both employment and earnings.
Second, $\frac{\partial \log(E_c)}{\partial \text{ProbMW}_{ct}} \approx 0$:
probability-weighted exposure should have no effect conditional on population-weighted
exposure. Third, the effect is increasing in the local-network wage gap. Fourth, network
exposure should increase labor market activity, particularly hiring. All four predictions
