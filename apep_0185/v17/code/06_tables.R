...
# ============================================================================
# Table 2: Main Results - Employment AND Earnings (Co-Primary)
# ============================================================================

cat("\nTable 2: Main results (Employment + Earnings, OLS vs 2SLS vs USD)...\n")

# Extract employment coefficients
...
# USD employment coefficient
usd_emp_coef <- if (!is.null(results$iv_usd_emp)) coef(results$iv_usd_emp)[1] else NA
usd_emp_se <- if (!is.null(results$iv_usd_emp)) se(results$iv_usd_emp)[1] else NA
usd_emp_p <- if (!is.null(results$iv_usd_emp)) fixest::pvalue(results$iv_usd_emp)[1] else NA

# Extract earnings coefficients
earn_ols_coef <- if (!is.null(results$ols_earn_pop)) coef(results$ols_earn_pop)[1] else NA
earn_ols_se <- if (!is.null(results$ols_earn_pop)) se(results$ols_earn_pop)[1] else NA
earn_ols_p <- if (!is.null(results$ols_earn_pop)) fixest::pvalue(results$ols_earn_pop)[1] else NA

earn_iv_coef <- if (!is.null(results$iv_earn_pop)) coef(results$iv_earn_pop)[1] else NA
earn_iv_se <- if (!is.null(results$iv_earn_pop)) se(results$iv_earn_pop)[1] else NA
earn_iv_p <- if (!is.null(results$iv_earn_pop)) fixest::pvalue(results$iv_earn_pop)[1] else NA

usd_earn_coef <- if (!is.null(results$iv_usd_earn)) coef(results$iv_usd_earn)[1] else NA
usd_earn_se <- if (!is.null(results$iv_usd_earn)) se(results$iv_usd_earn)[1] else NA
usd_earn_p <- if (!is.null(results$iv_usd_earn)) fixest::pvalue(results$iv_usd_earn)[1] else NA

# Helper
fmt_coef <- function(c, p) if (is.na(c)) "---" else paste0(sprintf("%.4f", c), stars(p))
fmt_se <- function(s) if (is.na(s)) "" else sprintf("(%.4f)", s)

n_obs <- format(nobs(results$iv_2sls_pop), big.mark = ",")
...
tab2_lines <- c(
  ...
  paste0("Full Network MW & ", fmt_coef(ols1_coef, ols1_p), " & ",
         fmt_coef(ols2_coef, ols2_p), " & ", fmt_coef(iv_coef, iv_p), " & ",
         fmt_coef(usd_emp_coef, usd_emp_p), " \\\\"),
  paste0(" & ", fmt_se(ols1_se), " & ", fmt_se(ols2_se), " & ",
         fmt_se(iv_se), " & ", fmt_se(usd_emp_se), " \\\\"),
  "\\addlinespace",
  "\\multicolumn{5}{l}{\\textit{Panel B: Log Earnings}} \\\\[3pt]",
  paste0("Full Network MW & --- & ", fmt_coef(earn_ols_coef, earn_ols_p), " & ",
         fmt_coef(earn_iv_coef, earn_iv_p), " & ",
         fmt_coef(usd_earn_coef, usd_earn_p), " \\\\"),
  paste0(" & & ", fmt_se(earn_ols_se), " & ",
         fmt_se(earn_iv_se), " & ", fmt_se(usd_earn_se), " \\\\"),
  "\\midrule",
  paste0("County FE & Yes & Yes & Yes & Yes \\\\"),
  paste0("Time FE & Yes & No & No & No \\\\"),
  paste0("State $\\times$ Time FE & No & Yes & Yes & Yes \\\\"),
  paste0("First Stage F & --- & --- & ", sprintf("%.1f", first_stage_f), " & ",
         if (!is.null(results$fs_usd)) sprintf("%.1f", (coef(results$fs_usd)[1] / se(results$fs_usd)[1])^2) else "---",
         " \\\\"),
...
# ============================================================================
# Table 11: Job Flow Results
# ============================================================================

cat("\nTable 11: Job flow mechanism results...\n")

if (file.exists("../data/job_flow_results.rds")) {
  jf <- readRDS("../data/job_flow_results.rds")

  if (jf$has_job_flows && length(jf$results) > 0) {
    fmt <- function(val) if (is.null(val) || is.na(val)) "---" else sprintf("%.4f", val)

    jf_rows <- list()
    for (r_name in names(jf$results)) {
      r <- jf$results[[r_name]]
      ols_c <- if (!is.null(r$ols)) coef(r$ols)[1] else NA
      ols_s <- if (!is.null(r$ols)) se(r$ols)[1] else NA
      ols_p <- if (!is.null(r$ols)) fixest::pvalue(r$ols)[1] else NA
      ...
      jf_rows[[length(jf_rows) + 1]] <- tibble(
        Outcome = r$label,
        `OLS Coef.` = paste0(fmt(ols_c), stars(ols_p)),
        `OLS SE` = sprintf("(%s)", fmt(ols_s)),
        `2SLS Coef.` = paste0(fmt(iv_c), stars(iv_p)),
        `2SLS SE` = sprintf("(%s)", fmt(iv_s)),
        `N` = format(r$n_obs, big.mark = ",")
      )
    }

    tab11_data <- bind_rows(jf_rows)

    # Coverage info
    coverage_pct <- round(100 * mean(jf$coverage, na.rm = TRUE), 0)

    tab11_latex <- tab11_data %>%
      kable(
        format = "latex",
        booktabs = TRUE,
        escape = FALSE,
        caption = "Job Flow Mechanism: Effect of Network Exposure on Hiring, Separations, and Job Flows",
        label = "jobflows",
        align = c("l", "c", "c", "c", "c", "c")
      ) %>%
      kable_styling(latex_options = "hold_position") %>%
      add_footnote(
        paste0("Notes: QWI job flow outcomes. HirA = all hires; Sep = separations; FrmJbC/FrmJbD = firm job ",
               "creation/destruction. Rates are computed as flows divided by beginning-of-quarter employment. ",
               "Approximately ", coverage_pct, "\\% of county-quarters have non-suppressed job flow data. ",
               "County and state $\\times$ time FEs. SEs clustered at state level. ",
               "*** p$<$0.01, ** p$<$0.05, * p$<$0.1."),
        notation = "none"
      )

    writeLines(tab11_latex, "../tables/tab11_jobflows.tex")
    cat("  Saved tab11_jobflows.tex\n")
  } else {
    cat("  No job flow results available, skipping Table 11.\n")
  }
} else {
  cat("  job_flow_results.rds not found, skipping Table 11.\n")
}

# ============================================================================
# Table 12: Distance-Credibility Table (CENTERPIECE)
# ============================================================================

cat("\nTable 12: Distance-credibility table...\n")

if (file.exists("../data/distance_credibility.rds")) {
  dist_cred <- readRDS("../data/distance_credibility.rds")

  if (length(dist_cred) > 0) {
    dc_df <- do.call(rbind, lapply(names(dist_cred), function(d) {
      r <- dist_cred[[d]]
      data.frame(
        dist = r$threshold,
        fs_f = as.numeric(r$first_stage_f),
        balance_p = as.numeric(r$balance_p),
        rf_pretrend_p = as.numeric(r$rf_pretrend_p),
        tsls_coef = as.numeric(r$tsls_coef),
        tsls_se = as.numeric(r$tsls_se),
        ar_ci_lower = as.numeric(r$ar_ci_lower),
        ar_ci_upper = as.numeric(r$ar_ci_upper),
        n_obs = as.numeric(r$n_obs),
        stringsAsFactors = FALSE
      )
    }))
    ...
    tab12_data <- dc_df %>%
      mutate(
        `Distance` = paste0("$\\geq$ ", dist, " km"),
        `FS F` = sprintf("%.1f", fs_f),
        `Balance p` = sprintf("%.3f", balance_p),
        `RF Pre-Trend p` = sprintf("%.3f", rf_pretrend_p),
        `2SLS` = ifelse(is.na(tsls_coef), "---", sprintf("%.3f", tsls_coef)),
        `SE` = ifelse(is.na(tsls_se), "---", sprintf("(%.3f)", tsls_se)),
        `AR 95\\% CI` = ifelse(is.na(ar_ci_lower), "---",
                               sprintf("[%.2f, %.2f]", ar_ci_lower, ar_ci_upper)),
        `N` = format(n_obs, big.mark = ",")
      ) %>%
      select(`Distance`, `FS F`, `Balance p`, `RF Pre-Trend p`, `2SLS`, `SE`, `AR 95\\% CI`, `N`)

    tab12_latex <- tab12_data %>%
      kable(
        format = "latex",
        booktabs = TRUE,
        escape = FALSE,
        caption = "Distance-Credibility Analysis: Instrument Strength, Pre-Trends, and Treatment Effects",
        label = "distcred",
        align = c("l", "c", "c", "c", "c", "c", "c", "c")
      ) %>%
      kable_styling(latex_options = c("hold_position", "scale_down")) %>%
      add_footnote(
        paste0("Notes: Each row uses out-of-state SCI connections beyond the distance threshold as the instrument. ",
               "FS F = first-stage F-statistic. Balance p = joint F-test of pre-treatment employment equality across IV quartiles. ",
               "RF Pre-Trend p = joint F-test of pre-period coefficients in the reduced-form event study (outcome regressed on instrument $\\times$ year). ",
               "AR CI = Anderson-Rubin 95\\% confidence set (weak-instrument robust). ",
               "State-clustered standard errors in parentheses."),
        notation = "none"
      )

    writeLines(tab12_latex, "../tables/tab12_distance_credibility.tex")
    cat("  Saved tab12_distance_credibility.tex\n")
  }
} else {
  cat("  distance_credibility.rds not found, skipping Table 12.\n")
}

# ============================================================================
# Table 13: LATE Complier Characterization
# ============================================================================

cat("\nTable 13: LATE complier characterization...\n")

if (file.exists("../data/complier_characterization.rds")) {
  comp_data <- readRDS("../data/complier_characterization.rds")
  comp_summary <- comp_data$complier_summary

  tab13_data <- comp_summary %>%
    arrange(iv_quartile) %>%
    mutate(
      `Quartile` = complier_label,
      `N Counties` = format(n_counties, big.mark = ","),
      `Mean IV Sensitivity` = sprintf("%.4f", mean_sensitivity),
      `Mean Employment` = format(round(mean_employment), big.mark = ","),
      `Mean Log Emp` = sprintf("%.3f", mean_log_emp),
      `Mean Earnings` = ifelse(is.na(mean_earnings), "---", format(round(mean_earnings), big.mark = ","))
    ) %>%
    select(`Quartile`, `N Counties`, `Mean IV Sensitivity`, `Mean Employment`, `Mean Log Emp`, `Mean Earnings`)

  tab13_latex <- tab13_data %>%
    kable(
      format = "latex",
      booktabs = TRUE,
      escape = FALSE,
      caption = "LATE Complier Characterization: County Characteristics by IV Sensitivity Quartile",
      label = "compliers",
      align = c("l", "c", "c", "c", "c", "c")
    ) %>%
    kable_styling(latex_options = "hold_position") %>%
    add_footnote(
      paste0("Notes: IV sensitivity = ratio of out-of-state to full network MW exposure (2013 baseline). ",
             "Q4 (High Compliers) = counties whose full network MW responds most to out-of-state variation. ",
             "These are the units that drive the LATE estimate. Employment and earnings from QWI."),
      notation = "none"
    )

  writeLines(tab13_latex, "../tables/tab13_compliers.tex")
  cat("  Saved tab13_compliers.tex\n")
} else {
  cat("  complier_characterization.rds not found, skipping Table 13.\n")
}
