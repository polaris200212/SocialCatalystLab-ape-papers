...
if (has_migration) {
  ...
  # Convert to numeric first if character, then format
  # Inflow: y2 = destination (focal county), y1 = origin
  inflow_clean <- inflow_raw %>%
    mutate(
      y1_statefips = as.integer(y1_statefips),
      y1_countyfips = as.integer(y1_countyfips),
      y2_statefips = as.integer(y2_statefips),
      y2_countyfips = as.integer(y2_countyfips)
    ) %>%
