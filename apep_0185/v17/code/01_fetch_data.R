...
# QWI API base URL and key
qwi_base <- "https://api.census.gov/data/timeseries/qwi/sa"
census_key <- Sys.getenv("CENSUS_API_KEY", "")

# Industry codes to fetch (one per request; multi-industry requests return HTTP 204)
industry_codes <- c("00", "44-45", "72", "52", "54")

# Function to fetch QWI for a state-year-quarter (loops over industries individually)
    ...
    for (attempt in 1:3) {
      tryCatch({
        ...
        if (status_code(resp) == 200) {
          ...
          message("  [WARN] HTTP ", status_code(resp), " for state=", state_fips,
                  " year=", year, " Q", quarter, " ind=", ind)
          break
        }
      }, error = function(e) {
        message("  [ERROR] state=", state_fips, " year=", year, " Q", quarter,
                " ind=", ind, ": ", conditionMessage(e))
      })
    }
    # No delay needed with Census API key
  }
...
# Fetch QWI for full panel: 2012-2022, all 4 quarters, all states
...
expected_cells <- length(state_fips_list) * length(years_to_fetch) * length(quarters)
cat("  Fetching QWI for", length(state_fips_list), "states x",
    length(years_to_fetch), "years x 4 quarters =", expected_cells, "cells...\n")
cat("  This may take several minutes...\n")

# Initialize results list and failure tracking
qwi_list <- list()
failed_cells <- list()
counter <- 0
...
for (st in state_fips_list) {
  for (yr in years_to_fetch) {
    for (qtr in quarters) {
      counter <- counter + 1
      if (counter %% 100 == 0) {
        cat("  Progress:", counter, "/", expected_cells, "\n")
      }
      ...
      if (!is.null(result)) {
        ...
        failed_cells[[length(failed_cells) + 1]] <- list(state = st, year = yr, quarter = qtr)
      }
...
# Report completeness
n_success <- length(qwi_list)
n_failed <- length(failed_cells)
coverage <- n_success / expected_cells * 100
cat("\n  === QWI FETCH COMPLETENESS ===\n")
cat("  Successful:", n_success, "/", expected_cells, sprintf("(%.1f%%)\n", coverage))
cat("  Failed:", n_failed, "\n")

if (n_failed > 0 && n_failed <= 20) {
  cat("  Failed cells:\n")
  for (fc in failed_cells) {
    cat("    state=", fc$state, " year=", fc$year, " Q", fc$quarter, "\n", sep = "")
  }
} else if (n_failed > 20) {
  cat("  First 20 failed cells:\n")
  for (fc in failed_cells[1:20]) {
    cat("    state=", fc$state, " year=", fc$year, " Q", fc$quarter, "\n", sep = "")
  }
}

if (coverage < 95) {
  stop("QWI coverage below 95% (", round(coverage, 1), "%). Cannot proceed with incomplete data.")
}
...
if (length(qwi_list) > 0) {
  ...
  stop("QWI API returned no data. Cannot proceed.")
}
