...
\title{Social Network Minimum Wage Exposure: \\ A New County-Level Measure Using the Facebook Social Connectedness Index\footnote{This paper is a revision of APEP-0185. See \url{https://github.com/SocialCatalystLab/ape-papers/tree/main/papers/apep_0185} for the original.}}
...
Our contribution is primarily descriptive and data-oriented. We construct and release a county-by-quarter panel containing each county's own-state minimum wage, SCI-weighted social network minimum wage (excluding own-state), and distance-weighted geographic minimum wage exposure. The panel also includes derived measures such as the gap between network and own-state minimum wages, as well as network community assignments based on Louvain clustering of the SCI graph.
...
While we do not pursue causal identification---doing so would require addressing challenging questions about endogeneity of network formation, omitted variables, and SUTVA violations---we provide an illustrative application demonstrating how our measure could be used in future research. We estimate associations between network exposure and county employment, present event study patterns around the Fight for \$15 policy shock, and conduct comprehensive robustness analyses. These results are suggestive rather than causal, but they demonstrate the potential of the data for policy-relevant research. Our primary goal is to provide a public good: a carefully constructed dataset and transparent methodology that future researchers can build upon.

The remainder of this paper proceeds as follows. Section 2 reviews the relevant literature on social networks in economics and minimum wage policy. Section 3 describes the data sources. Section 4 details the construction of our network exposure measure. Section 5 presents descriptive statistics and visualizations. Section 6 analyzes heterogeneity across regions and network communities. Section 7 presents an illustrative application examining associations between network exposure and employment. Section 8 reports comprehensive robustness analyses. Section 9 discusses potential applications and identification strategies for future research. Section 10 describes data availability. Section 11 concludes.
...
\textbf{SCI time-invariance.} We treat the Social Connectedness Index as time-invariant, using the 2018 vintage throughout our 2010--2023 analysis period. While \citet{bailey2018social} document high year-over-year correlations ($\rho > 0.97$), this assumption precludes examining how network structure evolves in response to policy changes. If minimum wage increases induce migration that restructures social networks, our time-invariant SCI would not capture these dynamics.

\textbf{Employment data temporal interpolation.} For labor market outcomes, we use Quarterly Census of Employment and Wages (QCEW) data. The publicly available QCEW bulk files provide annual averages rather than true quarterly data. We create quarterly observations by replicating annual values across four quarters. This approach preserves cross-sectional variation (across counties) and year-to-year variation, but does \textit{not} capture within-year employment fluctuations. Standard errors should be interpreted with this in mind---the effective sample size for temporal variation is years, not quarters. We document this limitation in our replication code.

\textbf{Anomalous value filtering.} We exclude county-quarter observations where network exposure falls below \$7.00 (the federal minimum wage floor), which affects approximately 8\% of observations. These anomalous values arise from data construction issues, including missing SCI links for some county pairs. Excluded observations are concentrated in rural counties with sparse SCI coverage. We report sensitivity analyses that include these observations (with winsorization) to verify that results are not driven by this exclusion.
...
\section{Illustrative Application: Network Exposure and Employment}

While we deliberately do not pursue causal identification in this paper, we present an illustrative application demonstrating how our network exposure measure could be used in future research. This analysis is suggestive rather than causal---we make no claims about identification---but it demonstrates the data structure and the types of patterns researchers might investigate.

\subsection{Methodological Context}

Before presenting our illustrative results, we situate our design within the modern econometric literature on difference-in-differences and shift-share designs.

Our network exposure measure resembles a shift-share (Bartik) instrument where the ``shares'' are pre-determined SCI weights and the ``shocks'' are minimum wage changes in connected states. \citet{goldsmith2020bartik} clarify when such designs identify causal effects: either the shares must be exogenous (predetermined and uncorrelated with unobserved factors affecting outcomes) or the shocks must be exogenous (as-good-as-random). In our setting, the SCI weights are predetermined (measured in 2018) but may be correlated with county characteristics that affect labor market outcomes. The minimum wage shocks reflect political decisions that may correlate with economic conditions. Neither exogeneity condition is clearly satisfied, motivating our cautious interpretation.

For staggered policy adoption designs, \citet{goodman2021difference} shows that two-way fixed effects (TWFE) estimators can produce biased estimates when treatment effects are heterogeneous and treatment timing varies. \citet{callaway2021difference} and \citet{sun2021estimating} provide robust alternatives that properly aggregate heterogeneous treatment effects. Our continuous exposure measure differs from the binary treatment case these papers address, but similar concerns about heterogeneity and negative weighting apply. \citet{borusyak2022quasi} provide inference procedures for shift-share designs that account for correlated shocks.

These methodological considerations explain why we present our application as illustrative rather than causal. Valid causal inference would require either (a) credibly arguing for shock or share exogeneity, or (b) employing the robust estimators developed in the recent literature. We leave this to future work.

\subsection{Specification}

We estimate a continuous exposure model with the following structure:
\begin{equation}
\log(\text{Emp})_{ct} = \beta \cdot \text{NetworkExposure}_{ct} + \alpha_c + \gamma_{st} + \varepsilon_{ct}
\end{equation}
where $\text{Emp}_{ct}$ is employment in county $c$ at time $t$, $\text{NetworkExposure}_{ct}$ is our SCI-weighted average minimum wage measure, $\alpha_c$ are county fixed effects, and $\gamma_{st}$ are state-by-time fixed effects. The state-by-time fixed effects absorb the own-state minimum wage, so $\beta$ captures the association between network exposure and employment conditional on the county's own-state minimum wage.

We cluster standard errors at the state level. Alternative clustering by network community (Section 6.3) produces similar standard errors, suggesting that state-level clustering adequately captures the relevant correlation structure.

\subsection{Three-Tier Results}

Table~\ref{tab:illustrative_results} presents results from three specifications of increasing restrictiveness:

\textbf{Tier 1: Naive specification.} County and time fixed effects only. This specification conflates own-state and network exposure effects since both vary by state.

\textbf{Tier 2: Main specification.} County and state-by-time fixed effects. This absorbs the own-state minimum wage, isolating within-state variation in network exposure.

\textbf{Tier 3: Horse race.} Includes both network and geographic exposure simultaneously. This tests whether network exposure provides information beyond geographic proximity.

\begin{table}[htbp]
\centering
\caption{Illustrative Regression Results: Network Exposure and Employment}
\label{tab:illustrative_results}
\begin{threeparttable}
\begin{tabular}{lccc}
\toprule
 & Tier 1 & Tier 2 & Tier 3 \\
 & Naive & Main & Horse Race \\
\midrule
Network Exposure & 0.012 & 0.008 & 0.005 \\
 & (0.006) & (0.005) & (0.004) \\
 & $[0.042]$ & $[0.103]$ & $[0.211]$ \\
\\
Geographic Exposure & & & 0.006 \\
 & & & (0.003) \\
 & & & $[0.045]$ \\
\midrule
...
\midrule
F-test: Network $|$ Geography & & & 1.56 \\
 & & & $[0.212]$ \\
\midrule
Observations & 159,907 & 159,907 & 159,907 \\
Counties & 3,068 & 3,068 & 3,068 \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
\small
\item \textit{Notes:} Standard errors in parentheses, clustered at the state level. $p$-values in brackets. Network Exposure is the SCI-weighted average minimum wage in socially connected counties in other states. Geographic Exposure is the inverse-distance-weighted average minimum wage in geographically proximate counties. The dependent variable is log employment. These results are illustrative---we make no causal claims due to identification concerns discussed in text.
\end{tablenotes}
\end{threeparttable}
\end{table}

The Tier 2 coefficient of 0.008 suggests that a \$1 increase in network exposure is associated with a 0.8\% increase in county employment. However, this estimate is not statistically significant at conventional levels ($p = 0.103$), and we emphasize that the identification assumptions required for causal interpretation may not hold.

The Tier 3 horse race shows that when both network and geographic exposure are included, the network coefficient attenuates and the F-test for additional contribution is not significant. This suggests that much of the apparent network effect may reflect geographic patterns that happen to correlate with social connections.

\subsection{Event Study Pattern}

We interact a county's time-averaged network exposure with year indicators, using 2012 as the reference year:
\begin{equation}
\log(\text{Emp})_{ct} = \sum_{k \neq 2012} \beta_k \cdot \ind[t=k] \cdot \overline{\text{NetworkExposure}}_c + \alpha_c + \gamma_{st} + \varepsilon_{ct}
\end{equation}

The event study reveals no clear pre-trend before 2015, with pre-period coefficients jointly insignificant (F-test $p = 0.34$). Post-2015 coefficients are positive but imprecisely estimated, with confidence intervals including zero. This pattern is consistent with either (a) a true but modest network effect that our design lacks power to detect, or (b) no effect combined with noise.

We emphasize that the parallel trends test supports---but does not prove---the identifying assumption. Counties with high and low network exposure may have been on parallel trends for reasons unrelated to the validity of our design.

\subsection{Industry Heterogeneity}

As a placebo-style test, we examine whether network exposure effects differ across industries with different exposure to minimum wage policy. ``High-bite'' industries (retail, accommodation/food services) employ many workers near the minimum wage, while ``low-bite'' industries (finance, professional services) employ few such workers. If network exposure affects employment through minimum wage information channels, we would expect stronger effects in high-bite industries.

Table~\ref{tab:industry_het} shows that the point estimate is indeed larger in high-bite industries (0.011) than low-bite industries (0.003), but neither estimate is statistically significant. The difference is suggestive but not conclusive.

\begin{table}[htbp]
\centering
\caption{Industry Heterogeneity in Network Exposure Effects}
...
\midrule
Network Exposure & 0.011 & 0.003 \\
 & (0.007) & (0.004) \\
 & $[0.118]$ & $[0.453]$ \\
\midrule
County FE & Yes & Yes \\
State $\times$ Time FE & Yes & Yes \\
Observations & 63,963 & 63,963 \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
\small
\item \textit{Notes:} Standard errors clustered at state level. High-bite industries: retail (NAICS 44-45) and accommodation/food services (NAICS 72). Low-bite industries: finance (NAICS 52) and professional services (NAICS 54). These results are illustrative only.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
These illustrative results suggest several patterns: (a) network exposure shows a positive but imprecise association with employment; (b) much of this association may reflect geographic patterns; (c) the event study shows no clear pre-trends but also no clear post-treatment effect; (d) heterogeneity by industry is in the expected direction but not statistically significant.

We interpret these results as demonstrating that our network exposure measure has the properties one would want for studying minimum wage spillovers---variation across space and time, correlation with plausible mechanisms---but that definitive causal conclusions require additional identification strategies beyond what we pursue here. Future researchers with access to additional data or natural experiments may be better positioned to credibly estimate causal effects.
...
Table~\ref{tab:loso} shows that the coefficient is relatively stable across exclusions, ranging from 0.006 to 0.011. No single state drives the result. The coefficient is largest when excluding California (0.011), suggesting that California counties---which have high network exposure---may be attenuating the estimate.
...
\midrule
None (baseline) & 0.008 & 0.005 \\
California & 0.011 & 0.006 \\
New York & 0.007 & 0.005 \\
Washington & 0.008 & 0.005 \\
Massachusetts & 0.009 & 0.005 \\
Florida & 0.006 & 0.005 \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
...
\begin{table}[htbp]
\centering
\caption{Lagged Exposure Specifications}
\label{tab:lags}
\begin{threeparttable}
\begin{tabular}{lcccc}
\toprule
...
\midrule
Network Exposure & 0.008 & 0.007 & 0.006 & 0.004 \\
 & (0.005) & (0.005) & (0.005) & (0.005) \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
\small
\item \textit{Notes:} Standard errors clustered at state level. Contemporaneous specification is the baseline from Table~\ref{tab:illustrative_results}.
\end{tablenotes}
\end{threeparttable}
\end{table}

The coefficient declines with longer lags, from 0.008 (contemporaneous) to 0.004 (4-quarter lag). This pattern could reflect (a) contemporaneous effects dominating lagged effects, (b) attenuation from measurement error accumulating at longer lags, or (c) no true effect with coefficients converging to zero.
...
\textbf{Pre-COVID (2012--2019):} Coefficient = 0.009 (SE = 0.006). Excluding the COVID period slightly strengthens the estimate.

\textbf{Post-2015 (2015--2022):} Coefficient = 0.010 (SE = 0.006). Focusing on the post-Fight-for-\$15 period, when variation in network exposure increased, produces a marginally larger estimate.

\textbf{Full sample (2012--2022):} Coefficient = 0.008 (SE = 0.005). The baseline estimate.

Results are qualitatively similar across time windows, suggesting that the patterns documented are not artifacts of the COVID period or the specific sample period chosen.
...
State-clustered SE: 0.005. Network-clustered SE: 0.006.
...
Our robustness analysis reveals that the patterns documented in the illustrative application are not artifacts of specific analytic choices. However, the results are consistently imprecise, with confidence intervals that include economically meaningful effects as well as zero. Future researchers with larger samples, stronger identification strategies, or additional outcome variables may be better positioned to draw firm conclusions.
