  ...
  tryCatch({
    ...
    if (status_code(resp) == 200) {
      ...
      if (length(data) > 1) {
        ...
        message("  [WARN] Empty response for state=", state_fips, " year=", year, " Q", quarter)
      }
    } else {
      message("  [WARN] HTTP ", status_code(resp), " for state=", state_fips, " year=", year, " Q", quarter)
    }
    return(NULL)
  }, error = function(e) {
    message("  [ERROR] state=", state_fips, " year=", year, " Q", quarter, ": ", conditionMessage(e))
    return(NULL)
  })
}
