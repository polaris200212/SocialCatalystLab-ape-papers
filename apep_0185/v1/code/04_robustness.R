...
# ============================================================================
# 2. Randomization Inference (Permute MW Timing)
# ============================================================================
...
if (has_qwi && "industry_type" %in% names(panel)) {
  ...
  cat("  High-bite industries:", round(coef(high_bite_check)[1], 4),
      "(p =", round(pvalue(high_bite_check)[1], 4), ")\n")
  cat("  Low-bite industries:", round(coef(low_bite_check)[1], 4),
      "(p =", round(pvalue(low_bite_check)[1], 4), ")\n")

  # Difference test
  if (pvalue(low_bite_check)[1] > 0.1) {
...
# Already have exposure categories from cleaning
tercile_fit <- feols(
  log_emp ~ i(social_exposure_cat, ref = "Low") | county_fips + state_fips^yearq,
  data = panel,
  cluster = ~state_fips
)

cat("  Medium vs Low:", round(coef(tercile_fit)[1], 4), "\n")
cat("  High vs Low:", round(coef(tercile_fit)[2], 4), "\n")
...
cat("Inference:\n")
cat("  Randomization inference p-value:", round(ri_pval, 4), "\n")
