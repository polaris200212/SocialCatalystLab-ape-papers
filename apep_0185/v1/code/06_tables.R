...
tab2_data <- tibble(
  ...
  `(1) Naive` = c(
    paste0(sprintf("%.4f", tier1_coef), stars(pvalue(results$tier1_naive)[1])),
    sprintf("(%.4f)", tier1_se),
    "", "",
    "", "Yes", "Yes", "No",
  ...
  `(2) Shift-Share` = c(
    paste0(sprintf("%.4f", tier2_coef), stars(pvalue(results$tier2_shiftshare)[1])),
    sprintf("(%.4f)", tier2_se),
    "", "",
    "", "Yes", "No", "Yes",
    format(nrow(panel), big.mark = ","),
  ...
  `(3) Horse Race` = c(
    paste0(sprintf("%.4f", tier3_social), stars(pvalue(results$tier3_horserace)["social_exposure"])),
    sprintf("(%.4f)", tier3_social_se),
    paste0(sprintf("%.4f", tier3_geo), stars(pvalue(results$tier3_horserace)["geo_exposure"])),
  ...
  tab3_data <- tibble(
    ...
    `(1) High-Bite` = c(
      paste0(sprintf("%.4f", coef(results$high_bite)[1]),
             stars(pvalue(results$high_bite)[1])),
    ...
    `(2) Low-Bite (Placebo)` = c(
      paste0(sprintf("%.4f", coef(results$low_bite)[1]),
             stars(pvalue(results$low_bite)[1])),
...
robustness_rows[[3]] <- c("Orthogonalized Network (Residual)",
                          ...
                          sprintf("%.3f", pvalue(robustness$ortho_only)[1]))
...
if (!is.null(robustness$lag1)) {
  robustness_rows[[4]] <- c("1-Quarter Lagged Exposure",
                            ...
                            sprintf("%.3f", pvalue(robustness$lag1)[1]))
}
...
if (!is.null(robustness$pre_covid)) {
  robustness_rows[[5]] <- c("Pre-2020 Sample",
                            ...
                            sprintf("%.3f", pvalue(robustness$pre_covid)[1]))
}
