...
ols1_coef <- coef(results$ols_simple)[1]
ols1_se <- se(results$ols_simple)[1]
ols1_p <- fixest::pvalue(results$ols_simple)[1]

ols2_coef <- coef(results$ols_statetime)[1]
ols2_se <- se(results$ols_statetime)[1]
ols2_p <- fixest::pvalue(results$ols_statetime)[1]

iv_coef <- coef(results$iv_2sls)[1]
iv_se <- se(results$iv_2sls)[1]
iv_p <- fixest::pvalue(results$iv_2sls)[1]

fs_coef <- coef(results$first_stage)[1]
fs_se <- se(results$first_stage)[1]
first_stage_f <- results$first_stage_f
...
tab2_data <- tibble(
  ...
  `(1) OLS` = c(
    ...
    "", "Yes", "Yes", "No",
    "--",
    format(nobs(results$ols_simple), big.mark = ",")
  ),
  `(2) OLS` = c(
    ...
    "", "Yes", "No", "Yes",
    "--",
    format(nobs(results$ols_statetime), big.mark = ",")
  ),
  `(3) 2SLS` = c(
    ...
    format(nobs(results$iv_2sls), big.mark = ",")
  )
)
...
tab3a_data <- tibble(
  ...
  `First Stage` = c(
    ...
    "", "Yes", "Yes",
    sprintf("%.1f", first_stage_f),
    format(nobs(results$first_stage), big.mark = ",")
  )
)
...
if (!is.null(results$ols_earn) && !is.null(results$iv_earn)) {
  ols_earn_coef <- coef(results$ols_earn)[1]
  ols_earn_se <- se(results$ols_earn)[1]
  ols_earn_p <- fixest::pvalue(results$ols_earn)[1]

  iv_earn_coef <- coef(results$iv_earn)[1]
  iv_earn_se <- se(results$iv_earn)[1]
  iv_earn_p <- fixest::pvalue(results$iv_earn)[1]
  ...
  tab5_data <- tibble(
    ...
    `(1) OLS` = c(
      ...
      "", "--",
      format(nobs(results$ols_earn), big.mark = ",")
    ),
    `(2) 2SLS` = c(
      paste0(sprintf("%.4f", iv_earn_coef), stars(iv_earn_p)),
      sprintf("(%.4f)", iv_earn_se),
      "", sprintf("%.1f", first_stage_f),
      format(nobs(results$iv_earn), big.mark = ",")
    )
  )
...
if (!is.null(results$reduced_form)) {
  rf_coef <- coef(results$reduced_form)[1]
  rf_se <- se(results$reduced_form)[1]
  rf_p <- fixest::pvalue(results$reduced_form)[1]
  ...
  tab6_data <- tibble(
    ...
    `Reduced Form` = c(
      ...
      sprintf("(%.4f)", rf_se),
      "", "Yes", "Yes",
      format(nobs(results$reduced_form), big.mark = ",")
    )
  )
