...
# New variable names from revised data construction:
# - network_mw_full: log of SCI-weighted MW (leave-own-county-out) - ENDOGENOUS
# - network_mw_full_dollar: same in dollars
# - network_mw_out_state: log of SCI-weighted MW (leave-own-state-out) - IV
# - own_min_wage: own-state minimum wage in dollars
# - geo_exposure: geographic (distance-weighted) MW exposure

# Convert log exposures to dollars for interpretable units
panel <- panel %>%
  mutate(
    network_mw = network_mw_full_dollar,  # Already in dollars
...
n_before <- nrow(panel)
panel <- panel %>%
  filter(!is.na(network_mw_full) & network_mw_full_dollar >= 7.00)
...
fig1 <- ggplot(map_cont) +
  ...
  labs(
    ...
    subtitle = "SCI-weighted mean of network minimum wages, 2012-2022"
  ) +
...
# Create baseline tercile (using first available year)
first_year <- min(panel$year, na.rm = TRUE)
baseline_tercile <- panel %>%
  filter(year == first_year) %>%
  group_by(county_fips) %>%
  summarise(baseline_exposure = mean(network_mw, na.rm = TRUE), .groups = "drop") %>%
  filter(!is.na(baseline_exposure)) %>%
  mutate(
    # Use ntile for robust tercile creation
    tercile = ntile(baseline_exposure, 3),
    tercile = factor(tercile, levels = 1:3, labels = c("Low", "Medium", "High"))
  )
...
fig3 <- ggplot(ts_data, aes(x = yearq, y = mean_mw, color = tercile, fill = tercile)) +
  ...
  labs(
    ...
    subtitle = paste0("Counties grouped by ", first_year, " network exposure level; shaded bands = 95% CI"),
    x = "Year",
...
# Round to 2 decimal places for consistency with Table 2
corr_val <- round(cor(panel$network_mw, panel$geo_mw, use = "complete.obs"), 2)
