...
\title{Friends in High Places: \\ How Social Networks Transmit Minimum Wage Shocks\footnote{This paper is a revision of APEP-0197. See \url{https://github.com/SocialCatalystLab/ape-papers/tree/main/papers/apep_0197} for the parent paper.}}
...
\begin{abstract}
\noindent
Can exposure to higher wages through social networks shift local labor market equilibria? We construct a novel measure of network minimum wage exposure---the population-weighted average of minimum wages in socially connected counties, using Facebook's Social Connectedness Index---and examine its relationship with county-level employment and earnings. The key insight is that \textit{information volume} matters: weighting connections by the population of destination counties produces dramatically different results than probability weighting. Using our population-weighted measure and instrumenting with out-of-state network exposure, our IV estimates indicate significant positive associations with both employment and earnings. In a USD-denominated specification, a \$1 increase in the network average minimum wage is associated with approximately 9\% higher county employment and 3.5\% higher average earnings, providing a directly interpretable measure of network spillover magnitude. The first-stage $F$-statistic exceeds 500. In contrast, the probability-weighted specification shows no significant effects, confirming that information \textit{volume}---not network share---drives these results. Analysis of Quarterly Workforce Indicators job flow data reveals that network exposure increases both hiring and separations, consistent with increased labor market dynamism through information transmission. IRS county-to-county migration flows (2012--2019) show no evidence that effects operate through physical migration. Our results are consistent with minimum wage policies generating spillover effects through social networks that reshape local labor market equilibria in both quantities and prices, though we note that a pre-trend test rejects parallel trends ($p = 0.008$), so our causal interpretation rests on complementary identification evidence including Anderson-Rubin inference, permutation tests, and leave-one-out stability diagnostics.
\end{abstract}
...
\textbf{Pre-Existing Differential Trends.} The most serious concern is that high-exposure and low-exposure counties were on different employment trajectories before the major minimum wage increases. Our balance tests (\Cref{tab:balance}) indicate that pre-treatment employment \textit{levels} differ across IV quartiles ($p = 0.002$): high-exposure counties have systematically higher baseline employment. However, our identification relies on parallel \textit{trends} in changes, not equal levels. County fixed effects absorb these level differences entirely. To assess whether trends are parallel despite level differences, we examine event-study specifications (\Cref{fig:event_study}) that allow effects to vary by year relative to the ``Fight for \$15'' shock. While Figure 6 confirms the level differences (higher-IV quartiles have higher mean employment throughout), the event study's pre-period evidence raises concern: the 2012 quarterly coefficients are positive and a joint $F$-test rejects parallel trends ($F(4,50) = 3.90$, $p = 0.008$). We therefore rely on complementary evidence---placebo shock tests, distance-restricted instruments, and \citet{rambachanroth2023credible} sensitivity analysis---to support the identifying assumption, rather than on the pre-period evidence alone.
...
Because the event-study evidence alone does not support parallel trends, our identification rests on three complementary approaches: (1) the distance-restricted instruments (\Cref{tab:distance}) that improve balance by limiting to geographically distant connections less correlated with local confounders; (2) the \citet{rambachanroth2023credible} sensitivity analysis (Section \ref{sec:robustness}) that bounds the treatment effect under violations of parallel trends; and (3) the placebo shock tests that confirm effects are minimum-wage-specific rather than driven by correlated economic shocks. The convergence of these complementary tests supports a causal interpretation despite the imperfect pre-trend evidence.
...
We conduct several additional robustness checks. The event-study specification (\Cref{fig:event_study}) shows positive 2012 pre-period coefficients, and the joint $F$-test rejects parallel trends ($F(4,50) = 3.90$, $p = 0.008$). As discussed in Sections~\ref{sec:identification} and~\ref{sec:event_study}, we rely on complementary evidence---distance-restricted instruments, placebo shock tests, and \citet{rambachanroth2023credible} sensitivity analysis---rather than the pre-period evidence alone to support identification.
...
If network exposure operates through information about minimum wages, effects should be concentrated in industries where minimum wages bind---``high-bite'' sectors such as retail trade (NAICS 44-45) and accommodation and food services (NAICS 72)---rather than in ``low-bite'' sectors such as finance and insurance (NAICS 52) or professional services (NAICS 54), where prevailing wages far exceed minimum wage floors. Industry-level QWI data---available by NAICS sector---would permit estimating our main specification separately for high-bite and low-bite sectors, providing a powerful test of the information transmission mechanism. We note this as an important direction for future work: if the information interpretation is correct, effects should concentrate in sectors where minimum wage floors are binding, with null effects in high-wage professional sectors where minimum wages are irrelevant to prevailing compensation.
...
\textbf{Counterfactual example.} To make the magnitude concrete in USD terms: El Paso, TX has a network average minimum wage approximately \$1.50 higher than Amarillo, TX, reflecting El Paso's dense social connections to California's high-wage labor market. Our USD estimates predict that this \$1.50 difference in the information environment is associated with approximately 13.5\% higher equilibrium employment and 5.2\% higher average earnings in El Paso, holding constant all observable county characteristics and state-level shocks.
