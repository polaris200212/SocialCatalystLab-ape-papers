...
\title{Information Volume Matters: \\ Causal Evidence on Network Transmission of Minimum Wage Effects\footnote{This paper is a revision of APEP-0189. See \url{https://github.com/SocialCatalystLab/ape-papers/tree/main/papers/apep_0189} for the original.}}
...
The construction of our analysis sample proceeds in several stages that merit detailed description for replication purposes. We begin with the universe of 3,143 county-equivalent units in the United States (counties, independent cities, and county-equivalents). We exclude Alaska (30 county-equivalents), Hawaii (5 counties), and territories (Puerto Rico and others, 78 units), leaving 3,053 continental U.S. counties in the SCI data. After merging with the QWI employment data, some additional counties enter due to independent city coding differences, yielding 3,053 unique counties in our final analysis panel.
...
\end{equation}
where $w^{pop}_{cj} = \frac{SCI_{cj} \times \text{Emp}_j}{\sum_{k \neq c} SCI_{ck} \times \text{Emp}_k}$ and $\text{Emp}_j$ is \textit{time-invariant} average employment in county $j$ (averaged over the full 2012--2022 sample period). This ensures that the ``shares'' in our shift-share design are predetermined, as required by \citet{borusyak2022quasi}. Both the SCI (2018 vintage) and the employment weights are fixed throughout the sample period; only the minimum wage ``shocks'' vary over time.
...
\begin{tablenotes}
\small
\item \textit{Notes:} Panel of 134,317 county-quarter observations, 2012--2022. Population-weighted exposure uses SCI $\times$ employment as weights. Probability-weighted exposure uses SCI only.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
The 2SLS estimate of 0.83 implies that a 10\% increase in population-weighted network exposure is associated with approximately 8.3\% higher employment. This is a large effect that warrants careful interpretation.

First, 2SLS estimates capture local average treatment effects (LATEs) among compliers---counties whose full network exposure responds strongly to variation in out-of-state network exposure. These compliers are counties with unusually strong cross-state connections, where information transmission through networks may be particularly effective. The average treatment effect across all counties may be smaller.

Second, the effect size should be interpreted relative to the magnitude of network exposure variation. The standard deviation of population-weighted network exposure is approximately 0.12 log points (roughly 12\%). A one-standard-deviation increase in exposure is thus associated with about 10\% higher employment---large but not implausible for counties moving from the bottom to the top of the exposure distribution.

Third, our estimates may capture multiple channels operating simultaneously: direct information effects, migration option values, and employer responses to workers' outside options. Disentangling these channels requires additional data on migration flows, job search behavior, and wage responses that our county-level analysis cannot provide.
...
\begin{center}
\url{https://github.com/SocialCatalystLab/ape-papers/tree/main/papers/apep_0190}
\end{center}
...
\begin{figurenotes}
Coefficients on interactions between population-weighted network exposure and year indicators, with 2013 as the reference year. Vertical bars represent 95\% confidence intervals. Pre-period coefficients (2012--2013) are small and statistically indistinguishable from zero, supporting the parallel trends assumption. Effects emerge after 2014 when major ``Fight for \$15'' minimum wage increases were announced.
\end{figurenotes}
\end{figure}
