...
if (file.exists(file.path(DATA_DIR, "displacement_regs.rds"))) {
  ...
  modelsummary(reg_list,
    ...
    notes = list(
      ...
      "Comparison: janitors, porters, guards/doorkeepers.",
...
tab5_tex <- kable(tab5, format = "latex", booktabs = TRUE, escape = FALSE,
  ...
  kable_styling(latex_options = c("hold_position")) %>%
  footnote(general = "Logit model: P(still elevator operator in 1950 $|$ elevator operator in 1940). AME = average marginal effect.",
