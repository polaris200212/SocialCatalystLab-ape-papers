...
# ═══════════════════════════════════════════════════════════════════════════════
# Figure 6: Newspaper Discourse Timeline (if classified data available)
# ═══════════════════════════════════════════════════════════════════════════════

cat("Figure 6: Newspaper timeline...\n")

if (!is.null(newspaper_stats)) {
  news_long <- melt(newspaper_stats, id.vars = "year",
                    variable.name = "Category", value.name = "count")
  # Remove V1 if present
  news_long <- news_long[Category != "V1"]

  p6 <- ggplot(news_long, aes(x = year, y = count, fill = Category)) +
    geom_area(alpha = 0.8) +
    scale_fill_brewer(palette = "Set2") +
    scale_x_continuous(breaks = seq(1900, 1960, 10)) +
    labs(
      title = "The Newspaper Record: Elevator Stories Over Time",
      subtitle = "Classified newspaper articles mentioning elevators, by category",
      x = NULL, y = "Number of articles",
      caption = "Source: American Stories corpus (Chronicling America). Classification: Gemini 3 Flash."
    ) +
    theme_apep()

  ggsave(file.path(FIG_DIR, "fig6_newspaper_timeline.pdf"), p6,
         width = 9, height = 5.5)
} else {
  # Create simple count-based figure from raw matches
  if (file.exists(file.path(DATA_DIR, "newspaper_matches_raw.parquet"))) {
    news_raw <- arrow::read_parquet(file.path(DATA_DIR, "newspaper_matches_raw.parquet"))
    news_counts <- as.data.table(news_raw)[, .(n_articles = .N), by = year]
    setorder(news_counts, year)

    if (nrow(news_counts) > 1) {
      p6 <- ggplot(news_counts, aes(x = year, y = n_articles)) +
        geom_col(fill = apep_colors[1], alpha = 0.8) +
        geom_text(aes(label = format(n_articles, big.mark = ",")),
                  vjust = -0.3, size = 2.5) +
        scale_x_continuous(breaks = news_counts$year) +
        labs(
          title = "Elevator Stories in the Press",
          subtitle = "Newspaper articles mentioning building elevators or operators",
          x = NULL, y = "Number of articles",
          caption = "Source: American Stories corpus (Chronicling America).\nSampled years; grain elevator articles excluded."
        ) +
        theme_apep() +
