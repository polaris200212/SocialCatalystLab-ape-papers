...
**Who is actually treated?** Properties inside the ZFE boundary (positive signed distance) are treated. Treatment = being located in a zone where polluting vehicles are banned. The treatment changes the local regulatory environment, potentially affecting air quality, traffic composition, and accessibility.

**Primary estimand population:** Property transactions within the MSE-optimal bandwidth of the A86 boundary, during the strong enforcement period (June 2021+, Crit'Air 4+ ban).

**Placebo/control population:** (1) Properties outside the ZFE within the same bandwidth (spatial control); (2) Transactions during weak enforcement (2020, Crit'Air 5 only) serve as temporal placebo for the difference-in-discontinuities.

**Design:** Spatial RDD (primary), with difference-in-discontinuities (supplementary) to remove time-invariant boundary effects. This is NOT a staggered DiD — the identification comes from the spatial discontinuity at the boundary.
