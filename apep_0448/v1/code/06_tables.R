...
main_latex <- paste0(
  ...
  sprintf("ATT & %.4f & %.4f & %.4f & %.4f \\\\\n",
          cs_agg$providers$overall.att, cs_agg$claims$overall.att,
          cs_agg$paid$overall.att, cs_agg$benes$overall.att),
  sprintf(" & (%.4f) & (%.4f) & (%.4f) & (%.4f) \\\\\n",
  ...
  sprintf("Early Term $\\times$ Post & %.4f & %.4f & %.4f & %.4f \\\\\n",
          coef(twfe$providers), coef(twfe$claims),
          coef(twfe$paid), coef(twfe$benes)),
  sprintf(" & (%.4f) & (%.4f) & (%.4f) & (%.4f) \\\\\n",
          se(twfe$providers), se(twfe$claims),
          se(twfe$paid), se(twfe$benes)),
  "[6pt]\n",
  "\\multicolumn{5}{l}{\\textit{Panel C: Placebo (Behavioral Health)}} \\\\\n",
  sprintf("ATT (CS) & %.4f & & & \\\\\n", cs_agg$bh_placebo$overall.att),
  ...
  "Panel B reports standard TWFE estimates with state and month fixed effects. ",
  "Panel C shows the placebo test using behavioral health providers (H-codes), who earn ",
  "higher wages and should be less affected by UI benefits. ",
...
robust_latex <- paste0(
  ...
  sprintf("RI $p$-value & & & 51 & $p$ = %.3f (500 perms) \\\\\n",
          robust$ri_pvalue),
  "\\hline\n",
  ...
  "Randomization inference permutes treatment assignment across states 500 times.} \\\\\n",
  "\\end{tabular}\n",
  "\\end{table}\n"
)
