...
\title{Does Local Climate Policy Build Demand for National Action? Evidence from Swiss Energy Referendums\thanks{Revision of apep\_0071 (Rating: 22.1). Key improvements: (1) documented treatment assignment with verifiable external sources (LexFind.ch, cantonal law databases); (2) fixed code pipeline to ensure all intermediate files are generated correctly; (3) implemented Difference-in-Discontinuities design following \citet{grembi2016fiscal} to control for permanent border effects; (4) added wild cluster bootstrap inference at the border-pair level; (5) tempered causal language to reflect identification limitations.}}
...
Border-pair (correct for RDD) & $\sim$12 & 0.045 \\
Wild cluster bootstrap (Webb weights) & --- & 0.058 \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
\small
\item Notes: P-values for the same-language RDD specification ($-5.9$ pp) under different clustering assumptions. Wild cluster bootstrap uses 9,999 replications with Webb weights recommended for few clusters \citep{card2008regression}.
\end{tablenotes}
\end{threeparttable}
\label{tab:inference}
\end{table}
