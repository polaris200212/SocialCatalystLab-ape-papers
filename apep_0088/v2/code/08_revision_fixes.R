...
if (has_wcb) {
  ...
  if (!is.null(wcb_result)) {
    cat("\nWild Cluster Bootstrap Results (Canton-level):\n")
  ...
  # Border-pair level wild cluster bootstrap (for RDD inference)
  if (exists("rdd_sample") && !is.null(rdd_sample) && nrow(rdd_sample) > 0) {
    cat("\n--- Border-Pair Level Wild Cluster Bootstrap ---\n")
    cat("(Correct clustering unit for spatial RDD with few border segments)\n\n")

    # Create border pair identifier for RDD sample
    rdd_for_wcb <- rdd_sample %>%
      mutate(
        border_pair = case_when(
          canton_abbr %in% c("AG", "ZH", "SO") ~ "AG_borders",
          canton_abbr %in% c("BE", "FR", "LU", "JU") ~ "BE_borders",
          canton_abbr %in% c("BL", "SO") ~ "BL_borders",
          canton_abbr %in% c("GR", "SG", "GL", "TI") ~ "GR_borders",
          TRUE ~ "other"
        ),
        treat = as.numeric(distance_to_border >= 0)
      ) %>%
      filter(border_pair != "other", !is.na(distance_to_border))

    if (nrow(rdd_for_wcb) > 50) {
      # Simple linear model near the cutoff
      rdd_lm <- lm(yes_share ~ treat + distance_to_border, data = rdd_for_wcb)

      wcb_borderpair <- tryCatch({
        boottest(
          rdd_lm,
          param = "treat",
          clustid = rdd_for_wcb$border_pair,
          B = 9999,  # More replications for few clusters
          type = "webb"  # Webb weights for few clusters
        )
      }, error = function(e) {
        cat(paste("Border-pair WCB error:", e$message, "\n"))
        NULL
      })

      if (!is.null(wcb_borderpair)) {
        cat("Wild Cluster Bootstrap (Border-Pair Clusters):\n")
        cat(paste("  Estimate:", round(wcb_borderpair$point_estimate, 2), "pp\n"))
        cat(paste("  Bootstrap p-value:", round(wcb_borderpair$p_val, 3), "\n"))
        cat(paste("  95% CI:", round(wcb_borderpair$conf_int[1], 2), "to",
                  round(wcb_borderpair$conf_int[2], 2), "pp\n"))
        cat(paste("  Number of clusters:", n_distinct(rdd_for_wcb$border_pair), "\n"))

        # Save to results
        wcb_bp_summary <- tibble(
          method = "WCB Border-Pair",
          estimate = wcb_borderpair$point_estimate,
          pvalue = wcb_borderpair$p_val,
          ci_lower = wcb_borderpair$conf_int[1],
          ci_upper = wcb_borderpair$conf_int[2],
          n_clusters = n_distinct(rdd_for_wcb$border_pair)
        )
        write_csv(wcb_bp_summary, file.path(tab_dir, "wild_cluster_bootstrap_borderpair.csv"))
      }
    }
  }
} else {
...
# =============================================================================
# SECTION 2b: INFERENCE COMPARISON TABLE
# =============================================================================

cat("\n--- Inference Comparison Table ---\n")
cat("Comparing p-values across different clustering assumptions:\n\n")

inference_comparison <- tibble(
  clustering = c("Municipality (biased)", "Canton (conservative)", "Border-pair (correct)"),
  description = c(
    "Standard OLS SE",
    "Cluster at treatment level",
    "Cluster at border segment"
  ),
  n_clusters = c(
    nobs(ols_base),
    n_distinct(gemeinde_data$canton),
    if(exists("rdd_for_wcb")) n_distinct(rdd_for_wcb$border_pair) else NA
  )
)

# Add estimates if available
if (exists("wcb_result") && !is.null(wcb_result) && exists("wcb_borderpair") && !is.null(wcb_borderpair)) {
  inference_comparison <- inference_comparison %>%
    mutate(
      pvalue = c(
        summary(ols_base)$coeftable["treated", "Pr(>|t|)"],
        2 * pnorm(abs(coef(ols_base)["treated"] / sqrt(vcov(ols_base)["treated", "treated"])), lower.tail = FALSE),
        wcb_borderpair$p_val
      )
    )
}

write_csv(inference_comparison, file.path(tab_dir, "inference_comparison.csv"))
