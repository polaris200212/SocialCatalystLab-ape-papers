...
if (nrow(placebo_energy) > 0) {
  # For each unique vote, sorted by date descending (most recent first)
  unique_votes <- unique(placebo_energy[, c("id", "name", "votedate")]) %>%
    arrange(desc(votedate))

  for (i in 1:nrow(unique_votes)) {  # Use all available pre-treatment placebos
    ...
    # Get this vote's data (raw swissdd data has canton_id, not canton_abbr)
    vote_data <- placebo_energy %>%
      ...
      left_join(canton_treatment, by = "canton_id")

    # Merge with distance data (no hard bandwidth cap; rdrobust selects
    # MSE-optimal bandwidth, consistent with main analysis)
...
fe_model <- feols(
  yes_share ~ signed_distance_km * treated | border_region,
  data = near_border_df,
  vcov = "hetero"
)
