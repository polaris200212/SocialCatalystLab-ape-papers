...
# APEP theme (larger fonts for readability)
theme_apep <- function() {
...
fig_rdd_corrected <- ggplot(binned_data, aes(x = mid, y = mean_yes, color = treated)) +
  ...
  annotate("text", x = -14, y = max(binned_data$mean_yes, na.rm = TRUE) - 2,
  ...
  annotate("label", x = 12, y = min(binned_data$mean_yes, na.rm = TRUE) + 2,
           label = paste0("RD = ", pooled_est, " pp\np = ", pooled_p),
...
fig_rdd_same_lang <- ggplot(binned_same_lang, aes(x = mid, y = mean_yes, color = treated)) +
  ...
  annotate("text", x = -12, y = max(binned_same_lang$mean_yes, na.rm = TRUE) - 1.5,
           label = "Control side", color = "#B2182B", size = 4, fontface = "bold") +
  annotate("text", x = 12, y = max(binned_same_lang$mean_yes, na.rm = TRUE) - 1.5,
           label = "Treated side", color = "#2166AC", size = 4, fontface = "bold") +
  # Prominent estimate annotation on the plot
  annotate("label", x = 12, y = min(binned_same_lang$mean_yes, na.rm = TRUE) + 2,
           label = paste0("RD = ", same_lang_est, " pp\np = ", same_lang_p),
           size = 4.5, fontface = "bold", color = "grey20",
           fill = "white", alpha = 0.85, label.size = 0.5) +
  theme_apep() +
  theme(
