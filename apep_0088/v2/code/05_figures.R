...
# APEP theme (override with function version for figures, larger fonts)
theme_apep <- function() {
...
if (!st_is_empty(border)) {
  ...
  fig3 <- ggplot(binned_data, aes(x = mid, y = mean_yes, color = treated)) +
    ...
    annotate("text", x = -12, y = max(binned_data$mean_yes) - 2,
             label = "Control side", color = "#B2182B", size = 4, fontface = "bold") +
    annotate("text", x = 12, y = max(binned_data$mean_yes) - 2,
             label = "Treated side", color = "#2166AC", size = 4, fontface = "bold") +
    # Prominent estimate annotation on the plot
    annotate("label", x = 12, y = min(binned_data$mean_yes) + 2,
             label = paste0("RD = ", round(rd_result$coef[1], 2), " pp\np = ", round(rd_result$pv[1], 3)),
...
if (file.exists(border_file)) {
  ...
  if ("border_pair" %in% names(border_data)) {
    ...
    for (bp in border_pairs) {
      bp_data <- border_data %>%
        filter(border_pair == bp, !is.na(signed_distance_km), !is.na(yes_share))

      if (nrow(bp_data) >= 20) {  # Only if sufficient observations
        # Create bins
        bp_data <- bp_data %>%
          mutate(bin = cut(signed_distance_km, breaks = seq(-10, 10, by = 2), include.lowest = TRUE)) %>%
          group_by(bin) %>%
          summarise(
            running_mid = mean(signed_distance_km, na.rm = TRUE),
