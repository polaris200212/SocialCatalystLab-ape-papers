...
# Selection rule: fetch date range, filter by name pattern, verify exactly one
# unique vote ID matches. If multiple match, select by explicit vote_id override.
fetch_referendum <- function(from_date, to_date, name_pattern, vote_type,
                             expected_vote_id = NULL) {
  tryCatch({
    ...
    # Filter by explicit vote ID if provided (deterministic selection)
    if (!is.null(expected_vote_id)) {
      votes <- votes %>% filter(id == expected_vote_id)
      if (nrow(votes) == 0) {
        message(paste("Vote ID", expected_vote_id, "not found for", vote_type))
        return(NULL)
      }
      cat(paste("  Selected vote ID", expected_vote_id, "for", vote_type, "\n"))
    } else if (!is.null(name_pattern)) {
      # Fallback: filter by pattern, but verify unique match
      votes <- votes %>%
        filter(str_detect(tolower(name), name_pattern))
      if (nrow(votes) == 0) return(NULL)

      unique_ids <- unique(votes$id)
      if (length(unique_ids) > 1) {
        # Log available matches and select chronologically first
        cat(paste("  WARNING:", length(unique_ids), "votes match pattern for",
                  vote_type, "- selecting earliest by date\n"))
        earliest_date <- votes %>%
          group_by(id) %>%
          summarise(votedate = first(votedate), name = first(name), .groups = "drop") %>%
          arrange(votedate)
        cat(paste("    Available:", paste(earliest_date$name, collapse = "; "), "\n"))
        votes <- votes %>% filter(id == earliest_date$id[1])
      }
      cat(paste("  Selected vote ID", unique(votes$id), "for", vote_type, "\n"))
    }
...
# Each referendum is identified by date range and a specific name pattern.
# The date ranges are narrow (single month) and patterns are specific enough
# to yield exactly one matching vote per period.
#
# Swiss referendum details (from Federal Chancellery):
#   - 2003-05-18: Moratorium Plus / Strom ohne Atom (nuclear moratorium)
#   - 2016-09-25: Grüne Wirtschaft (green economy initiative)
#   - 2016-11-27: Atomausstiegsinitiative (nuclear phase-out initiative)
#   - 2017-05-21: Energiegesetz (Energy Strategy 2050)
referendums <- list(
  # Pre-treatment (before 2010): nuclear moratorium vote
  list(from = "2003-05-01", to = "2003-05-31", pattern = "moratorium",
       type = "nuclear_2003", period = "pre"),

  # Post-treatment but before main outcome (2010-2016)
  list(from = "2016-09-01", to = "2016-09-30", pattern = "grüne wirtschaft|green economy",
       type = "green_2016", period = "post"),
  list(from = "2016-11-01", to = "2016-11-30", pattern = "ausstieg.*atom|atom.*ausstieg",
       type = "nuclear_2016", period = "post"),

  # Main outcome
  list(from = "2017-05-01", to = "2017-05-31", pattern = "energiegesetz|energiestrategie",
