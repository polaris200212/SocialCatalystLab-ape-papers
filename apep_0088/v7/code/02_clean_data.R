...
# -----------------------------------------------------------------------------
# 8. Create Border-Pair Data for RDD Analysis
# -----------------------------------------------------------------------------

message("\nCreating border-pair data for RDD analysis...")

# Define border pairs based on canton adjacency
# Treated cantons: GR (18), BE (2), AG (19), BL (13), BS (12)
border_pair_df <- analysis_df %>%
  mutate(
    # Assign each municipality to its nearest border pair
    border_pair = case_when(
      ...
      canton_abbr == "SO" & treated == FALSE ~ "AG-ZH/SO",
    ...
    # Indicator for same-language borders (German-German)
    same_language = border_pair %in% c("AG-ZH/SO", "BL-SO", "GR-SG/GL") |
                    (border_pair == "BE-multiple" & lang == "German"),
    # Running variable for RDD
    distance_to_border = signed_distance_km
  ) %>%
  filter(border_pair != "Other", border_pair != "BS-urban")

message(paste("Border-pair sample size:", nrow(border_pair_df)))
message("Border pair distribution:")
print(table(border_pair_df$border_pair))

# Save border-pair data
saveRDS(border_pair_df, file.path(data_dir, "border_pair_data.rds"))
message("Saved border_pair_data.rds")
