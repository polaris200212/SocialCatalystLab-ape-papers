...
message("\n=== LOADING VERIFIED TREATMENT ===")

# Load verified treatment from 00b_verify_treatment.R
# This ensures treatment assignment is documented with external sources
if (file.exists(file.path(data_dir, "canton_treatment_verified.rds"))) {
  canton_treatment <- readRDS(file.path(data_dir, "canton_treatment_verified.rds"))
  message("Loaded verified treatment from canton_treatment_verified.rds")
} else {
  message("Verified treatment not found. Running 00b_verify_treatment.R...")
  source(file.path(script_dir, "00b_verify_treatment.R"))
  canton_treatment <- readRDS(file.path(data_dir, "canton_treatment_verified.rds"))
}
...
# -----------------------------------------------------------------------------
# 2b. Fetch Pre-Treatment Placebo Votes for Robustness
# -----------------------------------------------------------------------------

message("\n=== FETCHING PLACEBO VOTES FOR ROBUSTNESS ===")

# Fetch pre-treatment referendums (before 2010, the first treatment year)
# These are used for placebo tests to validate the RDD design
placebo_votes_raw <- tryCatch({
  message("Fetching 2000-2009 referendums for placebo tests...")
  ...
  message(paste("   Fetched", nrow(placebo_all), "pre-treatment observations"))
  placebo_all
}, error = function(e) {
