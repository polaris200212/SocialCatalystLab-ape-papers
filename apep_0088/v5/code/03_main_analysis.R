...
# =============================================================================
# Save RD Results for Downstream Scripts
# =============================================================================

message("\n=== SAVING RD RESULTS ===")

# Create rd_results object for use by 04_robustness.R and other scripts
rd_results <- list(
...
# Add spatial RDD results if available
if (exists("rd_result") && !is.null(rd_result)) {
  rd_results$main <- rd_result
  message("Added main RDD result to rd_results")
}

# Add density test if available
if (exists("density_test") && !is.null(density_test)) {
  rd_results$density_test <- density_test
  message("Added density test to rd_results")
}

# Add donut RDD if computed
if (exists("rd_donut") && !is.null(rd_donut)) {
  rd_results$donut <- rd_donut
  message("Added donut RDD to rd_results")
} else {
  # Compute donut RDD (exclude municipalities within 2km of border)
  if (has_spatial && exists("border_sample") && nrow(border_sample) > 0) {
    donut_sample <- border_sample %>%
      st_drop_geometry() %>%
      filter(!is.na(yes_share) & !is.na(running_var)) %>%
      filter(abs(running_var) > 2000)  # Exclude within 2km

    if (nrow(donut_sample) >= 50) {
      rd_donut <- tryCatch({
        rdrobust(
          y = donut_sample$yes_share,
          x = donut_sample$running_var,
          c = 0
        )
      ...
      if (!is.null(rd_donut)) {
        rd_results$donut <- rd_donut
        message("Computed and added donut RDD to rd_results")
      }
    }
  }
}

saveRDS(rd_results, file.path(data_dir, "rd_results.rds"))
message("Saved rd_results.rds")
