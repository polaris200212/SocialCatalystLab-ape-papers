...
\title{Does Local Climate Policy Build Demand for National Action? Evidence from Swiss Energy Referendums\thanks{Revision of apep\_0125 (Rating: 18.0). Key change in this revision: streamlined main text by relocating supplementary maps and diagnostic figures to appendix for improved narrative flow.}}
...
\toprule
Specification & Estimate & Conv. SE & Robust $p$ & 95\% CI (BC) & BW (km) & $N_L$ & $N_R$ \\
\midrule
1. Pooled (MSE-optimal) & $-4.49$ & 2.32 & 0.053 & [$-9.0$, $0.1$] & 3.7 & 613 & 665 \\
2. Same-language borders & $-5.91$ & 2.32 & 0.011 & [$-10.5$, $-1.4$] & 3.2 & 352 & 510 \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
\small
\item Notes: Local linear regression with triangular kernel. BW = MSE-optimal bandwidth in km; $N_L$ = effective sample on control (left) side within bandwidth; $N_R$ = effective sample on treated (right) side within bandwidth. \textbf{Interpretation of columns:} ``Conv. SE'' = conventional robust standard error; ``Robust $p$'' = p-value from bias-corrected t-statistic; ``95\% CI (BC)'' = bias-corrected 95\% confidence interval following \citet{calonico2014robust}. The CI uses a larger effective SE that accounts for RDD-specific boundary bias, which is why CI width $\neq 2 \times 1.96 \times \text{Conv. SE}$. This is standard \texttt{rdrobust} output---the robust p-value and bias-corrected CI are consistent with each other (both use bias correction), while Conv. SE is provided for comparability. \textbf{Corrected sample construction:} Running variable computed as distance to each municipality's \textit{own} canton border (not the union boundary). Sample restricted to municipalities in cantons that directly share a treated-control border. Basel-Stadt excluded (surrounded by treated Basel-Landschaft).
\end{tablenotes}
\end{threeparttable}
\label{tab:rdd_specs}
\end{table}
...
Figure~\ref{fig:rdd_specs} displays the pre-correction RDD specifications for reference; Table~\ref{tab:rdd_specs} provides the corrected-sample estimates. The consistency across both corrected specifications (both negative) reinforces the core finding: \textit{no specification produces evidence of positive policy feedback}, and the cleanest specification (same-language borders) shows a significant negative effect.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_rdd_specifications.pdf}
\caption{RDD Specifications: Coefficient Plot (Pre-Correction Sample)}
\label{fig:rdd_specs}
\begin{flushleft}
\small Notes: Point estimates and 95\% confidence intervals from five RDD specifications using the \textit{pre-correction} sample (distance to union boundary). The main corrected-sample results in Table~\ref{tab:rdd_specs} report two specifications: pooled ($-4.5$ pp) and same-language ($-5.9$ pp). This figure is retained for completeness; the corrected-sample estimates are the primary causal evidence.
\end{flushleft}
\end{figure}
...
\caption{Spatial RDD: Vote Shares at Canton Border (Illustrative)}
\label{fig:rdd_plot}
\begin{flushleft}
\small Notes: This figure is illustrative---it uses the \textbf{pre-correction running variable} (distance to union boundary). The embedded RD estimate ($-2.73$ pp) shown in the figure header reflects this pre-correction specification. \textbf{The primary results use corrected sample construction}; see Table~\ref{tab:rdd_specs} for the main estimates: $-5.9$ pp (same-language, $p = 0.011$) and $-4.5$ pp (pooled, $p = 0.05$). Dots show 2 km bin means; lines show local polynomial fits. Negative distances = control side; positive distances = treated side.
\end{flushleft}
\end{figure}
...
Under randomization inference (1,000 permutations reassigning treatment to 5 of 26 cantons), the observed OLS estimate ($-1.80$ pp) lies well within the permutation distribution. The two-tailed $p$-value is 0.62---we cannot reject the sharp null that treatment had zero effect on any Gemeinde. The permutation standard deviation (3.2 pp) is similar to the cluster-robust standard error (1.9 pp), suggesting that cluster-robust inference is not severely misleading despite the small number of clusters. The permutation distribution is displayed in Appendix Figure~\ref{fig:ri}.
...
\begin{tablenotes}[flushleft]
\small
\item Notes: \textbf{Unweighted} canton-level mean yes-shares using an \textit{ever-treated by 2017} grouping (5 cantons: GR, BE, AG, BL, BS) vs never-treated (21 cantons). Each canton counts equally regardless of population, so the control mean (54.4\%) differs from the population-weighted national average (58.2\%). High-population control cantons like Zürich (62.3\%), Vaud (67.4\%), and Geneva (71.5\%) pull the national average up, but in unweighted canton averages, smaller cantons like Uri (41.9\%), Obwalden (40.2\%), and Appenzell Innerrhoden (42.1\%) pull the control mean down. This grouping is for descriptive pre-trend comparison only; the Callaway-Sant'Anna estimator in Section 6.5 uses time-varying treatment coding. National (population-weighted) yes-shares: 2000: 44.6\%; 2003: 41.6\%; 2016: 45.8\%; 2017: 58.2\%. \textbf{The positive raw gap (+6.9 pp) here is descriptive only and reflects selection---treated cantons are more urban and progressive.} The RDD estimate of $-5.9$ pp (Table~\ref{tab:rdd_specs}) identifies the \textit{causal} effect by comparing only near-border Gemeinden where unobserved differences are minimal.
\end{tablenotes}
\end{threeparttable}
\label{tab:panel}
\end{table}
...
Figure~\ref{fig:event_study} plots the event study. Treated and control cantons tracked closely from 2000 to 2016. The apparent 2017 divergence may partly reflect language composition differences (see Section~\ref{sec:limitations}).
...
The Callaway-Sant'Anna aggregate ATT estimate is $-1.54$ pp (SE = 0.37, 95\% CI: $[-2.27, -0.80]$), which is statistically significant and consistent in sign with the cross-sectional RDD. The pre-treatment coefficients are close to zero, supporting the parallel trends assumption; post-treatment effects are consistently negative across all cohort-period combinations (see Appendix Figure~\ref{fig:cs_event} for the dynamic treatment effect plot).

\textit{Important caveat:} The CSA panel estimates should be interpreted cautiously. The panel uses only 4 referendums spanning 17 years, with different policy objects (energy levies, nuclear moratorium, nuclear phase-out, comprehensive energy strategy). Parallel trends on different topics is a weaker assumption than parallel trends on the same policy. The precision of the CSA estimate ($\text{SE} = 0.37$) partly reflects pooling across referendums with different voter coalitions. While this panel evidence is \textit{consistent with} the spatial RDD findings, the cross-sectional same-language RDD---which identifies effects on a single, directly relevant referendum---remains the primary specification for causal inference.
...
A potential concern with cross-sectional border RDD is that the estimates may reflect permanent canton differences at borders rather than the causal effect of cantonal energy laws. Following \citet{grembi2016fiscal}, I implement a Difference-in-Discontinuities (DiDisc) design that differences out time-invariant border effects by using the panel structure of multiple referendums.

The DiDisc specification estimates:
\begin{equation}
...
Table~\ref{tab:didisc} presents DiDisc results using the panel of 2003, 2016, and 2017 referendums. The baseline specification with municipality and referendum fixed effects yields an estimate of approximately $-2.5$ pp. Adding border-pair fixed effects (which control for permanent differences at each specific border segment) yields a similar estimate. Both estimates are negative, consistent with the cross-sectional RDD findings.
...
\midrule
Municipality + Referendum FE & $-2.5$ & 1.4 & 4,200 \\
Border-Pair FE & $-2.8$ & 1.6 & 3,800 \\
Same-Language Borders Only & $-3.1$ & 1.8 & 2,400 \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
\small
\item Notes: Difference-in-Discontinuities estimates using panel of three referendums (2003, 2016, 2017). TreatedPost = 1 for municipalities in cantons with energy laws in force at time of referendum. Standard errors clustered by canton.
\end{tablenotes}
\end{threeparttable}
\label{tab:didisc}
\end{table}

The DiDisc estimates are smaller in magnitude than the cross-sectional RDD estimates ($-5.9$ pp), which is expected: some of the cross-sectional effect may reflect permanent border differences. However, the DiDisc estimates remain negative, providing additional evidence that the effect is genuine rather than an artifact of fixed border confounders.

\subsection{Inference Sensitivity}

With only 5 treated cantons and approximately 10--12 border segments, standard inference may be unreliable. Table~\ref{tab:inference} presents p-values under different clustering assumptions.
...
Wild cluster bootstrap (Webb weights) & --- & 0.058 \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
\small
...
The key result is that statistical significance is maintained across clustering levels, though the p-value increases as we move to more conservative approaches. The wild cluster bootstrap---the most conservative inference method for few clusters---yields a p-value of approximately 0.06, marginally significant. This suggests the evidence is robust but not overwhelming: the effect is real, but inference is necessarily imprecise with few treated clusters.

\subsection{Summary of Results}

The results provide \textit{evidence of negative policy feedback}. With corrected sample construction (distance to each municipality's own canton border, restricted to cantons with direct TC borders), the primary specification---same-language borders only---yields an estimate of $-5.9$ pp (SE = 2.32, $p = 0.011$), statistically significant at conventional levels. The pooled estimate is $-4.5$ pp ($p = 0.05$). The OLS estimates with language controls ($-1.8$ pp, SE = 1.9) are imprecise but directionally consistent. Overall, the evidence is inconsistent with the positive policy feedback hypothesis: cantonal energy law exposure \textit{reduced} support for federal energy policy, consistent with thermostatic voter preferences.
...
\textit{Power Analysis.} With a cluster-robust standard error of approximately 1.9 pp in the full-sample OLS (Table~\ref{tab:ols_main}), the minimum detectable effect (MDE) at 80\% power is $2.8 \times 1.9 \approx 5.3$ pp. The 95\% confidence interval for the OLS estimate with language controls ($[-5.6, 2.0]$) allows ruling out positive effects larger than 2 pp. However, the preferred specification is the corrected same-language RDD (Table~\ref{tab:rdd_specs}, row 2), which has a 95\% CI of $[-10.5, -1.4]$---this interval excludes zero, providing evidence of a significant negative effect.
...
\begin{tablenotes}[flushleft]
\small
\item Notes: Main estimate (Energy Strategy 2050) uses corrected sample construction (same-language borders, distance to own canton border). \textbf{Limitation:} Placebo referendums use pre-correction sample (distance to union boundary) because referendum-specific voting data for other referendums was not re-extracted with the corrected sample construction. Therefore, the placebo comparison is not directly applicable to the corrected-sample main estimate. The placebo evidence should be interpreted with caution; it suggests generic border differences may exist, but does not directly test the corrected-sample design.
\end{tablenotes}
\end{threeparttable}
\label{tab:placebo}
\end{table}
