...
\title{Testing the Substitution Hypothesis: Cannabis Dispensary Access and Alcohol Involvement in Fatal Crashes}
...
\begin{abstract}
\noindent
Does access to legal cannabis reduce alcohol involvement among fatal traffic crashes through substance substitution? I test this hypothesis using a spatial regression discontinuity design at the borders between legal and prohibition states in the western United States. Using 29,350 geocoded fatal crashes from the Fatality Analysis Reporting System (2016--2019) and 1,399 dispensary locations from OpenStreetMap, I estimate the effect of crossing from prohibition into legal territory on the share of fatal crashes involving alcohol-impaired drivers. The main specification yields a point estimate of 9.2 percentage points (SE = 5.9 pp), but the 95\% confidence interval includes zero (p = 0.127). Complementary analysis using distance to nearest dispensary within prohibition states also produces null results (coefficient: -0.006, SE = 0.014). These well-identified null findings challenge the popular narrative that cannabis legalization reduces alcohol-related harms through substitution. The evidence suggests that if substitution occurs, it does not affect the composition of fatal crashes at a detectable magnitude.
\end{abstract}

\vspace{1em}
\noindent\textbf{JEL Codes:} I12, I18, K32, R41 \\
\noindent\textbf{Keywords:} marijuana legalization, alcohol substitution, fatal crash composition, spatial RDD, null result
...
The main specification yields a point estimate of 9.2 percentage points: crashes on the legal side of the border actually show \textit{higher} alcohol involvement than those on the prohibition side. However, this estimate is not statistically significant at conventional levels (SE = 5.9 pp, p = 0.127). The 95\% confidence interval spans from -2.3 to +20.8 percentage points, encompassing both substitution and complementarity effects. The estimate is robust to bandwidth choice and polynomial order. Importantly, the direction of the point estimate---higher alcohol involvement in legal states---is inconsistent with the substitution hypothesis, even though statistical precision precludes strong conclusions.
...
\begin{enumerate}
    ...
    \item Effects should be larger for crashes involving younger drivers (who are more price-sensitive and more likely to substitute)
\end{enumerate}

The empirical analysis tests each of these predictions.
...
\bottomrule
\multicolumn{5}{l}{\textit{RDD Sample (Within 150km only):}} \\
Mean Dist. to Border (km) & \multicolumn{3}{c}{(computed for border sample only)} & 24.1 \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
\small
\item Notes: Data from FARS 2016--2019 in 13 western states. ``Ever-Legal States'' = CA, CO, NV, OR, WA (states with recreational retail by end of 2019). ``Never-Legal States'' = AZ, ID, KS, MT, NE, NM, UT, WY (no recreational retail through 2019). Note: For the RDD analysis, treatment is defined at the crash-date level (see Section 3.3); CA crashes before January 2018 and NV crashes before July 2017 are assigned to the control side. ``Within 150km'' restricts to crashes within 150km of a legal-control border at the time of crash. Alcohol involvement coded as 1 if any driver had BAC $\geq$ 0.08.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
The time-mismatch limitation is critical for the distance-to-dispensary analysis. Using 2020 dispensary locations to measure access costs for 2016--2019 crashes introduces systematic measurement error. For the distance analysis, I construct a \textit{time-varying} dispensary set: for each crash, the candidate dispensaries are only those in states with legal recreational retail as of the crash date (CO/OR/WA throughout; NV added from July 2017; CA added from January 2018). This ensures the distance measure reflects legal access at the relevant time.

Dispensary-level opening dates are not available in OSM, so the distance measure assumes that the dispensary network within each state is stable once that state's recreational market opens. This assumption is imperfect---dispensary counts in California increased substantially during 2018--2019---and introduces measurement error. However, this error should attenuate coefficients toward zero, making the null finding a conservative conclusion.

Given these limitations, the distance-to-dispensary analysis should be interpreted as a speculative robustness check rather than primary evidence. The main results come from the spatial RDD, where treatment is defined by legal status at the crash date and location, not by dispensary distances.
...
I estimate local polynomial regressions using the \texttt{rdrobust} package in R, which implements the MSE-optimal bandwidth selection procedure and bias-corrected inference developed by \citet{calonico2014}. The spatial RDD design follows the geographic boundaries framework of \citet{keele2015}, who formalize the conditions under which administrative borders identify causal effects. The seminal application of spatial RDD in economics is \citet{dell2010}, who exploits historical administrative boundaries in Peru. Standard RDD practice follows the guidelines in \citet{imbens2008}. The main specification uses:
...
However, statistical precision precludes strong conclusions. The confidence interval includes both modest substitution effects (up to -2.3 pp) and substantial complementarity effects (up to +20.8 pp). The data are consistent with no effect, modest substitution, or modest complementarity.
...
\begin{tablenotes}[flushleft]
\small
\item Notes: Standard errors clustered at state level in parentheses. Sample restricted to crashes in seven never-legal states that directly border a recreational-legal state (AZ, ID, KS, NE, NM, UT, WY); Montana is excluded because it does not share a border with any recreational-legal state. Distance is computed to the nearest dispensary in any state with legal recreational retail as of the crash date. Dependent variable is indicator for alcohol involvement (BAC $\geq$ 0.08). Weekend night defined as Friday or Saturday, 9pm--5am. \textit{Caution:} With only seven states, cluster-robust standard errors may be biased; interpretation should focus on point estimate magnitudes rather than precise inference.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\begin{tablenotes}[flushleft]
\small
\item Notes: Sample restricted to seven never-legal states that directly border a recreational-legal state (AZ, ID, KS, NE, NM, UT, WY). Montana is excluded because it does not share a border with any recreational-legal state. Mean distance computed to nearest dispensary in any legal state as of the crash date. States ordered by mean distance.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\begin{tablenotes}[flushleft]
\small
\item Notes: N refers to the \textit{effective sample within the RDD bandwidth} (35.6 km), not the total 150km sample. The effective N (1,446 total) is much smaller than the 150km sample (5,442) because the bandwidth focuses on crashes closest to the border. The higher rural share in the effective sample (62.5\%) vs the 150km sample (55.2\%) reflects that crashes closest to the border are disproportionately in rural areas. Note: Table 4 reports a different N (6,603) because it uses a separate sample (never-legal states that border a legal state, all distances) rather than the border RDD sample. None of the estimates are statistically significant at the 10\% level.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
The evidence does not support the substitution hypothesis at the fatal crash composition margin. Both the spatial RDD and the distance-to-dispensary specifications produce null results. The point estimates are small, imprecise, and sometimes in the ``wrong'' direction (higher alcohol involvement with greater cannabis access).
...
\textbf{Insufficient cost variation:} The ``cost of cannabis'' may not vary enough across the state border to generate detectable substitution. Even in prohibition states near the border, illegal cannabis may be readily available at prices not much higher than in legal states. The legal risk premium ($\lambda$ in the model) may be small in practice, especially for personal-use quantities.
...
This paper tests whether legal cannabis access reduces alcohol involvement among fatal crashes using a spatial regression discontinuity design at state borders in the western United States. The design exploits the sharp discontinuity in cannabis legal status at state lines, comparing fatal crashes occurring just inside legal states to those just inside prohibition states. If cannabis and alcohol are substitutes, legal access should reduce alcohol involvement in crashes.

The evidence does not support the substitution hypothesis at the fatal crash composition margin. The main RDD specification yields a point estimate of 9.2 percentage points (SE = 5.9 pp), suggesting that crashes in legal states actually have \textit{higher} alcohol involvement---the opposite of the substitution prediction. However, this estimate is not statistically significant (p = 0.127), and the 95\% confidence interval spans -2.3 to +20.8 percentage points, encompassing zero.

The null result is robust to specification choices. Alternative bandwidths, polynomial orders, and kernel functions all produce estimates with confidence intervals including zero. Complementary analysis using distance to the nearest dispensary within prohibition states also produces null results (coefficient: -0.006, SE = 0.014). Heterogeneity analysis finds no evidence of substitution in nighttime crashes, weekend crashes, or other subgroups where effects might be most pronounced.
...
First, the result establishes that the substitution mechanism, even if it operates at the population level, does not produce large effects on the composition of fatal crashes at state borders. The 95\% confidence interval rules out substitution effects larger than 2.3 percentage points---modest effects are possible, but large effects are unlikely. This should temper expectations that legalization will meaningfully reduce alcohol involvement in fatal crashes. Given the effective sample size within the optimal bandwidth (approximately 1,446 observations) and the outcome variance (alcohol involvement rate around 29\%), the minimum detectable effect (MDE) at 80\% power and $\alpha = 0.05$ is approximately 4--5 percentage points. The study is therefore well-powered to detect policy-relevant effects (a 15--20\% reduction in alcohol involvement), though it cannot detect very small effects.
...
\textbf{Measurement error in crash location:} The analysis uses crash locations rather than driver residence or drinking location. A prohibition state resident who drives to a legal state, drinks there, and crashes on the way home (on either side of the border) represents a misattribution of treatment exposure. The direction of bias from this measurement error is ambiguous: if cross-border visitors are more (less) likely to be alcohol-involved, the estimate could be biased in either direction. Future work could address this by conditioning on driver license state (available in FARS) to separate in-state from out-of-state drivers, though this would substantially reduce sample size.
