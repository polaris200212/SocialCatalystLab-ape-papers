...
###############################################################################
# Figure 10: DC Presence at the OZ Eligibility Threshold (v3 addition)
###############################################################################
cat("Figure 10: DC Presence RDD\n")

if ("dc_any" %in% names(rdd_sample) && sum(rdd_sample$dc_any, na.rm = TRUE) > 0) {
  binned_dc <- rdd_sample %>%
    filter(poverty_rate >= 5, poverty_rate <= 40) %>%
    mutate(bin = round(poverty_rate)) %>%
    group_by(bin) %>%
    summarize(
      dc_rate = mean(dc_any, na.rm = TRUE),
      n = n(),
      se = sqrt(dc_rate * (1 - dc_rate) / pmax(n, 1)),
      .groups = "drop"
    )

  p10 <- ggplot(binned_dc, aes(x = bin, y = dc_rate)) +
    geom_point(aes(size = n), alpha = 0.7, color = "darkorange") +
    geom_errorbar(aes(ymin = pmax(dc_rate - 1.96 * se, 0),
                      ymax = dc_rate + 1.96 * se),
                  width = 0.3, alpha = 0.5) +
    geom_vline(xintercept = 20, linetype = "dashed", color = "red", linewidth = 0.8) +
    geom_smooth(data = filter(binned_dc, bin < 20), method = "lm",
                se = TRUE, color = "grey50", fill = "grey80") +
    geom_smooth(data = filter(binned_dc, bin >= 20), method = "lm",
                se = TRUE, color = "darkorange", fill = "moccasin") +
    scale_y_continuous(labels = percent_format()) +
    scale_size_continuous(range = c(1, 5), guide = "none") +
    labs(
      x = "Tract Poverty Rate (%)",
      y = "Share of Tracts with Data Center",
      title = "Data Center Presence at the 20% Poverty Threshold",
      subtitle = "No discontinuity in direct data center location at the OZ eligibility cutoff"
    )

  ggsave(file.path(fig_dir, "fig10_dc_presence.pdf"), p10, width = 8, height = 5)
  cat("  Saved fig10_dc_presence.pdf\n")
} else {
  cat("  No DC data available for figure\n")
}


###############################################################################
# Figure 11: DC Vintage at Cutoff (v4 addition)
###############################################################################
cat("Figure 11: DC Vintage at Cutoff\n")

has_vintage <- "dc_any_post2018" %in% names(rdd_sample) &&
               (sum(rdd_sample$dc_any_post2018) > 0 || sum(rdd_sample$dc_any_pre2018) > 0)

if (has_vintage) {
  binned_vintage <- rdd_sample %>%
    filter(poverty_rate >= 5, poverty_rate <= 40) %>%
    mutate(bin = round(poverty_rate)) %>%
    group_by(bin) %>%
    summarize(
      post2018_rate = mean(dc_any_post2018, na.rm = TRUE),
      pre2018_rate = mean(dc_any_pre2018, na.rm = TRUE),
      n = n(),
      .groups = "drop"
    ) %>%
    pivot_longer(cols = c(post2018_rate, pre2018_rate),
                 names_to = "vintage", values_to = "rate") %>%
    mutate(
      vintage_label = ifelse(vintage == "post2018_rate",
                              "Post-2018 (Treatment-relevant)",
                              "Pre-2018 (Placebo)"),
      se = sqrt(rate * (1 - rate) / pmax(n, 1))
    )

  p11 <- ggplot(binned_vintage, aes(x = bin, y = rate, color = vintage_label)) +
    geom_point(aes(size = n), alpha = 0.7) +
    geom_errorbar(aes(ymin = pmax(rate - 1.96 * se, 0),
                      ymax = rate + 1.96 * se),
                  width = 0.3, alpha = 0.4) +
    geom_vline(xintercept = 20, linetype = "dashed", color = "red", linewidth = 0.8) +
    scale_color_manual(values = c("Pre-2018 (Placebo)" = "grey50",
                                   "Post-2018 (Treatment-relevant)" = "darkorange"),
                       name = NULL) +
    scale_y_continuous(labels = percent_format()) +
    ...
    labs(
      x = "Tract Poverty Rate (%)",
      y = "Share of Tracts with Data Center",
      title = "Data Center Presence by Vintage at the OZ Eligibility Threshold",
      subtitle = "Pre-2018 facilities serve as placebo; post-2018 captures OZ-era investment"
    )

  ggsave(file.path(fig_dir, "fig11_dc_vintage.pdf"), p11, width = 8, height = 5)
  cat("  Saved fig11_dc_vintage.pdf\n")
} else {
  cat("  No DC vintage data available for figure\n")
}
