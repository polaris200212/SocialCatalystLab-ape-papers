###############################################################################
# 03_main_analysis.R
# Main RDD analysis: OZ designation -> employment outcomes
# APEP-0445 v4
###############################################################################

this_file <- tryCatch(sys.frame(1)$ofile, error = function(e) NULL)
if (is.null(this_file)) this_file <- "."
source(file.path(dirname(this_file), "00_packages.R"))

rdd_sample <- readRDS(file.path(data_dir, "rdd_sample.rds"))
cat("RDD sample loaded:", nrow(rdd_sample), "tracts\n\n")

###############################################################################
# 1. McCrary Density Test
###############################################################################
cat("=== McCrary Density Test ===\n")

mccrary <- rddensity(rdd_sample$poverty_rate, c = 20)
cat(sprintf("  Test statistic: %.3f\n", mccrary$test$t_jk))
cat(sprintf("  P-value: %.4f\n", mccrary$test$p_jk))
cat(sprintf("  Result: %s\n",
            ifelse(mccrary$test$p_jk > 0.05, "PASS (no manipulation)", "FAIL")))

saveRDS(mccrary, file.path(data_dir, "mccrary_test.rds"))


###############################################################################
# 2. Covariate Balance Tests
###############################################################################
cat("\n=== Covariate Balance at Cutoff ===\n")

balance_vars <- c("total_pop", "pct_bachelors", "pct_white",
                   "median_home_value", "unemployment_rate",
                   "pre_total_emp", "pre_info_emp")

balance_results <- data.frame(
  variable = character(),
  coef = numeric(),
  se = numeric(),
  pval = numeric(),
  bandwidth = numeric(),
  stringsAsFactors = FALSE
)

for (v in balance_vars) {
  y <- rdd_sample[[v]]
  valid <- !is.na(y) & !is.na(rdd_sample$poverty_rate)

  if (sum(valid) > 100) {
    tryCatch({
      rd <- rdrobust(y[valid], rdd_sample$poverty_rate[valid], c = 20)
      balance_results <- rbind(balance_results, data.frame(
        variable = v,
        coef = rd$coef[1],
        se = rd$se[3],  # Robust SE
        pval = rd$pv[3],
        bandwidth = rd$bws[1, 1]
      ))
      cat(sprintf("  %-25s coef=%.3f  se=%.3f  p=%.3f  %s\n",
                  v, rd$coef[1], rd$se[3], rd$pv[3],
                  ifelse(rd$pv[3] > 0.05, "ok", "!")))
    }, error = function(e) {
      cat(sprintf("  %-25s ERROR: %s\n", v, substr(e$message, 1, 40)))
    })
  }
}

saveRDS(balance_results, file.path(data_dir, "balance_tests.rds"))


###############################################################################
# 3. First Stage: OZ Designation Probability (Nonparametric)
###############################################################################
cat("\n=== First Stage: OZ Designation Discontinuity ===\n")

first_stage <- rdrobust(
  as.numeric(rdd_sample$oz_designated),
  rdd_sample$poverty_rate,
  c = 20
)
cat(sprintf("  Discontinuity in OZ designation: %.3f (SE: %.3f)\n",
            first_stage$coef[1], first_stage$se[3]))
cat(sprintf("  P-value: %.4f\n", first_stage$pv[3]))
cat(sprintf("  Bandwidth: %.2f pp\n", first_stage$bws[1, 1]))
cat(sprintf("  Effective N (left): %d, (right): %d\n",
            first_stage$N_h[1], first_stage$N_h[2]))

saveRDS(first_stage, file.path(data_dir, "first_stage.rds"))


###############################################################################
# 3b. First Stage: Parametric with F-statistic
###############################################################################
cat("\n=== First Stage: Parametric F-statistic ===\n")

# Use optimal bandwidth from nonparametric first stage
fs_bw <- first_stage$bws[1, 1]
fs_sample <- rdd_sample %>%
  filter(abs(pov_centered) <= fs_bw)

# Linear first stage
fs_linear <- feols(oz_designated ~ eligible_poverty * pov_centered,
                   data = fs_sample, vcov = "HC1")

# Extract F-statistic for the treatment indicator
fs_coef <- coef(fs_linear)["eligible_povertyTRUE"]
fs_se <- sqrt(vcov(fs_linear)["eligible_povertyTRUE", "eligible_povertyTRUE"])
fs_f_manual <- (fs_coef / fs_se)^2

cat(sprintf("  Parametric first-stage coefficient: %.4f (SE: %.4f)\n", fs_coef, fs_se))
cat(sprintf("  First-stage F-statistic: %.1f\n", fs_f_manual))
cat(sprintf("  Stock-Yogo 10%% critical value: 16.38\n"))
cat(sprintf("  Instrument %s\n",
            ifelse(fs_f_manual > 16.38, "STRONG", "WEAK")))

# With covariates
fs_covs <- feols(oz_designated ~ eligible_poverty * pov_centered +
                   total_pop + pct_bachelors + pct_white + unemployment_rate,
                 data = fs_sample, vcov = "HC1")

fs_coef_covs <- coef(fs_covs)["eligible_povertyTRUE"]
fs_se_covs <- sqrt(vcov(fs_covs)["eligible_povertyTRUE", "eligible_povertyTRUE"])
fs_f_covs <- (fs_coef_covs / fs_se_covs)^2
cat(sprintf("  First-stage F (with covariates): %.1f\n", fs_f_covs))

saveRDS(list(
  linear = fs_linear,
  with_covs = fs_covs,
  f_stat = fs_f_manual,
  f_stat_covs = fs_f_covs,
  bandwidth = fs_bw,
  n_obs = nrow(fs_sample)
), file.path(data_dir, "first_stage_parametric.rds"))


###############################################################################
# 4. Reduced-Form RDD: Main Outcomes
###############################################################################
cat("\n=== Reduced-Form RDD Estimates ===\n")

outcomes <- list(
  "Total Employment (Post)" = "post_total_emp",
  "Info Employment (Post)" = "post_info_emp",
  "Construction Emp (Post)" = "post_construction_emp",
  "Log Total Emp (Post)" = "log_post_total_emp",
  "Log Info Emp (Post)" = "log_post_info_emp",
  "Delta Total Emp" = "delta_total_emp",
  "Delta Info Emp" = "delta_info_emp",
  "Delta Construction Emp" = "delta_construction_emp"
)

main_results <- list()
for (name in names(outcomes)) {
  var <- outcomes[[name]]
  y <- rdd_sample[[var]]
  valid <- !is.na(y) & !is.na(rdd_sample$poverty_rate)

  if (sum(valid) > 100) {
    tryCatch({
      rd <- rdrobust(y[valid], rdd_sample$poverty_rate[valid], c = 20)
      main_results[[name]] <- list(
        name = name,
        coef = rd$coef[1],
        se_robust = rd$se[3],
        pval = rd$pv[3],
        ci_lower = rd$ci[3, 1],
        ci_upper = rd$ci[3, 2],
        bandwidth = rd$bws[1, 1],
        N_left = rd$N_h[1],
        N_right = rd$N_h[2]
      )
      cat(sprintf("  %-30s b=%.3f  (SE=%.3f)  p=%.4f  bw=%.1f\n",
                  name, rd$coef[1], rd$se[3], rd$pv[3], rd$bws[1, 1]))
    }, error = function(e) {
      cat(sprintf("  %-30s ERROR: %s\n", name, substr(e$message, 1, 40)))
    })
  }
}

saveRDS(main_results, file.path(data_dir, "main_rdd_results.rds"))


###############################################################################
# 4b. Covariate-Adjusted RDD
###############################################################################
cat("\n=== Covariate-Adjusted RDD ===\n")

cov_vars <- c("total_pop", "pct_bachelors", "pct_white", "unemployment_rate")
cov_adjusted_results <- list()

for (name in c("Delta Total Emp", "Delta Info Emp", "Delta Construction Emp")) {
  var <- outcomes[[name]]
  y <- rdd_sample[[var]]
  x <- rdd_sample$poverty_rate

  # Build covariate matrix
  cov_cols <- rdd_sample[, cov_vars]
  valid <- !is.na(y) & !is.na(x) & complete.cases(cov_cols)

  if (sum(valid) > 100) {
    covs_matrix <- as.matrix(cov_cols[valid, ])

    tryCatch({
      rd_cov <- rdrobust(y[valid], x[valid], c = 20, covs = covs_matrix)
      cov_adjusted_results[[name]] <- list(
        name = name,
        coef = rd_cov$coef[1],
        se_robust = rd_cov$se[3],
        pval = rd_cov$pv[3],
        ci_lower = rd_cov$ci[3, 1],
        ci_upper = rd_cov$ci[3, 2],
        bandwidth = rd_cov$bws[1, 1],
        N_left = rd_cov$N_h[1],
        N_right = rd_cov$N_h[2]
      )
      cat(sprintf("  %-30s b=%.3f  (SE=%.3f)  p=%.4f\n",
                  name, rd_cov$coef[1], rd_cov$se[3], rd_cov$pv[3]))
    }, error = function(e) {
      cat(sprintf("  %-30s ERROR: %s\n", name, substr(e$message, 1, 50)))
    })
  }
}

saveRDS(cov_adjusted_results, file.path(data_dir, "cov_adjusted_rdd_results.rds"))


###############################################################################
# 5. Fuzzy RDD (IV Estimates)
###############################################################################
cat("\n=== Fuzzy RDD (IV) Estimates ===\n")

fuzzy_outcomes <- c("post_total_emp", "post_info_emp", "delta_total_emp", "delta_info_emp")

fuzzy_results <- list()
for (var in fuzzy_outcomes) {
  y <- rdd_sample[[var]]
  d <- as.numeric(rdd_sample$oz_designated)
  x <- rdd_sample$poverty_rate
  valid <- !is.na(y) & !is.na(d) & !is.na(x)

  if (sum(valid) > 100) {
    tryCatch({
      rd_fuzzy <- rdrobust(y[valid], x[valid], c = 20, fuzzy = d[valid])
      fuzzy_results[[var]] <- list(
        outcome = var,
        coef = rd_fuzzy$coef[1],
        se_robust = rd_fuzzy$se[3],
        pval = rd_fuzzy$pv[3],
        ci_lower = rd_fuzzy$ci[3, 1],
        ci_upper = rd_fuzzy$ci[3, 2],
        bandwidth = rd_fuzzy$bws[1, 1]
      )
      cat(sprintf("  %-25s b_IV=%.3f  (SE=%.3f)  p=%.4f\n",
                  var, rd_fuzzy$coef[1], rd_fuzzy$se[3], rd_fuzzy$pv[3]))
    }, error = function(e) {
      cat(sprintf("  %-25s ERROR: %s\n", var, substr(e$message, 1, 50)))
    })
  }
}

saveRDS(fuzzy_results, file.path(data_dir, "fuzzy_rdd_results.rds"))


###############################################################################
# 6. Parametric Specifications (for comparison)
###############################################################################
cat("\n=== Parametric RDD Specifications ===\n")

opt_bw <- ifelse(length(main_results) > 0,
                 main_results[[1]]$bandwidth, 10)

rdd_bw <- rdd_sample %>%
  filter(abs(pov_centered) <= opt_bw)

cat(sprintf("  Optimal bandwidth: %.1f pp\n", opt_bw))
cat(sprintf("  Observations in bandwidth: %d\n", nrow(rdd_bw)))

# Linear, no covariates
m1 <- feols(delta_total_emp ~ eligible_poverty * pov_centered,
            data = rdd_bw, vcov = "HC1")

# Linear, with covariates
m2 <- feols(delta_total_emp ~ eligible_poverty * pov_centered +
              total_pop + pct_bachelors + pct_white + unemployment_rate,
            data = rdd_bw, vcov = "HC1")

# Quadratic
m3 <- feols(delta_total_emp ~ eligible_poverty * (pov_centered + I(pov_centered^2)),
            data = rdd_bw, vcov = "HC1")

# Info employment
m4 <- feols(delta_info_emp ~ eligible_poverty * pov_centered,
            data = rdd_bw, vcov = "HC1")

m5 <- feols(delta_info_emp ~ eligible_poverty * pov_centered +
              total_pop + pct_bachelors + pct_white + unemployment_rate,
            data = rdd_bw, vcov = "HC1")

parametric_models <- list(m1 = m1, m2 = m2, m3 = m3, m4 = m4, m5 = m5)
saveRDS(parametric_models, file.path(data_dir, "parametric_models.rds"))

cat("\n  Parametric estimates (delta total emp):\n")
cat(sprintf("    Linear:            %.3f (%.3f)\n", coef(m1)["eligible_povertyTRUE"],
            sqrt(vcov(m1)["eligible_povertyTRUE", "eligible_povertyTRUE"])))
cat(sprintf("    Linear + covs:     %.3f (%.3f)\n", coef(m2)["eligible_povertyTRUE"],
            sqrt(vcov(m2)["eligible_povertyTRUE", "eligible_povertyTRUE"])))
cat(sprintf("    Quadratic:         %.3f (%.3f)\n", coef(m3)["eligible_povertyTRUE"],
            sqrt(vcov(m3)["eligible_povertyTRUE", "eligible_povertyTRUE"])))


###############################################################################
# 7. County-Clustered Parametric Specifications
###############################################################################
cat("\n=== County-Clustered Parametric Specifications ===\n")

if ("county_fips" %in% names(rdd_bw)) {
  # Linear, county-clustered
  m_clust1 <- feols(delta_total_emp ~ eligible_poverty * pov_centered,
                    data = rdd_bw, cluster = ~county_fips)

  m_clust2 <- feols(delta_total_emp ~ eligible_poverty * pov_centered +
                      total_pop + pct_bachelors + pct_white + unemployment_rate,
                    data = rdd_bw, cluster = ~county_fips)

  m_clust3 <- feols(delta_info_emp ~ eligible_poverty * pov_centered,
                    data = rdd_bw, cluster = ~county_fips)

  m_clust4 <- feols(delta_info_emp ~ eligible_poverty * pov_centered +
                      total_pop + pct_bachelors + pct_white + unemployment_rate,
                    data = rdd_bw, cluster = ~county_fips)

  cluster_models <- list(
    total_linear = m_clust1, total_covs = m_clust2,
    info_linear = m_clust3, info_covs = m_clust4
  )
  saveRDS(cluster_models, file.path(data_dir, "cluster_models.rds"))

  cat(sprintf("  County-clustered (total): %.3f (SE=%.3f)\n",
              coef(m_clust1)["eligible_povertyTRUE"],
              sqrt(vcov(m_clust1)["eligible_povertyTRUE", "eligible_povertyTRUE"])))
  cat(sprintf("  County-clustered (info):  %.3f (SE=%.3f)\n",
              coef(m_clust3)["eligible_povertyTRUE"],
              sqrt(vcov(m_clust3)["eligible_povertyTRUE", "eligible_povertyTRUE"])))
  cat(sprintf("  Number of clusters: %d\n", length(unique(rdd_bw$county_fips))))
} else {
  cat("  WARNING: county_fips not available, skipping clustered SEs\n")
}


###############################################################################
# 8. Direct Data Center Presence Outcomes (v3 addition)
###############################################################################
cat("\n=== Direct Data Center Presence RDD (v3) ===\n")

dc_results <- list()

if ("dc_any" %in% names(rdd_sample) && sum(rdd_sample$dc_any, na.rm = TRUE) > 0) {
  cat(sprintf("  DCs in RDD sample: %d tracts (%.3f%%)\n",
              sum(rdd_sample$dc_any), mean(rdd_sample$dc_any) * 100))

  # Binary outcome: any data center in tract
  tryCatch({
    rd_dc <- rdrobust(rdd_sample$dc_any, rdd_sample$poverty_rate, c = 20)
    dc_results[["DC Presence (Binary)"]] <- list(
      name = "DC Presence (Binary)",
      coef = rd_dc$coef[1],
      se_robust = rd_dc$se[3],
      pval = rd_dc$pv[3],
      ci_lower = rd_dc$ci[3, 1],
      ci_upper = rd_dc$ci[3, 2],
      bandwidth = rd_dc$bws[1, 1],
      N_left = rd_dc$N_h[1],
      N_right = rd_dc$N_h[2]
    )
    cat(sprintf("  DC Presence (Binary)       b=%.5f  (SE=%.5f)  p=%.4f  bw=%.1f\n",
                rd_dc$coef[1], rd_dc$se[3], rd_dc$pv[3], rd_dc$bws[1, 1]))
  }, error = function(e) {
    cat("  DC Presence (Binary): ERROR -", e$message, "\n")
  })

  # Count outcome: number of data centers per tract
  tryCatch({
    rd_dc_ct <- rdrobust(rdd_sample$dc_count, rdd_sample$poverty_rate, c = 20)
    dc_results[["DC Count"]] <- list(
      name = "DC Count",
      coef = rd_dc_ct$coef[1],
      se_robust = rd_dc_ct$se[3],
      pval = rd_dc_ct$pv[3],
      ci_lower = rd_dc_ct$ci[3, 1],
      ci_upper = rd_dc_ct$ci[3, 2],
      bandwidth = rd_dc_ct$bws[1, 1],
      N_left = rd_dc_ct$N_h[1],
      N_right = rd_dc_ct$N_h[2]
    )
    cat(sprintf("  DC Count                   b=%.5f  (SE=%.5f)  p=%.4f  bw=%.1f\n",
                rd_dc_ct$coef[1], rd_dc_ct$se[3], rd_dc_ct$pv[3], rd_dc_ct$bws[1, 1]))
  }, error = function(e) {
    cat("  DC Count: ERROR -", e$message, "\n")
  })

  # LPM parametric within optimal bandwidth
  if (length(dc_results) > 0) {
    dc_bw <- dc_results[[1]]$bandwidth
    dc_bw_sample <- rdd_sample %>% filter(abs(pov_centered) <= dc_bw)

    tryCatch({
      lpm_dc <- feols(dc_any ~ eligible_poverty * pov_centered,
                      data = dc_bw_sample, vcov = "HC1")
      dc_lpm_coef <- coef(lpm_dc)["eligible_povertyTRUE"]
      dc_lpm_se <- sqrt(vcov(lpm_dc)["eligible_povertyTRUE", "eligible_povertyTRUE"])
      cat(sprintf("  LPM (DC presence):         b=%.5f  (SE=%.5f)\n",
                  dc_lpm_coef, dc_lpm_se))

      dc_results[["DC LPM"]] <- list(
        name = "DC LPM",
        coef = dc_lpm_coef,
        se_robust = dc_lpm_se,
        pval = 2 * pnorm(-abs(dc_lpm_coef / dc_lpm_se)),
        ci_lower = dc_lpm_coef - 1.96 * dc_lpm_se,
        ci_upper = dc_lpm_coef + 1.96 * dc_lpm_se,
        bandwidth = dc_bw,
        N_left = sum(dc_bw_sample$pov_centered < 0),
        N_right = sum(dc_bw_sample$pov_centered >= 0)
      )
    }, error = function(e) {
      cat("  LPM: ERROR -", e$message, "\n")
    })
  }

  # Minimum Detectable Effect calculation
  cat("\n  --- Minimum Detectable Effect (MDE) ---\n")
  base_rate <- mean(rdd_sample$dc_any, na.rm = TRUE)
  if (length(dc_results) > 0 && !is.null(dc_results[["DC Presence (Binary)"]]) &&
      !is.null(dc_results[["DC Presence (Binary)"]]$se_robust)) {
    se_dc <- dc_results[["DC Presence (Binary)"]]$se_robust
    mde_80 <- 2.8 * se_dc  # 80% power at 5% significance
    cat(sprintf("  Base rate of DC presence: %.4f (%.2f%%)\n", base_rate, base_rate * 100))
    cat(sprintf("  SE of DC presence RDD: %.5f\n", se_dc))
    cat(sprintf("  MDE (80%% power): %.5f (%.2f pp)\n", mde_80, mde_80 * 100))
    cat(sprintf("  MDE as %% of base rate: %.1f%%\n", mde_80 / max(base_rate, 0.0001) * 100))

    dc_results[["MDE"]] <- list(
      base_rate = base_rate,
      se = se_dc,
      mde_80 = mde_80,
      mde_pct_base = mde_80 / max(base_rate, 0.0001) * 100
    )
  }
} else {
  cat("  No data center presence data available in RDD sample.\n")
  cat("  DC outcomes will be omitted from analysis.\n")
}

saveRDS(dc_results, file.path(data_dir, "dc_rdd_results.rds"))


###############################################################################
# 9. DC Vintage Analysis: Pre-2018 (Placebo) vs Post-2018 (Treatment) (v4)
###############################################################################
cat("\n=== DC Vintage RDD (v4: Pre-2018 Placebo vs Post-2018 Treatment) ===\n")

vintage_results <- list()

if ("dc_any_post2018" %in% names(rdd_sample)) {
  n_post2018 <- sum(rdd_sample$dc_any_post2018, na.rm = TRUE)
  n_pre2018 <- sum(rdd_sample$dc_any_pre2018, na.rm = TRUE)
  cat(sprintf("  Post-2018 DCs in RDD sample: %d tracts (%.3f%%)\n",
              n_post2018, mean(rdd_sample$dc_any_post2018) * 100))
  cat(sprintf("  Pre-2018 DCs in RDD sample: %d tracts (%.3f%%)\n",
              n_pre2018, mean(rdd_sample$dc_any_pre2018) * 100))

  # Use a wider bandwidth for vintage analysis since vintage DCs are very sparse
  # The nonparametric optimal bandwidth is too narrow for rare events;
  # use the full sample or a generous fixed bandwidth
  dc_bw_vin <- 15.0  # Wide bandwidth to ensure sufficient variation
  vin_bw_sample <- rdd_sample %>% filter(abs(pov_centered) <= dc_bw_vin)

  cat(sprintf("  Using bandwidth: %.1f pp (%d tracts)\n", dc_bw_vin, nrow(vin_bw_sample)))

  # Try rdrobust first; fall back to LPM if nonparametric fails
  run_vintage_rdd <- function(y, label) {
    result <- NULL
    # Try rdrobust
    tryCatch({
      rd <- rdrobust(y, rdd_sample$poverty_rate, c = 20)
      result <- list(
        name = label, coef = rd$coef[1],
        se_robust = rd$se[3], pval = rd$pv[3],
        ci_lower = rd$ci[3, 1], ci_upper = rd$ci[3, 2],
        bandwidth = rd$bws[1, 1],
        N_left = rd$N_h[1], N_right = rd$N_h[2],
        method = "rdrobust"
      )
      cat(sprintf("  %-30s b=%.6f (SE=%.6f) p=%.4f [rdrobust]\n",
                  label, rd$coef[1], rd$se[3], rd$pv[3]))
    }, error = function(e) {
      cat(sprintf("  %-30s rdrobust failed, using LPM\n", label))
    })

    # Fallback: parametric LPM within bandwidth
    if (is.null(result)) {
      tryCatch({
        bw_idx <- abs(rdd_sample$pov_centered) <= dc_bw_vin
        lpm_df <- data.frame(
          y_vin = y[bw_idx],
          eligible_poverty = rdd_sample$eligible_poverty[bw_idx],
          pov_centered = rdd_sample$pov_centered[bw_idx]
        )
        lpm <- feols(y_vin ~ eligible_poverty * pov_centered,
                     data = lpm_df, vcov = "HC1")
        lpm_coef <- coef(lpm)["eligible_povertyTRUE"]
        lpm_se <- sqrt(vcov(lpm)["eligible_povertyTRUE", "eligible_povertyTRUE"])
        lpm_pval <- 2 * pnorm(-abs(lpm_coef / lpm_se))
        result <- list(
          name = label, coef = lpm_coef, se_robust = lpm_se,
          pval = lpm_pval,
          ci_lower = lpm_coef - 1.96 * lpm_se,
          ci_upper = lpm_coef + 1.96 * lpm_se,
          bandwidth = dc_bw_vin,
          N_left = sum(lpm_df$pov_centered < 0),
          N_right = sum(lpm_df$pov_centered >= 0),
          method = "LPM"
        )
        cat(sprintf("  %-30s b=%.6f (SE=%.6f) p=%.4f [LPM]\n",
                    label, lpm_coef, lpm_se, lpm_pval))
      }, error = function(e) {
        cat(sprintf("  %-30s LPM also failed: %s\n", label, e$message))
      })
    }
    return(result)
  }

  # Post-2018 DC presence: main treatment-relevant outcome
  if (n_post2018 > 0) {
    vintage_results[["DC Post-2018 (Binary)"]] <- run_vintage_rdd(
      rdd_sample$dc_any_post2018, "Post-2018 DC presence")
    vintage_results[["DC Post-2018 (Count)"]] <- run_vintage_rdd(
      rdd_sample$dc_count_post2018, "Post-2018 DC count")
  } else {
    cat("  WARNING: No post-2018 DCs found in RDD sample\n")
  }

  # Pre-2018 DC presence: placebo (should show NO discontinuity)
  if (n_pre2018 > 0) {
    vintage_results[["DC Pre-2018 (Binary)"]] <- run_vintage_rdd(
      rdd_sample$dc_any_pre2018, "Pre-2018 DC presence (placebo)")
    vintage_results[["DC Pre-2018 (Count)"]] <- run_vintage_rdd(
      rdd_sample$dc_count_pre2018, "Pre-2018 DC count (placebo)")
  } else {
    cat("  WARNING: No pre-2018 DCs found in RDD sample\n")
  }

  # MDE for post-2018 outcome
  if (!is.null(vintage_results[["DC Post-2018 (Binary)"]])) {
    se_post <- vintage_results[["DC Post-2018 (Binary)"]]$se_robust
    base_post <- mean(rdd_sample$dc_any_post2018, na.rm = TRUE)
    mde_post <- 2.8 * se_post
    cat(sprintf("\n  Post-2018 base rate: %.4f (%.2f%%)\n", base_post, base_post * 100))
    cat(sprintf("  Post-2018 MDE (80%% power): %.5f (%.2f pp)\n", mde_post, mde_post * 100))
    vintage_results[["MDE_post2018"]] <- list(
      base_rate = base_post, se = se_post, mde_80 = mde_post,
      mde_pct_base = mde_post / max(base_post, 0.0001) * 100
    )
  }
} else {
  cat("  DC vintage variables not available in RDD sample\n")
}

saveRDS(vintage_results, file.path(data_dir, "dc_vintage_results.rds"))

cat("\n=== Main analysis complete ===\n")
