...
if (has_dc) {
  ...
  analysis <- analysis %>%
    left_join(dc_tract %>% select(tract_geoid, dc_count, dc_any,
                                   dc_any_pre2018, dc_any_post2018,
                                   dc_count_pre2018, dc_count_post2018),
              by = c("geoid" = "tract_geoid"))
  # Fill NAs with 0 for tracts without DCs
  for (v in c("dc_any", "dc_count", "dc_any_pre2018", "dc_any_post2018",
              "dc_count_pre2018", "dc_count_post2018")) {
    analysis[[v]][is.na(analysis[[v]])] <- 0L
  }
  cat("Data center presence merged:", sum(analysis$dc_any), "tracts with DCs\n")
  cat("  Pre-2018 DCs:", sum(analysis$dc_any_pre2018), "tracts\n")
  cat("  Post-2018 DCs:", sum(analysis$dc_any_post2018), "tracts\n")
} else {
