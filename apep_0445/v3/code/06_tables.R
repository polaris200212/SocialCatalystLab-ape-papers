...
if (file.exists(dc_file)) {
  ...
  if (length(dc_res) > 0 && !is.null(dc_res[["DC Presence (Binary)"]])) {
    ...
    tex6 <- paste0(
      ...
      "\\begin{tablenotes}\n",
      "\\small\n",
      "\\item \\textit{Notes:} Rows 1--2: nonparametric RDD using \\texttt{rdrobust} with MSE-optimal bandwidth and triangular kernel. Row 3: linear probability model within optimal bandwidth with HC1 standard errors. Data center locations from EIA Form 860 and EPA GHGRP, geocoded to census tracts.", mde_note, " * $p<0.10$, ** $p<0.05$, *** $p<0.01$.\n",
      "\\end{tablenotes}\n",
      "\\end{table}"
    )
