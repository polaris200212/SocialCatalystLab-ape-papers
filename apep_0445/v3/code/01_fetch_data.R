...
cat("=== Step 2: Fetch official OZ designation list from CDFI Fund ===\n")

# Primary source: CDFI Fund designated QOZ list (xlsx)
# This is the official Treasury-certified list of 8,764 Qualified Opportunity Zones
oz_urls <- c(
  "https://www.cdfifund.gov/sites/cdfi/files/2024-01/QOZ-List.xlsx",
  "https://www.cdfifund.gov/sites/cdfi/files/documents/designated-qozs.12.14.18.xlsx",
  "https://www.cdfifund.gov/sites/cdfi/files/2021-01/designated-qozs.xlsx"
)

oz_raw <- NULL
oz_file <- file.path(data_dir, "oz_list.xlsx")

for (oz_url in oz_urls) {
  if (!is.null(oz_raw)) break
  tryCatch({
    download.file(oz_url, oz_file, mode = "wb", quiet = TRUE)
    oz_raw <- readxl::read_excel(oz_file)
    cat("OZ list downloaded from:", oz_url, "\n")
    cat("  Records:", nrow(oz_raw), "\n")
    cat("  Columns:", paste(names(oz_raw), collapse = ", "), "\n")
  }, error = function(e) {
    cat("  Download failed from:", oz_url, "-", substr(e$message, 1, 60), "\n")
  })
}

# Alternative: CSV from data.gov
if (is.null(oz_raw)) {
  cat("All xlsx sources failed, trying CSV alternative...\n")
  ...
  tryCatch({
    ...
    cat("WARNING: All OZ download sources failed.\n")
    cat("Will fall back to approximation with loud warning.\n")
  })
}
...
if (!is.null(oz_raw)) {
  ...
  cat("Official OZ designation data saved\n")
} else {
  cat("*** FALLBACK: OZ designation data unavailable — approximation will be used ***\n")
...
cat("\n=== Step 5: Fetch ACS broadband/internet data ===\n")

# B28002: Presence and types of internet subscriptions in household
# B28002_001E = Total households
# B28002_004E = With broadband (cable, fiber, DSL)
# B28002_013E = No internet access
# NOTE: B28002 not available in 2015 ACS. Using 2017 ACS (still pre-OZ treatment).
broadband_list <- list()
for (st in states) {
  url <- paste0(
    "https://api.census.gov/data/2017/acs/acs5?get=",
    "B28002_001E,B28002_004E,B28002_013E",
    "&for=tract:*&in=state:", st
  )
  tryCatch({
    resp <- httr::GET(url)
    if (httr::status_code(resp) == 200) {
      ...
      df <- as.data.frame(raw[-1, ], stringsAsFactors = FALSE)
      names(df) <- raw[1, ]
      broadband_list[[st]] <- df
    }
  }, error = function(e) NULL)
  Sys.sleep(0.3)
}

broadband_raw <- bind_rows(broadband_list)
broadband_raw <- broadband_raw %>%
  mutate(
    geoid = paste0(state, county, tract),
    total_hh_internet = as.numeric(B28002_001E),
    broadband_hh = as.numeric(B28002_004E),
    no_internet_hh = as.numeric(B28002_013E),
    pct_broadband = ifelse(total_hh_internet > 0,
                            broadband_hh / total_hh_internet * 100, NA),
    pct_no_internet = ifelse(total_hh_internet > 0,
                              no_internet_hh / total_hh_internet * 100, NA)
  ) %>%
  select(geoid, pct_broadband, pct_no_internet, total_hh_internet)

saveRDS(broadband_raw, file.path(data_dir, "acs_broadband.rds"))
cat("Broadband data saved:", nrow(broadband_raw), "tracts\n")
