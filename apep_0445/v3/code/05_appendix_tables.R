...
for (outcome in c("Delta Total Emp", "Delta Info Emp", "Delta Construction Emp")) {
  ...
  # For outcomes with saved models, use them
  ...
  } else if (is.null(param_key_hc1)) {
    # For construction emp: estimate on the fly
    bw_opt <- 4.5
    bw_sub <- rdd_sample %>% filter(abs(poverty_rate - 20) <= bw_opt, !is.na(delta_construction_emp))
    bw_sub$pov_centered <- bw_sub$poverty_rate - 20
    tryCatch({
      m_hc1 <- feols(delta_construction_emp ~ eligible_poverty * pov_centered, data = bw_sub, vcov = "HC1")
      m_cl <- feols(delta_construction_emp ~ eligible_poverty * pov_centered, data = bw_sub, cluster = ~county_fips)
      hc1_coef_val <- coef(m_hc1)["eligible_povertyTRUE"]
      hc1_se_val <- sqrt(vcov(m_hc1)["eligible_povertyTRUE", "eligible_povertyTRUE"])
      hc1_pval <- 2 * pnorm(-abs(hc1_coef_val / hc1_se_val))
      col3_coef <- add_stars(fmt_3f(hc1_coef_val), hc1_pval)
      col3_se <- paste0("(", fmt_3f(hc1_se_val), ")")
      cl_coef_val <- coef(m_cl)["eligible_povertyTRUE"]
      cl_se_val <- sqrt(vcov(m_cl)["eligible_povertyTRUE", "eligible_povertyTRUE"])
  ...
  if (!is.null(clust_key) && file.exists(cluster_file)) {
    ...
    # else: col4 already initialized to "---" above
  }
  # else: col4 already set by on-the-fly estimation or initialized to "---"
