...
\textbf{Effects on health outcomes.} \citet{patrick2016implementation} find that states with robust PDMPs experienced reductions in opioid-related overdose deaths, though the magnitudes vary across specifications and the evidence is more mixed than for prescribing outcomes. \citet{meara2016state} show that stronger PDMP provisions reduce Schedule II prescribing among disabled adults. \citet{pardo2017pdmp} examines whether more robust PDMP programs reduce prescription opioid overdose, finding mixed evidence that depends on specific program features. \citet{fink2018association} provide a systematic review of associations between PDMPs and drug overdoses, highlighting heterogeneous effects across program types and the importance of mandate strength. However, some evidence suggests substitution to heroin and illicit fentanyl, which could partially offset or even reverse health gains from reduced prescribing \citep{meinhofer2018prescription, powell2020decline, alpert2018supply}.
...
\end{equation}
where $\alpha_s$ and $\lambda_t$ are state and year fixed effects, yields a coefficient $\beta^{TWFE}$ that is commonly interpreted as the average treatment effect. However, recent methodological advances have demonstrated that when treatment effects are heterogeneous across cohorts or over time, the TWFE estimator can be biased because it implicitly uses already-treated units as controls for newly-treated units \citep{goodmanbacon2021difference, dechaisemartin2020}. In staggered adoption settings, the TWFE coefficient is a weighted average of all possible two-by-two difference-in-differences comparisons, and some of these comparisons receive negative weights when treatment effects change over time \citep{roth2023s, wooldridge2021twoway}. This concern is especially relevant in the PDMP setting, where early adopters may have different treatment effects than late adopters due to differences in opioid crisis severity, policy environments, and labor market conditions. Alternative heterogeneity-robust estimators include the imputation approach of \citet{borusyak2024revisiting}, the two-stage method of \citet{gardner2022imputation}, the stacked event-study approach of \citet{cengiz2019effect}, and the synthetic difference-in-differences estimator of \citet{arkhangelsky2021synthetic}, which is particularly suited to settings with few never-treated units. In settings with near-universal adoption, interactive fixed effects models \citep{bai2009panel, xu2017generalized} offer another approach to constructing counterfactuals, though they require stronger functional form assumptions.
...
\end{equation}
where $Y_t(g)$ denotes the potential outcome under treatment by cohort $g$ and $Y_t(0)$ denotes the untreated potential outcome. The key identification assumption is the conditional parallel trends assumption \citep[see also][for a design-based perspective]{athey2022design}:
...
Third, must-access mandates vary substantially in their specific provisions---substances covered (Schedule II only versus all controlled substances), encounters covered (initial prescriptions only versus all encounters), exemptions (emergency departments, hospice), enforcement mechanisms, penalties for noncompliance, delegate access provisions, and integration with electronic health records \citep{pardo2017pdmp, fink2018association}. The analysis treats all mandates as homogeneous, coding a single 0/1 indicator. This is a potentially significant source of attenuation bias: if strong mandates (broad schedule coverage, strict enforcement, robust EHR integration) produce employment effects while weak mandates do not, the pooled binary indicator will understate the effect of the former. A dose-response analysis exploiting mandate-strength heterogeneity---constructing an index from PDAPS dimensions including schedule coverage, encounter requirements, enforcement provisions, delegate access, and interstate data-sharing capabilities---could yield substantially more informative estimates of which provisions matter. This is a high-priority direction for future work that could transform the current aggregate null into a more nuanced set of results.
...
The policy implications of this aggregate null are twofold. First, policymakers should not expect must-access PDMP mandates to generate measurable improvements in aggregate state-level employment over the medium run. The mandates may have other benefits---reduced prescribing, fewer new opioid dependencies, lower overdose rates---that justify their adoption on public health grounds, but employment gains should not be counted among the expected returns at the state aggregate level. The disconnect between the well-documented prescribing reductions and the absence of detectable employment effects suggests that the causal chain from mandate to prescribing to employment is attenuated at each step, with the aggregate effect diluted below the threshold of statistical detectability. This finding does not imply that mandates have no employment effects whatsoever---the power of this design is limited to detecting aggregate effects exceeding approximately 2 percent, while plausible subpopulation effects could be economically meaningful yet invisible in state-level data.
...
The null finding at the aggregate level does not preclude the possibility of meaningful effects on specific subpopulations or through specific channels. Several extensions would strengthen the evidence base. First, individual-level data---such as CPS microdata disaggregated by age, education, and industry---could test for effects among the populations most plausibly affected by prescribing changes, particularly prime-age workers in construction, mining, and manufacturing where opioid prescribing rates are highest \citep{krueger2017labor}. Second, outcome measures closer to the hypothesized mechanism---including labor force participation rates, disability insurance applications, and occupation-specific employment from the QCEW---would provide substantially more statistical power than state aggregate employment levels. Third, the near-universal adoption of must-access mandates creates an opportunity for researchers to examine dose-response heterogeneity across mandate strength, enforcement intensity, and local opioid crisis severity. Fourth, county-level data would enable border-county designs that compare outcomes on either side of state borders around mandate adoption dates, providing a more credible identification strategy than state-level DiD in a setting with near-universal adoption. Fifth, alternative counterfactual construction methods---including synthetic difference-in-differences \citep{arkhangelsky2021synthetic} and interactive fixed effects \citep{bai2009panel, xu2017generalized}---may provide more reliable causal estimates when the never-treated group is thin.
...
This research was conducted as part of the Autonomous Policy Evaluation Project (APEP). The analysis, code, and manuscript were produced with substantial assistance from Claude (Anthropic), an AI language model, under human supervision and editorial responsibility. All empirical specifications, identification assumptions, and interpretive claims reflect the judgment of the research team. AI-generated content was reviewed and validated by human researchers.
