...
## ---- 4. Pre-COVID Provider Exit Measure ----
cat("Computing pre-COVID provider exit intensity...\n")

# Active providers: billed in 2018-2019
active_2018_2019 <- provider_monthly[year %in% 2018:2019,
                                     .(billing_npi, state, is_hcbs, enumeration_date, taxonomy_1)] |>
  unique(by = c("billing_npi", "state"))

# Still active in 2020+: billed at least once after Feb 2020
active_2020_plus <- provider_monthly[month_date >= "2020-03-01",
                                     .(billing_npi)] |>
  unique()

# Exiters: active 2018-2019, NOT in 2020+
active_2018_2019[, exited := !(billing_npi %in% active_2020_plus$billing_npi)]

# State-level exit rates
state_exits <- active_2018_2019[, .(
  n_active       = .N,
  n_exited       = sum(exited),
  exit_rate      = mean(exited),
  n_hcbs_active  = sum(is_hcbs),
  n_hcbs_exited  = sum(exited & is_hcbs),
  hcbs_exit_rate = fifelse(sum(is_hcbs) > 0, sum(exited & is_hcbs) / sum(is_hcbs), NA_real_)
), by = state]

# Career length proxy: years since NPI enumeration (as of 2020-01-01)
active_2018_2019[, career_years := as.numeric(as.Date("2020-01-01") - enumeration_date) / 365.25]

# Shift-share instrument: national specialty exit rates × local composition
# Classify broad specialty from taxonomy
active_2018_2019[, specialty := fcase(
  is_hcbs, "hcbs",
...
# National exit rates by specialty
national_spec_exit <- active_2018_2019[, .(
  national_exit_rate = mean(exited)
), by = specialty]

# Local specialty shares
local_spec_share <- active_2018_2019[, .(
  n = .N
), by = .(state, specialty)]
local_spec_share[, share := n / sum(n), by = state]

# Bartik predicted exit rate
bartik <- merge(local_spec_share, national_spec_exit, by = "specialty")
bartik_state <- bartik[, .(
  predicted_exit_rate = sum(share * national_exit_rate)
), by = state]

state_exits <- merge(state_exits, bartik_state, by = "state", all.x = TRUE)

# Quartiles of exit intensity
state_exits[, exit_quartile := cut(exit_rate,
                                    breaks = quantile(exit_rate, probs = 0:4/4, na.rm = TRUE),
                                    labels = c("Q1 (Low)", "Q2", "Q3", "Q4 (High)"),
                                    include.lowest = TRUE)]
state_exits[, high_exit := exit_rate > median(exit_rate, na.rm = TRUE)]

cat(sprintf("Exit rates: median %.1f%%, range %.1f%%–%.1f%%\n",
            100 * median(state_exits$exit_rate),
            100 * min(state_exits$exit_rate),
            100 * max(state_exits$exit_rate)))
...
unemp <- rbindlist(unemp_list[!sapply(unemp_list, is.null)])
...
panel <- merge(panel, state_exits[, .(state, exit_rate, hcbs_exit_rate, exit_quartile,
                                       high_exit, predicted_exit_rate, n_active)],
               by = "state", all.x = TRUE)
