...
main_results <- readRDS("output/paper_84/data/cs_results.rds")
...
df_no_igaming <- read_csv("output/paper_84/data/analysis_no_igaming.csv", show_col_types = FALSE) %>%
...
df_early <- read_csv("output/paper_84/data/analysis_early_cohorts.csv", show_col_types = FALSE) %>%
  mutate(state_id = as.integer(factor(state_fips)), G = first_treat_quarter)
...
saveRDS(robustness_results, "output/paper_84/data/robustness_results.rds")
write_csv(robustness_summary, "output/paper_84/data/robustness_summary.csv")
