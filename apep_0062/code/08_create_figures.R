# Create publication figures for paper_80
library(ggplot2)
library(dplyr)

setwd("/Users/dyanag/auto-policy-evals/output/paper_80")
...
trends <- panel %>%
  filter(!suppressed, employment > 0) %>%
  mutate(treated_group = ifelse(G == 0, "Never Treated", "Eventually Treated")) %>%
  group_by(year, treated_group) %>%
  summarize(
    mean_emp = mean(employment),
    se_emp = sd(employment) / sqrt(n()),
    .groups = "drop"
  )

p3 <- ggplot(trends, aes(x = year, y = mean_emp, color = treated_group)) +
  geom_vline(xintercept = 2018.5, linetype = "dotted", color = "gray50") +
  geom_line(linewidth = 1) +
  geom_point(size = 2) +
  geom_ribbon(aes(ymin = mean_emp - 1.96 * se_emp, ymax = mean_emp + 1.96 * se_emp, fill = treated_group),
              alpha = 0.2, color = NA) +
  scale_color_manual(values = c("Never Treated" = "gray50", "Eventually Treated" = "steelblue")) +
  scale_fill_manual(values = c("Never Treated" = "gray50", "Eventually Treated" = "steelblue")) +
  labs(
