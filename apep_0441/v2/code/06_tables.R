## ============================================================================
## 06_tables.R — Generate all tables (v2: + border results)
...
rob_row("State-Pair $\\times$ Year FE",
        coef(twfe_pair)["treat_post"], se(twfe_pair)["treat_post"], n_baseline)
...
cat("\\begin{minipage}{0.85\\textwidth}\n")
cat("\\vspace{0.2cm}\n")
cat("\\footnotesize \\textit{Notes:} All specifications include district and year fixed effects. ``Placebo (1997)'' assigns a fake treatment date of 1997 using only pre-treatment data (1994--2000). ``Extended'' uses calibrated DMSP+VIIRS panel through 2023. Leave-one-out drops one state pair at a time. ``State-Pair $\\times$ Year FE'' absorbs common shocks within each parent--child pair. New state capitals: Dehradun (Uttarakhand), Ranchi (Jharkhand), Raipur (Chhattisgarh). Telangana uses VIIRS 2012--2023 with standard errors clustered at the district level (only 2 state clusters). Main specifications cluster at the state level (6 clusters). $^{*}p<0.10$, $^{**}p<0.05$, $^{***}p<0.01$.\n")
cat("\\end{minipage}\n")
cat("\\end{table}\n")
sink()
...
## ============================================================================
## Table 5: Border DiD Results
## ============================================================================

cat("\n=== Table 5: Border DiD Results ===\n")

sink(file.path(tab_dir, "tab5_border_did.tex"))
cat("\\begin{table}[htbp]\n")
cat("\\centering\n")
cat("\\caption{Border Discontinuity Design: Full Sample vs. Border Subsample}\n")
cat("\\label{tab:border}\n")
cat("\\begin{tabular}{lccccc}\n")
cat("\\hline\\hline\n")
cat(" & (1) & (2) & (3) & (4) & (5) \\\\\n")
cat(" & Full Sample & Border 100km & Border 150km & Border 200km & Border 300km \\\\\n")
cat("\\hline\n")
cat("\\addlinespace\n")

# Full sample estimate
b_full <- coef(twfe_basic)["treat_post"]
s_full <- se(twfe_basic)["treat_post"]
st_full <- stars_fn(b_full, s_full)
cat(sprintf("New State $\\times$ Post & %.4f%s", b_full, st_full))

# Border estimates at different bandwidths
for (bw in c("100", "150", "200", "300")) {
  if (bw %in% names(border_did_results)) {
    res <- border_did_results[[bw]]
    b <- coef(res$fit)["treat_post"]
    s <- se(res$fit)["treat_post"]
    st <- stars_fn(b, s)
    cat(sprintf(" & %.4f%s", b, st))
  } else {
    cat(" & ---")
...
# Spatial RDD estimate
if (!is.null(rdd_out)) {
  rdd_b <- rdd_out$coef[1]
  rdd_s <- rdd_out$se[1]
  rdd_st <- stars_fn(rdd_b, rdd_s)
  cat(sprintf("Spatial RDD & \\multicolumn{5}{c}{%.4f%s (%.4f), BW = %.0f km} \\\\\n",
              rdd_b, rdd_st, rdd_s, rdd_out$bws[1,1]))
}

cat("\\addlinespace\n")
cat("\\hline\n")
cat("District FE & Yes & Yes & Yes & Yes & Yes \\\\\n")
cat("Year FE & Yes & Yes & Yes & Yes & Yes \\\\\n")

# Districts row
cat(sprintf("Districts & %d", uniqueN(panel_2000$dist_id)))
for (bw in c("100", "150", "200", "300")) {
  if (bw %in% names(border_did_results)) {
    cat(sprintf(" & %d", border_did_results[[bw]]$n_dist))
  } else {
    cat(" & ---")
  }
}
...
# Treated districts row
n_treated_full <- sum(panel_2000$treated[panel_2000$year == panel_2000$year[1]] == 1)
cat(sprintf("Treated Districts & %d", n_treated_full))
for (bw in c("100", "150", "200", "300")) {
  if (bw %in% names(border_did_results)) {
    cat(sprintf(" & %d", border_did_results[[bw]]$n_treated))
  } else {
    cat(" & ---")
  }
}
...
# Observations row
cat(sprintf("Observations & %s", formatC(nrow(panel_2000), big.mark = ",")))
for (bw in c("100", "150", "200", "300")) {
  if (bw %in% names(border_did_results)) {
    cat(sprintf(" & %s", formatC(border_did_results[[bw]]$n_obs, big.mark = ",")))
  } else {
    cat(" & ---")
  }
}
