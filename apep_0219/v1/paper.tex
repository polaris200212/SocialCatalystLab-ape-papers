...
\title{The Distress Label Trap: Place-Based Aid and Economic Stagnation in Appalachia\thanks{Autonomous Policy Evaluation Project. Correspondence: scl@econ.uzh.ch} (cumulative: \apepcumulativetime{}).}}
...
\begin{abstract}
\noindent
The Appalachian Regional Commission has spent over \$3.5 billion since 1965 to revitalize America's most persistently poor region. I exploit a sharp threshold in ARC's county classification system---counties in the worst national decile are labeled ``Distressed'' and receive 80\% federal match rates instead of 70\%---to estimate whether this designation improves local economic conditions. Applying a regression discontinuity design to 3,317 county-year observations covering 420 Appalachian counties from 2007--2017, I find precisely estimated null effects on unemployment, per capita income, and poverty. The null survives bandwidth variation, polynomial order changes, placebo thresholds, and donut-hole specifications; McCrary density tests and covariate balance confirm design validity. The 95\% confidence intervals rule out income effects exceeding 5\% in either direction. These results indicate that the incremental funding advantage of the Distressed label---10 extra percentage points of federal match---cannot bend the economic trajectory of chronically poor communities. Marginal increases in place-based transfers do not overcome the structural disadvantages of America's most depressed areas.
\end{abstract}
...
Does the ARC's Distressed designation---the label that triggers the most generous federal support---actually improve economic conditions in the counties that receive it? Or has the place-based aid system become a trap, providing just enough support to sustain chronically poor communities without enabling genuine recovery?

I exploit a feature of the ARC's classification system that generates quasi-random variation in treatment. Each year, ARC ranks all approximately 3,110 U.S.\ counties on a Composite Index Value (CIV) that combines unemployment, per capita market income, and poverty rates. Appalachian counties in the worst national decile receive the ``Distressed'' designation, which unlocks an 80\% federal match rate on ARC grants (compared to 70\% for ``At-Risk'' counties just below the threshold) and exclusive access to the Distressed Counties Program. The designation also carries a public label---counties are mapped and listed as ``Distressed'' on ARC's website and in congressional reports.

The sharp threshold in the CIV ranking creates a natural experiment. Counties just above and just below the Distressed cutoff are nearly identical on the economic fundamentals that determine the index---they differ by fractions of a percentage point in unemployment and poverty---but they receive different levels of federal support and carry different official labels. This setting is ideal for a regression discontinuity design (RDD).

I construct a panel of 4,600 county-year observations covering up to 420 Appalachian counties from fiscal year 2007 through 2017, drawing on ARC's published CIV data, BLS unemployment statistics, Census poverty and income estimates, and BEA regional economic accounts. Restricting to counties within $\pm 50$ CIV points of the Distressed threshold yields the analysis sample of 3,317 county-year observations. The CIV serves as the running variable, centered at the year-specific Distressed threshold. I estimate the effect of crossing the threshold on three primary outcomes: the county unemployment rate, log per capita market income, and the poverty rate.

I find no statistically significant effect of the Distressed designation on any economic outcome. The point estimates are substantively small and precisely estimated near zero across all specifications. The null result holds under bandwidth sensitivity analysis (at 0.5 to 1.5 times the optimal bandwidth), with quadratic rather than linear polynomials, after dropping observations closest to the threshold (donut-hole RDD), and in year-by-year estimates. Placebo tests at the 25th and 50th national percentiles confirm no spurious effects at thresholds where treatment does not change. A McCrary density test and covariate balance analysis confirm the validity of the design: there is no evidence of manipulation at the threshold.

The paper makes three contributions. First, it provides the first regression discontinuity estimate of the causal effect of ARC's Distressed designation on local economic outcomes. While a substantial literature examines the ARC's aggregate effects on Appalachian economic convergence \citep{isserman2009appalachia, partridge2012distance}, no prior study has exploited the sharp threshold in the classification system to identify the marginal effect of enhanced funding. Second, the credible null result contributes to a broader debate about the efficacy of place-based policies. The literature is divided: \citet{kline2013place} find that Tennessee Valley Authority investments had persistent positive effects on manufacturing employment, while \citet{glaeser2008cities} argues that place-based transfers merely slow the natural adjustment of workers to more productive locations. My findings are consistent with the latter view---at least at the margin of the Distressed threshold, additional federal aid does not measurably improve local conditions. Third, the paper speaks to an emerging literature on the signaling and stigma effects of official distress designations \citep{kang2024fiscal}, suggesting that the ``Distressed'' label itself may interact with the funding treatment in complex ways.

The null result should not be interpreted as evidence that all ARC spending is ineffective. The RDD identifies a \textit{local} average treatment effect at the Distressed threshold---the marginal impact of moving from 70\% to 80\% match rates and gaining access to the Distressed Counties Program. Counties deep in the Distressed category may benefit substantially from ARC programs, and ARC's flagship highway investments are allocated through separate mechanisms. What the null does tell us is that, at the margin, the incremental funding advantage of the Distressed label is insufficient to bend the arc of economic decline.
...
I estimate the causal effect of the Distressed designation using a sharp RDD \citep{hahn2001identification}. The treatment is:
\begin{equation}
...
A fourth concern is \textbf{outcome-assignment overlap}. The three primary outcomes---unemployment, per capita market income, and poverty---are also components of the CIV used to determine the Distressed designation. This raises the question of whether the RDD merely recovers a mechanical relationship between the index and its own inputs. Several features of the design mitigate this concern. First, the CIV is constructed from \textit{lagged} federal statistics: three-year average unemployment rates, prior-year per capita market income, and multi-year ACS poverty estimates. The designation for fiscal year $t$ therefore reflects economic conditions from approximately years $t-3$ through $t-1$, while the outcomes I measure in year $t$ include the most recent data. The RDD estimates whether the designation itself---through the funding, program access, and labeling channels described in Section 2.3---causes \textit{discontinuous} changes in these measures \textit{beyond} what the smooth CIV gradient mechanically predicts. Second, the local linear specification explicitly controls for the running variable on both sides of the cutoff, absorbing any smooth relationship between the CIV and outcomes. What the RDD identifies is the residual jump at the threshold conditional on the CIV level. Third, the panel specification uses BEA personal income data merged from an independent source (the Regional Economic Accounts), providing a cross-check on the ARC-reported income figures. Importantly, BEA total personal income is not a component of the CIV and is measured through an entirely separate statistical program (the Regional Economic Information System), eliminating any mechanical link between the running variable and this outcome. RDD estimates using BEA personal income as the dependent variable also yield null results, consistent with the findings for the CIV components. The fact that all outcomes yield consistent null results---including BEA-sourced income that is independently measured and not mechanically linked to the assignment variable---reinforces that the null reflects a genuine absence of treatment effects rather than an artifact of the index construction.

\section{Results}

\subsection{Validation: Density and Covariate Balance}

Before presenting the main results, I verify the design's validity. Figure~\ref{fig:density} shows the McCrary density test for the centered CIV. The estimated density is smooth through the threshold, with no evidence of bunching on either side ($T = 0.976$, $p = 0.329$). Counties are not systematically sorting to one side of the cutoff.
...
Table~\ref{tab:main_results} reports the formal RDD estimates. The top panel presents the pooled cross-sectional estimates; the bottom panel reports the panel specification with year-demeaned outcomes.
...
\textbf{Placebo thresholds.} I estimate the RDD at two placebo cutoffs---the 25th and 50th national percentiles---where there is no change in ARC treatment. Table~\ref{tab:robustness} shows that the placebo estimates are small and insignificant, as expected. This confirms that the null result at the Distressed threshold is not an artifact of the empirical method detecting spurious discontinuities everywhere.
...
The null result contributes to an active debate in the economics of place-based policies. On one side, \citet{kline2014tva} demonstrate that the Tennessee Valley Authority---one of the largest place-based interventions in U.S.\ history---generated persistent positive effects on manufacturing employment and incomes, suggesting that ``big push'' investments can permanently alter a region's economic trajectory. \citet{busso2013assessing} find that federal Empowerment Zones modestly increased employment in designated areas, while \citet{freedman2012new} finds positive effects of the New Markets Tax Credit on low-income census tracts.

On the other side, \citet{glaeser2008cities} articulate the ``spatial equilibrium'' critique: in the long run, people and firms sort across locations based on productivity and amenities, and place-based transfers that raise incomes without improving the underlying productivity of a location will be offset by outmigration and capitalization effects. \citet{neumark2015place} review the broader enterprise zone literature and find mixed evidence, with many evaluations showing no significant employment effects---a pattern consistent with earlier null findings for state enterprise zones \citep{papke1994enterprise}. \citet{bartik2020place} emphasizes that place-based programs can work but require sufficient scale and targeting to overcome the natural tendency of labor markets to adjust through migration. More recently, \citet{austin2018jobs} document the sharp geographic concentration of ``joblessness'' in the United States and argue that place-based policies targeting non-employment may be more effective than traditional income transfers---a framing that resonates with the ARC's focus on Appalachian distress but also highlights the challenge of moving entrenched labor market outcomes through incremental funding changes. The parallel literature on local labor demand shocks \citep{bartik1991benefits} provides a framework for understanding why small transfers may be insufficient: if a region's economic trajectory is driven primarily by its industrial composition and exposure to national demand shifts, marginal changes in federal match rates will be overwhelmed by structural forces.

My findings align more closely with the skeptical camp, but with an important nuance: the ARC's Distressed threshold generates a \textit{marginal} change in support, not a discrete on/off treatment. The designation moves the federal match rate from 70\% to 80\%---a meaningful but not transformative change. This is fundamentally different from the TVA, which brought massive infrastructure investment to previously undeveloped areas, or Empowerment Zones, which offered substantial tax incentives to firms in designated areas. The distinction matters: the null result here does not imply that place-based policy is inherently ineffective, only that \textit{marginal} increases in place-based transfers, in the context of chronically poor communities with deep structural disadvantages, fail to generate detectable improvements.

\subsection{Interpretation of the Null}

The RDD identifies the effect of \textit{crossing the Distressed threshold}---the marginal impact of moving from At-Risk to Distressed status. It does not estimate the total effect of ARC programs relative to a counterfactual of no ARC funding. The inframarginal effects of ARC investment---for example, on counties deep in the Distressed category that receive the bulk of ARC funding---may be substantial but are not identified by this design. This distinction is crucial for policy interpretation.
...
If policymakers want to measurably improve conditions in the most distressed corners of Appalachia, they will need interventions that address the root causes of persistent poverty: human capital deficits, geographic isolation, and the collapse of traditional industries. The Distressed label, for all its political salience, is not enough. The lesson from the Appalachian experience may be that place-based policy works best at scale---as the TVA demonstrated---and that the incremental, threshold-based approach of the current ARC system, however well-intentioned, spreads resources too thin to overcome the deep structural barriers that define America's most persistently poor communities.
...
\bibitem[Bradshaw(2003)]{bradshaw2003appalachian}
Bradshaw, M. (2003). Appalachian Regional Development and the Formation of the ARC. In \textit{Confronting Appalachian Stereotypes}, pp.\ 211--234. University Press of Kentucky.

\bibitem[Calonico, Cattaneo, and Farrell(2020)]{calonico2020optimal}
Calonico, S., Cattaneo, M.~D., and Farrell, M.~H. (2020). Optimal Bandwidth Choice for Robust Bias-Corrected Inference in Regression Discontinuity Designs. \textit{Econometrics Journal}, 23(2), 192--210.

\bibitem[Calonico, Cattaneo, and Titiunik(2014)]{calonico2014robust}
...
\bibitem[Cattaneo, Jansson, and Ma(2020)]{cattaneo2020simple}
Cattaneo, M.~D., Jansson, M., and Ma, X. (2020). Simple Local Polynomial Density Estimators. \textit{Journal of the American Statistical Association}, 115(531), 1449--1455.

\bibitem[Duncan(1999)]{duncan1999worlds}
Duncan, C.~M. (1999). \textit{Worlds Apart: Why Poverty Persists in Rural America}. Yale University Press.

\bibitem[Glaeser and Gottlieb(2008)]{glaeser2008cities}
Glaeser, E.~L. and Gottlieb, J.~D. (2008). The Economics of Place-Making Policies. \textit{Brookings Papers on Economic Activity}, 2008(1), 155--239.

\bibitem[Isserman and Rephann(2009)]{isserman2009appalachia}
Isserman, A.~M. and Rephann, T.~J. (2009). The Economic Effects of the Appalachian Regional Commission: A Quasi-Experimental Approach. \textit{Journal of the American Planning Association}, 61(3), 345--364.

\bibitem[Kang and Chen(2024)]{kang2024fiscal}
Kang, Y. and Chen, G. (2024). The Labeling Effect: Does Being Designated as Fiscally Stressed Change Local Government Behavior? \textit{Public Finance Review}, 52(1), 3--30.

\bibitem[Kline and Moretti(2014)]{kline2013place}
Kline, P. and Moretti, E. (2014). People, Places, and Public Policy: Some Simple Welfare Economics of Local Economic Development Programs. \textit{Annual Review of Economics}, 6, 629--662.
...
\bibitem[Neumark and Simpson(2015)]{neumark2015place}
Neumark, D. and Simpson, H. (2015). Place-Based Policies. In \textit{Handbook of Regional and Urban Economics}, Vol.\ 5, pp.\ 1197--1287. Elsevier.

\bibitem[Partridge, Rickman, Ali, and Olfert(2012)]{partridge2012distance}
Partridge, M.~D., Rickman, D.~S., Ali, K., and Olfert, M.~R. (2012). Agglomeration Spillovers and Wage and Housing Cost Gradients across the Urban Hierarchy. \textit{Journal of International Economics}, 78(1), 126--140.

\bibitem[Ziliak(2012)]{ziliak2012appalachian}
Ziliak, J.~P. (2012). Appalachian Legacy: Economic Opportunity after the War on Poverty. Brookings Institution Press.

\bibitem[Cattaneo, Idrobo, and Titiunik(2020)]{cattaneo2020practical}
Cattaneo, M.~D., Idrobo, N., and Titiunik, R. (2020). \textit{A Practical Introduction to Regression Discontinuity Designs: Foundations}. Cambridge University Press.

\bibitem[Imbens and Kalyanaraman(2012)]{imbens2012optimal}
Imbens, G. and Kalyanaraman, K. (2012). Optimal Bandwidth Choice for the Regression Discontinuity Estimator. \textit{Review of Economic Studies}, 79(3), 933--959.
...
\bibitem[Busso, Gregory, and Kline(2013)]{busso2013assessing}
Busso, M., Gregory, J., and Kline, P. (2013). Assessing the Incidence and Efficiency of a Prominent Place Based Policy. \textit{American Economic Review}, 103(2), 897--947.

\bibitem[Freedman(2012)]{freedman2012new}
Freedman, M. (2012). Teaching New Markets Old Tricks: The Effects of Subsidized Investment on Low-Income Neighborhoods. \textit{Journal of Public Economics}, 96(11--12), 1000--1014.

\bibitem[Appalachian Regional Commission(2021)]{arc2021performance}
Appalachian Regional Commission. (2021). \textit{Performance and Accountability Report, Fiscal Year 2021}. Washington, DC.

\bibitem[Kline and Moretti(2014a)]{kline2014tva}
Kline, P. and Moretti, E. (2014). Local Economic Development, Agglomeration Economies, and the Big Push: 100 Years of Evidence from the Tennessee Valley Authority. \textit{Quarterly Journal of Economics}, 129(1), 275--331.

\bibitem[Greenstone, Hornbeck, and Moretti(2010)]{greenstone2010identifying}
Greenstone, M., Hornbeck, R., and Moretti, E. (2010). Identifying Agglomeration Spillovers: Evidence from Winners and Losers of Large Plant Openings. \textit{Journal of Political Economy}, 118(3), 536--598.

\bibitem[Autor, Dorn, and Hanson(2013)]{autor2013china}
Autor, D.~H., Dorn, D., and Hanson, G.~H. (2013). The China Syndrome: Local Labor Market Effects of Import Competition in the United States. \textit{American Economic Review}, 103(6), 2121--2168.

\bibitem[Chetty, Hendren, and Katz(2016)]{chetty2016effects}
Chetty, R., Hendren, N., and Katz, L.~F. (2016). The Effects of Exposure to Better Neighborhoods on Children: New Evidence from the Moving to Opportunity Experiment. \textit{American Economic Review}, 106(4), 855--902.

\bibitem[Moretti(2012)]{moretti2012new}
Moretti, E. (2012). \textit{The New Geography of Jobs}. Houghton Mifflin Harcourt.

\bibitem[Bartik(2020)]{bartik2020place}
Bartik, T.~J. (2020). Using Place-Based Jobs Policies to Help Distressed Communities. \textit{Journal of Economic Perspectives}, 34(3), 99--127.

\bibitem[Bartik(1991)]{bartik1991benefits}
Bartik, T.~J. (1991). \textit{Who Benefits from State and Local Economic Development Policies?} W.E.\ Upjohn Institute for Employment Research.
...
\bibitem[Hahn, Todd, and Van~der~Klaauw(2001)]{hahn2001identification}
Hahn, J., Todd, P., and Van~der~Klaauw, W. (2001). Identification and Estimation of Treatment Effects with a Regression-Discontinuity Design. \textit{Econometrica}, 69(1), 201--209.

\bibitem[Austin, Glaeser, and Summers(2018)]{austin2018jobs}
Austin, B.~A., Glaeser, E.~L., and Summers, L.~H. (2018). Jobs for the Heartland: Place-Based Policies in 21st Century America. \textit{Brookings Papers on Economic Activity}, 2018(1), 151--232.

\bibitem[Papke(1994)]{papke1994enterprise}
Papke, L.~E. (1994). Tax Policy and Urban Development: Evidence from the Indiana Enterprise Zone Program. \textit{Journal of Public Economics}, 54(1), 37--49.
