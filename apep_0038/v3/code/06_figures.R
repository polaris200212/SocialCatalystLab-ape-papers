...
if (requireNamespace("maps", quietly = TRUE)) {
  ...
  map_plot_data <- us_states %>%
    ...
    left_join(map_data_df, by = "state_abbr")
...
robustness_plot_data <- tibble(
  ...
  att = c(
    ...
    main_results$never_treated$att,
...
fig6 <- loo_results %>%
  ...
  labs(
    ...
    y = "Dropped State",
    title = "Leave-One-Out Sensitivity (All Treated States)",
...
# -----------------------------------------------------------------------------
# Figure 7: Wage Event Study (NEW in v3)
# -----------------------------------------------------------------------------

message("Creating Figure 7: Wage Event Study...")

if (file.exists("../data/wage_event_study.csv")) {
  wage_es <- read_csv("../data/wage_event_study.csv", show_col_types = FALSE)
  ...
  fig7 <- wage_es %>%
    ...
    labs(
      x = "Years Since Sports Betting Legalization",
      y = "Effect on Log(Weekly Wage)",
      title = "Event Study: Wage Effects of Sports Betting Legalization",
      subtitle = "Callaway-Sant'Anna estimator, outcome: log(average weekly wage) in NAICS 7132"
    ) +
    scale_x_continuous(breaks = seq(-8, 8, 2)) +
    theme_paper() +
    theme(
  ...
  ggsave(file.path(fig_dir, "fig7_wage_event_study.pdf"), fig7,
         width = 8, height = 5, device = cairo_pdf)
  ggsave(file.path(fig_dir, "fig7_wage_event_study.png"), fig7,
         width = 8, height = 5, dpi = 300)

  message("  Saved: fig7_wage_event_study.pdf")
} else {
  message("  Skipping wage event study figure (no data)")
}
