...
v7: adds within-GR horse race (HPI vs GR Bartik), Rotemberg weight diagnostics,
    COVID Bartik now standardized to SD=1
"""
...
bartik_covid = proc['bartik_covid']  # now standardized (mean=0, SD=1)
bartik_covid_raw = proc.get('bartik_covid_raw', bartik_covid)
bartik_covid_sd = proc.get('bartik_covid_sd', 0.0236)
hpi_boom = proc['hpi_boom']
saiz_instrument = proc.get('saiz_instrument', {})
shares_covid = proc.get('shares_covid', {})
shares_gr = proc.get('shares_gr', {})
...
# ──────────────────────────────────────────────────────────────
# Within-GR Horse Race: HPI vs GR Bartik (WS3 v7)
# ──────────────────────────────────────────────────────────────
print("\n" + "="*60)
print("WITHIN-GR HORSE RACE: HPI vs GR BARTIK")
print("="*60)

def lp_horse_race(paths, z1, z2, horizons, controls=None, min_n=10):
    """
    Horse race LP: Δy_{s,h} = α + β1*Z1_s + β2*Z2_s + X'γ + ε
    Returns results with both coefficients, SEs, VIF.
    """
    results = []
    for h in horizons:
        states_h = [st for st in sorted(paths.keys())
                    if st in z1 and st in z2 and h in paths[st]]
        n = len(states_h)
        if n < min_n:
            results.append({'h': h, 'beta_z1': np.nan, 'se_z1': np.nan,
                           'beta_z2': np.nan, 'se_z2': np.nan,
                           'n': n, 'r2': np.nan, 'vif': np.nan})
            continue

        y = np.array([paths[st][h] for st in states_h])
        x1 = np.array([z1[st] for st in states_h])
        x2 = np.array([z2[st] for st in states_h])
        ...
        try:
            ...
            # VIF: regress x1 on x2 + controls
            X_vif = np.column_stack([np.ones(n), x2])
            if controls is not None:
                X_vif = np.column_stack([X_vif, np.array(ctrl_rows)])
            b_vif = np.linalg.lstsq(X_vif, x1, rcond=None)[0]
            r_vif = x1 - X_vif @ b_vif
            r2_vif = 1 - np.sum(r_vif**2) / np.sum((x1 - np.mean(x1))**2)
            vif = 1 / (1 - r2_vif) if r2_vif < 1 else np.inf

            pval_z1 = 2 * (1 - stats.t.cdf(abs(beta_hat[1] / se[1]), n - k)) if se[1] > 0 else np.nan
            pval_z2 = 2 * (1 - stats.t.cdf(abs(beta_hat[2] / se[2]), n - k)) if se[2] > 0 else np.nan

            results.append({
                'h': h, 'beta_z1': float(beta_hat[1]), 'se_z1': float(se[1]),
                'pval_z1': float(pval_z1),
                'beta_z2': float(beta_hat[2]), 'se_z2': float(se[2]),
                'pval_z2': float(pval_z2),
                'n': n, 'r2': float(r2), 'vif': float(vif)
            })
        except np.linalg.LinAlgError:
            results.append({'h': h, 'beta_z1': np.nan, 'se_z1': np.nan,
                           'beta_z2': np.nan, 'se_z2': np.nan,
...
horse_race_horizons = [6, 12, 24, 36, 48, 60, 84, 120]
horse_race_results = lp_horse_race(gr_paths, hpi_boom, bartik_gr,
                                    horse_race_horizons, controls=gr_controls)

print("\n  HPI (demand) vs GR Bartik (industry) — both in same regression:")
print(f"  {'h':>5s}  {'β_HPI':>10s}  {'SE':>8s}  {'p':>6s}  {'β_Bartik':>10s}  {'SE':>8s}  {'p':>6s}  {'VIF':>6s}  {'R²':>6s}")
for r in horse_race_results:
    if not np.isnan(r['beta_z1']):
        sig1 = "***" if r.get('pval_z1', 1) < 0.01 else "**" if r.get('pval_z1', 1) < 0.05 else "*" if r.get('pval_z1', 1) < 0.1 else ""
        sig2 = "***" if r.get('pval_z2', 1) < 0.01 else "**" if r.get('pval_z2', 1) < 0.05 else "*" if r.get('pval_z2', 1) < 0.1 else ""
        print(f"  {r['h']:5d}  {r['beta_z1']:10.4f}{sig1}  ({r['se_z1']:.4f})  {r.get('pval_z1',1):.3f}  "
              f"{r['beta_z2']:10.4f}{sig2}  ({r['se_z2']:.4f})  {r.get('pval_z2',1):.3f}  {r['vif']:.2f}  {r['r2']:.3f}")

# ──────────────────────────────────────────────────────────────
# Rotemberg Weight Diagnostics for COVID Bartik (WS4 v7)
# ──────────────────────────────────────────────────────────────
print("\n" + "="*60)
print("ROTEMBERG WEIGHT DIAGNOSTICS: COVID BARTIK")
print("="*60)

def compute_rotemberg_weights(shares, shifts, instrument_vals):
    """
    Compute Rotemberg (2983) / Goldsmith-Pinkham+ (2020) weights.
    α_k = Σ_s ω_{s,k} * g_k / Σ_s B_s
    where ω_{s,k} is industry k share in state s, g_k is national industry shift.
    """
    ind_codes = sorted(shifts.keys())
    states = sorted(shares.keys())

    weights = {}
    for ind in ind_codes:
        g_k = shifts.get(ind, 0)
        if isinstance(g_k, str):
            g_k = float(g_k)
        # Numerator: sum of share * shift across states
        share_sum = sum(shares[st].get(ind, 0) for st in states)
        weights[ind] = share_sum * g_k
...
shifts_covid = proc.get('shifts_covid', {})
rotemberg_weights = compute_rotemberg_weights(shares_covid, shifts_covid, bartik_covid)

# Sort by weight
sorted_weights = sorted(rotemberg_weights.items(), key=lambda x: x[1], reverse=True)
print("\n  Top industries by Rotemberg weight:")
for ind, w in sorted_weights[:7]:
    print(f"    {ind}: {w:.4f} ({w*100:.1f}%)")

# Leave-one-out: drop top industry and re-run COVID LP at h=48
top_industry = sorted_weights[0][0] if sorted_weights else None
if top_industry:
    print(f"\n  Leave-one-out robustness: dropping {top_industry}")
    # Reconstruct Bartik without top industry for each state
    bartik_covid_loo = {}
    for st in sorted(shares_covid.keys()):
        total_emp = sum(shares_covid[st].get(ind, 0) for ind in shifts_covid.keys())
        loo_emp = total_emp - shares_covid[st].get(top_industry, 0)
        if loo_emp > 0:
            weighted = sum(shares_covid[st].get(ind, 0) * float(shifts_covid.get(ind, 0))
                          for ind in shifts_covid.keys() if ind != top_industry)
            bartik_covid_loo[st] = weighted / loo_emp if loo_emp > 0 else 0
        else:
            bartik_covid_loo[st] = 0

    # Standardize the LOO Bartik too
    loo_vals = np.array(list(bartik_covid_loo.values()))
    loo_mean = float(np.mean(loo_vals))
    loo_sd = float(np.std(loo_vals))
    if loo_sd > 0:
        bartik_covid_loo = {st: (v - loo_mean) / loo_sd for st, v in bartik_covid_loo.items()}

    loo_horizons = [3, 6, 12, 24, 36, 48]
    covid_loo_results = lp_ols(covid_paths, bartik_covid_loo, loo_horizons, controls=covid_controls)
    print(f"  LOO results (dropping {top_industry}):")
    for r in covid_loo_results:
        sig = "***" if r['pval'] < 0.01 else "**" if r['pval'] < 0.05 else "*" if r['pval'] < 0.1 else ""
        print(f"    h={r['h']:3d}: β={r['beta']:8.4f} ({r['se']:.4f}){sig}")
else:
    covid_loo_results = []
...
# ──────────────────────────────────────────────────────────────
print("\n" + "="*60)
print("ROMANO-WOLF STEPDOWN P-VALUES (1,000 permutations)")
print("="*60)

def romano_wolf_stepdown(paths, instrument, horizons, controls=None,
                         n_perms=1000, seed=42):
    """
    Compute Romano-Wolf (2005, 2016) stepdown-adjusted p-values.

    Accounts for multiple hypothesis testing across LP horizons by controlling
    the familywise error rate (FWER). Uses permutation resampling to
    approximate the joint null distribution.

    Algorithm:
      1. Compute observed |t|-statistics for each horizon.
      2. For each permutation, compute |t| at every horizon; record the max.
      3. Starting with all hypotheses, compare each observed |t| to the
         (1 - alpha) quantile of the max-|t| null distribution.
      4. Reject hypotheses that exceed the threshold.
      5. Remove rejected hypotheses, recompute max-|t| over remaining, repeat.
      6. Report adjusted p-values = (# permutations where max-|t|_null >=
         observed |t|_h) / n_perms at each stepdown stage.

    Returns dict: horizon -> {'pval_rw': float, 'pval_unadj': float,
                               't_obs': float, 'rejected_step': int or None}
    """
    rng = np.random.RandomState(seed)

    # Step 1: observed OLS estimates and t-statistics
    actual = lp_ols(paths, instrument, horizons, controls=controls)
    obs_t = {}
    obs_pval = {}
    for r in actual:
        h = r['h']
        if not np.isnan(r['beta']) and r['se'] > 0:
            obs_t[h] = abs(r['beta'] / r['se'])
            obs_pval[h] = r['pval']
        else:
            obs_t[h] = 0.0
            obs_pval[h] = 1.0
    ...
    # Step 2: permutation null — store |t| for each horizon at each permutation
    perm_t = {h: np.zeros(n_perms) for h in horizons}
    ...
    for p_idx in range(n_perms):
        ...
        for r in perm_results:
            h = r['h']
            if not np.isnan(r['beta']) and r['se'] > 0:
                perm_t[h][p_idx] = abs(r['beta'] / r['se'])
            else:
                perm_t[h][p_idx] = 0.0

    # Step 3-5: stepdown procedure
    active_horizons = list(horizons)  # hypotheses still being tested
    rw_pvals = {h: 1.0 for h in horizons}
    rejected_step = {h: None for h in horizons}
    step = 1

    while active_horizons:
        # Compute max |t| across active horizons for each permutation
        max_t_null = np.zeros(n_perms)
        for p_idx in range(n_perms):
            max_t_null[p_idx] = max(perm_t[h][p_idx] for h in active_horizons)

        # For each active hypothesis, compute stepdown p-value
        newly_rejected = []
        for h in active_horizons:
            # p-value = fraction of permutations where max-t >= observed t
            p_rw = np.mean(max_t_null >= obs_t[h])
            # Enforce monotonicity: RW p-value >= unadjusted p-value
            p_rw = max(p_rw, obs_pval[h])
            rw_pvals[h] = p_rw
            if p_rw < 0.10:  # reject at 10% FWER (generous for stepdown display)
                newly_rejected.append(h)
                rejected_step[h] = step

        if not newly_rejected:
            break  # no more rejections, stop

        # Remove rejected hypotheses and continue
        active_horizons = [h for h in active_horizons if h not in newly_rejected]
        step += 1
    ...
    for h in horizons:
        results[str(h)] = {
            'pval_rw': float(rw_pvals[h]),
            'pval_unadj': float(obs_pval[h]),
            't_obs': float(obs_t[h]),
            'rejected_step': rejected_step[h],
        }
    return results


# Run Romano-Wolf for GR (HPI instrument)
rw_horizons_gr = [3, 6, 12, 18, 24, 36, 48, 60, 84, 120]
print("\n--- Great Recession (HPI instrument) ---")
rw_gr = romano_wolf_stepdown(gr_paths, hpi_boom, rw_horizons_gr,
                              controls=gr_controls, n_perms=1000, seed=42)
print(f"  {'h':>5s}  {'|t| obs':>8s}  {'p unadj':>8s}  {'p RW':>8s}  {'step':>5s}")
for h in rw_horizons_gr:
    r = rw_gr[str(h)]
    step_str = str(r['rejected_step']) if r['rejected_step'] is not None else "—"
    sig_rw = "***" if r['pval_rw'] < 0.01 else "**" if r['pval_rw'] < 0.05 else "*" if r['pval_rw'] < 0.10 else ""
    print(f"  {h:5d}  {r['t_obs']:8.3f}  {r['pval_unadj']:8.3f}  "
          f"{r['pval_rw']:8.3f}{sig_rw}  {step_str:>5s}")

# Run Romano-Wolf for COVID (Bartik instrument)
rw_horizons_covid = [3, 6, 12, 18, 24, 36, 48]
print("\n--- COVID (Bartik instrument) ---")
rw_covid = romano_wolf_stepdown(covid_paths, bartik_covid, rw_horizons_covid,
                                 controls=covid_controls, n_perms=1000, seed=42)
print(f"  {'h':>5s}  {'|t| obs':>8s}  {'p unadj':>8s}  {'p RW':>8s}  {'step':>5s}")
for h in rw_horizons_covid:
    r = rw_covid[str(h)]
    step_str = str(r['rejected_step']) if r['rejected_step'] is not None else "—"
    sig_rw = "***" if r['pval_rw'] < 0.01 else "**" if r['pval_rw'] < 0.05 else "*" if r['pval_rw'] < 0.10 else ""
    print(f"  {h:5d}  {r['t_obs']:8.3f}  {r['pval_unadj']:8.3f}  "
...
# ──────────────────────────────────────────────────────────────
print("\n" + "="*60)
print("MONTIEL OLEA-PFLUEGER WEAK INSTRUMENT DIAGNOSTICS")
print("="*60)

def mop_weak_instrument_diagnostics(paths, endogenous, instrument, horizons,
                                     controls=None, min_n=10,
                                     tau=0.05, mop_cv=23.1):
    """
    Compute heteroskedasticity-robust effective first-stage F statistics
    (Montiel Olea & Pflueger, 2013) for the Saiz IV at each LP horizon.

    The effective F statistic is:
        F_eff = (pi_hat' * V_homo^{-1} * pi_hat) / (pi_hat' * V_robust^{-1} * pi_hat)
               * F_classical

    For a single instrument (K_z = 1), this simplifies to:
        F_eff = pi_hat^2 / V_robust(pi_hat)

    where V_robust is the HC1 variance of the first-stage coefficient on Z.

    MOP critical value: 23.1 for one endogenous regressor, one instrument,
    5% worst-case size distortion of a 5% Wald test (Table 1, MOP 2013).

    Parameters
    ----------
    paths : dict[str, dict[int, float]]
        State -> {horizon: outcome} employment paths
    endogenous : dict[str, float]
        State -> endogenous variable (HPI boom)
    instrument : dict[str, float]
        State -> instrument (Saiz elasticity)
    horizons : list[int]
        LP horizons to evaluate
    controls : dict[str, np.array] or None
        State -> control vector
    min_n : int
        Minimum sample size
    tau : float
        Worst-case size distortion tolerance (default 5%)
    mop_cv : float
        MOP critical value for given tau, K_z=1 (default 23.1)

    Returns
    -------
    list[dict] with keys: h, F_eff, F_classical, n, exceeds_mop, mop_cv
    """
    results = []

    for h in horizons:
        # Build aligned cross-section (same as lp_2sls)
        states_h = []
        for st in sorted(paths.keys()):
            if st in endogenous and st in instrument and h in paths[st]:
                states_h.append(st)
        ...
        if n < min_n:
            results.append({
                'h': h, 'F_eff': np.nan, 'F_classical': np.nan,
                'n': n, 'exceeds_mop': False, 'mop_cv': mop_cv,
                'pi_hat': np.nan, 'se_pi_robust': np.nan,
                'se_pi_homo': np.nan
            })
            continue

        x_endog = np.array([endogenous[st] for st in states_h])
        z = np.array([instrument[st] for st in states_h])

        # Build W matrix (controls + intercept)
        W = np.ones((n, 1))
        if controls is not None:
            ctrl_rows = [np.asarray(controls.get(st, np.zeros(5)), dtype=float)
                         for st in states_h]
            ctrl_matrix = np.array(ctrl_rows)
            W = np.column_stack([W, ctrl_matrix])
        ...
        # First stage: x_endog = W*delta + z*pi + v
        Z_first = np.column_stack([W, z])
        k_first = Z_first.shape[1]

        try:
            pi_all = np.linalg.lstsq(Z_first, x_endog, rcond=None)[0]
            pi_hat = pi_all[-1]  # coefficient on z (instrument)
            v_first = x_endog - Z_first @ pi_all

            # Classical (homoskedastic) F: restricted vs unrestricted
            pi_restricted = np.linalg.lstsq(W, x_endog, rcond=None)[0]
            ssr_restricted = np.sum((x_endog - W @ pi_restricted)**2)
            ssr_unrestricted = np.sum(v_first**2)
            k_w = W.shape[1]
            F_classical = ((ssr_restricted - ssr_unrestricted) / 1) / \
                          (ssr_unrestricted / (n - k_w - 1))

            # Homoskedastic SE of pi_hat
            sigma2_homo = ssr_unrestricted / (n - k_first)
            V_homo = sigma2_homo * np.linalg.inv(Z_first.T @ Z_first)
            se_pi_homo = np.sqrt(V_homo[-1, -1])

            # HC1 robust SE of pi_hat
            meat_first = np.zeros((k_first, k_first))
            for i in range(n):
                zi = Z_first[i:i+1, :]
                meat_first += (v_first[i]**2) * (zi.T @ zi)
            bread_first = np.linalg.inv(Z_first.T @ Z_first)
            V_robust = (n / (n - k_first)) * bread_first @ meat_first @ bread_first
            se_pi_robust = np.sqrt(V_robust[-1, -1])

            # Effective F statistic (MOP):
            # For single instrument: F_eff = pi_hat^2 / V_robust(pi_hat)
            F_eff = (pi_hat**2) / V_robust[-1, -1] if V_robust[-1, -1] > 0 else np.nan
            ...
            results.append({
                ...
                'n': n,
                'exceeds_mop': bool(F_eff >= mop_cv) if not np.isnan(F_eff) else False,
                'mop_cv': mop_cv,
                'pi_hat': float(pi_hat),
                'se_pi_robust': float(se_pi_robust),
                'se_pi_homo': float(se_pi_homo),
            })
...
mop_horizons = [6, 12, 24, 36, 48, 60, 84, 120]
mop_results = mop_weak_instrument_diagnostics(
    gr_paths, hpi_boom, saiz_instrument, mop_horizons,
    controls=gr_controls, mop_cv=23.1
)

print("\n  Saiz IV: Heteroskedasticity-robust effective F statistics")
print(f"  MOP critical value: 23.1 (5% worst-case size, K_z=1, 1 endog)")
print(f"\n  {'h':>5s}  {'F_eff':>8s}  {'F_class':>8s}  {'pi_hat':>8s}  "
      f"{'SE_rob':>8s}  {'SE_homo':>8s}  {'N':>4s}  {'Pass':>6s}")
for r in mop_results:
    if not np.isnan(r['F_eff']):
        pass_str = "YES" if r['exceeds_mop'] else "NO"
        print(f"  {r['h']:5d}  {r['F_eff']:8.2f}  {r['F_classical']:8.2f}  "
              f"{r['pi_hat']:8.4f}  {r['se_pi_robust']:8.4f}  {r['se_pi_homo']:8.4f}  "
              f"{r['n']:4d}  {pass_str:>6s}")
    else:
        print(f"  {r['h']:5d}  {'N/A':>8s}  {'N/A':>8s}  {'N/A':>8s}  "
              f"{'N/A':>8s}  {'N/A':>8s}  {r['n']:4d}  {'N/A':>6s}")

# Summary
n_pass = sum(1 for r in mop_results if r['exceeds_mop'])
n_total = len(mop_results)
print(f"\n  Summary: {n_pass}/{n_total} horizons pass MOP threshold (F_eff >= 23.1)")
if n_pass < n_total:
    weak_horizons = [r['h'] for r in mop_results if not r['exceeds_mop'] and not np.isnan(r['F_eff'])]
    if weak_horizons:
        print(f"  Weak instrument concern at horizons: {weak_horizons}")
        print(f"  → Use Anderson-Rubin CIs (already computed in 2SLS) for valid inference")
...
lp_results = {
    ...
    'rotemberg_weights': {k: float(v) for k, v in sorted_weights},
    'rotemberg_loo': covid_loo_results,
    'rotemberg_loo_industry': top_industry,
    'bartik_covid_sd': bartik_covid_sd,
    'romano_wolf': {
        'gr_hpi': rw_gr,
        'covid_bartik': rw_covid,
    },
    'mop_diagnostics': mop_results,
}
