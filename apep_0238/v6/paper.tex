...
\title{Demand Recessions Scar, Supply Recessions Don't:\\ Evidence from State Labor Markets\footnote{This paper is revision v6 of APEP-0238. See \url{https://github.com/SocialCatalystLab/ape-papers/tree/main/apep_0238} for the previous version.}}
...
Workers choose to remain in the labor force as long as $U_t(h, d) \geq V^{OLF}$. When unemployment duration is long enough and human capital has depreciated sufficiently, the expected value of continued search falls below the value of non-participation, and the worker exits the labor force. In simulations, I replace this endogenous stopping rule with the reduced-form hazard $\chi_t$ in \Cref{eq:exit_rate}, calibrated to match observed participation dynamics. This is the participation margin through which demand recessions generate permanent employment losses.
...
\small \textit{Notes:} Each point is a state. The x-axis plots the negated \citet{saiz2010geographic} housing supply elasticity (lower elasticity = more constrained); the y-axis plots the 2003Q1--2006Q4 log housing price boom. The strong negative relationship (supply-inelastic states experienced larger booms) validates the instrument's relevance. Fitted line with 95\% confidence band.
\end{minipage}
\end{figure}
...
COVID-19 defied this pattern. Panel B of \Cref{tab:main} tells a starkly different story. The Bartik-predicted COVID shock has a strong negative immediate effect ($\hat{\beta}_3 = -0.8279$, $p < 0.10$), confirming that states with more exposed industry compositions experienced larger initial employment declines. The $R^2$ of 0.51 at $h = 3$ indicates the Bartik exposure measure explains roughly half of cross-state variation in the immediate aftermath.

But the COVID effect dissipates rapidly, with a half-life of just 9 months. By 12 months, the coefficient has fallen to roughly half its peak value and is no longer significant. By 48 months, the point estimate is essentially zero ($+0.010$) with a wide confidence interval spanning zero---the cross-state relationship has effectively disappeared. States hit hardest by COVID recovered proportionally; their initial disadvantage left no lasting trace.

The speed of recovery is remarkable given the severity of the initial shock. The COVID Bartik shock has a larger cross-state standard deviation than the Great Recession Bartik (0.024 versus 0.010; see \Cref{tab:summary}), and peak-to-trough employment declines were 2.6 times larger. Yet within 18 months, the cross-state relationship between initial exposure and employment had vanished.
...
National JOLTS data confirm the mechanism. \Cref{fig:jolts_main} presents four key labor market flow rates during and after each recession. During the Great Recession, layoffs rose moderately but quits and job openings collapsed for five-plus years, reflecting sustained demand deficiency. The quit rate---a barometer of worker confidence---did not return to pre-recession levels until mid-2016, eight years after the NBER peak. During COVID, layoffs spiked to 11 million in March 2020 but normalized within three months; quits surged past pre-pandemic levels during the ``Great Resignation,'' and job openings reached record highs by spring 2021. The JOLTS evidence reinforces the central finding: the Great Recession destroyed labor demand for years, while COVID temporarily disrupted production without impairing the labor market's matching capacity.
...
\begin{minipage}{0.93\textwidth}
\small \textit{Notes:} National JOLTS data, seasonally adjusted. Top panels: layoffs and discharges (left) and quits (right). Bottom panels: job openings (left) and hires (right). Blue shows Great Recession window; red shows COVID window.
\end{minipage}
\end{figure}
...
& (17.4341) & (8.3801) & (3.3745) & (4.2800) & (3.8794) & & & \\[3pt]
...
\textit{No skill depreciation ($\lambda = 0$).} The employment decline at 48 months falls from 6.0\% to approximately 3.2\%, and the welfare loss from 39.6\% to 16.6\%. Skill depreciation accounts for \textbf{58\% of the demand shock's welfare cost}.

\textit{No participation exit ($\chi = 0$).} The welfare loss falls only from 39.6\% to 38.8\%---about 2\% of total welfare cost. Participation exit is empirically observable but quantitatively secondary to skill depreciation.
...
\Cref{tab:welfare} summarizes the welfare implications. The consumption-equivalent welfare loss from the demand shock is 39.6\%; from the supply shock, just 0.09\%. The demand/supply welfare ratio is \textbf{442:1}.
...
This enormous asymmetry arises from two sources: permanence (undiscounted output losses are larger) and amplification (the scarring channel compounds the initial productivity loss). Without scarring, the demand shock's welfare cost falls nearly in half but still exceeds the supply shock's by a factor of approximately 180. These welfare comparisons should be interpreted as illustrating the order-of-magnitude asymmetry between demand and supply shock costs rather than as precise point estimates; \Cref{tab:app_model_sensitivity} explores sensitivity to key calibration parameters.

These comparisons are illustrative of the asymmetry's magnitude rather than precise point estimates. The 442:1 ratio reflects risk-neutral utility and the absence of consumption smoothing (borrowing, savings); under CRRA preferences with risk aversion greater than one, the welfare costs would be substantially lower. The ratio is also sensitive to the discount rate and $\lambda$ (\Cref{tab:app_model_sensitivity} shows halving $\lambda$ to 0.06 reduces the CE loss to 30.9\%, while increasing to 0.18 raises it to 48.7\%). What is robust across all specifications is the qualitative asymmetry---demand shocks are orders of magnitude more costly---not the precise numbers. The welfare decomposition (skill depreciation 58\%, direct productivity 40\%, participation exit 2\%) is reported in \Cref{tab:welfare}.
...
A calibrated DMP model provides a unified explanation---and reveals the staggering magnitude of the asymmetry. Demand shocks depress hiring, extend durations, trigger human capital loss, and push workers out of the labor force---a vicious cycle that amplifies the initial shock into a 39.6\% consumption-equivalent welfare loss. Supply shocks create mass unemployment but preserve short durations, with a welfare cost of just 0.09\%. The demand-to-supply ratio is 442:1. Skill depreciation accounts for 58\% of the demand shock's welfare cost.
...
The total match surplus is $S(h) = W(h) - \max\{U(h, 0), V^{OLF}\} + J(h)$. Standard Nash bargaining with worker power $\gamma$ implies:
\begin{equation}
W(h) - \max\{U(h, 0), V^{OLF}\} = \gamma S(h), \quad J(h) = (1 - \gamma) S(h).
\label{eq:app_nash}
\end{equation}
...
\end{align}
The terminal value terms $\frac{\beta^T}{1-\beta}[\cdots]$ capture the infinite continuation beyond the simulation horizon, evaluated at the period-$T$ allocation. For $W^{ss}$, this is the steady state; for $W^{shock}$, this is the post-transition allocation (which for permanent demand shocks converges to a new, lower steady state). With $\beta = 0.996$ and $T = 120$, the terminal value accounts for approximately 60\% of total discounted welfare, making it essential for accurate computation. The CE welfare loss is:
\begin{equation}
\Delta = 1 - \frac{W^{shock}}{W^{ss}}.
\label{eq:app_CE}
\end{equation}
A value of $\Delta = 0.396$ means that a permanent 39.6\% reduction in steady-state consumption would leave the worker equally well off as experiencing the demand shock transition.
...
\begin{tablenotes}[flushleft]
\small
\item \textit{Notes:} Each column reports the coefficient from a cross-state regression of the change in the unemployment rate on recession exposure at horizon $h$ months. Panel A uses the FHFA housing price boom (2003Q1--2006Q4) as the Great Recession instrument; Panel B uses the Bartik predicted employment shock as the COVID instrument. The large COVID Bartik coefficients reflect the small standard deviation of the Bartik shock (0.023); a one-SD shock implies less than 1.5 percentage points of UR change at $h=3$. \textsuperscript{a}$h=60$ exceeds the 48-month post-COVID observation window. The negative COVID Bartik coefficients arise because the Bartik shock is itself negative (more exposed states have more negative Bartik values); hence a negative coefficient implies that greater exposure \textit{increases} the unemployment rate. HC1 robust standard errors in parentheses. $^{*}$~$p<0.10$, $^{**}$~$p<0.05$, $^{***}$~$p<0.01$.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\begin{tablenotes}[flushleft]
\small
\item \textit{Notes:} Each row uses a different base year for the pre-recession industry shares in the Bartik construction. COVID coefficients are sign-reversed (multiplied by $-1$) so that negative values indicate employment loss, consistent with Table~\ref{tab:main}. HC1 robust standard errors in parentheses. $^{*}$~$p<0.10$, $^{**}$~$p<0.05$, $^{***}$~$p<0.01$.
\end{tablenotes}
\end{threeparttable}
\end{table}
