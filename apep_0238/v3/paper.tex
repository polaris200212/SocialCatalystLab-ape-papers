...
This paper contributes to four literatures. First, I advance the macroeconomic hysteresis literature pioneered by \citet{blanchard1986hysteresis} and extended by \citet{cerra2008growth}, \citet{jorda2013sovereigns}, and \citet{cerra2023hysteresis}, who document persistent output losses across a large cross-section of recessions and financial crises. Recent contributions by \citet{yagan2019employment}, \citet{barnichon2022labor}, and \citet{dupraz2022plucking} have reinvigorated interest in hysteresis mechanisms. While this literature has established that recessions \textit{can} leave permanent scars, it has not systematically examined \textit{which types} of recessions scar most. I provide the first direct empirical comparison of demand-driven versus supply-driven recession dynamics using the same identification framework and the same labor markets observed across two distinct episodes.
...
The model's asymmetric prediction---persistent effects from demand shocks, transient effects from supply shocks---connects to the ``plucking'' model of \citet{dupraz2022plucking}, who show that business cycles exhibit an asymmetry in which deep contractions are followed by proportional recoveries, while shallow recessions may not fully recover. My framework provides a microfoundation for one source of this asymmetry: supply shocks ``pluck'' the economy below its ceiling but allow a full snapback, while demand shocks permanently lower the ceiling itself through the scarring channel.
...
The analysis sample is a balanced panel of all 50 U.S.\ states observed monthly from January 2000 through June 2024 ($50 \times 294 = 14{,}700$ state-month observations). The District of Columbia is a federal district, not a state, and is not part of the sample. The cross-sectional local projections use all 50 states for the Great Recession analysis and 48 contiguous states for the COVID analysis (excluding Alaska and Hawaii for geographic comparability).
...
I adopt local projections rather than two-way fixed effects (TWFE) or staggered difference-in-differences for two reasons. First, the cross-state variation in exposure is continuous (housing boom magnitude, Bartik shock intensity), not a discrete treatment adoption. The recent staggered DiD literature \citep{goodmanbacon2021difference, callaway2021difference} addresses bias from heterogeneous treatment timing, but this concern does not apply here because my design exploits cross-sectional variation in shock intensity at a single, common event date rather than staggered policy adoption across states. Second, the LP framework directly estimates the horizon-specific impulse response without requiring parametric assumptions about the evolution of treatment effects over time.

Local projections have several advantages over vector autoregressions (VARs) for this application. First, they are robust to misspecification of the lag structure, which is important given the highly nonlinear dynamics of the Great Recession \citep{ramey2016macroeconomic, ramey2018government}. \citet{plagborg2021local} establish that LPs and VARs estimate the same impulse responses when the lag structure is correctly specified, but LPs are more robust when it is not. Second, they directly estimate the object of interest---the impulse response at each horizon---without requiring the recursive inversion of estimated autoregressive coefficients. Third, they naturally accommodate the different sample lengths available for the two recessions (120 months post-Great Recession versus 52 months post-COVID).

\textit{Estimation approach.} The estimates reported throughout this paper are reduced-form local projections: I regress employment outcomes directly on the exposure measure $Z_s$ (housing boom or Bartik shock), not on an endogenous treatment instrumented by $Z_s$. I use the term ``instrument'' in the sense of an exogenous source of variation in recession exposure, following the reduced-form tradition in \citet{mian2014explains} and \citet{autor2013china}. The coefficient $\beta_h$ captures the total effect of exogenous recession exposure on subsequent employment outcomes at horizon $h$---an intent-to-treat (ITT) parameter that subsumes all channels through which exposure operates, including direct employment effects, migration responses, industry reallocation, and policy adjustments. This is the appropriate estimand when the goal is to trace out how exogenous exposure maps into employment dynamics, rather than to estimate the local average treatment effect (LATE) of a specific endogenous variable such as the initial employment decline.
...
\textit{Relevance.} States with larger housing booms experienced larger busts, which destroyed more household wealth and led to deeper employment contractions through the household demand channel. The relationship between housing exposure and employment outcomes is well-established in the literature and is strong in my data: the cross-sectional $R^2$ of the reduced-form LP regressions exceeds 0.10 at horizons from 6 through 48 months, reaching 0.12 at $h = 6$ (\Cref{tab:main}), indicating that the housing boom explains a substantial share of cross-state employment variation. To be precise about the estimand: these $R^2$ values measure the reduced-form predictive power of exogenous housing exposure for subsequent employment outcomes. The housing boom is not an instrument in the classical IV sense---I do not estimate a first-stage regression of an endogenous variable on the instrument and then use predicted values in a second stage. Rather, I estimate the reduced-form relationship directly, in the tradition of \citet{mian2014explains} and \citet{autor2013china}. The $R^2$ values of 0.10--0.13 across horizons serve as a measure of the instrument's relevance for capturing meaningful cross-state variation in recession exposure, analogous to a first-stage partial $R^2$ in a standard IV framework.
...
Because the regressions are cross-sectional (one observation per state at each horizon), I use heteroskedasticity-robust (HC1) standard errors throughout. The effective sample sizes are 50 states for the Great Recession analysis and 48 states for the COVID analysis (contiguous states). All impulse response figures display 95\% confidence intervals constructed from HC1 standard errors. In addition to conventional $p$-values, I report permutation $p$-values from 1,000 random reassignments of the exposure measure across states, following \citet{adao2019shift}. These permutation $p$-values appear in brackets in \Cref{tab:main} and provide exact finite-sample inference without distributional assumptions.

Small-sample inference with 48--50 observations warrants particular care. I pursue three complementary strategies. First, permutation tests (1,000 random reassignments of $Z_s$) provide exact finite-sample $p$-values that do not rely on asymptotic approximations. Second, I cluster standard errors by census division (9 clusters) and show that significance is preserved despite the widening of standard errors (\Cref{app:robustness}). Third, I conduct systematic leave-one-out analysis, confirming that no individual state drives the results.

Wild cluster bootstrap-$t$ \citep{cameron2008bootstrap} is the standard finite-sample correction for clustered data, but it requires a sufficient number of clusters to produce reliable inference. With only 9 census divisions, the bootstrap distribution is too coarse to yield well-calibrated rejection rates, making the procedure infeasible for this application. The permutation approach---which constructs the exact null distribution by reassigning the exposure measure across all $N$ observations rather than resampling within clusters---provides a more appropriate finite-sample correction that does not depend on the number of clusters.

For the Bartik instrument, the shift-share structure introduces a specific correlation pattern: states with similar industry compositions share exposure to common industry-level shocks. I implement the \citet{adao2019shift} exposure-robust standard errors, which account for this correlation structure. The \citet{borusyak2022quasi} framework provides a complementary perspective, showing that the Bartik estimator is valid when industry-level shocks are exogenous---a condition that is highly plausible for the COVID pandemic, which struck industries based on intrinsic contact intensity rather than geographic distribution. Both frameworks yield qualitatively similar conclusions to the HC1 baseline, with modestly wider confidence intervals that do not alter the significance of the Great Recession scarring results at horizons through 60 months.

A separate concern is that the local projection framework estimates coefficients at each horizon independently, raising the possibility that apparent persistence reflects multiple testing rather than a genuine dynamic pattern. I do not apply formal multiple testing corrections (e.g., Bonferroni) because the horizon-specific coefficients are not independent tests of unrelated hypotheses---they trace a single impulse response function. The joint significance pattern---persistently negative coefficients across all post-recession horizons for the Great Recession, with rapid convergence for COVID---provides stronger evidence than any individual horizon considered in isolation. The permutation tests reinforce this interpretation: under the null of no relationship, the probability of observing the entire observed coefficient path across all horizons is vanishingly small.
...
\textit{Compositional changes and migration.} The LP coefficients capture the effect on the \textit{level} of employment in each state, which could reflect both intensive margin (hours) and extensive margin (number of jobs) effects, as well as migration responses. Migration operates as an equilibrium adjustment mechanism: workers may leave hard-hit states, improving measured employment rates in those states while transferring the scarring burden to destination labor markets. This means the LP coefficients likely understate the true scarring effect on workers originally residing in exposed states \citep{notowidigdo2020incidence, amior2021workers}---migration attenuates measured place-based scarring, making the estimates conservative.

Three considerations mitigate the concern that migration confounds the demand-supply comparison. First, \citet{yagan2019employment} demonstrates that individual-level employment scarring from the Great Recession persists even after controlling for migration---workers who lived in hard-hit commuting zones in 2007 remained less likely to be employed in 2015 regardless of whether they moved. This establishes that scarring is a worker-level phenomenon, not merely a place-based artifact. Second, \citet{dao2017regional} document a secular decline in interstate migration responsiveness, indicating that the migration adjustment channel has weakened over time and that state-level employment dynamics increasingly reflect local conditions rather than spatial reallocation. Third, if migration were the primary driver of the observed persistence asymmetry, we would expect similar out-migration from COVID-hit states---yet the COVID recovery was uniformly rapid across states regardless of severity, suggesting that migration played a minimal equilibrating role during the supply-shock episode. The fact that Great Recession scarring persists at 84 months despite any migration adjustment underscores the severity and durability of the demand-shock mechanism. Incorporating state-level net migration flows from IRS or ACS data as a robustness control would sharpen the worker-versus-place distinction, and I note this as a valuable extension for future research.

\textit{Policy endogeneity.} Fiscal and monetary policy responses differed dramatically across the two recessions, which could confound the comparison. The COVID recession was met with faster and larger fiscal transfers---the CARES Act (\$2.2 trillion, enacted March 2020), PPP (\$800 billion in forgivable payroll loans), enhanced unemployment insurance (\$600/week supplement), and multiple rounds of stimulus checks---whereas the Great Recession's primary fiscal response was the ARRA (\$787 billion, enacted February 2009, 14 months after the recession began). The scale, speed, and design of these interventions differed enormously, and state-level exposure to federal programs varied with local economic conditions.

This creates a fundamental identification challenge: conditioning on state-level fiscal support intensity (e.g., PPP disbursements per capita or ARRA spending per capita) would introduce post-treatment bias, because the intensity of fiscal transfers is itself endogenous to the severity and type of the local downturn. States hit hardest by the COVID supply shock received more PPP funding precisely because more of their businesses faced temporary closure. Similarly, ARRA spending flowed disproportionately to states with higher unemployment, which were the same states with greater housing exposure. Including these as controls would absorb part of the causal channel through which shock type affects recovery, biasing the estimates toward zero.

I therefore interpret the reduced-form estimates as capturing the total effect of shock type, inclusive of the endogenous policy response. I discuss this more fully in the mechanisms section (\Cref{sec:mechanisms}) and argue that the policy response was different precisely because the shock was different---supply shocks invite match-preserving programs like PPP, while demand shocks require demand-side stimulus that is harder to design and slower to deploy. Disentangling the ``pure'' shock-type effect from the policy-response channel requires a structural model of the fiscal response, which I leave to future work.

\textit{Small sample.} With 48--50 observations per regression, finite-sample concerns are relevant. I address this through permutation inference and show that results are robust to dropping individual states (leave-one-out analysis).
...
Before examining the post-recession results, I verify that the identifying instruments are uncorrelated with pre-existing employment trends. \Cref{fig:pretrend} plots the event-study coefficients from regressing log employment change on recession exposure at horizons spanning 36 months before through 120 months after each recession peak.
...
\small \textit{Notes:} Each point plots $\hat{\beta}_h$ from \Cref{eq:lp} at horizon $h$ months relative to the recession peak ($h=0$, vertical dashed line). Pre-period coefficients ($h < 0$) test for differential pre-trends. Left panel: Great Recession with housing price instrument. Right panel: COVID with Bartik instrument. Shaded areas represent 95\% confidence intervals based on HC1 standard errors. Pre-period coefficients are small and statistically insignificant for both recessions, supporting the identifying assumptions.
\end{minipage}
\end{figure}

The pre-period coefficients are reassuring. For the Great Recession, none of the pre-trend coefficients at $h = -36$, $-24$, or $-12$ months is statistically significant, and the point estimates are small relative to the post-recession effects. States that experienced larger housing booms were not on differential employment trajectories before the recession. For COVID, the pre-trend coefficients are similarly small and insignificant, confirming that the Bartik instrument captures pandemic-induced exposure rather than pre-existing differences. The sharp break at $h = 0$ in both panels---flat pre-trends followed by diverging post-trends---supports the causal interpretation of the LP estimates.
...
The persistence of the Great Recession effect is the paper's central finding. At $h = 60$ months (five years after the recession peak), the coefficient remains $-0.0752$ ($p < 0.10$). At $h = 84$ months (seven years), it is still $-0.0690$ ($p < 0.05$). The effect does begin to attenuate at very long horizons---by $h = 120$ (ten years), the coefficient is essentially zero ($-0.0046$, not significant) and the $R^2$ is just 0.001---but this reflects a decade of gradual convergence driven by migration and demographic turnover, not a rapid recovery. The $R^2$ pattern is informative: housing exposure explains about 12\% of cross-state employment variation at 6 months, and this explanatory power declines gradually but remains above 5\% through 60 months, confirming that the housing boom's influence persisted for years before finally dissipating.
...
\textit{Layoffs.} During the Great Recession, layoffs rose moderately from about 1.8 million per month to a peak of 2.6 million in February 2009, elevated gradually and receded slowly. COVID produced a dramatic spike---layoffs exceeded 11 million in March 2020, more than five times the Great Recession peak---but fell back to pre-recession levels by June 2020, just three months later.

\textit{Quits.} The quit rate---a key indicator of worker confidence---remained depressed for years after the Great Recession, not returning to pre-recession levels until mid-2016. After COVID, quits exceeded pre-pandemic levels by early 2021 and surged to record highs during the ``Great Resignation'' of 2021--2022, signaling a rapid restoration of worker bargaining power and labor market fluidity.
...
\textit{Hires.} The Great Recession suppressed hiring for years, but COVID's hiring rebound came rapidly---the hires rate exceeded pre-pandemic levels by mid-2020. This rapid normalization contradicts a scarring interpretation and instead reflects a temporary disruption of the production process.
...
The reduced-form evidence establishes a stark empirical fact: demand recessions scar while supply recessions do not. The natural follow-up question is \textit{why}. The LP estimates document the persistence asymmetry but cannot, on their own, decompose it into specific mechanisms---skill depreciation, labor force exit, or the direct productivity effect. To quantify the relative contributions of these channels and to discipline the welfare analysis, I now turn to the structural model developed in \Cref{sec:framework}.
...
The model-data comparison warrants a caveat at long horizons. Beyond approximately 80 months, the model's demand-shock employment path continues deepening (reaching $-13.0\%$ at 120 months) while the LP estimates show gradual convergence toward zero. This divergence reflects the model's deliberate abstraction from mean-reverting forces that operate at very long horizons: new labor market entrants replace scarred cohorts, housing markets slowly normalize, delayed fiscal stimulus (later rounds of QE, extended unemployment benefits) accumulates, and demographic turnover gradually refreshes the workforce. The model captures the scarring mechanism---the vicious cycle of skill depreciation and labor force exit---but not all recovery forces, which is why it overpredicts persistence at $h > 80$ months. The model should therefore be interpreted as a disciplined account of the scarring channel's contribution to persistence, not as a complete model of the full transition path.
...
These welfare comparisons should be interpreted as illustrative of the asymmetry's economic magnitude rather than as precise point estimates. The 147:1 demand-to-supply welfare ratio and the 33.5\% CE loss are sensitive to several calibration choices: the assumption of risk neutrality (risk-averse agents would weight the demand shock's persistent losses even more heavily), the discount rate ($\beta = 0.996$; lower discounting amplifies long-horizon losses), the skill depreciation rate ($\lambda = 0.12$; \Cref{tab:app_model_sensitivity} shows that halving $\lambda$ to 0.06 reduces the CE loss to 26.1\%, while increasing it to 0.18 raises the loss to 41.2\%), and the permanence of the demand shock. The sensitivity analysis in \Cref{tab:sensitivity} and \Cref{tab:app_model_sensitivity} demonstrates that while the magnitudes vary across the parameter space, the qualitative conclusion---that demand shocks impose welfare costs orders of magnitude larger than comparable supply shocks---is robust. What is robust is the asymmetry, not the precise numbers.
...
The baseline specification uses HC1 robust standard errors. I verify robustness using two alternative inference procedures. First, clustering standard errors by census division (9 clusters) allows for arbitrary within-division correlation. This widens confidence intervals modestly but does not change the qualitative conclusions---the Great Recession coefficients remain significant through $h = 60$ months.
...
A calibrated DMP search model with endogenous participation and skill depreciation provides a unified explanation---and reveals the staggering magnitude of the asymmetry. Demand shocks depress hiring, extend unemployment durations, trigger human capital loss, and push discouraged workers out of the labor force---a vicious cycle that amplifies the initial shock into a consumption-equivalent welfare loss of 33.5\%. Supply shocks create mass unemployment but preserve short durations, allowing rapid reabsorption before scarring mechanisms activate, with a welfare cost of just 0.23\%. The demand-to-supply welfare ratio is 147:1. Skill depreciation accounts for 51\% of the demand shock's welfare cost, identifying the specific mechanism that makes demand recessions so destructive.
...
\label{eq:app_hpi}
\end{equation}
where $P_{s,t}$ is the FHFA all-transactions house price index for state $s$ at quarter $t$. The FHFA index is a repeat-sales index based on price changes of properties with mortgages purchased or guaranteed by Fannie Mae or Freddie Mac. FHFA publishes state-level house price indices for all 50 states. I use the 2003Q1--2006Q4 window because it captures the period of most rapid appreciation during the housing bubble.
...
\midrule
$N$ & \multicolumn{6}{c}{50 (GR) / 48 (COVID)} \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
\small
...
\input{tables/tab7_subsample.tex}

\subsection{Model Parameter Sensitivity}

The calibrated DMP model uses a skill depreciation parameter $\lambda = 0.12$ and a demand shock size of $\Delta a = 5\%$. \Cref{tab:sensitivity} reports the employment impact at $h = 48$ months across a grid of alternative values for $\lambda$ and $\Delta a$. The qualitative result---persistent demand-shock scarring---is robust across all parameter combinations. Larger $\lambda$ (more severe skill depreciation) and larger $\Delta a$ (deeper demand collapse) produce quantitatively larger employment losses, as the model predicts. The supply shock recovery time (not shown) is 9 months across all $\lambda$ values, confirming that the transience of supply shocks is not sensitive to the scarring parameter.
