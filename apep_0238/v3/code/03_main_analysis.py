...
v3: adds permutation inference, pre-trend estimation, subsample robustness
"""
...
def permutation_test(paths, instrument, horizons, n_perms=1000, seed=42):
    """
    ...
    actual = lp_ols(paths, instrument, horizons)
    ...
    for _ in range(n_perms):
        ...
        perm_results = lp_ols(paths, perm_instrument, horizons)
...
gr_perm_pvals = permutation_test(gr_paths, hpi_boom, perm_horizons_gr, n_perms=1000)
for h in perm_horizons_gr:
    print(f"  h={h:3d}: perm p-value = {gr_perm_pvals[h]:.3f}")

print("\n--- COVID (Bartik instrument) ---")
covid_perm_pvals = permutation_test(covid_paths, bartik_covid, perm_horizons_covid, n_perms=1000)
...
# Estimate event-study LP for both
gr_event_results = lp_ols(gr_pretrend_paths, hpi_boom, all_event_horizons_gr)
covid_event_results = lp_ols(covid_pretrend_paths, bartik_covid, all_event_horizons_covid)
...
for region, region_states in CENSUS_REGIONS.items():
    ...
    if len(region_paths) < 5:
        ...
        continue
    region_res = lp_ols(region_paths, hpi_boom, [60], min_n=5)
...
for label, state_list in [('Large (above median)', large_states), ('Small (below median)', small_states)]:
    ...
    sub_res = lp_ols(sub_paths, hpi_boom, [60])
