...
\small
\item \textit{Notes:} Panel A reports summary statistics for the balanced state-month panel. Nonfarm payrolls are from BLS Current Employment Statistics via FRED. Labor force participation rate has fewer observations ($N = 5{,}880$) because BLS LAUS seasonally adjusted state-level LFPR series are available for a shorter time span than employment and unemployment rate series. Panel B reports cross-state recession exposure measures. Bartik shocks are constructed using pre-recession industry employment shares weighted by national industry employment changes. Housing price boom is the log change in the FHFA state-level house price index from 2003Q1 to 2006Q4.
\end{tablenotes}
\end{threeparttable}
\label{tab:summary}
\end{table}
