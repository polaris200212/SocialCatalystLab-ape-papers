...
class DMPModel:
    ...
    def wage(self, a=None, h=1.0, theta=None):
        ...
        This omits the κθ term from the full Nash wage, consistent with
        the free-entry condition (Eq. 11) which yields closed-form θ.
        See Pissarides (2000, Ch. 1) for this standard simplification.
        """
...
def compute_welfare_loss(path, ss, model, T=None):
    """Compute consumption-equivalent welfare loss with terminal value.

    Uses the full simulated path (which may be >120 months) and applies
    the terminal-value approximation only after the path has converged.
    """
    if T is None:
        T = len(path['E']) - 1  # Use full simulated path
    beta = model.beta

    # SS welfare: PV of consumption in SS + terminal value
    ss_period = ss['e'] * ss['w'] + ss['u'] * model.b + ss['o'] * model.b_olf
    ss_welfare = sum(beta**t * ss_period for t in range(T))
    ss_welfare += (beta**T / (1 - beta)) * ss_period  # terminal value

    # Shock welfare + terminal value at period-T allocation
    shock_welfare = 0
    for t in range(T):
        E_t = path['E'][t]
        U_t = path['U'][t]
    ...
    # Terminal value: continuation at period-T allocation
    E_T = path['E'][T]
    U_T = path['U'][T]
    O_T = path['O'][T]
    w_T = path['w'][T]
    terminal_period = E_T * w_T + U_T * model.b + O_T * model.b_olf
    shock_welfare += (beta**T / (1 - beta)) * terminal_period

    # CE loss: 1 - (shock_welfare / ss_welfare)
    ce_loss = 1 - shock_welfare / ss_welfare
    return ce_loss

# Simulate extended paths for welfare (T=600 to ensure convergence)
T_WELFARE = 600
...
for label, pth in [('Demand', demand_path_long), ('Supply', supply_path_long),
    ...
    print(f"  {label}: emp@120={emp_120:.6f}, emp@600={emp_600:.6f}, "
          f"|Δ|={delta:.6f}, monthly change last year={monthly_change:.8f}")

demand_welfare = compute_welfare_loss(demand_path_long, ss, model)
supply_welfare = compute_welfare_loss(supply_path_long, ss, model)
cf1_welfare = compute_welfare_loss(cf1_path_long, ss, model_noscar)
cf2_welfare = compute_welfare_loss(cf2_path_long, ss, model_noolf)
...
# Save model results
# ──────────────────────────────────────────────────────────────
# Store display-length paths (T=120) for figures, plus long paths for welfare
model_results = {
    'steady_state': ss,
...
for name, overrides in welfare_scenarios:
    m = DMPModel(overrides)
    ss_m = m.steady_state()
    path_m = m.simulate_transition('demand', shock_size=0.05, T=T_WELFARE)
    welf = compute_welfare_loss(path_m, ss_m, m)
    emp48 = path_m['emp_change'][48]
    print(f"  {name}: emp_48={emp48:.4f}, CE welfare loss={welf*100:.2f}%")
    welfare_sens_results.append({
        'name': name,
        'overrides': overrides,
        'emp_48': float(emp48),
        'welfare_loss': float(welf),
    })
...
print(f"\n✓ All model results saved (including welfare sensitivity)")
