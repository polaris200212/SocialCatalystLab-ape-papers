...
\title{Demand Recessions Scar, Supply Recessions Don't:\\ Evidence from State Labor Markets\footnote{This paper is revision v7 of APEP-0238. See \url{https://github.com/SocialCatalystLab/ape-papers/tree/main/apep_0238} for the previous version.}}
...
Within each period, the following events occur. First, aggregate shocks are realized (changes to $a$ or the separation rate $\delta$). Second, employed workers separate from their jobs with probability $\delta_t$, entering unemployment. In steady state, $\delta$ reflects the average monthly separation rate. A supply shock operates by temporarily elevating $\delta_t$ above its steady-state value. Third, three transitions operate as \textit{competing hazards} on the unemployment pool: workers match with vacancies (probability $f(\theta_t)$), exit to non-participation (probability $\chi_t$), or remain unemployed (probability $1 - f(\theta_t) - \chi_t$). Separately, a fraction $\psi$ of non-participants re-enter unemployment. Fourth, unemployed workers who have been searching for at least $d^*$ periods suffer human capital depreciation: their productivity falls from $h$ to $h(1 - \lambda)$, where $\lambda \in (0,1)$ parameterizes the severity of scarring.
...
The value of employment for a worker with human capital $h$ determines the worker's willingness to accept job offers and remain employed:
\begin{equation}
W_t(h) = w_t(h) + \beta \left[ (1 - \delta_t) W_{t+1}(h) + \delta_t \cdot U_{t+1}(h, 0) \right],
\label{eq:value_employed}
\end{equation}
where $w_t(h)$ is the wage and separated workers enter unemployment with duration $d = 0$. The value of unemployment for a worker with human capital $h$ and duration $d$ periods is:
\begin{equation}
U_t(h, d) = b + \beta \left[ f(\theta_t) W_{t+1}(h) + \chi_t V^{OLF}_{t+1} + (1 - f(\theta_t) - \chi_t) U_{t+1}(h', d+1) \right],
\label{eq:value_unemployed}
\end{equation}
where $b$ is the flow value of unemployment (unemployment insurance benefits plus the value of leisure), $\chi_t$ is the probability of exiting to non-participation (a competing hazard with job finding), and $h'$ reflects potential skill depreciation:
\begin{equation}
h' = \begin{cases} h & \text{if } d < d^* \\ h(1 - \lambda) & \text{if } d \geq d^* \end{cases}.
\label{eq:depreciation}
\end{equation}
Workers who find employment before reaching duration $d^*$ retain their full human capital; depreciation applies only to those who remain unemployed beyond the threshold.

The value of non-participation is:
\begin{equation}
V^{OLF} = b_{OLF} + \beta \left[ \psi \cdot U_{t+1}(h_0, 0) + (1 - \psi) \cdot V^{OLF} \right],
\label{eq:value_olf}
\end{equation}
where $b_{OLF}$ is the flow value of home production, assumed to be less than the unemployment benefit ($b_{OLF} < b$), $h_0$ is the human capital at re-entry (assumed to be the pre-scarring level for simplicity), and re-entrants start with zero unemployment duration.

The reduced-form hazard $\chi_t$ in \Cref{eq:exit_rate} captures the participation margin: when unemployment durations are long and human capital has depreciated, more workers exit the labor force. This is the channel through which demand recessions generate permanent employment losses. The competing-hazard formulation---where job finding and OLF exit are parallel transitions from unemployment---is consistent with the aggregate laws of motion in \Cref{eq:lom_u,eq:lom_o}.

\subsection{Wage Determination}

Standard Nash bargaining with the stated Bellman equations yields a wage that depends on market tightness: $w = (1 - \gamma)b + \gamma(ah + \kappa\theta)$. For tractability in the transition dynamics, I adopt a simplified proportional-surplus wage rule that eliminates the tightness dependence:
\begin{equation}
...
\paragraph{Simulation approach.} The full model with duration-dependent depreciation and endogenous participation involves a high-dimensional state space (the joint distribution of workers over employment status, human capital, and unemployment duration). Rather than solve the Bellman system numerically at each point in time, I simulate the aggregate dynamics using a tractable approximation: the scarred fraction $s_t$ evolves as a reduced-form function of the job-finding rate (detailed in \Cref{app:model}), and the participation exit rate responds to $s_t$ through \Cref{eq:exit_rate}. The micro Bellman structure above provides the economic intuition; the quantitative simulations use the aggregate dynamic system. This approach follows the spirit of \citet{shimer2005cyclical}, who also works with aggregate flows rather than the full distribution.
...
\textit{Sign convention.} For the Great Recession, negative $\beta_h$ indicates that more-exposed states experienced larger employment declines. For COVID, the Bartik shock is constructed so that negative values indicate larger predicted employment declines. I multiply the COVID LP coefficients by $-1$ so that negative $\hat{\beta}_h$ indicates employment loss in both panels, matching the Great Recession convention. In both cases, convergence toward zero indicates recovery.
...
The scars do not heal. At five years, the coefficient remains $-0.0489$. At seven years, it is $-0.0507$. The effect attenuates at very long horizons---by ten years, it is $-0.0229$---but this reflects a decade of gradual convergence driven by migration and demographic turnover, not rapid recovery. Housing exposure explains about 37\% of cross-state employment variation at 6 months, declining gradually but remaining above 22\% through 60 months.

\Cref{tab:halflife} quantifies the persistence. Among the discrete horizons reported in \Cref{tab:main}, the peak LP response occurs at 48 months ($\hat{\beta}_{48} = -0.0527$). The half-life---computed from the full LP horizon grid estimated at 3-month intervals---is approximately 60 months from the peak. The effect remains within 90\% of its peak value through $h = 84$ months (as visible in Table~\ref{tab:main}, where $\hat{\beta}_{84} = -0.0507$) and crosses the half-peak threshold ($-0.0264$) between $h = 108$ and $h = 111$ months. The long plateau followed by late attenuation is consistent with gradual out-migration and demographic turnover eventually eroding cross-state differentials. As a summary measure of long-run persistence, I report the average effect across the four long-horizon estimates ($h \in \{48, 60, 84, 120\}$): $\bar{\beta}_{LR} = -0.037$ (wild cluster bootstrap 95\% CI: $[-0.069, -0.005]$). This single statistic avoids the multiple-testing concern inherent in interpreting horizon-by-horizon significance and confirms that long-run scarring is statistically distinguishable from zero at conventional levels.
...
\Cref{tab:horse_race} reports the results. The HPI coefficient retains its magnitude and significance at $h=6$ ($\hat{\beta}^{HPI} = -0.024$, $p < 0.05$), while the GR Bartik coefficient is positive and insignificant at all horizons. The variance inflation factor of 1.80 indicates moderate collinearity that does not compromise inference. This within-recession test strongly favors the demand interpretation: the housing wealth channel, not industry composition, generates persistence.
...
\midrule
Peak response ($\hat{\beta}_{peak}$) & $-0.0527$ & $-0.0193$ \\
Peak horizon (months) & 48 & 3 \\
Half-life (months) & 60 & 9 \\
$\hat{\beta}_{48}$ & $-0.0527$ & $0.0002$ \\
Persistence ratio ($\hat{\beta}_{48} / \hat{\beta}_{peak}$) & $1.000$ & $-0.010$ \\
\midrule
...
The model predicts that demand recessions scar through prolonged unemployment, while supply recessions preserve short durations. I test this directly by comparing the persistence of unemployment rate responses across recessions. If the scarring mechanism operates through duration, the UR response to the Great Recession should remain elevated at long horizons ($h = 24$--48 months), while the COVID UR response should converge to zero.

\Cref{tab:mechanism} reports the results. The Great Recession UR persistence ratio---defined as $\hat{\beta}_{48}/\hat{\beta}_{12}$---is 1.857, meaning the cross-state unemployment dispersion nearly \textit{doubled} between one and four years. This deepening is the hallmark of the scarring channel: initial job losses create long durations, which erode skills, which further depress reemployment, which extends durations further. By contrast, the COVID persistence ratio is 0.084, indicating that 91.6\% of the initial UR dispersion had dissipated by $h = 48$. COVID UR coefficients are now expressed per standard deviation of Bartik exposure, making them directly interpretable: a one-standard-deviation increase in COVID exposure raises the unemployment rate by 0.81 percentage points at $h = 6$, declining to just 0.03 points by $h = 48$. The asymmetry is dramatic: demand recessions compound through the duration channel; supply recessions self-correct.
...
The reduced-form evidence establishes \textit{that} demand recessions scar while supply recessions do not. To understand \textit{why}---and to quantify how much each channel contributes---I turn to the search-and-matching model developed in \Cref{sec:framework}. The LP estimates document the persistence asymmetry; the structural model decomposes it into skill depreciation, labor force exit, and direct productivity effects.
...
%% \input{tables/tab6_calibration.tex}  %% INLINED
\begin{table}[H]
\centering
...
The monthly discount factor $\beta = 0.996$ implies an annual discount rate of approximately 4.9\% ($\beta^{-12} - 1 \approx 0.049$). The matching function elasticity $\alpha = 0.5$ follows \citet{petrongolo2001looking} and satisfies the \citet{hosios1990welfare} condition with equal bargaining power $\gamma = 0.5$. Under the simplified proportional-surplus wage rule (Equation~\ref{eq:wage}), the Hosios condition does not strictly apply as it does under full Nash bargaining; $\gamma = 0.5$ is chosen as the standard symmetric parameterization rather than a strict efficiency condition. Matching efficiency $A = 0.60$ and vacancy cost $\kappa = 3.40$ are calibrated jointly to match the average monthly job finding rate of 0.40 \citep{shimer2005cyclical}. The separation rate $\delta = 0.034$ matches JOLTS total separations over 2001--2019.

The skill depreciation parameter $\lambda = 0.12$ implies 12\% human capital loss upon crossing the scarring threshold $d^* = 12$ months, calibrated to match earnings losses in \citet{jacobson1993earnings} and duration-dependent reemployment declines in \citet{kroft2016long}. The OLF exit rate has baseline component $\chi_0 = 0.008$ and duration-dependent component $\chi_1 = 0.004$, calibrated to match the cyclicality of labor force participation. Aggregate productivity is normalized to $a = 1$ in steady state. The calibrated steady state features an employment rate of 90.4\%, consistent with average US conditions over 2001--2019.
...
\textit{No skill depreciation ($\lambda = 0$).} The employment decline at 48 months falls from 6.0\% to approximately 3.2\%, and the welfare loss from 8.9\% to 3.0\%. Skill depreciation accounts for \textbf{two-thirds of the demand shock's welfare cost}.

\textit{No participation exit ($\chi = 0$).} The welfare loss falls from 8.9\% to 8.2\%---about 8\% of total welfare cost. Participation exit is empirically observable but quantitatively secondary to skill depreciation.
...
\Cref{tab:welfare} summarizes the welfare implications. The consumption-equivalent welfare loss from the demand shock is 8.9\%; from the supply shock, just 0.03\%. The demand/supply welfare ratio is \textbf{330:1}.

%% \input{tables/tab7_welfare.tex}  %% INLINED
\begin{table}[H]
\centering
...
\begin{tablenotes}[flushleft]
\small
\item \textit{Notes:} Consumption-equivalent (CE) welfare losses are computed as the permanent percentage reduction in consumption that would make agents indifferent between the steady state and the post-shock transition path. The demand shock reduces aggregate productivity by 5\% permanently. The supply shock doubles the separation rate for 3 months. Welfare is computed over a 600-month (50-year) simulation horizon to ensure convergence of the demand-shock transition path.
\end{tablenotes}
\end{threeparttable}
\label{tab:welfare}
\end{table}

This enormous asymmetry arises from two sources: permanence (undiscounted output losses are larger) and amplification (the scarring channel compounds the initial productivity loss). Without scarring, the demand shock's welfare cost falls to 3.0\% but still exceeds the supply shock's by a factor of approximately 100. These welfare comparisons should be interpreted as illustrating the order-of-magnitude asymmetry between demand and supply shock costs rather than as precise point estimates; \Cref{tab:app_model_sensitivity} explores sensitivity to key calibration parameters.

These comparisons are illustrative of the asymmetry's magnitude rather than precise point estimates. The 330:1 ratio reflects risk-neutral utility and the absence of consumption smoothing (borrowing, savings); under CRRA preferences with risk aversion greater than one, the welfare costs would be lower. The ratio is also sensitive to the discount rate and $\lambda$ (\Cref{tab:app_model_sensitivity} shows halving $\lambda$ to 0.06 reduces the CE loss to 4.6\%, while increasing to 0.18 raises it to 12.2\%). What is robust across all specifications is the qualitative asymmetry---demand shocks are orders of magnitude more costly---not the precise numbers. The welfare decomposition (skill depreciation 66\%, direct productivity 26\%, participation exit 8\%) is reported in \Cref{tab:welfare}.
...
A calibrated DMP model provides a unified explanation---and reveals the staggering magnitude of the asymmetry. Demand shocks depress hiring, extend durations, trigger human capital loss, and push workers out of the labor force---a vicious cycle that amplifies the initial shock into an 8.9\% consumption-equivalent welfare loss. Supply shocks create mass unemployment but preserve short durations, with a welfare cost of just 0.03\%. The demand-to-supply ratio is 330:1. Skill depreciation accounts for two-thirds of the demand shock's welfare cost.
...
\begin{equation}
W(h) = w(h) + \beta \left[ (1 - \delta) W(h) + \delta \cdot U(h, 0) \right],
\label{eq:app_W}
\end{equation}
where $w(h)$ is the wage from the simplified rule, $\delta$ is the exogenous separation rate, and a newly separated worker enters unemployment with duration $d = 0$. Rearranging:
\begin{equation}
W(h) = \frac{w(h) + \beta \delta \cdot U(h, 0)}{1 - \beta(1 - \delta)}.
\label{eq:app_W_solved}
\end{equation}

For an unemployed worker with human capital $h$ and duration $d < d^*$ (not yet scarred):
\begin{equation}
U(h, d) = b + \beta \left[ f(\theta) W(h) + \chi \cdot V^{OLF} + (1 - f(\theta) - \chi) U(h, d+1) \right].
...
\begin{equation}
U(\tilde{h}, d) = b + \beta \left[ f(\theta) W(\tilde{h}) + \chi \cdot V^{OLF} + (1 - f(\theta) - \chi) U(\tilde{h}, d+1) \right].
\label{eq:app_U_scar}
\end{equation}
...
In the full model, a worker would exit the labor force when the value of continued search falls below the value of non-participation ($U(h, d) < V^{OLF}$). Because $U(h, d)$ is decreasing in $d$, there exists a critical duration $\bar{d}(h)$ such that workers would exit after $\bar{d}$ periods. In simulations, I replace this endogenous stopping rule with the reduced-form competing hazard $\chi_t = \chi_0 + \chi_1 s_t$ (Equation~\ref{eq:exit_rate}), which enters the unemployment Bellman equation directly as a transition probability. The key economic content is preserved: when $f(\theta)$ is low (slack labor markets), the scarred fraction $s_t$ rises, increasing $\chi_t$ and generating higher labor force exit---the participation channel through which demand shocks create permanent employment loss.
...
The total match surplus is $S(h) = W(h) - U(h, 0) + J(h)$. Standard Nash bargaining with worker power $\gamma$ implies:
\begin{equation}
W(h) - U(h, 0) = \gamma S(h), \quad J(h) = (1 - \gamma) S(h).
\label{eq:app_nash}
\end{equation}
The full Nash solution yields $w = (1-\gamma)b + \gamma(ah + \kappa\theta)$, which makes free entry an implicit equation in $\theta$. For tractability in transition dynamics, I adopt the simplified proportional-surplus wage rule discussed in the main text:
\begin{equation}
...
\item Solve for market tightness from the free entry condition: $\theta_t = \left(\frac{A \cdot \beta(1 - \gamma)(a_t h_t^{eff} - b)}{\kappa [1 - \beta(1 - \delta_t)]}\right)^{1/\alpha}$.
...
\item Update the scarred fraction. Rather than tracking the full duration distribution and applying the $d^*$ threshold to each cohort---which would require a high-dimensional state space---I use a reduced-form proxy that captures the mapping from depressed job finding rates to the incidence of long-term unemployment:
\begin{equation*}
s_t = \min\left\{1,\; 0.95 \cdot s_{t-1} + 0.1 \cdot \max\left(0, 1 - \frac{f_t}{f^{ss}}\right)\right\}.
\end{equation*}
The $\min\{1,\cdot\}$ operator ensures $s_t \in [0,1]$, preserving its interpretation as a fraction. This specification has two desirable properties. First, the 0.95 persistence parameter ensures that scarring is slow to build and slow to dissipate, matching the observed dynamics of long-term unemployment shares. Second, the $\max(0, 1 - f_t/f^{ss})$ term activates only when the job finding rate falls below its steady-state value, so scarring occurs exclusively during recessions. The proxy is a simplification: it does not track individual workers' durations or the exact threshold $d^*$. The main text results are robust to alternative persistence parameters in the range [0.90, 0.98] and alternative sensitivity coefficients in [0.05, 0.15].
\end{enumerate}
\item Compute wages, employment changes, and welfare along the path.
\end{enumerate}

The algorithm converges rapidly because the model has a unique equilibrium for each value of the state variables. The transition path is computed for $T = 120$ months for display purposes. For welfare computation, the simulation is extended to $T = 600$ months (50 years) to ensure that the demand-shock path has converged to its new steady state before the terminal-value approximation is applied.
...
\end{align}
The terminal value terms $\frac{\beta^T}{1-\beta}[\cdots]$ capture the infinite continuation beyond the simulation horizon, evaluated at the period-$T$ allocation. For $W^{ss}$, this is the steady state; for $W^{shock}$, this is the post-transition allocation (which for permanent demand shocks converges to a new, lower steady state). With $\beta = 0.996$ and $T = 600$, the terminal value captures the infinite continuation beyond the simulation horizon. By $T = 600$, the demand-shock transition path has fully converged (monthly employment changes are less than $10^{-8}$ in the final year), validating the stationarity assumption embedded in the terminal-value approximation. The CE welfare loss is:
\begin{equation}
\Delta = 1 - \frac{W^{shock}}{W^{ss}}.
\label{eq:app_CE}
\end{equation}
A value of $\Delta = 0.089$ means that a permanent 8.9\% reduction in steady-state consumption would leave the worker equally well off as experiencing the demand shock transition.
...
\begin{tablenotes}[flushleft]
\small
\item \textit{Notes:} Each column reports the coefficient from a cross-state regression of the change in the unemployment rate on recession exposure at horizon $h$ months. Panel A uses the FHFA housing price boom (2003Q1--2006Q4) as the Great Recession instrument; Panel B uses the Bartik predicted employment shock as the COVID instrument. Appendix UR coefficients use the raw (unstandardized) Bartik instrument, sign-reversed (multiplied by $-1$) so that positive values indicate UR increases from greater COVID exposure. Multiply by the Bartik standard deviation (0.0233) for per-SD effects. The large COVID Bartik coefficients reflect the small standard deviation of the Bartik shock (0.023); a one-SD shock implies less than 1.5 percentage points of UR change at $h=3$. \textsuperscript{a}$h=60$ exceeds the 48-month post-COVID observation window. HC1 robust standard errors in parentheses. $^{*}$~$p<0.10$, $^{**}$~$p<0.05$, $^{***}$~$p<0.01$.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\begin{tablenotes}[flushleft]
\small
\item \textit{Notes:} Each column reports the coefficient from a cross-state regression of the change in the labor force participation rate on recession exposure at horizon $h$ months. LFPR data is from BLS LAUS for all 50 states. Appendix LFPR coefficients use the raw (unstandardized) Bartik instrument; multiply by the Bartik standard deviation (0.0233) for per-SD effects. COVID coefficients are large in absolute value because the Bartik shock has small cross-state variation (SD$\approx$0.03). A one-SD shock implies LFPR movements of less than 1 pp, consistent with observed patterns. ``Effect per 1-SD Bartik'' multiplies the COVID coefficient by the Bartik standard deviation (0.0233) to show the effect of a one-standard-deviation increase in exposure. ``---'' indicates horizon exceeds the 48-month post-COVID observation window. Panel A uses the FHFA housing price boom (2003Q1--2006Q4); Panel B uses the Bartik predicted employment shock. HC1 robust standard errors in parentheses. $^{*}$~$p<0.10$, $^{**}$~$p<0.05$, $^{***}$~$p<0.01$.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\midrule
Baseline ($\lambda = 0.12$, $\chi_0 = 0.008$) & $-6.0$ & 8.9 \\
Low scarring ($\lambda = 0.06$) & $-3.2$ & 4.6 \\
High scarring ($\lambda = 0.18$) & $-16.3$ & 12.2 \\
No participation exit ($\chi_0 = 0$) & $-5.0$ & 8.6 \\
High participation exit ($\chi_0 = 0.016$) & $-6.5$ & 8.9 \\
Low matching efficiency ($A = 0.45$) & $-18.5$ & 9.5 \\
High matching efficiency ($A = 0.75$) & $-1.3$ & 2.9 \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
\small
...
The model's qualitative predictions are robust across the parameter space. Higher skill depreciation ($\lambda$) amplifies the demand shock's 48-month employment impact and welfare cost, while lower values attenuate both. Matching efficiency ($A$) is the most powerful parameter: low matching efficiency ($A = 0.45$) dramatically extends durations and amplifies scarring, while high efficiency ($A = 0.75$) permits rapid reabsorption with minimal human capital loss. The participation exit channel has modest quantitative effects, consistent with the main-text counterfactual analysis. In all scenarios, the model's permanent demand shock produces employment declines that continue deepening beyond 120 months (model half-life $>$120), while the supply shock recovers within 9 months regardless of parameterization.
