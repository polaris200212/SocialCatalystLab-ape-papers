...
emp/pop paths, and industry shares matrix for apep_0238 v7

v7 changes: standardize COVID Bartik to SD=1, save raw Bartik for reference
"""
...
print(f"  COVID Bartik (raw): {len(bartik_covid)} states")
...
# ──────────────────────────────────────────────────────────────
# 2b. Standardize COVID Bartik to SD=1 (WS2 v7)
# ──────────────────────────────────────────────────────────────
print("\nStandardizing COVID Bartik to SD=1...")
bartik_covid_raw = dict(bartik_covid)  # preserve raw values
covid_vals = np.array(list(bartik_covid.values()))
covid_mean = float(np.mean(covid_vals))
covid_sd = float(np.std(covid_vals))
bartik_covid_std = {st: (v - covid_mean) / covid_sd for st, v in bartik_covid.items()}
print(f"  Raw Bartik: mean={covid_mean:.4f}, SD={covid_sd:.4f}")
print(f"  Standardized Bartik: mean={np.mean(list(bartik_covid_std.values())):.6f}, "
      f"SD={np.std(list(bartik_covid_std.values())):.6f}")
# Replace bartik_covid with standardized version for all downstream analysis
bartik_covid = bartik_covid_std
...
processed = {
    'bartik_gr': bartik_gr,
    'bartik_covid': bartik_covid,  # standardized (mean=0, SD=1)
    'bartik_covid_raw': bartik_covid_raw,  # original units for reference
    'bartik_covid_mean': covid_mean,
    'bartik_covid_sd': covid_sd,
    'hpi_boom': hpi_boom,
...
for st in sorted(STATE_FIPS.keys()):
    ...
    row = {
        ...
        'bartik_covid': bartik_covid.get(st, np.nan),  # standardized
        'bartik_covid_raw': bartik_covid_raw.get(st, np.nan),
