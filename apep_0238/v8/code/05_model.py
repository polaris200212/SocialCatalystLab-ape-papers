...
v8: Full duration distribution tracking, SMM estimation, CRRA welfare
"""
...
def smm_estimate(lp_results, verbose=True):
    ...
    Estimates 4 parameters (λ, χ₁, A, κ) by minimizing the distance between
    model-simulated moments and data moments from LP IRF estimates.

    Target moments (6, overidentified with 2 d.f.):
      1. GR employment response at h=12
      2. GR employment response at h=48
      3. GR employment response at h=84
    ...
    def simulate_moments(params):
        ...
        # Scale model employment response to match data
        # The LP coefficients are per-unit-of-instrument responses
        # The model produces per-5%-productivity-drop responses
        # We need a scaling factor: find the ratio at the peak
        model_peak = min(emp_d[:121])
        data_peak = min(m_data[0], m_data[1], m_data[2])
        if abs(model_peak) < 1e-10:
            return None
        scale = data_peak / model_peak
...
print("="*60)
print("STRUCTURAL MODEL: DMP WITH DURATION DISTRIBUTION (v8)")
print("="*60)
...
# Create model with SMM-estimated parameters
estimated_params = {
    'lam': smm_results['lam'],
...
lambda_vals = [0.01, 0.02, 0.03, 0.05, 0.10, smm_results['lam']]
