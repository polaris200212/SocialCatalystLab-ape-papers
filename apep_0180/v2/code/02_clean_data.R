...
# Assume this persists for T years with decay

consumption_gain_ppp <- egger_ge_effects %>%
  filter(outcome == "Consumption") %>%
  pull(recipient_effect)

# Convert to USD (divide by PPP factor)
...
# Wage earnings increased by $182 PPP (Egger et al.)
# But ~80% of rural Kenya is informal, so effective tax rate is low

wage_gain_ppp <- egger_ge_effects %>%
