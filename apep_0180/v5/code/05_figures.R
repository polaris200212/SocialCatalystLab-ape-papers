# ============================================================================
# 05_figures.R - Publication-quality figures (v5)
...
pv_stream_fig <- function(annual, years, retention, r) {
...
fig4_data <- expand.grid(
  ...
  rowwise() %>%
  mutate(
    pv_vat_i = pv_stream_fig(consumption_gain_usd * 0.16 * 0.50,
                              persistence, consumption_retention, discount),
    earn_yrs = min(persistence, 5),
    pv_inc_i = pv_stream_fig(wage_gain_usd * 0.185 * 0.20,
                              earn_yrs, earnings_retention, discount),
    net_i = 1000 - pv_vat_i - pv_inc_i,
