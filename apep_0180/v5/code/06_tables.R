# ============================================================================
# 06_tables.R - Generate all tables for the paper (v5)
...
cat("& & & & \\\\\n")
cat("\\textit{Treatment effects (annualized, USD)} & & & & \\\\\n")
cat(paste0("\\quad $\\Delta C$ (consumption) & \\$", round(consumption_gain_usd, 1),
    " & --- & $N(", round(consumption_gain_usd, 1), ", ",
    round(egger_ge_effects$recipient_se[1] / kenya_fiscal$ppp_factor, 1), "^2)$ & Egger et al. (2022) / 2.515 \\\\\n"))
cat(paste0("\\quad $\\Delta E$ (earnings) & \\$", round(wage_gain_usd, 1),
    " & --- & $N(", round(wage_gain_usd, 1), ", ",
    round(egger_ge_effects$recipient_se[egger_ge_effects$outcome == "Wage earnings"] / kenya_fiscal$ppp_factor, 1),
    "^2)$ & Egger et al. (2022) / 2.515 \\\\\n"))
...
# → paper.tex Table 9 (new in v5)

sink(file.path(tab_dir, "table9_variance_decomposition.tex"))
cat("\\begin{table}[htbp]\n")
cat("\\centering\n")
cat("\\caption{MVPF Variance Decomposition}\n")
cat("\\label{tab:variance_decomp}\n")
cat("\\begin{tabular}{lcc}\n")
cat("\\hline\\hline\n")
cat("Uncertainty Source & Variance Share (\\%) & Var. When Fixed \\\\\n")
cat("\\hline\n")
for (i in 1:nrow(variance_decomp)) {
  cat(paste(
    variance_decomp$source[i], "&",
    sprintf("%.1f", variance_decomp$variance_share_pct[i]), "&",
    sprintf("%.2e", variance_decomp$var_when_fixed[i]), "\\\\\n"
  ))
}
cat("\\hline\n")
cat(paste0("Total MVPF variance & 100.0 & ",
    sprintf("%.2e", variance_decomp$total_var[1]), " \\\\\n"))
cat("\\hline\n")
cat("\\multicolumn{3}{l}{\\footnotesize Notes: Variance share = 1 $-$ Var(MVPF $|$ source fixed at mean) / Var(MVPF).}\\\\\n")
cat("\\multicolumn{3}{l}{\\footnotesize Based on 5,000 bootstrap replications with spillovers ($\\rho = 0$).}\\\\\n")
cat("\\multicolumn{3}{l}{\\footnotesize Shares may not sum to 100\\% due to interaction effects.}\\\\\n")
cat("\\end{tabular}\n")
cat("\\end{table}\n")
sink()

cat("Table 9 generated.\n")

# ── Summary ───────────────────────────────────────────────────────────────

cat("\n=== All 9 Tables Generated ===\n")
...
cat("9. table9_variance_decomposition.tex\n")
