# ============================================================================
# 04_robustness.R - Sensitivity and robustness analysis (v5)
...
calc_mvpf_persistence <- function(years, retention, decay_type = "geometric") {
  ...
  earn_yrs <- min(years, 5)  # earnings persist at most 5 years
  pv_inc <- sum(sapply(1:earn_yrs, function(t)
    wage_usd * 0.185 * 0.20 * decay_fn(t) / (1 + r)^t))
...
formality_scenarios <- tibble(
  ...
  mutate(
    ...
    pv_income_tax_new = pv_stream(annual_income_tax, 5, earnings_retention, 0.05),
