...
# MVPF of Unconditional Cash Transfers in Kenya (v5)
# ============================================================================
#
...
# → Table 9 (new in v5)
# Hold each parameter group fixed at its mean, vary everything else.
# Variance contribution = 1 - Var(MVPF | group fixed) / Var(MVPF_full)

cat("\n=== VARIANCE DECOMPOSITION ===\n")

total_var <- var(mvpf_spillover_boot, na.rm = TRUE)

# (i) Fix treatment effects at means, vary fiscal params
mvpf_fix_te <- mapply(calc_mvpf,
  consumption_draw_usd = rep(cons_mean_usd, n_boot),
  earnings_draw_usd = rep(earn_mean_usd, n_boot),
  spillover_draw_usd = rep(spillover_mean_usd, n_boot),
  vat_coverage = vat_coverage_draws, informal_share = informal_draws,
  MoreArgs = list(mcpf = 1.0))
var_fix_te <- var(mvpf_fix_te, na.rm = TRUE)

# (ii) Fix fiscal params at means, vary treatment effects
mvpf_fix_fiscal <- mapply(calc_mvpf,
  consumption_draw_usd = cons_draws, earnings_draw_usd = earn_draws,
  spillover_draw_usd = spill_draws,
  vat_coverage = rep(0.50, n_boot), informal_share = rep(0.80, n_boot),
  MoreArgs = list(mcpf = 1.0))
var_fix_fiscal <- var(mvpf_fix_fiscal, na.rm = TRUE)

# (iii) Fix spillovers at mean, vary everything else
mvpf_fix_spill <- mapply(calc_mvpf,
  consumption_draw_usd = cons_draws, earnings_draw_usd = earn_draws,
  spillover_draw_usd = rep(spillover_mean_usd, n_boot),
  vat_coverage = vat_coverage_draws, informal_share = informal_draws,
  MoreArgs = list(mcpf = 1.0))
var_fix_spill <- var(mvpf_fix_spill, na.rm = TRUE)

# Shapley-style variance shares: contribution = total_var - var_when_fixed
share_te <- (total_var - var_fix_te) / total_var * 100
share_fiscal <- (total_var - var_fix_fiscal) / total_var * 100
share_spill <- (total_var - var_fix_spill) / total_var * 100

variance_decomp <- tibble(
  source = c("Treatment effect sampling error",
             "Fiscal parameter uncertainty (VAT coverage, informality)",
             "Spillover uncertainty"),
  variance_share_pct = round(c(share_te, share_fiscal, share_spill), 1),
  total_var = total_var,
  var_when_fixed = c(var_fix_te, var_fix_fiscal, var_fix_spill)
)

cat("Variance decomposition (% of total MVPF variance):\n")
print(variance_decomp %>% dplyr::select(source, variance_share_pct))
