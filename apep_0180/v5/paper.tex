...
\title{The Marginal Value of Public Funds for Unconditional Cash Transfers in a Developing Country: Evidence from Kenya\footnote{This paper is a revision of APEP-0192 (v4). See \url{https://github.com/SocialCatalystLab/ape-papers/tree/main/apep_0180} for the paper family and prior versions. Replication code and data are available in the same repository.}}
\author{APEP Autonomous Research\thanks{Autonomous Policy Evaluation Project. Correspondence: scl@econ.uzh.ch} \and @SocialCatalystLab \\ @anonymous, @ai1scl}
\date{\today}
...
Kenya's unconditional cash transfers deliver 87 cents of welfare per government dollar---between the US Earned Income Tax Credit (92 cents) and TANF (65 cents)---despite an economy where 80 percent of workers are informal and the government recaptures just 2.2 percent of each dollar transferred. This paper constructs the first MVPF estimate for a developing-country program and shows why: informality, not program design, is the binding constraint on transfer efficiency.
...
To illustrate: with a \$1,000 transfer (15\% admin), recipients value \$850. The government recoups \$14.78 in VAT (consumption gain $\times$ 16\% $\times$ 50\% coverage over 3 years) and \$7.27 in income tax (earnings gain $\times$ 18.5\% $\times$ 20\% formal over 5 years). Net cost is \$1,000 $-$ \$22.05 $=$ \$977.95. The MVPF is \$850/\$978 $=$ 0.869: each government dollar delivers 87 cents of welfare.
...
Care is needed to avoid double-counting. The spillover WTP represents genuine welfare gains---higher wages and business revenue for non-recipients---not gifts from recipients.\footnote{The distinction between pecuniary and real spillovers matters for welfare accounting. Pecuniary externalities---pure price-mediated transfers between agents---redistribute welfare without creating it and should not enter the MVPF numerator. Real externalities---productivity gains, business creation, expanded output---generate genuine surplus. The minimal price response documented by \citet{egger2022general} (0.1\% inflation) supports classifying most spillovers as real, but I test pecuniary shares of 0--100\% in Section~\ref{sec:sensitivity}.} The experimental evidence supports this: non-recipient gains arose from increased local demand propagating through the village economy.
...
\input{tables/table1_treatment_effects.tex}

\input{tables/table2_ge_effects.tex}
...
\input{tables/table3_fiscal_parameters.tex}

The VAT coverage rate of 50\% merits particular discussion. As \citet{pomeranz2015} demonstrates using a randomized audit experiment in Chile, VAT enforcement depends critically on the ``paper trail'' created by formal transactions---a mechanism largely absent in Kenya's rural informal markets. The Kenya Integrated Household Budget Survey (KIHBS 2015/16) shows that approximately 45\% of rural expenditure goes to food (much zero-rated: maize flour, milk, bread) and 20\% to housing and fuel (exempt). Of the remaining taxable spending, informal market purchases further reduce effective coverage. \citet{bachas2022} find that effective VAT rates in Kenya average 4--6\% on total consumption, implying 25--38\% of spending is effectively taxed at the 16\% statutory rate. My 50\% baseline is deliberately generous---biased toward finding larger fiscal externalities and thus a higher MVPF.
...
The MVPF is a ratio of estimated quantities, each carrying uncertainty. Following standard bootstrap inference for ratio estimands \citep{efron1994bootstrap} and the general framework for causal inference with experimental data reviewed by \citet{imbens2009recent}, I propagate uncertainty through correlated bootstrap simulation with 5,000 replications. In each draw, I sample consumption and earnings treatment effects jointly from a bivariate normal distribution:
\begin{equation}
...
\input{tables/table4_mvpf_components.tex}
...
At WTP = 0.95 (recipients value each dollar at 95 cents), the MVPF falls from 0.87 to 0.83. At WTP = 0.90, it falls to 0.78. At WTP = 0.85, the MVPF is 0.74. At WTP = 0.80, it reaches 0.70. The MVPF is linearly proportional to the WTP ratio, since the denominator is unchanged. This sensitivity is relevant for interpreting the MVPF of programs that impose conditions or restrict spending categories: if conditions reduce the effective value of the transfer to recipients, the MVPF falls proportionally.
...
\input{tables/table6_sensitivity.tex}
...
The MVPF ranges from 0.860 (persistence of 1 year) to 0.894 (50\% pecuniary spillovers) across single-parameter variations, excluding MCPF and WTP adjustments. The narrow range reflects the fundamental arithmetic: because fiscal externalities are small, even large changes in individual parameters move the MVPF modestly. The exception is the MCPF, which scales the entire denominator and can push the MVPF as low as 0.579 at MCPF = 1.5.
...
The informal sector share is the most consequential structural parameter. At 90\% informality (conservative), the MVPF is 0.866; at 60\% (optimistic), it rises to 0.876. Under full formalization---a hypothetical scenario where all earnings are taxed at 18.5\%---the MVPF reaches 0.896. This 3-percentage-point gap between observed (0.867) and full-formalization (0.896) MVPF represents unrealized fiscal efficiency from informality.
...
\input{tables/table8_covariance.tex}
...
Table~\ref{tab:variance_decomp} decomposes total MVPF variance into contributions from three uncertainty sources: treatment effect sampling error, fiscal parameter uncertainty (VAT coverage and informality), and spillover uncertainty. For each source, I hold that group fixed at its mean while varying all others, then compute the share of total variance attributable to that source.

\input{tables/table9_variance_decomposition.tex}

Treatment effect sampling error dominates, accounting for virtually all MVPF variance. Fiscal parameter uncertainty---the unknown extent of VAT coverage and labor market formality---contributes less than 1\%. This decomposition has a practical implication: additional precision on fiscal parameters (e.g., better informality surveys) would barely tighten the MVPF confidence interval. The binding source of uncertainty is the treatment effect estimates themselves, particularly the spillover estimate from \citet{egger2022general} whose standard error is large relative to its point estimate. Researchers seeking to narrow MVPF confidence intervals for developing-country programs should invest in larger experiments rather than better fiscal data.
...
\input{tables/table7_government_scenarios.tex}
...
Ultimately, the 5-percentage-point gap between Kenya's MVPF (0.87) and the US EITC (0.92) does not reflect a failure of program design---it reflects the fiscal reality of informality. Until developing countries can tax the behavioral responses that transfers generate, cash will remain efficient but never self-financing. The binding constraint on transfer efficiency is not what governments give, but what they can recapture.
