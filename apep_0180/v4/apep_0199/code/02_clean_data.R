...
# This matches the source used in 03_main_analysis.R for recipient effects.
# Egger et al. recipient effects are reserved for GE/spillover analysis.

consumption_gain_ppp_monthly <- haushofer_shapiro_effects %>%
  filter(outcome == "Total consumption") %>%
  pull(treatment_effect)

# Annualize and convert to USD
consumption_gain_usd <- (consumption_gain_ppp_monthly * 12) / kenya_fiscal$ppp_factor
...
# Earnings increased by $17 PPP/month (Haushofer & Shapiro 2016, Non-agricultural revenue)
# This matches the source used in 03_main_analysis.R for recipient effects.
# ~80% of rural Kenya is informal, so effective tax rate is low

wage_gain_ppp_monthly <- haushofer_shapiro_effects %>%
  filter(outcome == "Non-agricultural revenue") %>%
  pull(treatment_effect)

wage_gain_usd <- (wage_gain_ppp_monthly * 12) / kenya_fiscal$ppp_factor
