...
# Base treatment effects (pulled from data objects, not hard-coded)
base_consumption <- haushofer_shapiro_effects %>%
  filter(outcome == "Total consumption") %>%
  pull(treatment_effect)

base_earnings <- haushofer_shapiro_effects %>%
  filter(outcome == "Non-agricultural revenue") %>%
  pull(treatment_effect)

base_consumption_se <- haushofer_shapiro_effects %>%
  filter(outcome == "Total consumption") %>%
  pull(se)

base_earnings_se <- haushofer_shapiro_effects %>%
  filter(outcome == "Non-agricultural revenue") %>%
  pull(se)

cat(sprintf("Base consumption effect: $%.0f PPP/month (SE: $%.0f)\n", base_consumption, base_consumption_se))
cat(sprintf("Base earnings effect: $%.0f PPP/month (SE: $%.0f)\n", base_earnings, base_earnings_se))
...
for (sim in 1:n_sims) {
  # Draw consumption and earnings effects using published SEs
  c_draw <- rnorm(1, base_consumption, base_consumption_se)
  e_draw <- rnorm(1, base_earnings, base_earnings_se)
