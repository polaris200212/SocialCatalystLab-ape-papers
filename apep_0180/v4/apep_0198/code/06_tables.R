...
# Load sensitivity values from computed robustness results
if (!file.exists("../data/robustness_results.RData")) {
  stop("robustness_results.RData not found. Run 04_robustness.R first.")
}

baseline_mvpf <- mvpf_results$mvpf_direct

# Extract computed values from robustness objects
vat_25_mvpf <- vat_sensitivity$mvpf[vat_sensitivity$vat_coverage == 0.25]
vat_100_mvpf <- vat_sensitivity$mvpf[vat_sensitivity$vat_coverage == 1.00]
informal_90_mvpf <- formality_sensitivity$mvpf[formality_sensitivity$informal_share == 0.90]
informal_60_mvpf <- formality_sensitivity$mvpf[formality_sensitivity$informal_share == 0.60]
disc_3_mvpf <- discount_sensitivity$mvpf[discount_sensitivity$discount_rate == 0.03]
disc_10_mvpf <- discount_sensitivity$mvpf[discount_sensitivity$discount_rate == 0.10]

sensitivity_summary <- tibble(
  parameter = c("Baseline", "VAT coverage 25%", "VAT coverage 100%",
                "Informality 90%", "Informality 60%",
                "Discount rate 3%", "Discount rate 10%"),
  mvpf = c(baseline_mvpf, vat_25_mvpf, vat_100_mvpf,
           informal_90_mvpf, informal_60_mvpf,
           disc_3_mvpf, disc_10_mvpf),
  change_from_baseline = round((c(baseline_mvpf, vat_25_mvpf, vat_100_mvpf,
                                   informal_90_mvpf, informal_60_mvpf,
                                   disc_3_mvpf, disc_10_mvpf) / baseline_mvpf - 1) * 100, 1)
)
...
if (!is.null(mvpf_results$variance_decomp)) {
  table7 <- mvpf_results$variance_decomp %>%
    mutate(
      Source = source,
      Variance = sprintf("%.6f", variance),
      `Share (%)` = sprintf("%.1f%%", share_pct)
    ) %>%
    select(Source, Variance, `Share (%)`)

  write_csv(table7, file.path(tab_dir, "table7_variance_decomp.csv"))
...
# -----------------------------------------------------------------------------
# Table 8: Component-Level Uncertainty
# -----------------------------------------------------------------------------

if (!is.null(mvpf_results$component_uncertainty)) {
  table8 <- mvpf_results$component_uncertainty %>%
    mutate(
      Component = component,
      `Point Estimate` = sprintf("$%.2f", point_estimate),
      SE = ifelse(se < 0.01, "---", sprintf("$%.2f", se)),
      `95% CI` = sprintf("[%.1f, %.1f]", ci_lower, ci_upper)
    ) %>%
    select(Component, `Point Estimate`, SE, `95% CI`)

  write_csv(table8, file.path(tab_dir, "table8_component_uncertainty.csv"))
...
# -----------------------------------------------------------------------------
# Table 9: WTP Sensitivity
# -----------------------------------------------------------------------------

if (exists("wtp_sensitivity")) {
  table9 <- wtp_sensitivity %>%
    mutate(
      `WTP Multiplier` = sprintf("%.1f", multiplier),
      Interpretation = interpretation,
      `MVPF (Direct)` = sprintf("%.3f", mvpf_direct),
      `MVPF (Total)` = sprintf("%.3f", mvpf_total)
    ) %>%
    select(`WTP Multiplier`, Interpretation, `MVPF (Direct)`, `MVPF (Total)`)

  write_csv(table9, file.path(tab_dir, "table9_wtp_sensitivity.csv"))
...
# -----------------------------------------------------------------------------
# Table 10: Pecuniary vs Real Spillover Sensitivity
# -----------------------------------------------------------------------------

if (exists("pecuniary_sensitivity")) {
  table10 <- pecuniary_sensitivity %>%
    mutate(
      `Pecuniary Share` = sprintf("%.0f%%", pecuniary_share * 100),
      `Spillover WTP` = sprintf("$%.0f", spillover_wtp),
      MVPF = sprintf("%.3f", mvpf)
    ) %>%
    select(`Pecuniary Share`, `Spillover WTP`, MVPF)

  write_csv(table10, file.path(tab_dir, "table10_pecuniary_spillover.csv"))
...
cat("7. table7_variance_decomp.csv - Variance decomposition\n")
cat("8. table8_component_uncertainty.csv - Component-level uncertainty\n")
cat("9. table9_wtp_sensitivity.csv - WTP sensitivity\n")
cat("10. table10_pecuniary_spillover.csv - Pecuniary spillover sensitivity\n")
