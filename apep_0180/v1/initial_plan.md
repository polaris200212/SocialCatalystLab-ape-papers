...
**Method:** Doubly Robust (DR) estimation applied to RCT data

The identification comes from:
1. **Village-level randomization:** 653 villages randomized to high saturation (2/3 treated) or low saturation (1/3 treated)
2. **Household-level randomization:** Within treatment villages, eligible households randomized to receive or not receive transfer
3. **DR enhances RCT:** While RCT provides clean identification, we use DR estimation to:
   - Improve precision by controlling for baseline covariates
   - Account for any residual imbalance
   - Enable heterogeneity analysis
