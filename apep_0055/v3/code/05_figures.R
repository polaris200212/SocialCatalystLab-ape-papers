...
# 05_figures.R - Generate all figures including first-stage
# ============================================================================
...
# ============================================================================
# Figure 6: First-Stage Evidence (Insurance Coverage by Age)
# ============================================================================

cat("Generating Figure 6: First-Stage Evidence...\n")

first_stage <- readRDS(file.path(data_dir, "first_stage_results.rds"))

if ("AGEP" %in% names(first_stage)) {
  # ACS PUMS data available
  fs_plot <- first_stage[, .(
    Age = AGEP,
    `Employer Coverage` = employer_rate * 100,
    `Any Private` = private_rate * 100,
    `Medicaid` = medicaid_rate * 100,
    `Uninsured` = uninsured_rate * 100
  )]

  fs_long <- melt(fs_plot, id.vars = "Age",
                  variable.name = "Coverage", value.name = "Percent")

  pdf(file.path(fig_dir, "figure6_first_stage.pdf"), width = 10, height = 6)

  ggplot(fs_long, aes(x = Age, y = Percent, color = Coverage, shape = Coverage)) +
    geom_point(size = 3) +
    geom_line(data = fs_long[Age < 26], linewidth = 0.8, alpha = 0.7) +
    geom_line(data = fs_long[Age >= 26], linewidth = 0.8, alpha = 0.7) +
    geom_vline(xintercept = 25.5, linetype = "dashed", color = "grey30", linewidth = 0.8) +
    scale_color_manual(values = c("Employer Coverage" = "#377EB8",
                                  "Any Private" = "#4DAF4A",
                                  "Medicaid" = "#E41A1C",
                                  "Uninsured" = "#984EA3")) +
    scale_shape_manual(values = c(16, 17, 15, 18)) +
    labs(
      title = "First Stage: Health Insurance Coverage by Age",
      subtitle = "Sharp drop in private coverage and rise in Medicaid/uninsured at age 26",
      x = "Age",
      y = "Coverage Rate (%)",
      caption = "Data: ACS 1-Year PUMS, 2016-2023 (weighted). Dashed line = age 26 cutoff."
    ) +
    theme_apep() +
    theme(legend.position = "bottom")
  ...
  # Natality-based proxy first stage
  fs_plot <- first_stage[, .(
    Age = MAGER,
    `Private Insurance` = private_rate * 100,
    `Medicaid` = medicaid_rate * 100,
    `Self-Pay` = selfpay_rate * 100
  )]

  fs_long <- as.data.frame(melt(fs_plot, id.vars = "Age",
                  variable.name = "Payment", value.name = "Percent"))

  pdf(file.path(fig_dir, "figure6_first_stage.pdf"), width = 10, height = 6)

  print(ggplot(fs_long, aes(x = Age, y = Percent, color = Payment, shape = Payment)) +
    geom_point(size = 3) +
    geom_line(data = subset(fs_long, Age < 26), linewidth = 0.8, alpha = 0.7) +
    geom_line(data = subset(fs_long, Age >= 26), linewidth = 0.8, alpha = 0.7) +
    geom_vline(xintercept = 25.5, linetype = "dashed", color = "grey30", linewidth = 0.8) +
    scale_color_manual(values = c("Private Insurance" = "#377EB8",
                                  "Medicaid" = "#E41A1C",
                                  "Self-Pay" = "#4DAF4A")) +
    scale_shape_manual(values = c(16, 17, 15)) +
    labs(
      title = "First Stage: Payment Source for Delivery by Mother's Age",
      subtitle = "Sharp shift from private to Medicaid payment at age 26",
      x = "Mother's Age at Delivery",
      y = "Percent of Births",
      caption = "Data: CDC Natality 2016-2023. Dashed line = age 26 cutoff."
    ) +
    theme_apep() +
    theme(legend.position = "bottom"))
