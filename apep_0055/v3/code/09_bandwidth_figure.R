...
# Create figure (fixed: no axis truncation, show full range including BW=5)
pdf(file.path(fig_dir, "figure9_bandwidth.pdf"), width = 8, height = 5)
...
ggplot(bw_table, aes(x = Bandwidth, y = RD_Estimate)) +
  ...
  labs(
    ...
    x = "Bandwidth (Years)",
    y = "RD Estimate",
    caption = "Note: Points show RD estimates; bars show 95% robust CI.\nEffect stable across bandwidths. Data: CDC Natality 2016-2023."
  ) +
  theme_apep()
