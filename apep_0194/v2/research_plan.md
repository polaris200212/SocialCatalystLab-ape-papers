...
- **Who is actually treated?** All firms operating in states that enact comprehensive data privacy laws. The treatment is a state-level policy that applies to firms processing consumer data of state residents, regardless of firm headquarters.
...
- **Placebo/control population:** Same industries in never-treated states (32 states + DC) and not-yet-treated states (12 states with effective dates after 2024Q4, including Montana reclassified as not-yet-treated).
- **Design:** Staggered DiD (not triple-diff). Treatment varies at state × time level. Subsector heterogeneity is examined by running separate regressions for each NAICS code, not via interaction terms.
...
**Who is actually treated?** State comprehensive data privacy laws apply to businesses that collect personal data from residents of the enacting state. The treatment operates at the state level: all firms operating within a treated state face compliance costs, while firms in never-treated states do not.

**Primary estimand population:** Employees in state × NAICS industry cells. The treatment captures the extensive margin of privacy law adoption (binary: effective vs. not yet effective). We measure employment effects at the state-quarter level using QCEW data.

**Placebo/control population:** Non-tech sectors (NAICS 52 Finance, NAICS 23 Construction) serve as within-state placebos. These sectors face minimal direct compliance burden from data privacy laws, so any detected effects would indicate violations of the parallel trends assumption or confounding from concurrent policies.

**Design:** Staggered DiD with 8 states whose privacy laws became effective within the 2015Q1–2024Q4 sample window. The remaining 43 states (including DC) serve as never-treated controls. The Callaway-Sant'Anna estimator produces group-time ATTs that are robust to heterogeneous treatment effects across adoption cohorts.

**Power Assessment:**
- Pre-treatment periods: 20+ quarters for the earliest cohort (CA, 2020Q1), ≥5 quarters for all cohorts
- Treated clusters: 8 states with effective dates in sample window
- Post-treatment periods per cohort: 1–20 quarters depending on adoption timing
- MDE: With 51 state clusters and 40 quarters, minimum detectable effect is approximately 3–5% of log employment given within-state variation
