...
if (length(rob_rows) > 0) {
  ...
  rob_tex <- kable(rob_tab, format = "latex", booktabs = TRUE,
    ...
    footnote(general = "Panel A: Sun-Abraham (2021) IW estimator. Panel B: Excludes California. Panel C: Non-tech placebo sectors. Panel D: Fisher randomization inference with 1,000 permutations.")
...
###############################################################################
# Table 7: Cohort-Specific ATTs
###############################################################################

cat("  Table 7: Cohort ATTs\n")

cohort_results <- tryCatch(readRDS(file.path(DATA_DIR, "cohort_results.rds")),
                           error = function(e) list())

if (length(cohort_results) > 0) {
  # Load period_map to convert periods back to yearqtrs
  qcew_tmp <- read_csv(file.path(DATA_DIR, "qcew_panel.csv"), show_col_types = FALSE)
  all_yearqtrs <- sort(unique(qcew_tmp$yearqtr))
  ...
  for (ind in names(cohort_results)) {
    ...
    for (i in seq_along(agg$egt)) {
      ...
      if (length(yq) == 0) yq <- NA

      # Convert yearqtr to label
      yr <- floor(yq)
      q <- round((yq - yr) * 4) + 1
      label <- sprintf("%dQ%d", yr, q)

      cohort_rows[[length(cohort_rows) + 1]] <- tibble(
        Industry = ind,
        Cohort = label,
        ATT = sprintf("%.4f", agg$att.egt[i]),
        SE = sprintf("(%.4f)", agg$se.egt[i]),
        `95% CI` = sprintf("[%.4f, %.4f]",
                           agg$att.egt[i] - 1.96 * agg$se.egt[i],
                           agg$att.egt[i] + 1.96 * agg$se.egt[i])
      )
    }
  }

  if (length(cohort_rows) > 0) {
    cohort_tab <- bind_rows(cohort_rows)
    cohort_tex <- kable(cohort_tab, format = "latex", booktabs = TRUE,
                        caption = "Cohort-Specific ATT Estimates (Callaway-Sant'Anna)",
                        label = "tab:cohort_att") %>%
      kable_styling(latex_options = c("hold_position")) %>%
      footnote(general = "Group-level ATTs from CS-DiD with never-treated controls. Cohort = first treatment quarter. CIs based on state-clustered SEs.")
    writeLines(cohort_tex, file.path(TABLE_DIR, "tab7_cohort_att.tex"))
    cat("  Cohort ATT table saved.\n")
  }
}

###############################################################################
# Table 8: Employment per Establishment + Wage + Establishment (Combined)
###############################################################################

cat("  Table 8: Emp/estab results\n")

emp_estab_results <- tryCatch(readRDS(file.path(DATA_DIR, "emp_estab_results.rds")),
                              error = function(e) list())

if (length(emp_estab_results) > 0) {
  ee_rows <- list()
  for (ind in names(emp_estab_results)) {
    fit <- emp_estab_results[[ind]]
    ee_rows[[length(ee_rows) + 1]] <- tibble(
      Outcome = "Log Emp/Estab",
      Industry = ind,
      Estimate = format_coef(coef(fit)["treat"], se(fit)["treat"], fixest::pvalue(fit)["treat"]),
      SE = format_se(se(fit)["treat"]),
      N = nobs(fit)
  ...
  ee_tab <- bind_rows(ee_rows)
  # Append to existing wage/estab table if possible, or create standalone
  ee_tex <- kable(ee_tab, format = "latex", booktabs = TRUE,
                  caption = "Employment per Establishment Effects",
                  label = "tab:emp_estab") %>%
    kable_styling(latex_options = c("hold_position")) %>%
    footnote(general = "TWFE with state and time FE. Outcome: log(avg employment / establishments). Clustered SEs by state. Rising emp/estab with falling total employment indicates small-firm exit.")
  writeLines(ee_tex, file.path(TABLE_DIR, "tab8_emp_estab.tex"))
}
