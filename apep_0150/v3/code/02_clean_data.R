...
cat("Placebo cancer (pre):", ifelse(!is.null(placebo_cancer), nrow(placebo_cancer), "not available"), "\n")
cat("Placebo heart (pre):", ifelse(!is.null(placebo_heart), nrow(placebo_heart), "not available"), "\n")
cat("Placebo cancer (post):", ifelse(!is.null(placebo_cancer_post), nrow(placebo_cancer_post), "not available"), "\n")
cat("Placebo heart (post):", ifelse(!is.null(placebo_heart_post), nrow(placebo_heart_post), "not available"), "\n")
...
# are suppressed by CDC due to small cell sizes. Vermont is reclassified as not-yet-treated
# (first_treat = 0) in the estimation sample, joining the other 8 not-yet-treated states
# as a control unit. Sensitivity analysis with Vermont included is in 04_robustness.R.
panel <- panel %>%
...
cat("\n=== Merging Placebo Outcomes (Pre + Post Treatment) ===\n")

# Combine pre-treatment (1999-2017) and post-treatment (2020-2023) placebo data
