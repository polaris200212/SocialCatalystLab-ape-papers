...
for (i in 1:nrow(table2)) {
  ...
  # Mark states reclassified as not-yet-treated in estimation
...
cat("exposure (first January 1 under the law). ``Treatment Year'' reflects the ")
cat("\\textit{policy} effective date, not the estimation cohort assignment.\n")
cat("\\item $^{\\ddagger}$Reclassified as not-yet-treated in estimation because ")
cat("treatment onset postdates the 2023 data endpoint.\n")
cat("\\item $^{\\dagger}$Vermont is reclassified as not-yet-treated in estimation ")
cat("despite a 2022 policy treatment year, because post-treatment mortality data are ")
cat("suppressed by the CDC due to small cell sizes.\n")
cat("\\item See Table~\\ref{tab:cohorts} for estimation cohort assignments.\n")
...
if (!is.null(main_results)) {
  ...
  tryCatch({
    ...
    # Post-process: add \label to the talltblr table
    main_tex <- readLines("tables/table3_main_results.tex")
    main_tex <- sub(
      "caption=\\{Effect of Insulin Copay Caps on Diabetes Mortality",
      "caption={Effect of Insulin Copay Caps on Diabetes Mortality",
      main_tex
    )
    # Add label after caption line
    main_tex <- sub(
      "(caption=\\{[^}]+\\}),",
...
if (!is.null(robustness)) {
  ...
  # 0. CS-DiD baseline (for cross-reference with Table 3)
  main_results <- tryCatch(readRDS("data/main_results.rds"), error = function(e) NULL)
  ...
  # Post-treatment placebo tests (full panel incl. 2020-2023)
  if (!is.null(robustness$cs_placebo_heart_full) && !is.null(robustness$cs_placebo_heart_full$agg)) {
    ...
    robustness_table <- rbind(robustness_table, data.frame(
      Specification = "Placebo: Heart Disease (CS-DiD, Full Panel)",
      ATT = agg$overall.att,
      SE = agg$overall.se,
  ...
  if (!is.null(robustness$cs_placebo_cancer_full) && !is.null(robustness$cs_placebo_cancer_full$agg)) {
    ...
    robustness_table <- rbind(robustness_table, data.frame(
      Specification = "Placebo: Cancer (CS-DiD, Full Panel)",
      ATT = agg$overall.att,
      SE = agg$overall.se,
      CI_lower = agg$overall.att - 1.96 * agg$overall.se,
      CI_upper = agg$overall.att + 1.96 * agg$overall.se,
  ...
  cat("SE is not reported (``--'') because the bootstrap procedure produces $p$-values and confidence intervals directly.\n")
  ...
  cat("and cancer mortality for all ages, age-adjusted (unrelated cause).\n")
  cat("\\item Pre-treatment placebo data covers 1999--2017. Full-panel placebos cover 1999--2023 (incl.\\ post-treatment).\n")
  cat("\\end{tablenotes}\n")
  cat("\\end{table}\n")
  sink()
...
if (!is.null(robustness)) {
  ...
  if (length(het_models) > 0) {
    tryCatch({
      # Compute number of states (clusters) for each model
      panel <- readRDS("data/analysis_panel.rds")
      state_counts <- data.frame(
        term = "Num. States"
      )
      for (name in names(het_models)) {
        model_data <- het_models[[name]]$model_info$data
        # Count unique states from model
        n_states <- length(unique(het_models[[name]]$fixef_id$state_id))
        state_counts[[name]] <- as.character(n_states)
      }

      # Also compute treated state counts
      treated_counts <- data.frame(
        term = "Treated States"
      )
      low_treated <- panel %>% filter(cap_category == "Low ($25-30)") %>% distinct(state_fips) %>% nrow()
      med_treated <- panel %>% filter(cap_category == "Medium ($35-50)") %>% distinct(state_fips) %>% nrow()
      high_treated <- panel %>% filter(cap_category == "High ($100)") %>% distinct(state_fips) %>% nrow()
      if ("(1) Low ($25-30)" %in% names(het_models)) treated_counts[["(1) Low ($25-30)"]] <- as.character(low_treated)
      if ("(2) Medium ($35-50)" %in% names(het_models)) treated_counts[["(2) Medium ($35-50)"]] <- as.character(med_treated)
      if ("(3) High ($100)" %in% names(het_models)) treated_counts[["(3) High ($100)"]] <- as.character(high_treated)

      add_rows_df <- rbind(state_counts, treated_counts)
...
cat("\\begin{table}[htbp]\n")
cat("\\centering\n")
cat("\\caption{Pre-Treatment Balance: Treated vs.\\ Control States (1999--2017)}\n")
...
cat("\\begin{tablenotes}\n")
cat("\\small\n")
cat("\\item \\textit{Notes:} Pre-treatment period: 1999--2017 (NCHS historical data, before any state adopted caps; 2018--2019 not available from either CDC source).\n")
...
# ============================================================
# Table A4: MDE Dilution Mapping
# ============================================================

cat("\n=== Appendix: MDE Dilution Mapping Table ===\n")
...
if (file.exists("data/dilution_table.rds")) {
  ...
  cat("\\begin{table}[htbp]\n")
  cat("\\centering\n")
  cat("\\caption{MDE Dilution Mapping: Population-Level vs.\\ Treated-Group Detectable Effects}\n")
  ...
  cat("where $s$ is the share of mortality attributable to the treated subpopulation ")
  cat("(privately insured diabetics using insulin), and $\\Delta_T$ is the true effect on that group.\n")
  cat("\\item Population-level MDE is computed at 80\\% power, 5\\% significance using TWFE SE.\n")
  cat(sprintf("\\item Mean baseline diabetes mortality rate: %.2f per 100,000.\n",
              dilution_table$mean_mortality[1]))
  cat("\\item For realistic treated shares ($s = 3$--$5\\%%$), the MDE on the treated group ")
  cat("exceeds 100\\% of baseline mortality, meaning the design cannot detect ")
  cat("plausible effects without subpopulation-specific data.\n")
  cat("\\end{tablenotes}\n")
  cat("\\end{table}\n")
  sink()
  ...
  cat("  Dilution table not found — run 04_robustness.R first\n")
}
