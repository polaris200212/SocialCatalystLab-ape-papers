...
if (!is.null(recent_weekly)) {
  ...
  # Mark suppressed states (deaths = 0 due to weekly suppression)
...
placebo_cancer_post <- tryCatch({
  ...
  if (status_code(resp) == 200) {
    ...
    df <- df %>%
      ...
      left_join(all_states, by = "state_name") %>%
      filter(!is.na(state_fips)) %>%
      filter(mortality_deaths_cancer > 0)
    ...
    df <- df %>%
      ...
      left_join(pop_all, by = c("state_fips", "year")) %>%
      mutate(
        mortality_rate_cancer = ifelse(!is.na(population) & population > 0,
...
placebo_heart_post <- tryCatch({
  ...
  if (status_code(resp) == 200) {
    ...
    df <- df %>%
      ...
      left_join(all_states, by = "state_name") %>%
      filter(!is.na(state_fips)) %>%
      filter(mortality_deaths_heart > 0)
    ...
    df <- df %>%
      ...
      left_join(pop_all, by = c("state_fips", "year")) %>%
      mutate(
        mortality_rate_heart = ifelse(!is.na(population) & population > 0,
