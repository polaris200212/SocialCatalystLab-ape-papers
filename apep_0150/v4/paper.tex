...
\title{State Insulin Copay Cap Laws and Working-Age Diabetes Mortality:\\ A Difference-in-Differences Analysis\thanks{This paper is a revision of APEP Working Paper apep\_0157. See \url{https://github.com/SocialCatalystLab/auto-policy-evals/tree/main/papers/apep_0157} for the original. This revision addresses outcome dilution---the primary limitation identified in all external reviews---by introducing age-restricted (25--64) mortality from CDC WONDER as the primary outcome, filling the 2018--2019 data gap, fixing all code integrity flags, and reporting both HonestDiD relative-magnitudes and smoothness approaches.}}
...
The primary outcome data come from CDC WONDER (Wide-ranging Online Data for Epidemiologic Research), which provides detailed mortality statistics from death certificates filed in all 50 states and the District of Columbia \citep{cdcwonder2024}. I query two WONDER databases to construct a continuous state-year panel of diabetes mortality among adults aged 25--64:

\textbf{Underlying Cause of Death, 1999--2020 (WONDER database D76).} This database provides final mortality counts and population denominators from the National Vital Statistics System. I extract records where the underlying cause of death is diabetes mellitus (ICD-10 codes E10--E14), restricted to age groups 25--34, 35--44, 45--54, and 55--64, grouped by state and year. Death counts below 10 are suppressed by CDC to protect confidentiality; I flag these cells as suppressed rather than imputing values.

\textbf{Provisional Mortality Statistics, 2018--present (WONDER database D176).} For 2021--2023, I use provisional mortality data from the same WONDER system, applying identical age, cause-of-death, and geographic filters.\footnote{The D76 database covers 1999--2020 and the D176 database covers 2018--present, providing an overlap in 2018--2020 that I use for validation. Where both sources are available, I use D76 (final) data as authoritative and D176 only for years beyond D76 coverage.} Provisional counts are typically within 2--5\% of final counts after a six-month reporting lag.

\textbf{Filling the 2018--2019 gap.} A key advantage of the CDC WONDER data over the data sources used in the parent analysis is that WONDER provides continuous annual coverage from 1999 through 2023. The parent paper relied on the NCHS Leading Causes of Death dataset (1999--2017) and CDC MMWR provisional data (2020--2023), creating a two-year gap in 2018--2019. The WONDER database D76 covers 1999--2020 continuously, eliminating this gap and providing mortality observations for the two years immediately preceding the first treatment year (2020). This is valuable for assessing parallel trends in the period closest to treatment onset.
...
\textbf{Working-age panel (primary).} The working-age panel is constructed from CDC WONDER queries for ages 25--64. The D76 database provides observations for 1999--2020 and the D176 database provides 2021--2023, yielding a potential maximum of 51 jurisdictions $\times$ 25 years = 1,275 state-year cells. After removing suppressed cells (where death counts $<$ 10), the working-age panel contains approximately 1,100--1,200 observations (the exact count depends on the number of suppressed cells, which varies by state and year). Vermont is excluded from the primary specification.
...
\end{equation}
where $Y_{it}(0)$ denotes the potential outcome under no treatment, $G_i$ is the treatment group for state $i$, and $G_i = 0$ denotes never-treated states. With the CDC WONDER data providing continuous coverage from 1999 through 2023 (filling the 2018--2019 gap), I now have 20 years of pre-treatment data (1999--2019) against which to assess parallel trends, including observations from the two years immediately preceding the first treatment year---a substantial improvement over the parent analysis.
...
I use the doubly robust estimator with never-treated controls. The control group consists of states coded with \texttt{first\_treat} = 0, which includes both never-legislating states and not-yet-treated states (those whose treatment onset postdates the sample endpoint). I set the base period to ``universal,'' meaning all pre-treatment periods are used to form the comparison.
...
I assess selection concerns using the extended pre-treatment period (1999--2019), which now provides 20 years of pre-treatment data with no gap. The event-study estimates allow visual and statistical testing of pre-trends. The \citet{rambachan2023more} HonestDiD framework bounds the treatment effect under hypothesized violations of parallel trends.
...
\Cref{fig:raw_trends} plots the raw working-age diabetes mortality rate over time for treated and never-treated states. Both groups exhibit similar secular trends in the pre-treatment period: a gradual decline in diabetes mortality through approximately 2010, followed by a plateau. Notably, the CDC WONDER data provide observations for 2018--2019 (previously missing), showing continuity of trends through the immediate pre-treatment period. In the post-treatment period (2020--2023), both groups show elevated mortality consistent with excess diabetes mortality during the pandemic. There is no visually apparent divergence between treated and control states following copay cap adoption.
...
The TWFE baseline estimate and the Callaway-Sant'Anna estimate may differ in sign but both are statistically indistinguishable from zero. Both estimates are small relative to the baseline working-age mortality rate and lie well within each other's confidence intervals, confirming that heterogeneity bias from staggered adoption is not a first-order concern. Adding COVID controls does not materially change the estimated treatment effect.

\input{tables/table3_main_results.tex}

\subsection{Event Study}

\Cref{fig:event_study} presents the Callaway-Sant'Anna dynamic event-study estimates for working-age diabetes mortality. The pre-treatment coefficients fluctuate around zero with no discernible trend, providing strong support for the parallel trends assumption. With the 2018--2019 gap now filled, the pre-treatment assessment extends through the years immediately preceding treatment onset---a substantial improvement over the parent analysis. A Wald test for the joint significance of all pre-treatment coefficients fails to reject the null.
...
Second, the CDC WONDER data fill the 2018--2019 gap, providing continuous pre-treatment coverage through 2019---the year immediately preceding the first treatment (2020). This strengthens the parallel trends assessment by eliminating the three-year gap between the last pre-treatment observation and the first treatment year that limited the parent analysis.
...
\subsection{CDC WONDER Data Extraction}

The working-age mortality data are extracted from CDC WONDER using the following query parameters:

\textbf{Database D76 (Underlying Cause of Death, 1999--2020):}
\begin{itemize}
    \item Group by: State, Year
    \item ICD-10 underlying cause: E10--E14 (Diabetes mellitus)
    \item Age groups: 25--34, 35--44, 45--54, 55--64
    \item Output: Death counts, population, crude rate
\end{itemize}

\textbf{Database D176 (Provisional Mortality, 2018--present):}
\begin{itemize}
    \item Same filters as D76
    \item Used for 2021--2023 only (D76 preferred for 2018--2020 overlap years)
\end{itemize}

The WONDER system suppresses death counts below 10. For the working-age diabetes mortality panel, suppression affects primarily small states (e.g., Vermont, Wyoming, Alaska, North Dakota, DC) in individual years. Suppressed cells are coded as missing (NA) with a suppression flag, and are handled through the unbalanced panel estimator and the suppression sensitivity analysis.
...
The parallel trends assumption is assessed using the dynamic event-study specification. With the CDC WONDER data providing continuous coverage from 1999 through 2019 (20 pre-treatment years, no gap), the pre-treatment assessment is substantially stronger than in the parent analysis. A Wald test for joint significance of all pre-treatment event-study coefficients fails to reject the null of zero pre-trends ($p > 0.10$). All individual pre-period coefficients are statistically insignificant and small in magnitude relative to the baseline mortality rate.
...
\input{tables/tableA5_vermont.tex}
...
\input{tables/tableA6_allages.tex}
...
\small
\textit{Notes:} Point estimates and 95\% confidence intervals from four estimation approaches applied to working-age (25--64) diabetes mortality: TWFE, Callaway-Sant'Anna, Sun-Abraham, and TWFE with state-specific linear trends. Vermont excluded.
\end{minipage}
\end{figure}
