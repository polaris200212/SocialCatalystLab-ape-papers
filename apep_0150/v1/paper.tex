...
\title{State Insulin Copay Cap Laws and Diabetes Mortality:\\ A Difference-in-Differences Analysis}
...
Third, this paper contributes to the rapidly evolving econometric literature on staggered difference-in-differences. I implement the full suite of modern DiD diagnostics recommended by \citet{roth2023s}, including heterogeneity-robust estimation, decomposition diagnostics, sensitivity analysis for parallel trends violations, and placebo outcome tests. The application illustrates both the promise and the limitations of these methods in a setting with short post-treatment periods and a contemporaneous aggregate shock.
...
The COVID-19 pandemic is a major contemporaneous shock that complicates causal inference in this setting. Diabetes is a significant risk factor for severe COVID-19 outcomes and death \citep{barron2020associations, holman2020risk}, and excess mortality during the pandemic disproportionately affected the diabetic population. To address this, I construct state-year COVID-19 death counts from CDC provisional data and include them as time-varying controls in robustness specifications. I also estimate models that exclude the COVID-affected years of 2020 and 2021 entirely \citep{woolf2021excess}.
...
\small
\textit{Notes:} Figure shows the year in which each state's insulin copay cap law first fully applied (first full calendar year of exposure). States with treatment onset in 2024--2025 are reclassified as not-yet-treated because the mortality data end in 2023. Vermont is also excluded from the treated group due to suppressed post-treatment data. The seventeen effectively treated states span treatment cohorts from 2020 through 2023. Treatment timing follows the convention described in Section~4.2.
\end{minipage}
\end{figure}
...
Adding COVID controls (state-level COVID death rates, COVID year indicators) does not materially change the estimated treatment effect, suggesting that the null result is not driven by confounding from differential pandemic severity across treated and control states.
...
\textbf{Cluster-robust inference.} With 51 state-level clusters, standard cluster-robust standard errors are well-powered. Applying the small-sample correction recommended by \citet{cameron2015practitioner} (adjusting for the effective degrees of freedom consumed by fixed effects) slightly increases the standard errors but does not change the statistical conclusion. The coefficient remains insignificant under all variance estimators considered.

\textbf{Log specification.} Re-estimating the Callaway-Sant'Anna model with the log of the diabetes mortality rate as the outcome (allowing the treatment effect to be interpreted as a percentage change) yields a small and insignificant estimate, consistent with the levels specification.
...
An important consideration for interpreting the null result is whether the research design has sufficient statistical power to detect plausible effect sizes. A back-of-envelope calculation is informative. The residual standard deviation of age-adjusted diabetes mortality in the estimation sample is approximately 6 deaths per 100,000. With 17 treated states (contributing an average of 2.6 post-treatment years each) and 34 control states, the effective sample size for estimating the aggregate ATT is modest. Under standard assumptions for cluster-robust inference with 51 state-level clusters \citep{cameron2008bootstrap, bertrand2004how}, the minimum detectable effect (MDE) at 80 percent power and a 5 percent significance level is approximately 3--4 deaths per 100,000, or roughly 13--17 percent of the mean diabetes mortality rate.
...
\texttt{covid\_death\_rate} & State-year COVID-19 death count (from CDC provisional data) \\
...
Both approaches are implemented using the \texttt{HonestDiD} R package. The coefficient vector and diagonal variance-covariance matrix are extracted from the Callaway-Sant'Anna event-study output. The diagonal variance approximation is conservative (it ignores positive covariances between event-study coefficients, which would tighten the confidence intervals).
