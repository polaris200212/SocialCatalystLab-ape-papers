...
if (!is.null(covid_deaths)) {
  # Merge and compute crude COVID death rate
  panel <- panel %>%
    left_join(covid_deaths, by = c("state_fips", "year")) %>%
    mutate(
      covid_deaths = ifelse(is.na(covid_deaths), 0, covid_deaths),
      # Use a simple per-capita measure (deaths per death_count ratio)
      covid_death_rate = covid_deaths  # Keep as raw count, control directly
    )
} else {
  # If COVID data unavailable, set to 0
  panel <- panel %>%
    mutate(covid_death_rate = 0, covid_deaths = 0)
}
