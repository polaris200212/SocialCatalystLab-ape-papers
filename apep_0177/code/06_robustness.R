...
for (status in c("Married", "Unmarried")) {
  if (status == "Married") {
    df_sub <- natality[married == 1 & MAGER >= 22 & MAGER <= 30]
  } else {
    df_sub <- natality[married == 0 & MAGER >= 22 & MAGER <= 30]
  }
  df_sub[, age_c := MAGER - 26]

  rd_sub <- rdrobust(y = df_sub$medicaid, x = df_sub$age_c, c = 0)

  het_results[[status]] <- data.frame(
    Group = status,
    N = nrow(df_sub),
    RD_Estimate = rd_sub$coef["Conventional"],
    Robust_SE = rd_sub$se["Robust"],
    CI_Lower = rd_sub$ci["Robust", "CI Lower"],
    CI_Upper = rd_sub$ci["Robust", "CI Upper"],
    p_value = rd_sub$pv["Robust"]
  )

  cat(sprintf("%s (N=%s): RD = %.4f (SE = %.4f, p = %.3f)\n",
              status, format(nrow(df_sub), big.mark=","),
              rd_sub$coef["Conventional"],
              rd_sub$se["Robust"],
              rd_sub$pv["Robust"]))
}
...
cat("\nDifference (Unmarried - Married): ",
    sprintf("%.4f", het_table$RD_Estimate[2] - het_table$RD_Estimate[1]), "\n")
