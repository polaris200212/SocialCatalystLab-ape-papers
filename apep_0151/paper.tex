...
\title{Technological Obsolescence and Populist Voting: \\ Evidence from U.S. Metropolitan Areas\footnote{This paper is a data-corrected revision of APEP-0143. Earlier versions (APEP-0141, APEP-0143) incorrectly used simulated data. This revision uses the actual technology vintage data provided by Prof.~Tarek Hassan (Boston University). See Appendix~\ref{sec:data_provenance} for details.}}
...
Our primary independent variable is the modal age of technologies employed within each CBSA. The data were provided by Prof.~Tarek Hassan (Boston University) and cover 917 CBSAs from 2010 to 2023, measuring the typical age (in years) of capital equipment and production technologies across different industries within each metropolitan area.
...
County-level presidential election returns for 2008 and 2012 come from the MIT Election Data and Science Lab's County Presidential Election Returns 2000-2020 dataset \citep{mit2020county}. Returns for 2016, 2020, and 2024 come from county-level compilations that aggregate data from the MIT Election Data Science Lab and official state reporting sources. Note: The 2024 presidential election occurred in November 2024; this paper was prepared in early 2026 using certified county-level results.
...
We are grateful to Prof.~Tarek Hassan (Boston University) for providing the technology vintage data and for identifying the data error in earlier versions of this paper. Election data are from the MIT Election Data Science Lab. Full replication materials are available at \url{https://github.com/SocialCatalystLab/auto-policy-evals}.
...
The technology data were provided by Prof.~Tarek Hassan (Boston University). For each CBSA-year, we observe approximately 45 industry-level modal age values and collapse to the CBSA-year level by computing the mean.
...
\subsection{Data Provenance and Correspondence}
\label{sec:data_provenance}

This research was initiated based on correspondence from Prof.~Tarek Hassan (Boston University) to Prof.~David Yanagizawa-Drott suggesting evaluation of the hypothesis that technological obsolescence is related to the rise of populist support.

\textbf{Important Note on Data Correction:} Earlier versions of this paper (APEP-0141, APEP-0143) incorrectly used simulated data rather than the actual technology vintage data provided by Prof.~Hassan. Upon review, Prof.~Hassan identified this error in February 2026:

\begin{quote}
\textit{``Hi David,}

\textit{Thanks so much, this is great. I had a look and it seems it took data from some other (Acemoglu) paper, rather than from the dropbox link I sent. Can you try again with this link (this time as csv)?''}

\textit{--- Prof. Tarek Hassan, February 2026}
\end{quote}

This revision uses the correct data from Prof.~Hassan's Dropbox link (\texttt{modal\_age.csv}). The results presented here reflect the actual technology vintage data as intended.

\textbf{Data Sources:}
\begin{itemize}
\item Technology vintage data: Provided by Prof.~Tarek Hassan (Boston University) via Dropbox
\item Election data (2008, 2012): MIT Election Data and Science Lab
