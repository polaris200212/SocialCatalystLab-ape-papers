...
# Filter to target years for LaTeX table (fix indexing bug)
summary_table <- summary_by_year %>% filter(year %in% c(2016, 2020, 2024))
...
cat(sprintf("Trump Vote Share (\\%%) & %s & %s & %s \\\\\n",
            summary_table$`Trump Vote Share (%)`[summary_table$year == 2016],
            summary_table$`Trump Vote Share (%)`[summary_table$year == 2020],
            summary_table$`Trump Vote Share (%)`[summary_table$year == 2024]))
cat(sprintf("Modal Technology Age & %s & %s & %s \\\\\n",
            summary_table$`Modal Technology Age`[summary_table$year == 2016],
            summary_table$`Modal Technology Age`[summary_table$year == 2020],
            summary_table$`Modal Technology Age`[summary_table$year == 2024]))
cat(sprintf("N (CBSAs) & %s & %s & %s \\\\\n",
            summary_table$`N (CBSAs)`[summary_table$year == 2016],
            summary_table$`N (CBSAs)`[summary_table$year == 2020],
            summary_table$`N (CBSAs)`[summary_table$year == 2024]))
cat("\\hline\n")
