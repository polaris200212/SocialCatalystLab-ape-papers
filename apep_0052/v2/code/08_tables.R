...
main_results <- if (file.exists("tables/main_results.csv")) {
  mr <- read_csv("tables/main_results.csv", show_col_types = FALSE)
  # Normalize column names to lowercase
  if ("ATT" %in% names(mr) && !"att" %in% names(mr)) {
    mr <- mr %>% rename_with(tolower)
  }
  # Add specification column if missing
  if (!"specification" %in% names(mr) && "estimator" %in% names(mr)) {
    mr$specification <- "baseline"
  }
  mr
} else {
...
if (!is.null(main_results)) {
  ...
  spec_labels <- setNames(paste0("(", seq_along(specs), ") ", gsub("_", " ", specs)), specs)
  ...
  for (out in outcomes_order) {
    ...
    for (sp in specs) {
      ...
      if (nrow(row_data) == 0) {
        ...
        r <- row_data[1, ]
        p <- ifelse("p_value" %in% names(r), r$p_value,
  ...
  footer_cg       <- "Never-treated"

  # Save CSV
  write_csv(tab3_df, "tables/table3_main_did.csv")

  # LaTeX
  col_names <- c("Outcome", unname(spec_labels[specs]))
...
if (!is.null(main_results)) {
  ...
  pref_spec <- if ("specification" %in% names(main_results)) {
    intersect(c("with_controls", "no_controls", "baseline"), unique(main_results$specification))[1]
  } else {
    "baseline"
  }
  ...
  tab4_rows <- lapply(foundation_outcomes, function(out) {
    r <- main_results %>%
      filter(outcome == out)
    if ("specification" %in% names(main_results)) {
      r <- r %>% filter(specification == pref_spec)
    }
    if (nrow(r) == 0) {
      return(tibble(
...
if (!is.null(main_results)) {
  baseline <- main_results %>%
    ...
    slice(1)
  if (nrow(baseline) > 0) {
    bp <- ifelse("p_value" %in% names(baseline), baseline$p_value,
...
# Helper to extract ATT/SE/p from heterogeneity tables (handles mixed case column names)
extract_het_row <- function(r, label_col = "subgroup", fallback_label = "Unknown") {
  # Determine the label
  lbl <- fallback_label
  for (col in c("subgroup", "group", "Subgroup", "Group")) {
    if (col %in% names(r) && !is.na(r[[col]])) { lbl <- as.character(r[[col]]); break }
  }
  # Get ATT (could be ATT or att)
  att_val <- NA_real_
  for (col in c("ATT", "att")) {
    if (col %in% names(r)) { att_val <- as.numeric(r[[col]]); break }
  }
  # Get SE
  se_val <- NA_real_
  for (col in c("SE", "se")) {
    if (col %in% names(r)) { se_val <- as.numeric(r[[col]]); break }
  }
  # Get p-value
  p_val <- NA_real_
  for (col in c("p_value", "pval", "p")) {
    if (col %in% names(r)) { p_val <- as.numeric(r[[col]]); break }
  }
  if (is.na(p_val) && !is.na(att_val) && !is.na(se_val) && se_val > 0) {
    p_val <- 2 * pnorm(-abs(att_val / se_val))
  }
  # Get N
  n_val <- NA_integer_
  for (col in c("N", "n")) {
    if (col %in% names(r)) { n_val <- as.integer(r[[col]]); break }
  }
  tibble(Subgroup = lbl, ATT = att_val, SE = se_val, N = n_val, p_value = p_val)
}
...
if (!is.null(het_partisan)) {
  for (i in seq_len(nrow(het_partisan))) {
    r <- het_partisan[i, ]
    row_out <- extract_het_row(r, fallback_label = paste("Partisan", i))
    het_rows[[row_out$Subgroup]] <- row_out
  }
}
...
if (!is.null(het_rural)) {
  for (i in seq_len(nrow(het_rural))) {
    r <- het_rural[i, ]
    row_out <- extract_het_row(r, fallback_label = paste("Rural", i))
    het_rows[[row_out$Subgroup]] <- row_out
  }
}
...
if (!is.null(het_moral)) {
  for (i in seq_len(nrow(het_moral))) {
    r <- het_moral[i, ]
    row_out <- extract_het_row(r, fallback_label = paste("Moral", i))
    het_rows[[row_out$Subgroup]] <- row_out
  }
}
...
if (has_mde || has_equiv) {
  ...
  if (has_mde) {
    panelA <- mde_results %>%
      transmute(
        Outcome    = outcome,
        MDE_raw    = if ("MDE_80_raw" %in% names(mde_results)) MDE_80_raw
                     else if ("mde_raw" %in% names(mde_results)) mde_raw
                     else if ("mde" %in% names(mde_results)) mde
                     else NA_real_,
        MDE_sd     = if ("MDE_80_SD" %in% names(mde_results)) MDE_80_SD
                     else if ("mde_sd" %in% names(mde_results)) mde_sd
                     else NA_real_,
        Outcome_SD = if ("outcome_SD" %in% names(mde_results)) outcome_SD
                     else if ("outcome_sd" %in% names(mde_results)) outcome_sd
                     else NA_real_,
        Power      = 0.80
      )
  } else {
  ...
  if (nrow(panelB) > 0) {
    for (i in seq_len(nrow(panelB))) {
      r <- panelB[i, ]
      bound_val <- if ("bound_sd" %in% names(r)) r$bound_sd
                   else if ("bound" %in% names(r)) r$bound
                   else if ("delta" %in% names(r)) r$delta
                   else NA_real_
      reject_val <- if ("equivalent_05" %in% names(r)) r$equivalent_05
                    else if ("reject" %in% names(r)) r$reject
                    else NA
      p_tost_val <- if ("p_TOST" %in% names(r)) r$p_TOST
                    else if ("p_upper" %in% names(r)) r$p_upper
                    else if ("pval" %in% names(r)) r$pval
                    else NA_real_
      tab7_parts <- c(tab7_parts, list(tibble(
        Col1 = sprintf("\\quad $\\delta = %.2f$ SD", bound_val),
        Col2 = ifelse(!is.na(reject_val) && reject_val, "Reject $H_0$", "Fail to reject"),
        Col3 = ifelse(!is.na(p_tost_val), sprintf("$p = %.3f$", p_tost_val), "---"),
        Col4 = "",
        Col5 = ""
      )))
    }
