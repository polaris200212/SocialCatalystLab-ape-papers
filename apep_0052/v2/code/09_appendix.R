...
# Prepare gname for did package: must be numeric double, 0 for never-treated
analysis <- analysis %>%
  filter(!is.na(log_income)) %>%
  mutate(
    gname = as.numeric(ifelse(treated, treat_year, 0)),
    id_numeric = as.numeric(factor(place_id))
  )
...
# Load main results and normalize column names
...
# Helper function for formatting coefficients with significance stars
...
if (!is.null(pretrend_results)) {
  # Use pre-computed results; normalize column names to match formatting expectations
  tabA2 <- pretrend_results %>%
    rename(Outcome = outcome, Chi2 = chi2) %>%
...
tabA2_latex <- tabA2 %>%
  select(Outcome, Chi2, df, p_value, Power) %>%
  mutate(
...
tabA3_rows <- lapply(comparison_outcomes, function(out) {
  ...
  if (!is.null(main_results)) {
    ...
    if (nrow(cs_row) > 0) {
      cs_att <- cs_row$att
      cs_se  <- cs_row$se
      cs_p <- ifelse("p_value" %in% names(cs_row), cs_row$p_value,
...
if (nrow(tabA5) > 0) {
  ...
  # Normalize column names from pre-computed CSV (lb/ub/includes_zero/outcome)
  if ("lb" %in% names(tabA5) && !"CI_Lower" %in% names(tabA5)) {
    tabA5 <- tabA5 %>%
      rename(CI_Lower = lb, CI_Upper = ub) %>%
      mutate(Includes_Zero = as.logical(includes_zero))
  }

  # If multi-outcome, pick universalism_index as focal; show all M values
  if ("outcome" %in% names(tabA5)) {
    tabA5_focal <- tabA5 %>% filter(outcome == "universalism_index")
    if (nrow(tabA5_focal) == 0) tabA5_focal <- tabA5 %>% slice_head(n = 5)
  } else {
    tabA5_focal <- tabA5
  }

  tabA5_latex <- tabA5_focal %>%
    mutate(
      M             = sprintf("%.2f", M),
      CI_Lower_fmt  = sprintf("%.4f", CI_Lower),
      CI_Upper_fmt  = sprintf("%.4f", CI_Upper),
      CI_95         = sprintf("[%s, %s]", CI_Lower_fmt, CI_Upper_fmt),
